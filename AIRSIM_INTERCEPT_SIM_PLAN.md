...
本计划的当前运行主线已经从早期单目标/固定系留节点方案推进到 D1-D7 统一 runtime：拦截资源使用 SimpleFlight，入侵目标使用脚本驱动的无人机 mesh actor，高空侦察节点为可机动、带高性能光电云台的二级协调/观测节点。main 统一启动一次 Blocks，并以 reset 分隔 episode；模块算法不得写死 2v2/5v5，规模由 `--drone-count`、资源数和目标数输入。

截至 2026-07-13 已形成以下真实 AirSim 证据：

- strict dense crossing：4 m/2 m 各 20 seeds，共 40 episode；D1 governed replay 与 D2 truth-isolated 评分可用。
- D4 episode-time 故障矩阵：六类、10 seeds、60/60 safety outcome 通过；不等同真实 RF/硬件网络验证。
- M5N2：高威胁目标使用 `2 primary + 1 standby reserve`，不要求同时到达；40 个 SimpleFlight episode 中最佳 profile coalition 为 `5/10`，总体 `8/40`，仍是开放 P1。
- D5 原生 MOT：ByteTrack/BoT-SORT 18-case screening 已完成但 0 个候选准入；默认继续使用 AirSim `simGetDetections`。
- D7 继续使用既有位置 PN 与 `png_guidance_delivery` 视觉 PNG，核心公式未修改；D6 已统一输出七源 CSV/JSON/中文 Markdown/PNG。

当前没有新的运行级 P0 blocker。下一阶段优先关闭第二 primary 视觉获取/锁定、远距检测尺度、D3 逐时刻计划历史和真实网络时序等 P1，不提前以 ROS 2、PX4、MPC 或完整外部高阶算法替换当前主线。

## 1. 项目目标

本方案面向科研仿真验证，使用 AirSim 构建入侵无人机、拦截无人机、地面/中心节点和机动高空侦察无人机共同参与的对抗场景，验证地面雷达探测、图像识别、目标配准、融合跟踪、导引拦截和虚拟命中判定的完整闭环。
...
- 单目标与 2v2 闭环已经跑通，当前主线为可变 M/N 和高威胁目标多资源协同。
- 当前想定允许中心节点、二级系留侦察节点、拦截无人机之间交换数据；通信仅作为仿真消息链路建模，不涉及真实无线链路、频点或抗干扰实现。
- 正常状态由中心节点统一融合、统一分配、统一评估；机动高空侦察无人机作为二级区域节点，健康时按雷达/GlobalTrack cue 调整云台并提供高视角图像/航迹 cue，中心失效时优先接管局部协调。
- 当中心节点仍在线但定位不确定度升高、分配计划延迟过大、末端视觉配准与中心计划持续不一致时，场景允许触发主动降级仲裁；二级节点不可用后才进入完全无中心协商。
...
默认约束 = 一架拦截机同一时刻最多执行一个主任务；普通目标默认单资源，高威胁目标可声明 k>1 需求，当前基线为2个active primary+1个standby reserve
节点假设 = 中心C2 + 机动高空二级侦察/协调节点 + 拦截资源节点
通信假设 = C2-二级节点、C2-拦截机、二级节点-拦截机、拦截机之间均可交换仿真数据；视频/图像 cue 主要由二级节点定向提供给小范围资源
```
...
当前想定允许拦截机之间、拦截机与中心、拦截机与二级节点以及二级节点与中心交换数据；二级节点还可向局部资源发送视频或图像 cue。中心化正常模式下，分配结果仍以中心版本化计划为规范事实源，机间链路主要承载状态、证据和保底协同，不允许本地节点据此改写 `global_track_id` 或绕过计划版本。
...
1. 中心正常：中心节点统一分配，机间/二级链路提供状态和观测辅助。
2. 中心健康但末端软不一致：请求二级观测辅助；持续硬失配请求中心重规划，不转移 owner。
3. 中心失效：由满足 readiness、lease 和版本条件的二级节点接管。
4. 二级失效：必要成员 ACK、epoch、lease 全部有效后才进入完全分布式联盟。
5. 通信受限研究：注入延迟、丢包、分区和恢复；缺少执行合同则 fail closed。

是否需要无人机间通信的结论：

- **做中心化多目标分配**：机间通信不是求解器必要输入，但当前系统允许并记录该链路。
