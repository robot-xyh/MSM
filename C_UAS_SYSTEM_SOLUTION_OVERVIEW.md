...
当前文件系统可用空间约 13 GiB，低于 20 GiB 固定保护线，正式大批量运行暂停。本轮没有
降低保护线，也没有启动 900-cell 或完整多 seed 写盘。
...
D4 实际区域策略在 20 个校准 seed、420 个样本上得到 76 个安全非零输出和 344 个资源
不可行主分类。当前谱系候选构建器已将训练和模型选择限定在 train/validation，并绑定
源码、数据、拆分、配置和权重；共享工作区不干净时拒绝生成实物。D5 行为克隆已加入类别
