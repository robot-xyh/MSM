...
实现不一致。实际装配器退出码为 2，没有生成 admitted bundle。D3、D4 和 A3 的开发性
证据装配与运行桥已经形成，但现有模型仍未获得正式准入。A3 进一步补齐了零检测帧、观测
触发和 episode 证据尾窗：默认 1% 通信丢包的 20-seed 开发复跑为 492/488 条可配对，
零丢包控制为 500/500；全部权限保持为假。下一步仍需形成绑定 clean commit、严格未见
seed、实际采用、物理结果和成对非退化的新证据。获准模型完成正式作用域后，再由 D6
审计逐 cell 实际采用、物理结果和同键 R0 非退化。D6 只评价证据，不授予模型晋级或
控制权限。
