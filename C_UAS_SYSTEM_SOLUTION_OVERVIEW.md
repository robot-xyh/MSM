...
统一 episode 总线、匿名视觉投影、稀疏跨视角图和学习辅助接口。规则基线 R0 的正式矩阵
仍按 9 类场景、5 档规模和 20 个保留 seed 设计。既有正式证据保持不动，不能跨提交拼接。
2026-07-28 实测文件系统可用空间约 32 GiB，高于 20 GiB 固定保护线。磁盘启动门当前
满足，但正式未见种子、模型权限、运行确认和成对非退化尚未闭合，因此没有启动 900-cell
或完整多 seed 写盘，也没有降低保护线。
...
当前权限边界已经关闭旧 D3/D4 bundle 缺省准入、D5 development scorer 越权和调用方
手工构造 admitted bundle 等路径。D5 G1 v5 已完成合成匿名候选图上的 external audit、
生产装配和 post-assembly 文件链，证明模型制品具备受控影子评分资格；六项运行权限仍
全部为 false，尚无获批 G1 运行实例，现实相机泛化和中心身份绑定结果仍不可用。

D3 新增冻结帧动作裕量校准。正式证据保持 20/20 代价矩阵变化、0/20 binding 变化；开发
夹具在 `alpha=0.25` 出现 3 条差异，只用于定位 Hungarian 离散边界。A1 隔离批次已增加
公共严格加载器，能够重算文件布局、摘要、seed/帧范围、候选选择守恒和版本连续性，但不
产生计划发布、运行确认或物理结果。

D4 历史区域策略在 20 个校准 seed、420 个样本上得到 76 个安全非零输出和 344 个资源
不可行主分类。当前谱系候选已从 clean commit `b0d498d9...` 构建并复核，候选 manifest
和权重分别为 `7cc10ad7...de64`、`fd1b9c4c...0047`。固定门限开发诊断得到 train
168/180、validation 54/60 个安全非零动作；test、历史 calibration 和保留 seed 均未
读取。该实物仍为 development/shadow，全部运行权限为 false。D5 行为克隆已加入类别
不平衡权重、逐动作门和训练语料结构审计。语料缺少 `hold`、少数动作或侦察相机样本时
拒绝训练，复制、过采样和重加权不能替代独立 episode 和 seed。

D4 区域调度已增加规划权与执行权分离。20 目标、20 资源、8 区域的 seed 29 规则专项中，
中心 source 计划由 17 条分配开始；D4 建议从资源相对富余区向短缺区转移 1 个资源，D3
下一周期形成 18 条分配，未分配目标由 3 降到 2。目标区域只有重规划资格，分配、联盟、
接管和控制执行权限均为 false。中心在 2.0 秒失效的负例没有转移、消费或后继计划。
D6 已从实际 episode 总线复核两条链，分别判定为合同链通过和故障代际围栏通过。该专项
使用规则建议器；D4 v4 尚未注册，也没有独立同键规则 episode 和多 seed 配对结果，不能
据此声明学习策略收益。

D6 正式运行准备度已升为 v2。manifest 自报 facts 和十类通用自签 gate 文件不能进入正式
结论。冻结种子审计之外，G1 `model_source` 已通过固定 13 项原始制品、模型身份和实现
谱系的可信适配器复核，只证明 D5 图模型来源可验证。G1 的实际采用、运行确认、物理窗口、
同键 R0、成对非退化、真值审计、有限状态和外部权限仍不可用；A1、A2、A3 也尚无正式
模型来源。六个学习变体因此全部未就绪。D6 只评价证据，不授予模型晋级或控制权限。
