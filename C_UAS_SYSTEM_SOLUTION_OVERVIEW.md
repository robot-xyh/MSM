...
夹具在 `alpha=0.25` 出现 3 条差异，只用于定位 Hungarian 离散边界。D4 实际区域策略在
20 个校准 seed、420 个样本上得到 76 个安全非零输出和 344 个资源不可行主分类，但候选
实现谱系与当前代码不一致，因此不能形成 A2 证据。D5 行为克隆已加入类别不平衡权重和
逐动作门，99:1 夹具的总体准确率 0.99 仍因少数动作召回为 0 而失败关闭。

D6 正式运行准备度已升为 v2。manifest 自报 facts 和十类通用自签 gate 文件不能进入正式
结论；当前只复用既有 canonical seed split 严格审计链，其他模型来源、采用、确认、物理
窗口、同键 R0、非退化和外部权限门均保持 unavailable。六个学习变体因此全部未就绪。
下一步先按既有 producer/auditor schema 补齐受信 adapter，再生成与当前实现谱系一致、
严格未见 seed 的实际采用和物理证据。D6 只评价证据，不授予模型晋级或控制权限。
