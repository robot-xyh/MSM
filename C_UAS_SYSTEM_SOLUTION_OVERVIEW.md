...
后续身份审计确认严格身份交换指标尚不可用。20 个 episode 中有 118 个航迹帧发生
多真值混轨，另有 2,464 个受评分映射缺少明确离线标签。部分身份下界只用于定位，
没有回填严格指标。当前修复顺序是先处理 D1 雷达/视觉跨模态一致性和离线标签合同，
再由 D2、D6 重算严格身份交换、连续性及 D1 真值精度。
