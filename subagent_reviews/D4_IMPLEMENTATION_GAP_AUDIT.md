...
| 二级节点 lifecycle 和链路 freshness | 已实现 `SecondaryNodeLifecycleSummary`、`CommunicationSummary`、video/cue freshness、gimbal pointing、coverage ratio、requested coverage match、fixed/mobile secondary classification、link stale/fresh、cue stale 和 heartbeat stale 判断；传入通信摘要时二级节点必须有新鲜链路才可被选为辅助/接管节点 | `models.py`；`active_degradation.py`；`adapter.py`；`tests/test_active_degradation.py` |
...
| D6 event metadata | 已实现 `D4DecisionRecord.to_event_record_kwargs()`，metadata 含 `d4_action`、`degradation_mode`、`selected_coordinator`、`coverage_cell`、trigger/decision timestamp、三值 review label、pre/post review window、secondary lifecycle、secondary diagnostic、secondary takeover plan lifecycle、plan activation delay、takeover necessity/success、D5 secondary coverage/conversion evidence 和未配准诊断等字段 | `adapter.py`；`tests/test_arbitration_adapter.py` |
...
| main runtime bus 真实 episode 接线 | D4 adapter 可消费对象/dict 摘要，并返回 `D4DecisionRecord` 与 D6 event kwargs；main episode bus 已持续调用 D4 adapter、写入 D4 event，并保留 D1/D2/D3/D5 摘要；P1 D4/D5 calibration sweep 已接入，D6 标准 report bundle 已自动生成；2026-07-08 mobile recon stress 的 3 seeds 均 connected=True，9 个 episode 均为 13/13 image frames，三类 D4 action 与预期一致；D4 record 已输出三值 review label、pre/post window 和 secondary diagnostic 字段 | 仍需真实 Blocks 多 seed 校准二级 coverage/heartbeat/link freshness、人工/离线 review label 填充、二级接管必要性和 D5 peer evidence 分布 | main 需要继续用同一 adapter schema 形成 D6 多 seed 统计 |
...
- AirSim stress 继续维护并校准二级节点 coverage、heartbeat、lease、video/cue freshness、link stale、二级接管必要性和 secondary takeover lifecycle；2026-07-08 mobile recon stress 已确认 gimbal/cue/capability 正常，但二级网络同帧全覆盖仍不足，主要 coverage 断点是 `not_all_targets_visible` / `network_union_incomplete`，D5 注册断点是 `geometry_gate_rejected`。D4 只消费这些摘要，不修正视觉几何注册。
...
1. **P1 AirSim D4/D5 stress 多 seed 校准**：main/integrated runtime 已调用 `D4ArbitrationAdapter.evaluate()` 并写入 D6 event；P1 D4/D5 calibration sweep 已接入，D6 标准 AirSim calibration report bundle 已自动生成；2026-07-08 mobile recon stress 已验证三类 D4 action 正常，下一步是继续用真实 Blocks 多 seed 校准主动降级阈值、dwell/release、review label 填充、false degradation rate、active degradation precision、二级接管必要性和 D5 peer evidence 合流。
2. **P1 secondary coverage/heartbeat/link freshness 校准**：基于真实 episode 标定 heartbeat stale、coverage/coverage ratio、lease、video/cue freshness、gimbal pointing、secondary relay freshness、plan activation delay 分布和恢复合并窗口；当前断点是 `not_all_targets_visible` / `network_union_incomplete` 和 D5 `geometry_gate_rejected`，D4 剩余任务是解释这些断点对接管必要性的影响，而不是执行视觉注册。
