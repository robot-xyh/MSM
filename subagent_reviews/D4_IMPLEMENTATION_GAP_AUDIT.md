...
1300 条 secondary-case D4 决策提供了门限级证据：heartbeat/link/cue/gimbal、visible 和 registered 全部通过，capability score 无低于 0.70；1285 条的 network full-view < 0.80，因而 readiness 为 `registration_usable`，其中 600 条同时 coverage < 0.65。仅 50 m seed 2/5 的三个 frame 产生 15 条 `takeover_ready` 记录，但均为 `pending_secondary_plan`，0 条 `secondary_plan_active`、0 条 executable，随后回落 distributed。D4 安全门控行为正确，不应放宽；剩余 P1 是能力证据的时间稳定性、coverage-cell 聚合、pending 到 active plan/lease 的运行层闭环，以及网络分区和误降级的成对标定。

逐决策证据仍有一个 P1 接线缺口：D5/D6 episode 汇总中的 stable registration/not-registered 计数已经存在，但 1300 条 D4 capability input 的 `stable_cross_view_registration_count` 和 `not_registered_count` 均为 `null`；D4 通过 `cross_view_association_count=2..5` 的兼容路径将 registered 判为 true。该兼容路径没有造成此次失败，但 main/D5 应把逐决策稳定注册摘要送入 D4，以支持持续性门限、回落原因和 takeover audit。
...
| 二级能力评估 | 已完成 D4 合同层，保持回归。`SecondaryNodeLifecycleSummary` 输出 `secondary_visible`、`secondary_registered`、`secondary_takeover_capable`、`secondary_capability_score/reasons`、`secondary_readiness_class` 和 score inputs；event metadata 顶层输出 readiness `secondary_capability_class`。接管选择和 D7 handoff 都要求 `secondary_capability_class=takeover_ready`；visible-only 和 registration-usable 只可辅助/诊断/标定，不可接管 | `tests/test_active_degradation.py::test_visible_but_not_registered_secondary_is_not_takeover_capable`；`::test_registered_secondary_with_low_network_full_view_is_not_takeover_ready`；`tests/test_arbitration_adapter.py::test_adapter_reports_secondary_detect_visible_without_cross_view_registration` |
...
| 二级节点 lifecycle 和链路 freshness | 已实现 `SecondaryNodeLifecycleSummary`、`CommunicationSummary`、video/cue freshness、gimbal pointing、coverage ratio、network full-view rate、stable registration count、not-registered count、requested coverage match、lease expiry、fixed/mobile secondary classification、link stale/fresh、cue stale、heartbeat stale、visible/registered/takeover_capable、readiness class 和 capability score inputs；传入通信摘要时二级节点必须有新鲜链路才可被选为辅助/接管节点，接管还必须 `takeover_ready` | `models.py`；`active_degradation.py`；`adapter.py`；`tests/test_active_degradation.py` |
...
| D7 二级接管门控辅助 | 已实现 `build_d7_secondary_handoff()`，`degrade_to_secondary` 阶段 1 不放行 visual PNG，阶段 2 必须有新 plan id/version；只有显式 `takeover_ready` 可进入 visual PNG gate，`visible_only`、`registration_usable` 或缺失 readiness 均不放行 | `active_degradation.py`；`tests/test_airsim_phase1_dry_run_contracts.py::test_d7_handoff_rejects_visible_only_secondary_capability` |
...
| D6 event metadata | 已实现 `D4DecisionRecord.to_event_record_kwargs()`，metadata 含 `d4_action`、`degradation_mode`、`selected_coordinator`、`coverage_cell`、trigger/decision timestamp、三值 review label、`active_degradation_necessity_label`、pre/post review window、secondary lifecycle、secondary diagnostic、readiness `secondary_capability_class`、capability inputs、secondary takeover plan lifecycle、lease/executable/reject reason、hard/soft risk、false-trigger candidate、plan activation delay、takeover necessity/success、stable/not-registered count、D5 secondary coverage/conversion evidence 和未配准诊断等字段 | `adapter.py`；`tests/test_arbitration_adapter.py` |
...
| secondary takeover plan version 闭环 | pending/active metadata、lease/epoch strictness、D7 handoff 和 controlled 2v2 owner/version 基线已有；10-seed 已证明 readiness evidence 可进入 D4 | 20 个 secondary case 最终均 distributed；15 条瞬时 takeover-ready 全部 pending，0 active/executable。主失败门限为 network full-view < 0.80，部分记录还 coverage < 0.65 | main/D3/D7 新增持续 readiness 到 active plan 的回归，记录 activation delay 和回落原因；不降低 D4 门限 |
| D1/D2/D3/D5 evidence adapter | D4 侧 adapter 可消费对象/dict 摘要；10-seed 中 cross-view association、coverage、freshness 已进入 readiness | D5/D6 有 episode stable/not-registered 统计，但逐决策 D4 input 两字段均为 null；D5 peer evidence 合流仍需校准 | main/D5 将逐决策 stable/not-registered 摘要统一送入 `D4ArbitrationAdapter.evaluate()` |
...
1. **P1 AirSim D4/D5 定向校准**：10-seed 常规基线已经完成；先补齐逐决策 stable/not-registered 摘要，再构造持续 network full-view 和 coverage-cell 切换 case，统计 `registration_usable -> takeover_ready -> pending -> active` 驻留时间、回落原因、false degradation 和接管必要性，不再把“继续增加相同 seed”作为验收。
2. **P1 secondary freshness/plan lifecycle 校准**：分别注入 heartbeat stale、lease expiry、video/cue stale、link stale 和 gimbal failure，并提供可持续的全覆盖对照，确认每个门限能独立阻断接管；同时闭合 active plan/lease、plan activation delay 和恢复合并窗口。
