...
- **验证日期/样本/结果**：2026-07-20，区域资源建议专项 32/32，D4 全量 **335/335 passed**，验收阈值零失败。覆盖 3/5/8/32 区、守恒、断边/partition、中心/多二级/distributed owner、旧 epoch/过期 lease/缺 ACK/fault fence、formal commit 保护、BC/PPO 有限更新、bundle SHA、全部回退路径和 shadow verdict 不变。
- **新增/重分类 GAP**：main scalable 3D 接线缺口已关闭为质点接口证据；学习实现管线缺口已关闭。assist 资格、至少 20 个实际未见 seed、训练后 checkpoint、paired backlog/transfer/churn/communication/fail-closed/safety/latency 结果、AirSim 和真实网络仍开放，不能由合成 evaluator fixture 替代。
...
- **验证日期/样本/结果**：2026-07-20，23 个确定性 pytest case，无随机 seed。五档参数分别为 5/20/50/100/200 region，每档构造相同数量 active task/resource metadata；其余 case 覆盖声明节点数上限、中心/二级连续失效、双区域 coverage、全层完整/缺失 ACK、D5 member hold、单成员多能力、跨区域 capacity、旧 generation、最早 lease 和全层分区。新增 23/23、当时 D4 全量 **303/303 passed**；当前全量为 335/335。
...
2026-07-15 当日 D4 全量验收阈值为零失败，结果 280/280 passed。此前 278/278 未覆盖两个公开 helper 的 sustained/source/epoch `None`，不能证明所有公开入口闭锁；新增逐字段 `None`、完整正例、same-plan 维持和 distributed bypass 后才关闭该 P0。该历史计数先由 303/303、再由当前 335/335 回归取代，P0 判定不变。
...
该轮 2026-07-14 历史验收为 211/211 passed；2026-07-15 的 278/278 与 280/280、区域阶段 303/303 都是历史结果，当前 D4 全量为 335/335。未运行新 AirSim episode。member replacement 仍只是测试手工给定替换成员后的 replay，不是自主 reserve 激活、补位、缩编或整盟重组。
...
| 区域资源学习建议 | **管线已实现，assist 未验收** | 专项 32/32、D4 全量 335/335；默认 disabled/shadow，少于 20 个实际未见 seed 不得 assist，不改变 formal D4/D3/D7 裁决 |
