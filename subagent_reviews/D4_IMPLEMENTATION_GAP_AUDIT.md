...
D4 模块内 pending/active 合同也已补齐：只有 sustained readiness 才能进入 pending；active 还要求 source 与选中二级节点一致、plan version 单调或保持同一已激活 secondary plan、plan lease epoch 不低于节点要求且 lease 未过期。事件输出 previous/current transition、pending since、activated at、activation delay 和 fallback reason。2026-07-11 修复 `not_ready -> takeover_ready` 时 `ready_since_s` 未初始化的问题，并增加“3 个不同时间戳后 sustained、同帧不累计、回落后重新计时”回归。D2 online truth 指标不可用时，IDSW/continuity 占位不再作为硬风险，在线 ambiguity/duplicate/quality 风险保持有效。单元测试还覆盖 heartbeat/link/cue/gimbal 回落、过期 lease、stale lease epoch、错误 source、能力回落和 distributed fallback；当前 D4 测试为 95 项通过。剩余 P1 是 main/D3/D5/D7 的真实 AirSim active-plan 接线、coverage-cell 聚合、网络分区和误降级成对标定。
