...
| secondary takeover plan version 闭环 | `degrade_to_secondary`、lifecycle、D7 两阶段 handoff、D4 record pending/active metadata 已有；固定系留/机动高空二级均可作为 source node，`mobile_high_recon` 已作为二级候选能力进入 D4；D4 record 已输出 plan activation delay/pending duration、readiness class 与 takeover necessity/success 字段；main/D3/D7 已完成 secondary owner/version P1 基线，并通过 controlled 2v2 secondary visual PNG 回归 | 本轮 mobile recon gimbal/cue 正常，但二级网络同帧全覆盖仍为 0.0，case mean 联合覆盖约 0.65-0.69；registration v2 单 seed 已有 stable registration 51/55/53，但 degradation case not-registered 仍为 35/35；真实 Blocks 多 seed 中的 mobile recon heartbeat、coverage ratio、gimbal、link freshness、稳定注册、接管必要性标签和恢复合并窗口还未标定 | main/D3/D7 需要继续把二级新 plan id/version、source node 和恢复后双轨校验写入同一 episode log |
...
| AirSim D4/D5 stress | D4 合同测试覆盖 case_001/002/003；main 层脚本和 runtime 已有基线 stress/controlled regression 接线；D4 事件字段已能承载 coverage、freshness、stable cross-view registration、not-registered、readiness class 和 review label | D4 不直接运行 AirSim；真实 Blocks 多 seed 的阈值、freshness、false degradation rate、active degradation precision 和二级接管必要性标签尚未校准 | main 需要用统一 D4 输入口径跑多 seed 并输出 D6 报告 |
