...
D4 已经实现了可运行的离线骨架：`C2Health` 状态机、被动降级优先级、主动降级仲裁、二级侦察节点模型、二级节点生命周期摘要、通信摘要、主动降级迟滞/防抖、简化 CBBA、中心恢复合并、AirSim phase-1 dry-run 合同测试，以及 main 到 D4 的摘要适配器。当前 D4 侧 P0/P1 接口已经补齐；剩余缺口主要是 main/integrated runtime 显式调用该 adapter、把 D4 事件持续写入统一数据总线、真实 5v5 CV stress 的运行时日志贯通，以及成熟开源 CBBA/拍卖/合同网实现的对照接入。
...
**2026-07-03 P1 补充实现状态**：D4 已新增 `SecondaryNodeLifecycleSummary`、更完整的二级节点 heartbeat/lease/coverage/video-cue freshness 字段、`ActiveDegradationConfig` 的 dwell/release/window 防抖参数，以及更丰富的 D6-compatible decision metadata。D6 侧也已具备主动/被动降级基础聚合，因此 D4 模块内 P1 接口缺口已关闭；后续主要是 main 真实 episode 接线和参数校准。
...
| 主动降级：中心在线但 D1/D2/D3/D5 风险导致仲裁 | **已实现离线规则版、D4 侧 adapter 和 P1 防抖配置**。`ActiveDegradationArbiter` 消费 D1 定位、D2 关联、D3 分配、D5 终端关联、C2/secondary health，输出 continue/replan/assist/secondary/distributed/hold；`D4ArbitrationAdapter` 可自动构造 summary；`ActiveDegradationConfig` 支持 dwell/release/window 防抖 | `active_degradation.py`；`adapter.py`；`tests/test_active_degradation.py`；`tests/test_arbitration_adapter.py`；`tests/test_airsim_phase1_dry_run_contracts.py` | 阈值为人工默认值，尚未用 5v5 批量实验校准；main runtime 尚未统一调用 D4 adapter 替换各处手工 summary | main 需要接入 adapter 并用多 seed episode 校准 dwell/release/window 参数 | P1 已完成模块基线 |
...
| 二级系留/高空侦察节点：区域协调者、视频/检测 cue 提供者、`coordinator_only` | **已实现模块基线**。`NodeRole.SECONDARY_RECON`、`coordinator_only`、`coverage_cell`、lease/priority、heartbeat 和 `SecondaryNodeLifecycleSummary` 已建模；主动仲裁要求覆盖匹配与链路新鲜；D4 adapter 可消费 LinkRecord-like 二级链路摘要 | `models.py`；`active_degradation.py`；`adapter.py`；`coordinator.py`；`tests/test_coordinator.py`；`tests/test_arbitration_adapter.py`；`tests/test_airsim_phase1_dry_run_contracts.py` | 二级节点仍是 summary/leader 模型，不持有区域图像流或局部 TrackSummary 缓存；真实图像/检测 cue 由 main/D5 产生 | main/AirSim 需生成 `Secondary_Recon_*` 的视频元数据、检测摘要、覆盖区、通信时延；D5/D1 需消费二级 cue | P1 已完成模块基线 |
| 二级节点失效后的二次被动降级 | **已实现模块基线**。二级 availability 为 none、heartbeat stale 或链路 stale 时会选择 distributed；coordinator 也能在二级不可用时选 cluster representative | `active_degradation.py`；`adapter.py`；`coordinator.py`；`models.py`；`tests/test_active_degradation.py`；`tests/test_arbitration_adapter.py`；`tests/test_coordinator.py` | 持续运行的 episode 级 lifecycle 仍由 main 维护，D4 只消费每帧 summary | main 需要在 episode 中写入 secondary heartbeat、lease epoch、link stale 与 coverage mismatch 事件 | P1 已完成模块基线 |
...
| 中心/二级/拦截机之间的数据与视频链路用于仲裁 | **已实现摘要级基线**。主动仲裁会用 fresh `c2_direct/secondary_relay/video_cue` 链路判断二级节点可用；无新鲜链路不选二级；adapter 可把 D6 `LinkRecord` 风格对象转换为 D4 `CommunicationSummary` | `active_degradation.py`；`adapter.py`；`tests/test_active_degradation.py`；`tests/test_arbitration_adapter.py` | 链路只作为摘要，不包含真实视频帧或无线网络协议实现 | main 需要从中心、二级节点、拦截机之间的数据/视频事件生成 LinkRecord/CommunicationSummary；D6 负责统计 link stale、latency、secondary selected | P1 已完成模块基线 |
...
| AirSim D4/D5 stress：case_001/002/003 | **已实现 D4 dry-run 合同测试；main 层已有 D4/D5 stress 分析脚本**。D4 测试覆盖 continue_center、degrade_to_secondary、degrade_to_distributed 和 metrics 字段；`airsim_runtime/d4d5_stress.py` 生成 D5 观测、D4 decisions 和 case 报告 | `tests/test_airsim_phase1_dry_run_contracts.py`；`research_modules/airsim_runtime/d4d5_stress.py`；`README.md` | stress 脚本和 integrated runtime 的 D4 输入口径仍需持续统一到 adapter；D6 episode 级汇总由 main 统筹 | main 需要把真实 episode 的 D4 decision event 稳定写入 D6 collector | P1 main 统筹 |
| D4 输出指标：`d4_action`、`degradation_mode`、`target_node_id`、风险因素等 | **已实现模块基线**。`ActiveDegradationDecision.to_metrics()` 输出主要字段，`D4DecisionRecord.to_event_record_kwargs()` 输出 D6 EventRecord 兼容 metadata，CBBAResult 输出 rounds/conflict/completion/messages | `active_degradation.py`；`adapter.py`；`models.py`；`tests/test_arbitration_adapter.py`；`tests/test_airsim_phase1_dry_run_contracts.py` | episode-level 聚合由 D6/main 负责，D4 不直接生成全局报告 | D6/main 基于 D4 decisions/events 计算 active/passive 次数、二级接管率、分布式冲突率；D6 基线已可消费这些字段 | P1 已完成模块基线 |
...
| 主动降级迟滞/防抖 | **已实现配置基线**。`ActiveDegradationConfig` 已包含 `min_dwell_s`、`release_consecutive_consistent_frames`、`risk_window_size` 和 `risk_window_threshold`，测试覆盖主要状态 | `active_degradation.py`；`tests/test_active_degradation.py` | 当前参数仍是保守默认值，未用真实 5v5 批量 episode 标定 | 需要 5v5 批量 episode 统计、误触发率、恢复条件、最短保持时间和动作切换矩阵 | P1 已完成模块基线 |
...
1. **模块边界原因**：D4 不直接依赖 D1-D3/D5 的内部类型，也不修改 main runtime。D4 侧 summary adapter、二级 lifecycle summary、迟滞配置与 D6-compatible metadata 已完成；剩余缺口是 integrated_simulation/main 在真实 episode 中调用 adapter、写入统一事件总线，并生成 episode 级聚合。
...
3. **P1 已完成模块基线：二级节点生命周期与 coverage 路由**
   D4 已提供 `SecondaryNodeLifecycleSummary`、heartbeat/lease/coverage/video cue freshness 和 coverage 过滤规则。下一步在 main episode 中维护这些字段并覆盖多区域多二级节点冲突。
