...
2026-07-11 新增 M 对 N 专项审计：高威胁目标要求 `k_j=3` 时，当前 single-winner `CBBANegotiator` 不能被解释为多机联盟分配。基础 CBBA 是成熟分布式基线，但 CCBBA、consensus-based grouping 和 distributed coalition formation 的公开代码成熟度不足。该项不是新 P0；它是需要跨 D3/D4/D6/D7/main 定义合同并验证的 P1。证据见 `D4_M_TO_N_DISTRIBUTED_COALITION_REVIEW.md`。

2026-07-11 第一阶段安全语义已落地：`CoalitionSafetyEvidence` 可序列化输出 D3 schema v2 的 plan/coalition version、需求、完整性、授权成员、锁成员和冲突原因。中心可用且联盟合法时保留中心路径，但若 arbiter 候选动作会进入尚不支持原子联盟的 secondary/distributed，则改为 `request_center_replan`；中心不可用时统一输出 `coalition_fallback_unsupported`/`hold_or_revoke`。event 记录 candidate/gated action。合法联盟内多个授权资源锁同一 `global_track_id` 不再计 duplicate；联盟外/超额成员及旧 plan/coalition version fail closed。模块测试覆盖 `k=1` 回归、`k=3` 中心正常/失效、中心可用主动 distributed 候选、第四成员和旧双版本。二级/完全分布式 coalition commit、ACK、reserve/补位/重构仍 deferred。
...
本轮 P0/P1 复核：无 P0 blocker。P0-B 已在 D4 模块内闭合到单元测试层；2026-07-09 已补齐 heartbeat sliding window/miss threshold/dwell，单次 heartbeat 延迟不会直接 failed；secondary takeover plan 新增 lease expiry、epoch monotonic、executable/reject reason 和恢复双轨审计字段，过期或非单调替换二级 plan 不可执行，当前 plan owner 已是 secondary 且 current/secondary plan id/version 相同时可保持 `secondary_plan_active`；secondary capability score 现在输出 `not_ready|visible_only|registration_usable|takeover_ready` readiness class，并消费 coverage ratio、network full-view rate、heartbeat/link/cue freshness、gimbal、stable registration count、not-registered count 和 reject reason；主动降级继续保留硬/软风险分层、dwell/release/window 防抖和 false-trigger metadata，避免 `d3_assignment_cost_margin_low`、D5 低置信度、D5 持续无冲突 `ambiguous/reacquire` 等软证据造成名义场景每帧中心重规划或分布式降级。D4 现在区分“末端暂时看不清/重捕获”和“末端观测与分配冲突”：前者继续中心或请求二级 cue，后者才进入中心重规划、二级接管或分布式保底。2026-07-08 已补齐机动高空侦察节点元数据：`role/capability_class=mobile_high_recon/mobile_secondary_recon` 会进入二级候选和 lifecycle/D6 evidence，并与 `fixed_tethered_secondary/tethered_recon` 区分；但侦察图像、cue freshness、云台指向或 coverage ratio 本身不触发接管。D4 record 中 secondary takeover `pending_secondary_plan`/`secondary_plan_active` metadata、三值 review label、pre/post window、`active_degradation_necessity_label`、plan activation delay、二级接管必要性/成功统计字段、D5 distributed evidence 到 CBBA 的风险加权、CBBA cost gap helper、CBBA D6 report metadata 和 D6 event metadata 均已补齐。中心重规划 lifecycle、assignment freshness 与既有合同均有模块回归，当前 D4 测试 121 项通过；D4 仍只输出仲裁和审计，main/D3 负责请求状态和新 plan 的事实源，D5/D7 保持独立执行门控。

最新 main 测试上下文为 `research_modules/airsim_runtime/outputs/p1_gap_closure_calibration_20260710/` 及其 50/200 m case 目录：10 seeds、3 个二级节点、FOV 110 度、1920x1080，共 60 个 5v5 case。20 个 `degrade_to_secondary` case 的最终帧和 dominant action 均为 `degrade_to_distributed`。50 m/200 m 的 network joint full-view 均值为 0.023/0.000，coverage 均值为 0.685/0.708；projection valid 均为 1.0，cross-view association 均值为 4.6/4.0，stable registration 均值为 86.3/96.7，not-registered 均为 0。该批次表明注册链已显著改善，但接管所需的网络同帧全覆盖仍不稳定。

1300 条 secondary-case D4 决策提供了门限级证据：heartbeat/link/cue/gimbal、visible 和 registered 全部通过，capability score 无低于 0.70；1285 条的 network full-view < 0.80，因而 readiness 为 `registration_usable`，其中 600 条同时 coverage < 0.65。仅 50 m seed 2/5 的三个 frame 产生 15 条 `takeover_ready` 记录，但均为 `pending_secondary_plan`，0 条 `secondary_plan_active`、0 条 executable，随后回落 distributed。D4 保持这些硬门限，并新增默认 3 个不同时间戳决策、至少 0.2 s、evidence gap <= 1.0 s 的持续 readiness gate；单帧和同时间戳重复判定均不能接管。
...
D4 模块内 pending/active 合同也已补齐：只有 sustained readiness 才能进入 pending；active 还要求 source 与选中二级节点一致、plan version 单调或保持同一已激活 secondary plan、plan lease epoch 不低于节点要求且 lease 未过期。事件输出 previous/current transition、pending since、activated at、activation delay 和 fallback reason。2026-07-11 修复 `not_ready -> takeover_ready` 时 `ready_since_s` 未初始化的问题，并增加“3 个不同时间戳后 sustained、同帧不累计、回落后重新计时”回归。D2 online truth 指标不可用时，IDSW/continuity 占位不再作为硬风险，在线 ambiguity/duplicate/quality 风险保持有效。单元测试还覆盖 heartbeat/link/cue/gimbal 回落、过期 lease、stale lease epoch、错误 source、能力回落和 distributed fallback；当前 D4 测试为 121 项通过。剩余 P1 是 main/D3/D5/D7 的真实 AirSim active-plan 接线、coverage-cell 聚合、网络分区和误降级成对标定。

2026-07-11 truth-isolated smoke 复核未关闭该 P1。`p1_runtime_truth_isolated_d4d5_smoke_20260711`（200 m、2 secondary）、`p1_runtime_truth_isolated_d4d5_50m_20260711`（50 m、2 secondary）和 `p1_runtime_truth_isolated_d4d5_secondary5_20260711`（200 m、5 secondary）中，中心保持正例均为 `continue_center`，二级不可用负例均为 `degrade_to_distributed`。三组预期二级接管正例同样为 `degrade_to_distributed`，其共同证据是 `secondary_network_joint_full_view_frame_rate=0.0`、readiness 非持续 `takeover_ready`。5-secondary 配置虽把 `secondary_network_mean_coverage_ratio` 提升到约 0.80，仍未形成同帧全目标联合覆盖。结论是安全回落正确，但二级接管正向能力尚未验收；不能把平均覆盖、累计检测或单帧可见性解释为 active secondary plan，也不得通过降低门限关闭缺口。
...
| D2 online truth 隔离语义 | 已实现 `truth_metrics_available`/`continuity_available` 透传与门控；不可用的 IDSW/continuity 数值占位不产生 `d2_id_switch_observed`/`d2_track_continuity_low`，在线 ambiguity、duplicate track risk 和 track-quality-derived association risk 保持有效，既有 duplicate risk 0.5 转换门限不变 | `active_degradation.py`；`adapter.py`；`tests/test_arbitration_adapter.py` |
...
| main runtime bus 真实 episode 接线 | 已完成 10-seed、50/200 m、60 case 的 D4/D5 sweep 和 D6 聚合；D4 record 已包含 review label、readiness、capability inputs 和 secondary diagnostic | 现有 sweep freshness 全部正常，尚未覆盖通信/heartbeat/cue 故障分布或持续 active secondary plan | main 增加通信退化和持续接管 case，继续沿用当前 schema |
...
| secondary takeover plan version 闭环 | D4 模块内 sustained readiness、pending/active/source/lease/timing/fallback 合同和负例已完成 | 真实 AirSim 尚无 main/D3 回填 active plan 的多 seed 正例，历史 15 条瞬时 ready 按新规则应被连续门控阻断 | main 复用同一 adapter；D3 回填新 plan；D7 校验 sustained/current binding；不降低门限 |
...
| AirSim D4/D5 stress | 10-seed 真实 Blocks sweep 和 D6 聚合已完成，active degradation label/precision、coverage、registration 和 readiness 可审计 | 二级接管成功率仍为 0，现有正常 freshness 数据不能标定故障阈值；需补持续全覆盖、链路退化和 active-plan case | main 使用统一 D4 schema 增加针对性 seed，不重复仅改变随机检测序列的同类 sweep |
| M 对 N 联盟降级 | 第一阶段已读取 D3 schema v2 的 demand、coalition/member 和双版本；中心有效的 secondary/distributed 候选改为中心重规划，中心失效/越权/超额/旧版本时 hold；event 输出 candidate/gated action；合法授权多锁不算 duplicate；coordinator 禁止 `k>1` 进入 CBBA | 二级/完全分布式原子 coalition commit、member ACK、reserve、缩编/补位/整盟重组、时序可达性和 coalition digest 恢复仍未实现 | D3/D4/D6/D7/main 继续确定 lifecycle、ACK、lease/epoch、D7 可达性和 D6 指标；分布式原子联盟保持 deferred |
...
0. **P1 M 对 N 联盟合同**：先定义 target demand、coalition id/member role、timing policy、reserve、ACK、lease/epoch、形成/执行/重构状态和中心恢复 digest；再设计中心、二级、完全无中心三种模式的同场景验收。不得把同一目标复制三次或把现有 single-winner CBBA 宣称为已支持 `k_j=3`。
...
2. **P1 secondary plan 运行层闭环**：D4 heartbeat/link/cue/gimbal/lease/source/能力回落负例已完成；main/D3 现在需要生成并回填真实新 plan，D7 验收 current binding，D6 统计 pending->active delay 和回落。真实正例不足时补场景，不降低门限。
3. **P1 网络分区与误降级成对标定**：注入 center-secondary、secondary-interceptor、peer split/recovery，记录 partition/digest/duplicate-owner/merge 指标；同 seed 配置正常对照和故障真值，由 D6 统计 false/missed degradation、动作混淆和恢复时间。分区或接管样本不足时补场景，不降低门限。
...
5. **P2 optional auction baseline**：在多 seed CBBA gap benchmark 稳定后，可选实现最小 single-round auction baseline，用同一 summary/task/resource 输入与 CBBA 对照。
6. **P2 optional MIT/CA-CBBA adapter**：完成许可证和依赖审查后，以 optional benchmark 接入，不替换默认轻量 CBBA。
