...
**2026-07-21 保留 seed 阈值拒绝诊断更新**：`RegionResourcePairedArmEvidence` 已升级为 `d4-region-resource-paired-arm-evidence-v2`，逐 arm 持久化候选 confidence、`minimum_confidence`、OOD 状态、latency limit、finite 状态以及 confidence/OOD/latency/finite/external-failure 五项门结果；拒绝码明确分解为 `candidate_low_confidence`、`candidate_ood_rejected`、`candidate_inference_timeout` 和 `candidate_output_nonfinite`，旧 generic reason 与 v1 JSON reader 保持兼容。对权威 nominal 5v5 seed 1000-1019 正式记录做只读复算：confidence min/mean/max 为 **0.508893/0.563426/0.569492**，在未下调的 `minimum_confidence=0.6` 下通过 **0/20**；OOD 与 finite 均通过 **20/20**，正式 latency P95/max 为 **35.608/42.302 ms**，在 `50 ms` 门下通过 **20/20**，因此明确拒绝原因是 `candidate_low_confidence: 20`，候选有效数仍为 0 且全部执行规则回退。专项 **33/33**、D4 全量 **482/482 passed**；pair input、冻结 bundle identity、确定性投影和 next-cycle safety gate 均未放宽，也未修改 bundle、权重或 manifest。manifest 仍明确标记 confidence head uncalibrated，本轮结果只形成独立校准/重训计划，不宣称候选有效，不开放 PPO、assist 或 authority。
