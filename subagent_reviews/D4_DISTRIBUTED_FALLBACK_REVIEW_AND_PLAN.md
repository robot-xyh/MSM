...
**2026-07-21 保留 seed 配对干预更新**：D4 已补齐 `d4-region-resource-paired-intervention-spec-v1`、arm evidence v1、manifest v1 和稳定的隔离候选加载/执行入口。seed 严格为 1000-1019，每个 seed 的规则 control 与候选 treatment 重复绑定同一场景配置、初始状态、通信/故障时序和区域快照 lineage SHA。加载器只接受冻结的 `region_resource_bc_900_20260720` development bundle，复核 manifest、权重、训练清单、数据集与切分 SHA，并在每次推理前后重新检查文件指纹。control 始终执行现有确定性规则；treatment 只读生成 raw learned candidate，再进入现有确定性投影和 next-cycle gate。加载、推理、分布外、时限、置信、数值或投影失败均如实记录并回退规则。安全采用不形成线上 ACK，不改变 D4 authority 或生产 advisor 准入。专项 **26/26**、D4 全量 **475/475 passed**，冻结文件读取前后 SHA 不变。本轮未运行正式保留 seed episode，D6 sidecar 未接入，outcome、paired non-degradation、counterfactual、causal、PPO、assist 和 authority 均 unavailable/false。
