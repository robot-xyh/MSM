...
**2026-07-21 保留 seed 配对干预更新**：D4 已增加 `d4-region-resource-paired-intervention-spec-v1`、arm evidence v1 和 manifest v1。seed 严格为 1000-1019，每个 seed 的规则 control 与候选 treatment 重复绑定同一场景配置、初始状态、通信/故障时序和区域快照 lineage SHA。control 始终执行现有确定性规则；treatment 候选只有通过 bundle/阈值/有限值检查和 owner/version/epoch/lease、fault fence、联盟 ACK、邻接、容量、备用与资源守恒投影后，才可在隔离 arm 进入下一周期。安全采用不形成线上 ACK，不改变 D4 authority。manifest 拒绝缺 arm、跨 arm 实际快照不同、hash/truth/非有限值等不完整证据。新增专项 20/20、相关回归 132/132、D4 全量 **469/469 passed**。本轮未运行正式保留 seed episode，D6 sidecar 未接入，outcome、paired non-degradation、counterfactual、causal、PPO、assist 和 authority 均 unavailable/false。
