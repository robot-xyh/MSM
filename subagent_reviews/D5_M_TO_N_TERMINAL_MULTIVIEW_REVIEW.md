...
## 2026-08-10 时序几何层对 M 对 N 的作用边界

D5 已增加相机内的有状态时序几何关联。相同 `local_track_id` 按资源、相机和流分别维护，短时
漏检可在 0.25 秒内保留带增长协方差的匿名证据；全局引用改变需要连续两个实测帧和足够代价
优势。该机制可减少联盟成员各自视场内的单帧跳变，并为 D6 提供 pending、held、expired 和
recovered 事件。

这一能力不把多个相机的局部轨迹自动合并为同一目标，也不替代现有跨视角几何、共同可见窗口、
联盟成员确认或 D4 分配合同。每个相机的 `measured_assignments` 仍需由跨视角层核对同一中心
`global_track_id`、时间和计划版本。predicted/lost 记录不能作为第二个联盟成员的实测锁定证据，
也不能形成 D7 控制许可。

2026-08-10 的 13 项 D5 专项和 904 项全量测试覆盖确定性接口。main 已在长距离 runtime 中
按相机接入该接口，联合专项为 23 项通过；只有实测关系驱动发现、提示和完成，coast 不授权。
这关闭了相机内状态接线，不关闭 M 对 N 跨相机共同视场、部分可见、交叉目标、多 seed 和物理
共同完成 P1。

冻结 seed `20260810` 的 D6 离线报告是旧 v2 基线，准确率 `0.9979317477`、短缺口 3、绑定
切换 7、交叉窗口可评价 `3/31`，并保持 `fail_closed`。它没有运行新 v3 时序接线，不能作为
新算法的 M 对 N 性能证据。

## 2026-08-10 真实 AirSim 长距离旧 v2 证据对 M 对 N 的边界

main 已在 ComputerVision 中运行原始尺度 `coverage_safe` 20 秒取证。seed 为 `20260810`，
20 个 `Quadrotor1` actor 距中心约 2.8-3.2 千米、速度 50 米/秒，拦截相机距目标群约
500 米，逻辑更新率为 100 Hz。中心执行偏航/俯仰二维扫描，拦截相机执行二维提示。

主场景发现和五帧确认均为 `20/20`，拦截提示为 `20/20`，观察和完成为 `19/20`。
1934 条几何关联的准确率为 `0.9979317477`。连续可见段身份切换为 0、纯度和连续性均为
1.0；仍有 3 次短缺口中断、48 次长期重发现和 7 次几何绑定切换。31 个预定交叉窗口仅 3 个可评价，
可评价窗口均无身份切换。专项总门直接失败于短缺口中断 `3 > 0`。

取证包保存 98 幅原图和 98 幅在线标注图，19/20 条全局航迹形成双相机图像证据。在线标注和
关联不使用 actor/object/truth ID，真值只进入离线混淆矩阵，`global_track_id` rewrite=0。
中心轨迹来自 `synthetic_d1_d2_fixture`。场景仍只有一个中心相机和一个拦截相机，因此不证明
多个联盟成员的共同可见、共同完成、三角定位或物理拦截。中心相机 3 千米原始图接近 AirSim 渲染/像素
极限，detect 框不能作为真实光电辨识证据。短漏检、交叉覆盖、几何绑定、真实误差注入和多 seed 继续
是 P1。

2 倍网格诊断达到 `20/20` 双相机证据，但关联准确率为 `0.9892194912`，几何绑定切换增至 42，
短缺口仍为 3，总门仍失败。该诊断只说明放大目标对可见性和框重叠歧义的影响，不替代原始尺度
主场景。本轮没有运行图神经网络。

## 2026-08-10 窄视场顺序观察补充

D5 在全时可见两相机基线之外增加了窄视场顺序观察。默认 20 个目标在三维空间前后、高度
错列，A/B 参考像面形成 10/6 对交叉或明确近距交叉。A 先扫描并确认本地匿名轨迹，再把已绑定
的中心航迹逐条提示给 B；B 同一时刻只观察提示方向，不要求在一个画面内看到全部目标。该过程
更接近 M 对 N 场景中局部相机分批建立关系链的情况。

标准 seed 表明，机械转速达到 180 度/秒也不等于形成连续覆盖：1.8 度光轴步长会跨过 0.621 度
视场。把步长限制为视场宽度的 80% 后，理想模型在 1.61 秒内完成 20/20 两级链，10 个 seed
均无 ID 切换、重复或全局编号改写。该结果仍只有一个 B，提示使用精确三维航迹，且无误差、
漏检、遮挡和通信中断。多个拦截相机之间的部分可见重叠、异步提示、联盟共同锁定和真实图像
关联仍属于开放 P1；图表没有执行图神经网络。

## 2026-08-10 两相机二十目标理想基线边界

D5 已用两台相机和 20 个运动目标验证中心侧两级配准。A 侧先把中心三维航迹与 A 匿名像素
轨迹匹配，再把同一批已绑定航迹投影到只提供像素轨迹的 B 相机完成第二级匹配。目标数由参数
决定，算法使用完整 N×N 代价矩阵，没有 2v2 或 5v5 数组假设。10 个 seed 的两级和端到端
准确率均为 1.0，`id_switch_count`、重复分配、未匹配、在线 truth 使用和中心编号改写均为 0。

该实验只有一个中心相机和一个拦截相机，所有 20 个目标在两个视场中持续可见，且相机几何、
时间和像素观测均无误差。它证明两级投影、时间窗代价和匈牙利关系链可作为 M 对 N 研究的
确定性对照，不证明多个拦截相机部分可见时的共同目标、三角定位、联盟成员共同锁定、遮挡恢复、
异步通信或物理完成。三层 60 节点图是候选关系示意，没有执行图神经网络。现有 M 对 N P1
状态不因本次理想结果关闭。

## 2026-08-03 A3 v3 正式来源对 M 对 N 的边界

A3 v3 正式来源已经完成 `104/104`，覆盖 sequence `0-103`、104 个互异 seed
`24000-24103`，development/future-held-out 为 `72/32`。来源提交为
`64dfc088cf4990304a95b2a362b905136092c70a`，request SHA-256 为
`9d16ef30180584b3df8a5af82b082d9427172f118c6e438e602d4cd291508dcc`。原失败 seed
`24097` 已形成 1936 个样本；该条和聚合在线 truth 使用、中心 ID 创建/改写均为 `0/0/0`。

该结果关闭 A3 v3 主动视觉来源未生成项，为后续研究多资源相机观察意图提供冻结输入。它没有
证明 M 对 N 多相机共同可见、跨视角配准准确率、联盟共同完成或相机动作收益。当前没有训练、
validation 消费、future-held-out 模型消费或 D6 独立审计，运行与控制权限继续关闭。M 对 N
在线主线仍使用确定性投影、几何门、版本/联盟合同和中心只读 `global_track_id`。

旧 `6737b44` failed output 不得恢复或迁移。本次状态同步只使用 main 提供的 manifest 级证据，
没有读取 future-held-out 样本载荷。

## 2026-08-01 A3 v2 候选对 M 对 N 的边界

本次行为克隆输入覆盖 5v5、20v20、50v50、100v100 和 200v200，按实际相机与候选动作数量
生成特征，不写死 N。test 各规模精确动作准确率为 `0.960317`、`0.974937`、`0.970615`、
`0.970142`、`0.955918`。这些总体值主要来自 hold/reacquire，不能证明多相机共同锁定、联盟
完成或主动观察收益。

模型对 `observe_target` 与 `search_sector` 的 test 召回均为 0，侦察相机精确动作准确率只有
`0.656527`，因此无法为 M 对 N 场景提供可靠的主动跨视场调度。development precheck 已失败
关闭，A3/R0 paired shadow 未启动。当前跨视角注册、联盟版本、成员 ACK、友方冲突和
`global_track_id` 只读规则仍全部由确定性路径处理。

该候选只允许 development shadow 复现，不发布任何相机动作，也不改变 D3/D4 联盟或 D7
导引门。feature-boundary OOD 为 0 不能替代多相机几何、遮挡、通信退化、AirSim 或真实相机
分布外验证。后续若研究少数意图，应使用新的 M 对 N 开发/评估 seed，并保持当前 test 为冻结
失败基线。
本次相关回归为 `35 passed in 4.29s`。训练批次内 D5 全量为
`779 passed, 2 warnings in 102.40s`，收尾复跑为
`779 passed, 2 warnings in 124.10s`；这些测试不构成 M 对 N 物理共同窗口或运行收益证据。
...
runtime 中退化为一系列单相机图。D5 已增加有界活跃相机快照：本次实测批次锚定图，其他相机
只有在双时间戳、外参年龄、missed-frame 和 TTL 合法时才以原匿名实测状态参与。快照不预测、
不改编号，不改变中心 `global_track_id` 所有权。

异步同目标 fixture 已形成 `2` 节点、`1` 边；不同目标没有形成边。5v5 seed `1000`、
...
为 0.720073，在线 truth、同相机互斥违规和中心 ID 改写为 0。新增训练视图覆盖遮挡重现、相似
运动干扰和独立尺度抖动。

该证据仍固定候选拓扑，没有验证 M 对 N 联盟中部分视场重叠、候选门重新构图、多个 primary
不同可见窗口或物理完成。训练来源为 dirty，D6 外部审计和 G1 assembler 未运行，因此候选不能
进入 M 对 N 在线辅助路径。clean commit 重跑和真实匿名多相机回放完成前，确定性投影、几何门、
受约束聚类和中心绑定继续作为默认方案。
...
clean `f80b5bd` 三 seed 10 秒冻结重放中，终端路径配对中位耗时下降 `15.34%/15.88%/18.27%`。逐帧核心、最终 binding、操作数哈希保持；在线真值使用和中心 ID 改写为 0。该结果降低动态 M/N 图候选的索引开销，不证明高威胁联盟共同可见、第二 primary 连续锁定、长时线性或物理完成，这些 P1 状态不变。

## 2026-07-22 f80b5bd M 对 N 集成边界

main 在 clean `8f86192 -> f80b5bd` 上完成 nominal 200v200、10.0 秒、seeds `42000-42002` 的统一三维对照。D5 终端关联累计耗时三 seed 均值为 `2.545876 -> 1.974446 s`，约下降 `22.45%`；主动视觉为 `4.174315 -> 4.183797 s`，基本持平。投影 DTO 缓存命中/未命中逐 seed 保持 `68/48`、`71/48`、`70/48`，最终 binding 保持 `22/29/28`。

逐条视觉 binding 与主动视觉载荷语义相同。跨提交审计只处理独立运行生成的 D3 opaque `plan_id`，按计划出现次序和版本建立映射；ACK 原始来源载荷 SHA-256 先行验证，owner、version、coalition、`global_track_id`、command 等 M 对 N 业务字段仍逐项比较。三个候选 episode 均 finite，online truth use=0，D1/D2/D3/D5/D7 最终数量相同。

该次证据中的中心预测工作区只发生在单次 `process()` 的同量测时刻相机批内，不跨调用。完整局部候选和中心候选仍参与投影、门控、聚类和 Hungarian binding，也不授予 D5 创建或换绑中心身份的权限。2026-07-26 新增的跨调用活跃相机快照只复用匿名实测相机状态，不改变这里所述中心预测作用域。该证据证明动态 M/N 输入在完整状态机中保持业务等价，但不证明高威胁联盟共同可见、第二 primary 连续锁定、物理拦截或长时单次成本线性；这些 P1 仍开放。

## 2026-07-22 M 对 N 中心预测复用边界

当前中心投影矩阵仍按实际局部节点数和中心候选数构建，不设置 2v2、5v5 或 200v200 特例。优化只把单次调用内同一量测时刻的中心 position/covariance 预测共享给多个相机：seed 42000 短/长冻结日志从 `76/715` 份相机级预测物化收敛为 `23/116` 份时刻级只读预测。`13615/499505` 个投影单元、`13415/472288` 个 binding 单元、全部合法候选和 Hungarian 唯一绑定继续保留。

配对 10 秒路径平均单次成本中位数下降 `26.6%`，业务/binding/操作数哈希不变，online truth use 和中心 ID 改写为 0。该结果证明 M/N 动态输入路径可减少重复物化，不证明 M 对 N 规模线性、真实多相机共同可见、第二 primary 连续锁定或物理完成。当前源码三种子系统复跑已由上节关闭，D6 正式操作数聚合仍是 P1；AirSim、G1、assist 和 authority 状态不变。

## 2026-07-22 200v200 clean 运行边界

提交 `8f86192` 已在统一三维 200v200、10 秒、seeds `42000-42002` 中持久化 D5 性能快照。终端关联阶段三种子均值为 `2.5459 s`，相对旧 clean 候选下降 `5.7%`，调用次数逐 seed 不变。seed 42000 处理 2493 个图节点和 33315 次局部匹配对比较；在线 truth 使用和中心 `global_track_id` 改写为 0。

该证据证明 D5 的动态 M/N 输入、匿名局部轨迹和性能诊断可进入完整状态机。短长序列单次成本仍增长 `2.423x`，所以不能据此认定 200v200 实时性已经闭合。D6 三种子仅为 clean descriptive calibration，也没有证明高威胁多资源联盟的共同可见、第二 primary 连续锁定或物理完成。冻结跨视角图 paired shadow 和本节系统集成计时继续分开评价。

## 2026-07-22 paired shadow 对 M 对 N 的证据边界

正式 paired shadow v2 已覆盖 `high_threat_m_to_n` 场景标签及 5/20/50/100/200 五个规模标签，
并在 20 个保留 seed 上完成 45 个场景规模单元的同图规则/模型对照。模型边级和簇对级 F1 均为
1.0，候选召回为 1.0，45/45 cell 无质量退化；同相机候选边、在线真值和中心
`global_track_id` 改写均为 0。

该证据仍是合成局部 tracklet 图评估，不等于实际 M 对 N 联盟运行。当前保留图的
`shared_global_track_count` 全部为 0，无法评价多个中心投影候选或共享投影线索存在时的分层性能；
尺度差、尺度率差和角速度差对离线标签接近确定性可分，单特征最佳方向 AUC 约 0.9973。因此满分
不能用于宣称多资源共同可见、第二 primary 连续锁定、联盟 ACK、跨节点重获取或物理完成。

后续 M 对 N 验收应保留 v2 冻结阈值，另建包含部分视场重叠、同运动困难负样本、异步相机、外参
漂移和 `shared_global_track_count=1` 的独立评估集。D6 独立审计、真实/代表性多相机回放和联盟
合同闭环完成前，G1、assist、authority 继续关闭，规则回退保持启用。

当前最终源码的 paired-shadow 专项为 `5 passed in 3.21s`，D5 全量为
`534 passed in 141.66s`。下列早期章节是当时的管线状态，当前同图评估指标只采用 v2。

## 2026-07-21 有界候选预算修复（历史，已由 v2 更新）

四相机局部困难图的最大节点数为 17，最大可能跨相机度数为 14。旧最终 cap=8 小于该局部几何需求，
造成 clean canonical test 候选召回降至 `11409/16698=0.683255`。几何门只拒绝 21/370,211 个
pair，主要损失来自最终 cap 删除 125,158 条门后边。

默认 cap 现与前置预算统一为 24。该常数覆盖当前四相机局部图，同时保留每节点度数 24 和边数
`<=12V` 的稀疏上界；它不是取消 M 对 N 的候选预算。在线排序不使用 evaluator truth，也不改变
同相机互斥、中心绑定和 `global_track_id` 所有权。seed 5 困难帧的软件回归得到候选召回 1.0、实际
最大度数 12；D5 全量 529 项通过。

该修复尚未在 4,500 帧 clean corpus、200 规模真实运行图或保留集上复跑。旧数据支持 pass 重新降为
历史证据。main 需先重建 clean corpus 和组合视图，再重训、运行 held-out 与 paired shadow；此前
不能形成 M 对 N 模型准入或 G1/assist 权限。

## 2026-07-21 保留集对 M 对 N 的验证范围（历史管线阶段）
...
## 2026-07-21 Composite 训练入口对 M 对 N 的边界（历史预检）
...
## 2026-07-21 Tracklet 困难样本对 M 对 N 的边界（历史首轮语料）
