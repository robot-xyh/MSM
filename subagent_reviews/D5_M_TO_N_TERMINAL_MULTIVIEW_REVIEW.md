...
配对 10 秒路径平均单次成本中位数下降 `26.6%`，业务/binding/操作数哈希不变，online truth use 和中心 ID 改写为 0。该结果证明 M/N 动态输入路径可减少重复物化，不证明 M 对 N 规模线性、真实多相机共同可见、第二 primary 连续锁定或物理完成。当前源码三种子系统复跑和 D6 操作数聚合仍是 P1；AirSim、G1、assist 和 authority 状态不变。
