...
中心预测复用只发生在单次 `process()` 的同量测时刻相机批内。完整局部候选和中心候选仍参与投影、门控、聚类和 Hungarian binding，缓存不跨调用，也不授予 D5 创建或换绑中心身份的权限。该证据证明动态 M/N 输入在完整状态机中保持业务等价，但不证明高威胁联盟共同可见、第二 primary 连续锁定、物理拦截或长时单次成本线性；这些 P1 仍开放。
