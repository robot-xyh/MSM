...
## 2026-08-03 动态任务基数与联盟边界

场景 `task_count` 只描述配置中的标称目标数。D4 在线任务集合由当前 D2/D3 唯一航迹假设
决定，可因虚警、新生目标或动态增减短时超过标称值。每条在线任务仍只能绑定一个中心所有
的 `global_track_id`；需要多个资源的目标通过 `required_member_count` 和联盟成员集合表达，
不得复制任务绕过身份唯一性。
...
`collecting_acks` 无法在再次执行时转为 `committed`。缺确认时继续
`execution_allowed=false`，符合原子联盟安全合同。

失败的 episode 级分类为：16 个 D3-D4 计划代次错位，11 个当前代次 ACK 晚到但没有
触发中心正常路径重评，1 个有界重发和终止尾窗不足。另有 35 个现有通过样本也存在旧
D4 计划与最后 D3 计划错位。当前 M 对 N 验收必须增加“最后 D4 联盟绑定最后 D3 计划”
这一条件，旧计划 `committed` 不能证明当前计划可执行。

本轮没有修改联盟算法或降低 ACK、版本、epoch、lease、成员身份与原子提交门。D4 代码
未变，正式 R0 未重跑，当前全量回归 903/903。main 修复计划顺序、ACK 事件重评、重发和
尾窗后，需从新 clean source 重跑完整 900 项。逐 cell 分类见 D4 正式 R0 联盟确认
专项报告。

## 2026-07-29 v5 外部评价与联盟边界

M16N20 来源独立外部评价只检查区域资源 actor 和置信校准器，不形成 M 对 N 联盟。外部
数据 63 帧与旧开发数据精确可观测键零重合，但冻结 actor 没有匹配 2 个规则安全正动作，
actor-derived 正类分母不可用。固定 0.60 门通过 0/63，全部规则回退。

该结果没有选择联盟成员，没有生成 D3 后继计划、成员 ACK、原子 commit、secondary
takeover 或 D7 控制许可。v5 未注册、准入关闭，coalition commit 权限为 false。现有
中心、二级和完全分布式路径继续使用确定性 owner/version/epoch/lease、必要成员 ACK、
故障围栏和规则区域建议。

本次不关闭 M 对 N 的自主成员形成、reserve 激活、成员补位、重构、时序可达性或物理
连续性 P1。D6 已完成独立复核且结果与 D4 一致，但 actor-derived 正类仍为 0，正类
分母不可用。来源独立 actor 正类和 D3 successor 均成立前，不得把 v5 外部负类拒绝
结果解释为联盟算法或联盟收益证据。
...
`feature_ood`，模型实际执行为 0。该结果不产生联盟成员、确认或原子提交。M 对 N 路径继续
使用确定性规则与既有 ACK/lease/epoch 门控。

后续复合训练视图可覆盖默认 8 区域主要特征并补充非零区域动作，但区域建议仍不等于联盟
形成。新候选即使通过运行预检，也必须独立满足 D3 成员计划、required-member ACK、原子
commit 和物理窗口，才能进入 M 对 N 证据链。2 区域几何仍不在当前声明适用域。
...
request-replan。它不形成 M 对 N 成员联盟，不修改联盟成员，不授予执行权。候选输出进入
隔离 treatment 后，仍必须通过原子联盟的 required-member ACK、current plan/epoch/lease、
有效 partition generation 和确定性安全投影。

本轮 420/420 校准 gate-pass 只证明候选在训练域内可进入后续隔离试验。seed 1000-1019 的
center failure、secondary failure 和 active risk 联盟采用尚未运行，因此成员确认、严格
后继计划、物理连续性和规则基线非退化仍是 P1。联盟状态机、异步 ACK 累积和分区失败关闭
逻辑没有修改。

## 2026-07-26 区域学习与联盟证据边界

本节中的 0/20 数据属于旧冻结候选。新版候选尚未运行保留 seed，不能用 calibration
420/420 替代联盟采用证据。
