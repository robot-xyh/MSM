...
**2026-07-21 运行时确认边界同步**：`d4-region-resource-runtime-ack-evidence-v2` 区分新执行计划采纳和同代评估刷新采纳。后者要求前序与当前资源-目标 binding、coalition ID/version 和 member role 完全一致，因此只能证明区域建议在既有联盟上被评估采用，不能证明联盟形成、成员 ACK 或执行状态变化。真实 main 5v5 刷新集成与篡改专项 5/5，运行时专项合计 33/33，该阶段 D4 全量 430/430；加入区域 reward 合同后当前为 449/449。该证据不是 `CoalitionMemberAck`，不能证明物理执行或策略收益；冻结 900 episode 仍没有对应 runtime 字段，PPO、assist 和 authority 保持关闭。
