...
**2026-07-21 配对干预与联盟边界同步**：保留 seed 1000-1019 的规则 control/候选 treatment 合同复用区域 `coalition_ack_complete`、owner/plan/version/epoch/lease 和 fault fence。任一必要成员 ACK 不完整、联盟 generation 旧、租约到期或故障栅栏生效时，候选不得进入 treatment 下一周期；安全投影后的区域配额建议仍不能形成、解散或改写目标级联盟。新增专项 20/20、D4 全量 469/469。该合同只建立同输入实验边界，未执行正式 20-seed 联盟 paired shadow，也没有成员级物理 outcome 或因果证据。
...
**2026-07-21 运行时确认边界同步**：`d4-region-resource-runtime-ack-evidence-v2` 区分新执行计划采纳和同代评估刷新采纳。后者要求前序与当前资源-目标 binding、coalition ID/version 和 member role 完全一致，因此只能证明区域建议在既有联盟上被评估采用，不能证明联盟形成、成员 ACK 或执行状态变化。真实 main 5v5 刷新集成与篡改专项 5/5，运行时专项合计 33/33，该阶段 D4 全量 430/430；加入区域 reward 合同后当前为 469/469。该证据不是 `CoalitionMemberAck`，不能证明物理执行或策略收益；冻结 900 episode 仍没有对应 runtime 字段，PPO、assist 和 authority 保持关闭。
