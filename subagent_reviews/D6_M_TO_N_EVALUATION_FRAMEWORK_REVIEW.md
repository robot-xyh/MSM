...
## 2026-07-31 高威胁 clean smoke

clean commit `49e43ea` 的 6-cell smoke 覆盖 5、100、200 三档和 seed `7/17`。当前
计划标识、版本、时期、租约和 49 个多成员联盟目标均为 `6/6` 通过。权威发布、计划
身份和计划确认均为 10，通信处置 16101 条通过逐记录审计。

4 个 100/200 规模重规划 cell 在新计划发布后仍输出旧计划区域建议，且没有匹配最终
计划的建议。该现象会使现有 targeted/full 正式门失败关闭。它不改变已闭合的联盟，但
破坏建议证据的代次完整性，不能通过只过滤全部历史记录解决。正式 900-cell 应在运行时
修正建议快照失效与发布前代次核对后再启动。

ID Switch 完整指标为 `3/6 available`，不可用项继续保留原因和部分下界。100/200
规模未达到实时。该 smoke 只用于准入判断，不更新历史正式高威胁 M 对 N 通过率。
...
episode 的旧计划区域建议仍使建议聚合失败关闭。后续 clean smoke 证明抽取样本还存在
发布时错代，需要同时定义 superseded 证据口径和发布前当前代次门。
