...
## 2026-08-01 多资源末端本地身份分组补充

M 对 N 场景允许多个资源同时观察或拦截同一 `global_track_id`。不同资源、不同相机使用不同
`local_track_id` 是合法状态，不能直接计为末端 ID switch。D6 当前计数器只按全局航迹分组，
在多成员联盟中可能把多个本地流拼成一个时间序列。本项列为 P1，关闭前不得将该指标用于
M 对 N 正式 actual-execution 验收。

关闭条件是以全局航迹、资源和相机/流身份组成固定分组键，并验证跨流稳定为 0、单流真实
切换可计数、记录顺序不影响结果、缺流身份保持不可用或走版本化兼容规则。2026-08-01 的真实
AirSim 2 对 2 受控专项不含同一全局航迹的多资源/多相机观测；其中两个诊断计数均为各自流内
变化，只能作为风险发现样本，不能替代 M 对 N 校准。

## 2026-07-31 高威胁 clean smoke 修复后复核

clean commit `b063535` 的 6-cell smoke 覆盖 5、100、200 三档和 seed `7/17`。当前
计划标识、版本、时期、租约和 49 个多成员联盟目标均为 `6/6` 通过。权威发布、计划
身份和计划确认均为 10，通信处置 16101 条通过逐记录审计。

12 条区域建议全部匹配发布时最新计划，发布时旧代为 0。4 个 100/200 规模重规划 cell
均补齐最终 v2 建议，最终计划建议覆盖和低层 clean formal 均为 `6/6`。历史 v1 建议仍
保留，D6 按其发布时有效代次审计，没有通过过滤记录改变结论。

10 条当前代故障诊断均为 shadow、非 assist、正式决策未改变。没有消费记录时采用保持
unavailable。D4 建议代次预准入通过；正式 targeted/full posterior 仍要求冻结 900-cell
execution plan 和规范分片。ID Switch 完整指标为 `3/6 available`，100/200 规模未达到
实时。该 smoke 不更新历史正式高威胁 M 对 N 通过率。

## 2026-07-31 高威胁 v5 时期租约复验

v5 开发态批次的当前计划标识、版本、时期、租约和联盟闭合均为 `100/100`。当前计划
包含 644 个多成员联盟目标。151 次权威 D3 发布对应 151 个不同计划身份和 151 次计划
确认，48 次同身份评价刷新不产生第二次发布或续租。

v4 的时期/租约不可用缺口已在开发证据层关闭。D4 航迹回退在 28 个 episode 维持任务
连续性，共 391 个快照；当前联盟仍闭合，D7 当前航迹门控不变。51 个真实重规划
episode 的旧计划区域建议保留为 v5 历史证据。修复前 smoke 证明抽取样本存在发布时
错代；修复后 `b063535` smoke 已验证发布时旧代 0、最终计划覆盖 `6/6`。

100 项均为 dirty source，不能与历史正式范围的 `72/100` 合并，也不能替代新门禁下的
900-cell R0。D2 身份切换 12 项不可用和 200 对 200 非实时仍为 P1。

## 2026-07-30 高威胁 100 项开发态修复复验

v4 开发态批次覆盖 5、20、50、100、200 五个规模和 seed `1000-1019`。最终当前计划
标识/版本和联盟闭合均为 `100/100`。151 次 D3 权威发布没有同身份重复，
`payload_digest_mismatch` 和 `cross_binding_invalid` 均为 0。v3 的
`GT3D-000011` 最终快照断点已在开发证据层关闭。

当前结果不能与历史正式范围的 `72/100` 合并为提升量。两批来源、代码状态、运行长度和
门禁版本不同；v4 仍是 dirty-source、2 秒开发复验，formal acceptance 为 `0/100`。
D3 区域时期/租约对照仍不可用，补齐后需要完整重跑 clean formal 900 项。

## 2026-07-30 当前计划联盟审计补强

M 对 N 的联盟成功分母现在绑定最后 D3 计划。D6 逐目标核对最后 D4 决策是否属于同一
`plan_id/plan_version`，并核对当前 required/acked/missing、原子提交、执行授权和租约。
旧计划的 committed 联盟不能计入当前计划。`collecting_acks` 和 `proposed` 继续失败关闭，
且失败原因保留目标标识。

联盟目标分母不再使用“`coalition_id` 非空”作为充分条件。普通一对一 assignment 即使
携带该字段也不进入原子提交分母；同一目标当前分配多个资源，或同代 D4 明确要求提交时，
才执行联盟审计。

审计器已与 main runtime 的逐消息通信处置落盘合同核对一致；旧 episode 没有该文件时
只报告 unavailable。本轮完成代码和单元回归，没有重跑正式 900 项，因此不更新现有
M 对 N 通过率或正式分母。

## 2026-07-30 正式 R0 高威胁 M 对 N 结果

完整 R0 审计首次给出高威胁 M 对 N 在 5、20、50、100、200 五个规模和 20 个 seed 上的
同一规则基线结果。该子范围共 100 个 cell，clean formal、实验矩阵资格和 D1/D2 generation
均为 100/100，skip 为 0，pending 全排空。

严格系统门通过 72/100。其余 28 项在 episode 终点仍为
`d4_fail_closed:collecting_member_acks`。按规模的失败数为 5、4、5、6、8。失败关闭说明
未完成的多成员联盟没有被误放行为完整执行；它同时暴露 D4/main 的联盟 ACK 收敛、lease、
timeout 和终止窗口 P1。

该结果没有同时到达、分波物理效果或学习算法对照。完整父矩阵仍为 900/5700，G1、A1、
A2、A3 尚未形成同范围 treatment。D6 保留 28 个失败 cell，不用 clean formal 或 generation
通过替代联盟完成。
...
资源策略评价。来源、运行分布、影子动作和实际采用分开报告。模型动作缺失和规则回退不改变
分布兼容布尔，但没有可辨识非零动作时不能形成 treatment。

D6 的 5 资源/5 目标、2 区域、6 帧确定性 fixture 为 6/6 OOD、模型动作 0、规则回退 6，
只用于合同回归。main 实际预检另有 5v5/2 区域 seed 2000 的 3/3 OOD，以及
200v200/8 区域 seed 2001 的 2/2 OOD。当前没有 treatment，不能进入 M 对 N 非退化分母。
分布内 no-op 回归虽通过分布门，采用和配对收益仍 unavailable。至少 20 个预注册未见
seed、兼容运行记录、完整采用链和独立 R0 尚未提供。D6 只读评价边界及全部 false 权限
保持不变。
