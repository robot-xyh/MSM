...
当前 99fa 实物的目录与安全门通过，但整体未通过。证据实现摘要
`81968e0d...066e7f` 与当前摘要 `ff8c744e...8a1b7` 不同；单特征最高 AUC 为 `0.997340`；
五类扰动最低边/簇 F1 为 `0.563264/0.572845`。对应冻结门为 `0.98`、`0.9` 和 `0.9`。五类
扰动均未重新构建候选图，该限制在 JSON 中单独披露。
...
专项测试 `13 passed`，D6 全量 `943 passed, 1 warning in 80.56s`，`SHA256SUMS` 校验通过。
warning 是既有 Matplotlib `Axes3D` 环境提示。当前无 D6-owned P0；下一步不应启动正式 G1
矩阵，应先由 D5 在当前实现上重建证据并处理合成捷径和扰动退化。
