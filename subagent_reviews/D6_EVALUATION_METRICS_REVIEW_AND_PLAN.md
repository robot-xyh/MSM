...
## 2026-07-31 高威胁 clean smoke 评审

评审接受 clean commit `49e43ea` 的来源与核心合同证据。6 个 episode 的配置哈希、有限
状态、在线真值隔离、当前计划标识/版本/时期/租约和联盟闭合均通过。10 次权威发布、
10 个计划身份和 10 次确认守恒；49 个当前联盟目标和 16101 条通信处置完成审计。

评审不接受据此启动正式 900-cell。100 和 200 规模的 4 个重规划 episode 均在新计划
发布后产生一条绑定旧计划的 D4 `shadow` 建议，并且没有匹配最终计划的建议。第一条旧
计划建议是随后 superseded 的历史有效证据；第二条在发布时已错代。现有 formal 门将
四项失败关闭，clean formal 仅为 `2/6`。

该建议没有改写正式决策，不能据此放宽证据门。main/D4 应先修正建议快照生命周期和发布
前代次核对，再复跑同一 clean smoke。准入门为过时建议 0、当前建议覆盖 `6/6`、clean
formal `6/6`，其余计划和联盟合同不退化。完整 ID Switch 只有 `3/6 available`，大规模
实时性也继续作为 P1。
...
重规划 episode 的旧计划区域建议仍使 D4 建议聚合失败关闭；后续 clean smoke 已证明
抽取的重规划样本包含发布时错代，而不只是最终聚合中的历史记录。该项与最终计划绑定
通过并不矛盾，列为跨模块 P1。

通信处置 `100/100 available/verified`，当前联盟目标 644 个。D4 航迹回退 28 项、
391 个快照，没有放宽 D7 门控。200 对 200 墙钟均值/P95 为
`14.209/15.566` 秒，实时倍率均值 0.142。下一步是清理建议 supersession 合同、补齐
身份分母，并从 clean checkout 重跑规范 900 项。
