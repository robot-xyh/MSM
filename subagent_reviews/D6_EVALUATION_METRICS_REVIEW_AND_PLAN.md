...
## 2026-08-10 D5长距离视觉配准专项评审

评审接受 D6 的离线消费者边界。新入口只读取 main 的 episode 文件，按相机和全局层输出指标，
每项保留可用性、原因和来源。v2 缺少时序文件时不将保持、过期和绑定振荡写成零；几何预检不
进入实际交叉窗口分母。D6 不导入 D5 运行代码，不参与在线关联或控制，不创建、合并或改写
`global_track_id`。

冻结 seed `20260810` 的只读结果与 main 基线一致：`1930/1934` 正确关联，准确率
`0.9979317476732161`；连续可见身份切换 `0`，短缺口 `3`，长期重发现 `48`，几何绑定切换 `7`；
实际交叉可评分 `3/31`。重复分配、在线真值和全局编号改写均为 0。结果失败关闭，原因是有效
短缺口未归零、绑定振荡证据不可用以及交叉窗口数量和比例不足。

评审只接受“D6 离线接口和历史兼容已实现”。七项专项测试通过，D6 全量为
`1425 passed, 16 skipped, 1 warning in 129.29s`；v3 正例是合成夹具。真实 main 行结构和无表头
空 CSV 已进入兼容回归，空文件不会被补成零事件。真实 v3 episode、至少 10 seeds、交叉窗口
改善和跨模块 P1 结论仍由 main 后续组织，当前不得宣称 P1 已关闭。
...
状态、在线真值隔离、当前计划标识/版本/时期/租约和联盟闭合均通过。10 次权威发布、
10 个计划身份和 10 次确认守恒；49 个当前联盟目标和 16101 条通信处置完成审计。

12 条 D4 建议全部匹配各自发布时刻的最新正式代次，发布时旧代为 0。100 和 200 规模的
4 个重规划 episode 均在 v2 计划后形成 v2 建议，最终计划建议覆盖为 `6/6`。现有低层
formal 门未放宽，`formal_acceptance_eligible=6/6`。修复前报告中的 D4 advice 代次
阻断已关闭。

10 条当前代故障诊断均为 `shadow`、`assist_eligible=false`，正式决策没有改写。它们
按发布时当前代记录，不误记为 stale。原始制品没有消费记录，采用状态保持 unavailable。
D4 建议代次的 6-cell 预准入通过。正式 900-cell 仍需冻结 execution plan、规范分片和
存储条件后执行；完整 ID Switch `3/6 available` 和大规模非实时继续作为 P1。

## 2026-07-31 高威胁 v5 时期租约复验评审

评审接受 v5 100 个开发态 episode 的只读结果：最终计划标识、版本、时期、租约和当前
联盟闭合均为 `100/100`。151 次权威发布、151 个不同计划身份和 151 次运行时计划确认
守恒；同身份重复为 0，48 次评价刷新只产生诊断，不续租。

评审据此关闭 v4 的时期/租约 availability P1，但只关闭开发证据缺口。100 个 manifest
均为 dirty source，不能替代正式 R0。D2 身份切换仍为 `88/100 available`。51 个真实
重规划 episode 的旧计划区域建议保留为 v5 历史证据；修复前 smoke 证明其中抽取样本
存在发布时错代，修复后 `b063535` smoke 已验证发布时旧代 0、最终计划覆盖 `6/6`。

通信处置 `100/100 available/verified`，当前联盟目标 644 个。D4 航迹回退 28 项、
391 个快照，没有放宽 D7 门控。200 对 200 墙钟均值/P95 为
`14.209/15.566` 秒，实时倍率均值 0.142。下一步是补齐身份分母，并从冻结 clean
checkout 重跑规范 900 项。

## 2026-07-30 高威胁开发态 100 项修复复验评审

评审接受 D6 对 v4 100 个 episode 的只读重算，不接受将其登记为 formal 证据。五个规模
的最终计划标识/版本和当前联盟闭合均为 `20/20`，合计 `100/100`。当前计划共审计 644
个多成员联盟目标，v3 三个最终快照断点不再出现。

151 次权威 D3 发布对应 151 个不同计划身份，同一 `(plan_id, plan_version)` 重复权威
发布为 0。`payload_digest_mismatch`、`cross_binding_invalid` 均为 0。通信处置
195838 条全部通过逐记录格式检查，100 项文件记录数与 summary 对账一致。有限状态和
在线真值零使用均为 `100/100`。

D3 没有发布区域时期和租约对照字段，两项保持 `0/100 available`。离线身份指标 88 项
可用，可用部分合计 52 次，12 项保持不可用。下一步先补齐该跨模块合同，再从 clean
source 完整运行正式 900 项。v4 的 dirty-source、2 秒开发结果不能进入正式分母。
定向回归为 `12 passed`，D6 全量为 `1263 passed, 1 warning in 154.43s`；warning 是
既有 Matplotlib `Axes3D` 环境提示。

## 2026-07-30 正式 R0 当前计划绑定评审

评审接受 D6 对审计规则的补强。最后 D3 计划现在是当前代次的唯一基准。最后 D4 每个区域
必须与其 `plan_id/plan_version` 一致；可用时继续核对权威 epoch 和 lease。旧 D4 的
committed 状态不再作为新 D3 计划的联盟证据。

当前计划需要多成员协同时，D6 要求唯一 commit、成员集合闭合、原子提交、执行授权和有效
租约。`collecting_acks`、`proposed`、成员缺失、计划错代或租约失效均失败关闭并保留具体
目标和原因。逐消息通信处置文件是可选输入：缺失记 unavailable，存在则严格校验，不使用
summary 总数补造消息级证据。

联盟分母已按真实合同更正。单成员 assignment 的 `coalition_id` 只作为来源字段；只有
同目标多资源分配或同代 D4 `commit_required=true` 才要求原子提交。20 对 20 合同 smoke
由误报 16 对 2 修正为 2 对 2 并通过。

代码和测试已就绪，专项组合为 `27 passed`，D6 全量为
`1261 passed, 1 warning in 128.21s`。main runtime 已完成同代租约冻结、ACK 重评、
有限重发、重发耗尽失败关闭、尾部排空和逐消息处置落盘，D6 已核对消费合同。本轮没有
正式 900 项重跑。历史 `872/900` 只代表旧门禁；需在新的 clean source 上完整重跑，再
评审新结果。
...
v5 七文件布局、四类 evidence、`SHA256SUMS`、admission report v2、authority contract v2
和公共严格加载路径。结构与 D6 post-assembly v2 的预期一致，不需要兼容分支或门限调整。

新增正例在 D6 测试内生成冻结输入，最终 v5 由真实 `assemble_tracklet_g1_bundle()` 发布。
D5 严格加载器先确认该包有效，随后 D6 审计确认 lineage 文件摘要、900 条记录、900 个唯一
episode UID、准入报告三个 lineage 字段、六项 false 权限、D6 外审文件/内容摘要和十文件
运行实现摘要全部一致。对该生产产物篡改 lineage 或删除 lineage 均失败关闭。
...
main 退回意见成立。external audit 的权限字段由四项增加到六项后，继续使用
`d6.d5-g1-external-audit.v1` 会使同一 schema 具有两种字段语义。D6 已将主输出升为
`d6.d5-g1-external-audit.v2`。
输入 spec 和 consumer 字段集合没有变化，分别保留 input v1 和 consumer v1。

post-assembly 边界同时升级。当前 verifier 只接受 `d5.tracklet-model-bundle.v5`、
admission report v2、
authority contract v2 和 external audit v2。bundle v5 必须包含 held-out、paired-shadow、
paired lineage 和 external audit 四类证据。lineage 文件由 D6 独立解析，不只核对 manifest
中的计数。六权限、D6 文件/内容摘要、三类证据和运行实现摘要必须在 manifest、报告、权限合同
及 external audit 间完全一致。

external audit 和 post-assembly 输出中的模型晋级、G1 辅助、默认路径、分配、故障接管和控制
权限全部为 false。真实相机泛化、中心 binding 正确率和物理闭环结果仍为 unavailable。通过
结果只说明证据完整性，不触发 D5、D3、D7 或 main 运行路径。
...
2026-07-26，D6 独立复核 clean source commit `64cb865b...b05` 的正式 R0 制品。批次
`SHA256SUMS` 8834 项、D6 报告清单 3 项均通过。batch manifest 内容 SHA-256 为
`448b5ff1...94c5`，aggregate 内容 SHA-256 为 `dc84c90b...22af`。dataset manifest
SHA-256 `5ee284fd...247` 与 sidecar 绑定一致；sidecar 文件 SHA-256 为
`f0db1b13...1a1e`。2670 个 episode、2670 条 sidecar 记录和 2670 个稳定坐标精确对应。

正式结果包含 20 个 seed、2670 帧、16842 个节点和 4658 条候选边。4645 个时间合格真值对中
保留 4642 条真边，另有 16 条假边。微平均 precision、recall、F1 和 false rate 为
`0.9965650494`、`0.9993541442`、`0.9979576481` 和 `0.0034349506`。逐 seed F1 均值为
`0.9976519241`，bootstrap 95% CI 为 `[0.9953251507, 0.9995705026]`。标签和候选召回
声明全覆盖，硬违规为 0。

本次关闭正式 R0 单臂候选图证据缺口。它没有 G1 prediction、cluster、center binding、
control 或 physical result，不能写成 G1 模型收益或系统闭环收益。专项 fixture
`12 passed, 1 warning`，D6 全量 `1022 passed, 1 warning in 88.77s`。下一步只在具备
同坐标 G1 dataset 和独立 prediction sidecar 后比较 G1；真实相机泛化继续开放。
...
时延为 `1.0/0.0/1.0/0.8715935983 ms`。五类扰动最低边/簇 F1 均为 1.0，单特征最高
AUC 为 `0.7200734257`。在线真值、同相机候选边/互斥违规和 `global_track_id` 创建或换绑
违规均为 0。

最终结果为 `pass`，blocker 为空。输出目录为
`/tmp/MSM-d5-g1-current-runtime-d6-external-audit-64cb865-20260726-v2/`，JSON 文件/内容
SHA-256 为 `24c8b0cd...ad7d` / `f17acecf...135f`。同输入重复运行四项输出逐字节一致。
...
checkpoint 均为 complete，各 45/45；shard 8、18 为 paused，各 21/45。总计 177/900。
新 D6 专项不采用 `targeted_formal_d6` 聚合，独立复核五个原失败 cell：
...
五项均为 clean-formal，基础与矩阵 formal eligibility 为 true，generation contract 为
verified，integrity 为 true，episode/matrix/variant failure reasons 全为空；source、
plan、shard、cell result 和 artifact tree 完整性通过，D1 final 与 D2 consumed 一致，
...
全量 `1243 passed, 1 warning in 150.38s`。完整输出 SHA 校验、语法检查和限定路径
`git diff --check` 通过。warning 为既有 Matplotlib `Axes3D` 环境提示。
...
## 2026-07-31 D3 A1 来源独立 v2 外部审计评审

评审接受 D6 新增的只读外部审计接口。接口不导入 D3 高层评价代码，独立验证结果文件、冻结
合同、生成源、匿名数据集、模型 bundle、评价源码树和逐帧记录。审计前后输入摘要一致，
`SHA256SUMS` 全部通过。

评审接受以下事实：

1. 数据源为 100 个 episode、292 帧、seed 20000-20099；episode 和 frame split 分别为
   `60/20/20` 与 `178/57/57`。
2. 在线真值使用、禁止 truth/Actor/Object 身份字段、训练与正式 seed 重叠均为 0。
3. 安全换绑为 `13/110=11.82%`，教师完全匹配为 `8/110=7.27%`，负类规则保持为
   `182/182=100%`，均满足预注册总体门限。
4. 94 个拒绝帧的矩阵和绑定全部回退规则基线。非零修正 98、OOD 27；拒绝原因和场景分布
   与 D3 aggregate 完全闭合。
5. 重复资源、硬禁边、M 对 N 原子性、版本、规则矩阵突变和模型越权输出均为 0。
6. 数据集路径与 generation root 固定子目录一致，逐帧 inventory 独立 split hash 为
   `f1380dd6...ca5`；292 个规则矩阵摘要均按连续小端双精度字节独立闭合。
7. CSV 固定 21 列、292 行，与 JSONL 逐行不一致数为 0。R0、candidate、effective 各复算
   21637 条边，索引越界、资源容量超额、硬禁边和原子性违规均为 0；总体安全门使用
   independent effective 结果。

评审不接受以下扩展结论：

1. test 子组教师完全匹配为 `0/25`，不能把总体门通过写成各子组均通过或泛化问题关闭。
2. 本次没有读取正式 seed 1000-1019，也没有正式保留集准入结论。
3. 离线机器门不等于运行采用、计划收益、控制收益、物理拦截或生产准入。
4. D6 不能授予 runtime、assist、assignment、plan、control、physical、formal 或
   production authority；上述权限继续全部为 false。

评审结论为“离线结果完整性与预注册总体机器门通过，未准入”。专项 `18 passed`，其中新增
9 个失败关闭负例；D6 全量 `1348 passed, 1 warning in 139.42s`。后续由 main/D3 提供新的
预注册正式保留集和同键物理对照，D6 继续按只读方式审计。

## 2026-08-01 D1 GlobalTrack A95 物化配对评审

评审接受 D6 的只读配对接口及三面审计口径。外生输入由场景、离线真值和传感器观测构成；
sequence、内部模块发布、通信投递时间、消息顺序和传输摘要归入运行时总线面。D1/D2 业务面
不删除状态、协方差、双时间戳、身份或谱系字段，时序漂移造成的业务变化必须失败。

source `4166fe8` 的 seed 43000-43009 结果为外生输入等价 `10/10`、业务等价 `10/10`、
操作合同通过 `10/10`。运行时总线完全等价为 `0/10`，未观察到 D1/D2 分歧。候选批量处理
103609 个矩阵，执行 518 次批量特征值调用；reference 执行 103609 次标量 A95。候选更快
`6/10`，墙钟中位改善 `1.05%`，D1 包含式计时中位改善 `2.04%`。

评审结论为“业务等价通过，性能证据不足，候选保持 default-off”。该 10-seed 开发批次不构成
正式晋级，不修改既有正式结论，也未读取保留 seed 1000-1019。
专项 `13 passed`，D6 全量 `1397 passed, 1 warning in 136.01s`；既有 Matplotlib
`Axes3D` warning 与本次纯文本/数值审计无关。
