...
训练数据谱系、20-seed held-out、同权重 final paired-shadow、当前运行实现和三项安全计数。
D5 后续装配器不应再分别读取多个 D6 报告，也不能用调用方构造的正向布尔值代替实物复核。

审计使用固定形式化目录和严格 availability。seed、episode、场景规模单元、在线真值字段、
`global_track_id` 改写和同相机互斥违规只有在来源字段存在、类型正确、交叉谱系一致时才输出
数值。文件缺失或字段 unavailable 时 consumer 值为 null，整体失败关闭。

当前 99fa 实物的目录与安全门通过，但整体未通过。首次审计的证据实现摘要
`81968e0d...066e7f` 与当时当前摘要 `ff8c744e...8a1b7` 不同。D5 后续把 G1 evidence assembler
纳入运行时实现集合；D6 已按同一十文件集合和规范 JSON 对齐，D5 API 与 D6 独立实算均为
`41381db3...4b07`。

装配器后复核继续使用同一历史证据，没有增加实验样本。旧证据没有 assembler 哈希，
`tracklet_model_bundle.py` 也与当前文件不一致，因此实现证据 unavailable 和实现谱系 mismatch
同时成立。单特征最高 AUC 仍为 `0.997340`；五类扰动最低边/簇 F1 仍为
`0.563264/0.572845`。对应冻结门为 `0.98`、`0.9` 和 `0.9`。五类扰动均未重新构建候选图，
该限制在 JSON 中单独披露。

审计输出的 authority 字段始终关闭。即使未来 evidence audit pass，D6 也不直接授予模型晋级、
G1 辅助或控制权限。D5 需验证本审计文件和内容哈希后再执行自己的装配；main 启动 G1 作用域后，
D6 再使用 `learning_scope_formal_audit` 对实际采用、回退、在线真值、物理结果和同键 R0 做
第二层审计。

新审计位于独立 post-assembler 目录，主 JSON 文件/内容 SHA-256 为
`98bf9e02...c8ed` / `40a42af0...90d`，`SHA256SUMS` 校验通过，原审计未覆盖。专项测试
`14 passed`，D6 全量 `944 passed, 1 warning in 80.12s`。warning 是既有 Matplotlib
`Axes3D` 环境提示。当前无 D6-owned P0；下一步不应启动正式 G1 矩阵，应先由 D5 在当前十文件
实现上重建证据并处理合成捷径和扰动退化。
