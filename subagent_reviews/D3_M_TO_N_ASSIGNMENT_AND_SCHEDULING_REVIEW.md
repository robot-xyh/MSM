...
## v3 开发来源请求状态（2026-08-01）

A1 v2 失败归因没有改变现有需求槽匈牙利、多机需求全有或全无、primary/reserve 角色或
版本化计划算法。v3 development data request 新增 100t100r 和 200t200r 高威胁 M-to-N
单元，并要求采集需求升高、需求降低、联盟不完整、主备角色变化和困难负类。该请求用于
后续辨别候选排序与需求槽结构影响，当前没有分配 seed、生成数据、训练模型或开放执行
权限。M-to-N 当前实现和既有试验结论保持不变。

## 来源独立 M-to-N 证据（2026-07-31）

A1 v2 来源独立评价已覆盖 `high_threat_m_to_n-100v100-v1` 和
`high_threat_m_to_n-200v200-v1`，各 10 个 seed，并与其余八个单元共同形成 100 个
episode、292 帧的冻结评价。整体机器门中 M-to-N 原子性违规、重复资源、硬禁边和版本
违规均为 0；正类安全换绑 `13/110=11.82%`、教师完全匹配 `8/110=7.27%`、负类
exact-R0 `182/182=100%`，均通过预注册门限。

高威胁 M-to-N 场景是当前分布外和投影拒绝的主要来源：两个规模单元合计出现 17 个
`feature_ood` 帧，并各有 20 个投影拒绝帧。该结果表明 all-or-none 安全外壳在新来源上
保持有效，也表明冻结特征范围对高威胁大规模场景仍较保守。由于评价未授予运行、分配、
计划或物理权限，这些统计不能证明联盟运行完成率或物理效果。D6 后续已完成离线完整性和
总体机器门复核；正式与物理证据仍为开放 P1。2026-07-31 D3 全量回归为
...
物理结果和同键 R0 非退化检查能力。当前没有实际 D6 A1 formal-scope 审计输出，旧
sidecar 的 runtime ACK、
