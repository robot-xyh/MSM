...
**D1-owned 无 P0；运行时代码和五项定向验证已关闭；正式 clean source 已冻结为
`1e5ed8ddcf27f375e922a447decfbd875d21bfdf`，execution plan SHA-256 为
`8804ecb4dd0513db55906905f031832711012974fc911546df40e09fb297d373`。shards `0/5/9`
已完成 `135/900`；原失败项 seeds `1000/1005/1009` 为 clean-formal，正式闭合 `3/5`；
seeds `1008/1018` 和完整 900-cell formal acceptance 仍开放。运行因磁盘只比 `20 GiB`
下限多约 `65 MB` 暂停**。完整诊断见
...
`per_checkpoint_prefix_rebuild_v1`，避免把快照范围与 replay-prefix 实现同时作为
treatment。required ID 只能来自同一 release cycle 的当前 source observations 和已物化公开
航迹 `latest_observation_id`。D1 不生成该集合，也不读取 truth、目标真实编号或 D6 标签。
...
上述 fixture 与 7 对微基准保留为候选形成历史，不能覆盖正式多种子判定。正式矩阵的
short seeds 为 1151-1160、每臂 2.2 秒；long seeds 为 1151-1153、每臂 10 秒。D6
记录 0 reused、0 failed。失败门为 short 更快 `5/10 < 8/10`、short D1 改善
`0.959611% < 1%`、short bootstrap 上界 `0.619827% > 0%`、short core 改善
`-0.256641% < 0.25%`、long core 改善 `-1.930083% < 0.25%`。long D1 改善
`2.361778%`、内部物化减少 `52.150746%`、RSS/D2 均值门通过，但准入要求全部门通过。

正式 bundle 位于 `research_modules/d6_evaluation_metrics/outputs/`
`d1_replay_prefix_summary_multiseed_20260725_formal_7d2e987_d6/`。该项没有改变 AirSim
producer、DTO、runtime bus DTO、坐标、时间或 episode 数据合同。证据仅覆盖三维质点
仿真；AirSim、硬件和正式融合精度保持开放。
...
long 3 pair 共 13 pair/26 episode，全部预注册 gate 通过。

| GAP | 当前状态 | D1-owned 证据 | 剩余关闭条件 |
| --- | --- | --- | --- |
| raw batch 到 `SensorScanFrame` 的逐量测和转换后集合重复检查 | **P1 正式准入已关闭；D1 默认已提升** | short/long scan-input 改善 `38.289241%/36.275282%`，core wall 改善 `4.252745%/4.916501%`；candidate closed `2665/2665`、fallback 0、online truth use 0；业务语义、实现身份、有限状态、审计守恒、RSS 和 D2 均值门全部通过 | 保持默认 candidate、显式 reference、实现 ID、失败关闭、fallback 和守恒回归；不得修改冻结 matrix/config 或历史正式制品 |
...
| D2 单 pair 尾部容量 | **P1 观察项** | D2 association short/long 均值增幅 `2.113047%/2.830616%` 通过冻结门，但 `short_seed_1125` 和 `long_seed_1121` 单 pair 增幅为 `15.778858%/14.408510%` | 保持均值门不变，补更长时容量、P95/max、峰值内存和抖动观察；不得把均值通过写成尾部风险关闭 |
| 系统实时、AirSim 和融合精度 | **P1 开放** | candidate 最低 RTF `0.204490 < 1.0`；正式证据仅为 2026-07-25 三维质点仿真 | 在冻结目标环境达到 RTF `>=1.0`，并独立完成 AirSim、目标硬件、实机/实飞、RMSE、NEES 和 NIS 验收 |
...
`ONLINE_BATCH_FRAME_DEFAULT_IMPLEMENTATION` 当前指向 candidate；builder/helper 未显式
selector 时统一使用 candidate，显式
`implementation=ONLINE_BATCH_FRAME_REFERENCE_IMPLEMENTATION` 可单参数回退 reference。
`candidate_default_enabled=true` 反映当前声明默认状态，显式 reference 仍由 selector、
实现 ID、路径计数和守恒诊断审计。

该正式结论不改变下述不透明来源缓存正式拒绝，也不构成系统实时或 AirSim 准入。
...
`_limit_covariance_diagonal` 累计 `0.822 s`，`eigvalsh` 调用 70,183 次、累计
`0.847 s`。D1 已实现默认关闭的六维 Cholesky 快检候选。严格正定矩阵直接返回；失败矩阵
只有在归一化行列式通过 `9.094947017729282e-13` 安全门后才直接返回；其余矩阵完整执行
原有特征值检查、正半定投影和对角回退。半正定、近奇异、机器精度附近不定、非有限拒绝、
业务 reason 和发布操作数均未改变。当前实现 ID 为
`d1.fusion.covariance_psd_check.cholesky_6x6_relative_determinant_guard_then_eigvalsh.v2`。

确定种子专项使用 2,000 个六维矩阵、每样本 10 轮和 9 次交替采样。reference/candidate
中位墙钟 `0.558490 -> 0.588263 s`，candidate 慢 `5.33%`，`0/9` 配对更快。20,000 次
候选尝试形成 19,800 次成功和 200 次回退；输出 covariance 和 reason 摘要严格一致。
D1 全量 `404 passed in 21.39s`。

| GAP | 当前状态 | 证据 | 剩余关闭条件 |
| --- | --- | --- | --- |
| 六维 PSD 判定热点 | **P1 候选已实现并完成模块验证；性能准入否决** | v2 显式实现 ID、默认关闭、安全门、attempt/success/fallback 守恒、严格正定/近奇异/半正定/机器精度不定/一般不定/非有限/非别名回归；当前代码 9 次交替正式重跑 | candidate 慢 `5.33%` 且 `0/9` 更快。安全门前旧计时作废，不启动 main 多 seed A/B；只有目标处理器或真实冻结负载改变后才重新评估 |

该候选不关闭系统实时 P1，也不形成 AirSim、硬件、RMSE、NEES 或 NIS 证据。
...
`064cbb979d3bab68fee995e476df25709eb666db`。两臂共同包含 D1 PSD 修复
`064cbb979d3bab68fee995e476df25709eb666db` 与 D2 误警审计修复 `e4147b8`。正式
manifest SHA-256 为
`40669d10fff8367aa31e24624bab802d8bc3de6b01aaa1e5c92d054753ed93ec`。

| GAP/合同 | 当前状态 | 证据 | 剩余关闭条件 |
| --- | --- | --- | --- |
| D1 发布协方差完整正半定合同 | **P0 已关闭；正式多 seed/长时回归通过** | seed 1103、200v200、10 s 在发布时刻 `7.85180018473111 s` 暴露 `global_track_031` 非正定输出。限制前最小特征值 `+7.506060086e-04`，单个交叉项 pairwise 裁剪后为 `-9.247657800e-04`。故障前 58,776 次 scalar/vectorized 双算逐元素一致，确认是共有数学缺口。当前使用完整正半定投影与确定性对角回退；固定失败、1-6 维各 96 组随机/极端、路径等价、双时间戳/谱系/6 s fixed-lag 回归通过。正式 v3 的 26/26 episode 未再发生 PSD/D2 断点 | P0 不再开放。保持 D2 严格 PSD 门和性质回归；RMSE/NEES/NIS、AirSim、目标硬件及系统实时性属于独立 P1 |
| 预测/重放协方差成对限制热点 | **P1 向量化优化准入已关闭；系统实时 P1 仍开放** | 正式 v3 中 short 融合累计墙钟 `4.029165 -> 3.652252 s`，改善 `9.35462%`、10/10 更快，配对原始变化 95% CI `[-10.914359,-8.113134]%`；long 为 `32.954357 -> 30.768826 s`，改善 `6.631993%`、3/3 更快，CI `[-7.279095,-5.406805]%`。单次融合 P95 改善 short/long 为 `6.652902/6.655511%`。D6 判定 `d1_optimization_admitted=true` | `system_realtime_gap_closed=false`，candidate 最低 RTF `0.143397`。继续补目标运行环境、AirSim、RMSE/NEES/NIS 和长期质量证据，不回退向量化默认路径 |
| A1 原子 publication overlay API / A2 clean 原子复核 | **D1-owned 原子优化已实现并完成单测；main clean 接线与安全子门闭合；A2 性能和有效 treatment GAP 未关闭** | D1 原子入口内部完成 prepare、evaluate、可选 detached assemble 和 post-integrity，不公开 prepared descriptor；公共三步 API 保持兼容。聚焦 `36 passed`，D1 全量 `324 passed`。main clean commit `7cc2d0cfd598a72d60c6ba8c7d4a283f4e5a897d` 完成 seed 1100、200v200、2.2 s 成对复核：两份 manifest 均 `repository_dirty=false`；9 publications/46 decisions，0 accepted/46 `oosm_scan` rejected、0 error；9/9 post-integrity 通过，atomic failure/materialized shadow/禁止写入/D2-D3 shadow 消费/在线 truth/全局编号变化均为 0；D1/D2/D3 两臂均为 `202/201/186` | control/shadow 墙钟 `10.735151270986535/19.449935468961485 s`，增加 `81.1799%`；RTF `0.204934/0.113111`；shadow P50/P95/max `1024.838/1536.429/1549.436 ms`。全拒绝路径旧 prepared-handle 本来就跳过 assemble，原子入口不能消除前后完整摘要和规范准备/post-integrity 的主要开销。安全边界闭合不等于 A2 准入；性能门失败且没有 accepted treatment，停止 A3/A4 和 seeds 1101/1102 |
| A1 规范发布准备对象与只读 metadata 装配 | **D1-owned 优化与过期摘要阻断已实现并完成单测；main 已显式接线，但 A2 性能门和有效 treatment 门失败** | 2026-07-23 增加 experimental/offline 准备对象，在 evaluate/assemble 间复用一次完整规范航迹描述。准备对象不持有可变航迹、metadata 或 NumPy 引用；每个复用边界核对对象绑定并重算每条航迹完整规范载荷 SHA-256。数组、嵌套 metadata、source support、identity、全局编号、时间戳和分级变化均 fail closed。接受路径可按值复制嵌套只读 `Mapping`、tuple、frozenset 和 NumPy 值。2/3/5 成员 decision 与提交 `de73cb2` 基线逐字节一致；200 航迹接受夹具为 1 次完整描述、2 次完整载荷复核和 400 条复核摘要。聚焦 `21 passed`，D1 全量 `308 passed in 19.69s`。main 提交 `2b976a7` 的成对开发复跑为 seed 1100、200v200、2.2 s、`recon_count=2`；9/9 次显式 prepared handle 完整性校验通过，46 evidence 为 0 accepted/46 `oosm_scan` rejected。过滤专属审计并归一化不透明编号和总线序号后，3294/3294 条业务记录逐条一致，归一化 SHA-256 均为 `bb7eabca...c3855a2`；truth NPZ 和离线 truth/proximity 文件一致，D1/D2/D3 均为 `202/201/186`，错误、禁止写入、下游消费和在线 truth 均为 0 | control/shadow 墙钟 `10.712171729/19.376483415 s`，增加 `80.8829%`；RTF `0.205374/0.113540`，shadow P95 `1532.999 ms`。before/prepare/evaluate/after 均值 `224.461/345.095/195.421/207.312 ms`，最大载荷 `11,275,939 bytes`，水位 `8/1024`。manifest 为 `repository_dirty=true`，只属开发证据。安全接口和业务非干预子门通过，但性能门失败且没有 accepted treatment 可评估有效性；A2 不准入，停止 A3/A4 和 seeds 1101/1102 |
| 结构歧义下一候选设计（2026-07-23 基线） | **A1 `IMPLEMENTED_UNIT_TESTED_OFFLINE_PROTOTYPE`；A2 当时未接线；B 暂缓，C 交 D2 规划** | 提交 `de73cb2` 实现独立 publication overlay 纯函数，拒绝返回原业务序列，接受只修改 detached DTO；完整键排序、SHA-256、generation、冲突、容量和非有限输入均 fail closed。基线聚焦 `7 passed`，当时 D1 全量 `294 passed` | 当前状态由上一行更新。A1 仍未接 `FusionAdapter` 或默认路径；experimental decision 仍不是在线 schema。A3/A4 的匿名 treatment、多 seed、RMSE/NEES/NIS、D2/D3 可用性和系统收益仍未开始 |
| 结构歧义证据侧车与 prediction-only 分量 | **D1-owned 侧车和身份中性共同质心候选均已实现；受控冻结扫描已形成一次 treatment；真实 clean 系统 P1 开放** | 固定提交 `ff88131` 的旧两臂中，hold 把 strict IDSW `9->3`，但 track/coverage continuity `.865/.870 -> .826667/.828333`。D1 默认关闭候选不选边、不 hit/lineage/identity commit、速度和相对位置不变，且已关闭跨 generation 累积和无界幂等表；专项当时 `62 passed`，D1 全量当时 `282 passed in 17.81s`。main 先在 dirty 工作树得到零 treatment 诊断，后在固定提交 `7e15dac9cdaf6743999dfe045a70676fd31a17d6` 完成 `repository_dirty=false` 的 seed 1100 同输入复跑：200v200、`recon_count=2`、2.2 s、配置哈希 `20ef5248...b840`；两臂 89 批传感器主题哈希同为 `bc064834...51518`，D2 在线记录哈希同为 `da7089fa...f8d2f`。hold-only 与 hold+centroid 的 D1/D2/D3、strict IDSW、track/coverage continuity 均相同，为 `202/201/186`、`3`、`0.8266666667/0.8283333333`；可用/不可用/未承诺映射均为 `1491/218/76`，commitment coverage 均为 `0.9574706212`。重复分配、在线 truth 使用、未承诺来源/候选绑定违规均为 0。D3 门拒绝 11 个目标；main 在一次 hold 事件中累计撤回或清除 13 条运行时绑定，两者统计口径不同。candidate `46/0/46` 个候选/施加/拒绝，拒绝原因为 `oosm_scan=30`、`unbalanced_component=16`。D1 后续复用 governed replay、扫描组织器和在线批融合入口运行三类受控冻结输入：同步平衡纯交替环 `2/2` 分量施加一次 `15.000000 m` 共同平移且协方差不收缩；乱序平衡分量以 `oosm_scan` 拒绝；成员/观测 `2/1`、free row/column `1/0` 的分量以 `unbalanced_component` 拒绝。两臂冻结帧签名一致，专项 `5 passed`，当前 D1 全量 `287 passed in 18.03s` | D1-owned 状态语义、存储边界、受控有效施加窗口和 main 构造参数接线已完成，系统效果未关闭。受控诊断没有改变真实 clean seed 1100 的零 treatment，也不证明共同质心收益或恢复 hold 可用性。停止 seeds 1101/1102，默认关闭。后续使用新的真实匿名冻结扫描做未见 seed、多 seed、RMSE/NEES/NIS、D2/D3 可用性、P95 和长时内存/吞吐验收；不得通过忽略 OOSM 或放宽数量门制造 treatment。完整规则见 `D1_STRUCTURAL_AMBIGUITY_HOLD_CAUSAL_AUDIT_CN.md` |
...
共同质心拒绝路径的证据口径已补充：`oosm_scan` 与 `unbalanced_component` 两个受控场景均为
`applied_component_count=0`，共同质心 correction 未施加；候选臂仍在拒绝后执行一次
publication-base replay + replace 清除旧临时修正。分段预测与单段历史重放的当前离散 CV
过程噪声不满足半群等价，候选减控制协方差差最小特征值分别为
`-0.0071928353214153066`、`-0.004617076466238031`。差值已 bitwise 归因到 replacement，
只作诊断，不能声称拒绝路径对状态和协方差严格无副作用；两项边界均为
`candidate_not_promoted`，不关闭
现实匿名输入的系统效果 P1。
...
交替路径。同配置 recon=2 A/B 虽使 strict availability 从 `1/3` 提升为 `3/3`，但 D2/D3
可用数量下降，seed1001 continuity 下降约 `0.055`，并抑制
`1.12%/6.61%/3.98%` 的 radar observations；因此生产默认恢复基线 Hungarian，v1 被拒绝。
...
第三条候选不再把图论分量全部记作 suppression。D1 已实现结构歧义 evidence 侧车并让相关成员
...
main 已接入构造参数。seed 1100 先有 dirty 开发诊断，随后由固定提交 `7e15dac` 的 clean
同输入复跑确认 46 个候选全部因 OOSM 或分量不平衡拒绝，实际施加为 0。两臂业务结果完全相同
不能作为收益证据。D1 受控冻结扫描已在不放宽安全合同下形成一次合法施加，并保持 OOSM 与
数量不平衡边界 fail closed；该子项不等于真实匿名输入收益。seeds 1101/1102 已停止，后续
使用新的匿名冻结扫描和未见 seed 验收。
...
`200x199` 图、`0.80058/1.58216` gate-valid 替代边和 1,966 条无真实径向速度量测只用于证明
free-row 数学缺口与速度字段不可用。10 s 的 7 个 radar+vision ambiguous mappings 不能单独
证明 radar-only 根因，但长期 coast 与跨模态传播属于后续集成验收，不能从关闭条件中排除。
