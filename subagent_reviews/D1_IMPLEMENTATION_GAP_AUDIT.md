...
| --- | --- | --- | --- |
| A1 原子 publication overlay API | **D1-owned experimental/offline 优化已实现并完成单测；main 尚未接入，A2 性能 GAP 未关闭** | 2026-07-24 冻结单个同步入口，内部完成 prepare、evaluate、detached assemble 和 post-integrity verify，不向调用方暴露 prepared descriptor。公共 prepare/evaluate/assemble 的逐边界完整内容校验保持不变。2/3/5 成员 canonical decision bytes 与 `de73cb2` 基线一致。200 航迹 accepted 夹具为 1 次完整描述、200 条描述摘要、1 次后置完整性复核、200 条规范复核摘要，并另生成 200 条 detached shadow 摘要；rejected 路径 shadow 复制和摘要均为 0。调用内 state/covariance、嵌套 metadata、source support、identity、`last_nis`、全局编号、时间戳和分级变化均丢弃 shadow、撤销状态推进并 fail closed；装配异常同样不公开部分结果。公开结果支持标准 JSON 和确定性字节编码，canonical/shadow 发布摘要语义一致；只读嵌套 Mapping/tuple/frozenset/NumPy 值可安全复制。聚焦 `36 passed`，D1 全量 `324 passed` | main 需在默认关闭审计路径中显式采用原子入口，只复跑 seed 1100 成对开发场景并重算墙钟、RTF、P95、载荷、业务非干预和 accepted treatment。当前不得用模块工作量计数声明 A2 性能通过；提交 `2b976a7` 的 `+80.8829%` 和 0 accepted/46 rejected 仍是系统证据。A2 不准入，A3/A4 与 seeds 1101/1102 继续停止 |
