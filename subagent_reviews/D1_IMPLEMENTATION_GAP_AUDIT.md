...
| --- | --- | --- | --- |
| A1 规范发布准备对象与只读 metadata 装配 | **D1-owned 优化与过期摘要阻断已实现并完成单测；main A2 性能门未通过** | 2026-07-23 增加 experimental/offline 准备对象，在 evaluate/assemble 间复用一次完整规范航迹描述。准备对象不持有可变航迹、metadata 或 NumPy 引用；每个复用边界核对对象绑定并重算每条航迹完整规范载荷 SHA-256。数组、嵌套 metadata、source support、identity、全局编号、时间戳和分级变化均 fail closed。接受路径可按值复制嵌套只读 `Mapping`、tuple、frozenset 和 NumPy 值。2/3/5 成员 decision 与提交 `de73cb2` 基线逐字节一致；200 航迹接受夹具为 1 次完整描述、2 次完整载荷复核和 400 条复核摘要。聚焦 `21 passed`，D1 全量 `308 passed in 19.69s` | main 提供的 seed 1100、200v200、2.2 s、`recon_count=2` 开发复跑为 46 evidence、9 evaluations、0 accepted/46 rejected，shadow P95 约 `2216 ms`，RTF 约 `0.205 -> 0.094`，未通过 P95 `+5%` 门。本批拒绝路径原本不重复描述，不能用模块单测推断系统性能改善。完整性复核仍遍历全部 metadata，main 需接新接口并拆分规范化、前后禁止写入摘要、prepare/evaluate/assemble 和日志物化计时；A2 通过前停止 A3/A4 和 seeds 1101/1102 |
