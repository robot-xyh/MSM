...
| --- | --- | --- | --- |
| 结构歧义下一候选设计 | **A1 `IMPLEMENTED_UNIT_TESTED_OFFLINE_PROTOTYPE`；A2/A3/A4 未实现；B 暂缓，C 交 D2 规划** | 提交 `de73cb2` 新增独立 experimental publication overlay 纯函数模块；输入只读规范 `GlobalTrack` 快照与 `StructuralAmbiguityEvidence`，输出 detached decision/member overlays 和有界 generation 状态。拒绝装配直接返回原业务序列；接受只复制 DTO 并增加统一 NED 位置平移和 PSD 位置协方差增量。完整键排序、canonical JSON/SHA-256、重复/倒退代、摘要冲突、重叠组件、容量和非有限输入均确定性 fail closed。2026-07-23 聚焦 `7 passed`，覆盖 2/3/5 成员、拒绝透传、全排列、幂等/冲突/容量和输入不变；D1 全量 `294 passed`。A1 未接 `FusionAdapter`，未修改 `fusion.py`，无运行开关，experimental decision 不是在线 schema | A1 只关闭离线纯函数单元原型。A2 仍需在冻结扫描上接入默认不发布的 shadow，记录规范快照、shadow DTO 和全部禁止写入对象摘要，并验证业务输出、P95/RSS；A3/A4 的匿名 treatment、多 seed、RMSE/NEES/NIS、D2/D3 可用性和系统收益均未开始。B 只有在事件排序、过程噪声分段和一致性门槛冻结后才可实现；C 由 D2 owner 另行规划。seeds 1101/1102 继续停止 |
