...
尚未实现的主要是外部成熟框架集成：Stone Soup、FilterPy、ROS 2 `tf2`、`message_filters`、UKF、IMM、真实 AirSim ComputerVision/Blocks 运行时适配。这些目前有文档计划或占位类，但未作为运行依赖接入。原因主要是当前阶段强调依赖轻、可复现、离线测试稳定，且缺少 ROS 2 runtime、真实 AirSim topic/log schema、外参标定链路和多模型评估基准。

优先级建议：

- **P0**: 保持当前 NumPy EKF、传感器观测模型、延迟补偿和 AirSim dry-run 合同稳定。
- **P1**: 补 `TrackUncertaintySummary`、JSONL/CSV replay、AirSim CV detection 输入合同、可配置协方差参数。
...
|---|---|---|---|---|---|
| 统一 `SensorObservation` 数据合同 | 已实现。支持 `radar/acoustic/eo/lidar`、`measurement_timestamp`、`arrival_timestamp`、`frame_id`、`covariance`、质量字段和通信元数据 | `research_modules/d1_sensor_fusion/src/d1_sensor_fusion/types.py`; `research_modules/d1_sensor_fusion/tests/test_interfaces.py` | 不适用 | 后续需与主程序 JSONL schema 保持字段一致 | P0 |
...
| 协方差输出与航迹分级 | 已实现。输出 6x6 协方差、`a95_m`、`coarse/stable/handover`、NIS 通过率参与分级 | `research_modules/d1_sensor_fusion/src/d1_sensor_fusion/fusion.py`; `research_modules/d1_sensor_fusion/src/d1_sensor_fusion/metrics.py` | 不适用 | `TrackUncertaintySummary` 尚未代码化；主动降级字段仍需标准化 | P1 |
| `TrackUncertaintySummary` | 未实现代码。已在文档中设计字段，用于 D4 主动降级 | `research_modules/d1_sensor_fusion/docs/ALGORITHM_AND_IMPLEMENTATION.md`; `subagent_reviews/D1_SENSOR_FUSION_REVIEW_AND_PLAN.md` | 当前主线先通过 `GlobalTrack.metadata` 传递必要字段 | 需要 D4/D6 确定字段消费方式、区域聚合窗口和阈值 | P1 |
...
| AirSim dry-run fake fixture | 已实现。可从 fake fixture 生成 radar/acoustic/eo/lidar `SensorObservation[]`，不连接真实 AirSim | `research_modules/d1_sensor_fusion/src/d1_sensor_fusion/airsim_dry_run.py`; `research_modules/d1_sensor_fusion/tests/test_airsim_dry_run.py`; `research_modules/d1_sensor_fusion/docs/AIRSIM_INTEGRATION_PLAN.md` | 不适用 | 需要与 main 的 AirSim CV/Blocks JSONL schema 对齐 | P0 |
...
| 真实 AirSim ComputerVision / Blocks runtime | 未在 D1 内实现。D1 只提供 fake fixture 和 SensorObservation 适配 | `research_modules/d1_sensor_fusion/src/d1_sensor_fusion/airsim_dry_run.py`; `research_modules/d1_sensor_fusion/docs/AIRSIM_INTEGRATION_PLAN.md` | 用户要求阶段内不连接真实控制/API；真实 AirSim runtime 由 main/shared 层负责 | 需要 main 提供 target actor truth、camera pose、simGetDetections、JSONL/CSV replay schema | P1/P3 |
...
| JSONL/CSV replay 输入合同 | 部分实现。D1 有 simulation/fake fixture，但没有统一 JSONL/CSV reader | `research_modules/d1_sensor_fusion/src/d1_sensor_fusion/simulation.py`; `research_modules/d1_sensor_fusion/src/d1_sensor_fusion/airsim_dry_run.py` | 当前测试直接生成 Python 对象；main replay schema 尚未固定 | 需要 main 确定 `truth_actors.jsonl`、`detections.jsonl`、`synthetic_radar.jsonl`、`sensor_observations.jsonl` 格式 | P1 |
...
2. **消息合同未完全稳定**: 真实 AirSim ComputerVision 的 actor truth、camera pose、`simGetDetections`、检测框和通信日志 JSONL/CSV schema 仍需要 main 统一。
...
2. **P1 补接口缺口**: 实现 `TrackUncertaintySummary`、JSONL/CSV replay reader、AirSim CV detection schema 测试、协方差参数配置和 source de-dup 基线。
