...
| --- | --- | --- | --- |
| D1 发布协方差完整正半定合同 | **P0 D1-owned 缺口已关闭；clean 多 seed/长时矩阵需由 main 重启** | seed 1103、200v200、10 s 在发布时刻 `7.85180018473111 s` 暴露 `global_track_031` 非正定输出。限制前最小特征值 `+7.506060086e-04`，单个交叉项 pairwise 裁剪后为 `-9.247657800e-04`。故障前 58,776 次 scalar/vectorized 双算逐元素一致，确认是共有数学缺口。当前在既有对角和相关上界之后执行相关矩阵特征值投影、单位对角归一化和单位阵凸组合，有限复核失败时保留治理对角并对角回退；reason 和操作数可审计。固定失败、1-6 维各 96 组随机/极端、隐藏非正定、路径等价、双时间戳/谱系/6 s fixed-lag 测试通过；专项合计 `28 passed`，D1 全量 `352 passed in 20.52s`。修复后同配置 10 s 集成运行完成，10,554 观测、finite、online truth 0、RTF `0.157583` | main 在固定提交重新完成 13-pair clean 矩阵并由 D6 生成正式报告；多 seed、长时增长、RMSE/NEES/NIS、AirSim 和系统实时 P1 不因本修复关闭 |
| 预测/重放协方差成对限制热点 | **D1-owned 标量裁剪热点已关闭；clean full-stack 与系统实时 P1 仍开放** | seed 1100、200v200、2.2 s 冻结输入含 89 扫描、2,035 匿名观测、202 终态航迹，SHA-256 `54bed9d7...0ec`，在线 truth 0。调用审计确认 10,832 次预测限制、1,789 次更新后重放限制和 202 次新生限制都对应新协方差，不能缓存跳过。保留 `vectorized_covariance_limit=False` 标量 reference，默认优化路径对同一 pairwise correlation bound 做批量上三角裁剪；只复用不可写索引拓扑，不缓存协方差。floor/ceiling、reason、对称化、非法状态重置、在线 covariance fail-closed、NED、双时间戳、6 s fixed-lag、全部观测、门限、谱系和全局编号合同不变。预热后交错 5 轮，reference/optimized 均值 `3.001196/2.610975 s`，P50 `3.011440/2.614061 s`，P95 `3.023308/2.660813 s`，P50/P95 加速 `1.152x/1.136x`，5/5 更快；cProfile limiter 累计 `1.047145/0.426826 s`。预热、5 轮和 profile 均通过逐扫描后验/物化航迹、终态航迹、证据、操作数、累计诊断和延迟审计严格等价。10 s 长夹具另只运行一对，771 扫描、11,889 观测、4,009 次 fixed-lag rebase 和 11,888 条 OOSM 在两臂一致。专项 `18 passed`，D1 全量 `342 passed in 19.73s` | 本项证据来自当前 D1 工作树的冻结三维质点回放，不是 clean full-stack、多 seed、AirSim 或实时验收。main 应在固定提交复跑同输入全栈，核对 D1/核心墙钟、P50/P95、RSS 和跨模块语义；更多未见 seed、长时增长及正式 RMSE/NEES/NIS 继续开放 |
