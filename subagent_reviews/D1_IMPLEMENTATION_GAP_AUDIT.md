...
| --- | --- | --- | --- |
| 结构歧义下一候选设计 | **A/B/C 设计已完成，均未实现；A 最小原型为 D1 下一步，B 暂缓，C 交 D2 规划** | `research_modules/d1_sensor_fusion/docs/STRUCTURAL_AMBIGUITY_NEXT_CANDIDATE_DESIGN_CN.md` 已明确数学语义、概念数据结构、确定性排序键、风险、阶段和预注册门槛。A 只在 detached 发布 DTO 上形成 overlay，拒绝时直接发布规范快照且不 replay/replace；B 明确当前 `Q(h)=G(h)qG(h)^T` 单段/分段不等价，零更新事件也会改变协方差分段；C 保持 D1 只发布 evidence，由 D2 研究概率/多假设消费。本轮无 Python、开关、schema、测试或运行证据 | 先完成 A1 纯函数原型和 A2 离线 shadow，逐拒绝原因证明业务发布与 control bitwise 相同，且 state/covariance/history/checkpoint/cache/lineage/source support/`global_track_id` 摘要不变；未通过前不得运行效果试验。B 只有在事件排序、过程噪声分段和 RMSE/NEES/NIS 一致性门槛冻结后才可实现。C 由 D2 owner 另行规划。seeds 1101/1102 继续停止 |
