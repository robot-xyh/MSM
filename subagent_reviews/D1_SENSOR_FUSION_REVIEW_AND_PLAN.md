...
## 36. GlobalTrack 批量 A95 候选评审（2026-08-01）

### 36.1 研究选择

对现有 seed 42000、2.2 秒非正式 200v200 冻结回放重新 profile 后，reference
`global_tracks` 累计 `0.443068 s`，大于同轮 scan-input 墙钟 `0.138621 s`。本轮只推进
`d1.publication.global_track_materialization.batched_a95_summary.v1`。三个已拒绝的历史候选
均未复用，scan-input 没有并行实现。

### 36.2 合同边界

候选只把同一发布帧内二维位置协方差的 A95 特征值计算批量化。协方差限制、质量门限、逐航迹
状态/协方差复制、双时间戳、NED、谱系、完整 metadata 和 `global_track_id` 保持不变。
构造开关默认 `False`，candidate 必须显式开启。在线真值使用和规范身份写权限均为 0。

### 36.3 模块门

输入含 86 个扫描、2,051 条匿名观测；SHA-256 为
`c6dcc69d58b0fc9a51e9cfcf2368b4faeb882d5a90991ffcdf1f7605bba55e53`。7 对交替
fresh-process 中 candidate 7/7 更快，模块墙钟 P50 从 `0.228742 s` 降至 `0.190582 s`，
改善 `16.6824%`。P95/max 从 `0.229523/0.229629 s` 降至
`0.194719/0.195816 s`；配对差 bootstrap 95% 区间为
`[-0.044637, -0.031457] s`。RSS P50 为 `165528/165312 KiB`。

逐扫描后验、协方差、NIS、门控编号、完整发布载荷、一致性证据、业务操作数和最终离线导出
7/7 严格一致。每臂物化 11,188 条航迹；reference 标量 A95 11,188 次，candidate 对相同
11,188 个矩阵执行 56 次非空批量调用。

### 36.4 评审结论

D1 模块门通过。候选仍 default-off，等待 main 独立集成评审。正式 seeds 1000--1019、正式
R0、AirSim、目标硬件、正式 RMSE/NEES/NIS 和系统实时均未运行或关闭。main 若接线，应另行
冻结 clean source 和未见开发输入，验证核心墙钟、RSS、D2 消费及长期容量。
