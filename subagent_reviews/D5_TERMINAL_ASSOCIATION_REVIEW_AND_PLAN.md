...
## 2026-08-10 有状态时序几何关联复核

D5 已完成有状态包装层，原无状态几何关联 API 保持兼容。新接口以相机流为状态边界，输出瞬时
几何结果、迟滞后的实测关系、当前只读绑定、coast 预测、候选间隔和六类绑定事件。默认 coast
为 0.25 秒，预测协方差显式增长；预测和丢失证据不授权。挑战者需要连续两个不同实测帧，且
相对原绑定的总代价优势不小于现有 `min_lock_margin`。成对交换、近似平局和重复占用失败关闭。

专项覆盖 0.06/0.10/0.17 秒缺口、0.26 秒过期、单帧与双帧挑战、成对交换、时戳回退、
多相机/多流隔离和 truth/global ID 不变式，结果为 `13 passed`；D5 全量为
`904 passed, 1 warning in 146.08s`。唯一警告是既有 Matplotlib `Axes3D` 环境提示。

main 已在 `long_range_cv_scan` 中按相机创建 episode 独立实例并完成 reset 隔离。发现、提示和
完成只消费 `measured_assignments`；coast 仅写日志且不授权。运行输出增加
`temporal_binding_events.csv` 和 `dropout_events.csv`，main-D6 联合专项为 `23 passed`。

冻结 seed `20260810` 的 D6 报告仍是接线前旧 v2 基线，状态为 `fail_closed`：准确率
`0.9979317477`、短缺口 3、绑定切换 7、交叉窗口可评价 `3/31`。这些数值不能归入新时序算法。
后续需运行 v3 原始尺度单 seed 和不少于 10 个 seed，由 D6 分别统计原始漏检、预测保持、恢复、
长期重发现、绑定切换和交叉窗口可评价率；真实 AirSim P1 继续开放。

## 2026-08-10 真实 AirSim 长距离 20 目标复核（冻结旧 v2 基线）

D5 复核了 main 于 2026-08-10 形成的原始尺度取证和 2 倍网格诊断证据。主场景目录为
`research_modules/airsim_runtime/outputs/d5_cv_long_range_20target_visual_evidence_20260810/`。
运行采用 ComputerVision；seed `20260810`，20 个 `Quadrotor1` actor，速度 50 米/秒，时长
20 秒，逻辑更新率 100 Hz。中心距目标约
2.8-3.2 千米，拦截相机距目标群约 500 米。

主场景把检测转换为 NED 世界视线并使用速度预测。`coverage_safe` 累计发现和五帧确认
均为 `20/20`，拦截提示 `20/20`，观察及完成 `19/20`。1934 条可评分几何关联准确率为
`0.9979317477`。

匿名多目标跟踪在连续可见段的身份切换为 0，轨迹纯度和连续性均为 1.0。仍有 3 次短缺口中断
和 48 次长期重发现。31 个预定交叉窗口只有 3 个可评价；可评价窗口身份切换为 0、纯度和
连续性为 1.0。专项总门的直接失败原因是短缺口中断 `3 > 0`。几何绑定切换为 7，作为开放 P1
保留。

运行与记录门通过。取证包保存 98 幅原图和 98 幅在线标注图，19/20 条全局航迹形成双相机图像
证据。在线文件和标注不含 actor/object/truth ID；真值只用于离线混淆矩阵。在线真值使用和
`global_track_id` 改写均为 0。
中心航迹来自 `synthetic_d1_d2_fixture`，本轮没有验证真实 D1/D2 融合误差或中心身份正确性。

中心相机 3 千米原始尺度图像接近 AirSim 渲染/像素极限，`simGetDetections` 框不等价于真实
光电辨识。2 倍网格诊断虽达到双相机 `20/20`，但准确率为 `0.9892194912`，几何绑定切换增至
42，短缺口仍为 3，总门仍失败。该诊断不替代原始尺度主场景。

后续 P1 聚焦三次短漏检、交叉窗口可评价率 `3/31`、7 次几何绑定切换、真实光电小目标数据，
以及位置、姿态、外参、时间和检测误差的多 seed 标定。本轮没有运行图神经网络，不改变 G1、assist、
authority 或确定性默认关联路径，也不关闭真实设备、M 对 N 联盟完成和物理拦截缺口。

## 2026-08-10 不规则交叉与窄视场扫描复核

D5 已在旧全时可见基线之外完成独立扫描 runner。20 个默认目标位于 A 前方约 2.8-3.2 千米，
初始径向跨度 390.0 米，全时段最小三维间距 30.0 米。成对相向横向运动使 A/B 参考像面出现
10/6 对线段相交或明确近距交叉。B 与目标群保持 500 米后方偏置随群平移，只接收 A 已绑定
中心航迹的提示，中心编号保持只读。

复核确认了两类扫描行为。机械模式在 100 Hz 下每帧移动 1.8 度，超过 A 的 0.621 度视场，
标准 seed 实际发现 0/20；安全覆盖模式每帧移动 0.4968 度，保留 20% 重叠，在 1.61 秒内完成
20/20。D5 只在连续五帧、0.05 秒确认后执行事件级完整矩阵和匈牙利匹配。10 个 seed 的两级
准确率与完整链均为 1.0，IDSW、重复、未匹配、在线 truth 使用和全局 ID 改写均为 0。

本次证据关闭理想解析扫描和分批提示配准的软件基线。它没有运行 AirSim、真实检测、多目标
跟踪或图神经网络，也没有加入相机位姿、云台、时钟和通信误差。下一轮应保持相同冻结场景，
按单变量加入误差并重新标定覆盖重叠、确认时间和候选代价间隔；在此之前不得把 20/20 写成
真实吊舱覆盖率或 M 对 N 物理完成率。

## 2026-08-10 理想二十目标两级配准复核

D5 已形成一个与 AirSim、检测器和学习模型隔离的两级几何基线。中心节点 A 持有 20 条既有
三维 `GlobalTrack` 和 A 相机的 20 条匿名像素轨迹，但不知道对应关系；拦截节点 B 只提供
20 条匿名像素轨迹。中心先把全局航迹投影到 A 图像，通过最近五帧位置与帧间位移代价完成
第一级匈牙利匹配，再把已绑定航迹投影到 B 图像完成第二级匹配。最终公开结果保留
`global_track_id -> A_local_track_id -> B_local_track_id`，D5 未生成、归一化或改写中心编号。

2026-08-10 对 seed `20260810-20260819` 运行 10 组 15 秒理想质点场景。每组包含 76 个图像
时刻，A 侧、B 侧和端到端准确率均为 1.0，完整链和双相机全时段可见率均为 1.0；身份切换、
重复分配、未匹配、在线 truth 使用和全局编号改写均为 0。标准 seed 导出每级 30,400 条候选
代价、1,520 条公开关系链、13 张步骤图和 76 帧 GIF。目标数量由入口参数决定，专项测试同时
覆盖非 20 目标配置。

本次复核确认了算法顺序、数据隔离、时间戳/协方差合同和输出审计能力。输入使用精确相机内参、
位姿和云台方向，所有目标持续可见，预测像素与匿名观测完全一致，因此正确项代价为 0。该数值
不能作为真实门限。下一轮应依次加入像素噪声、位姿误差、时间偏差、漏检、虚警、遮挡和本地
轨迹中断；只有确定性基线形成稳定失败样本后，才用冻结的同输入数据比较图匹配方法。

三层候选图只用于解释完整代价矩阵中的候选收缩，没有执行图神经网络。本轮不是 AirSim、实飞、
真实检测或 M 对 N 联盟完成证据，不改变 D5 对真实几何 drift、MOT 连续性、跨视角共同窗口和
物理闭环的既有 P1 判断。
...
train/validation/future-held-out 的 `48/24/32`、9 个场景族、5 个规模、416 个意图窗口和
208 个困难混淆 assignment；配置元数据未出现 truth/actor/object 身份，权限保持全 false。

五个非正式运行探针覆盖四类意图、两类相机角色和五类困难混淆，共形成 693 帧。在线 truth
使用、`global_track_id` 创建和改写计数均为 0。完成能力审计后，D5 更新两个实际漂移的 main
文件引用，并级联重算 schedule 与 request 自哈希。当前 schedule file SHA 为
`d14b19d8...1082e`，generation-only request file SHA 为 `157166b8...80b3`。

main generation API 的两次临时有界调用分别把库存推进到 1 和 2，只处理 train seed
`24000/24001`。future-held-out 没有 staged episode 或 payload 文件，训练、validation
consumption、held-out read、shadow、assist、runtime、control、camera command 和中心 ID 写权限
均未开启。定向回归 `51 passed, 1 warning in 20.13s`，D5 全量
`877 passed, 2 warnings in 139.85s`。正式 104 条来源生成和后续模型流程仍由 main 另行授权。

## 2026-08-01 A3 v3 真实 producer 可达性复核

main 的真实首 episode 暴露了自检与冻结生产条件不一致。旧 schedule 为 6 秒，四窗各 1.5 秒，
每窗要求 24 个唯一样本；真实基础配置为 0.1 秒视觉周期。seed `24000` 共形成 50 个主动视觉帧，
首帧 0.85 秒，`intent-window-1` 只有 `1.55-2.95` 秒的 15 个唯时间样本。旧 smoke 使用 0.05 秒
周期和 2 个侦察相机，不能证明正式冻结配方可写出。

D5 将 schedule 实例升级为 v3。episode 时长为 8 秒，窗口为 `0-2/2-4/4-6/6-8` 秒，侦察相机
下限为 4。每窗 24、episode 96 的最低配额保持不变。readiness v4 新增 104-cell viability audit，
按真实 0.1 秒周期、1.4 秒最大启动时间和 0.5 秒最大尾段缺口计算保守容量。最不利窗口容量为
24；任一 entry、窗口、角色数量或 producer 配置漂移导致容量不足时，请求直接失败关闭。

真实 producer 复核覆盖 entry `0/1/2/70/72/95`。这些配方覆盖 train、validation、
future-held-out、两种相机角色排列和五类困难混淆，实际窗口最低计数为 32，在线 truth 使用为 0。
首条配方已完成严格 evidence 校验和临时 development staging。当前 request v2 文件 SHA-256 为
`157166b8...80b3`；更早 request 与授权不再有效。定向测试 `64 passed`，D5 全量
`875 passed, 2 warnings in 119.98s`。正式 104 episode、训练和 future 语义评估未执行，所有
运行、相机命令、控制和中心 ID 写权限保持 false。

## 2026-08-01 A3 v3 来源生成请求与恢复复核

D5 已形成独立、版本化、可机器验证的 generation-only request。request 以仓库相对路径和
SHA-256 绑定冻结 protocol、schedule、allocation binding、main global seed registry 和 D5
episode staging 实现。validator 重新核对 104 条 episode、`48/24/32` split、精确 seed
`24000-24103`、future-held-out one-shot 合同、在线 truth 隔离和 permission 集；任一不一致
均不返回 ready。readiness 顶层与 producer capability 暴露相同的 request path/hash/ready
稳定字段，便于 main 统一消费。

原 `stage_a3_v3_episode_evidence()` 保持 create-only。新增只读 inventory 允许下一进程在运行
episode 前确认哪些冻结 recipe 已完整落盘；新增 resume helper 只在 incoming evidence 与已落盘
descriptor/hash 完全一致时幂等返回，不重写文件。descriptor v2 自带内容哈希，恢复路径拒绝
部分文件、损坏、online/offline hash 漂移、split 漂移和 development/future 根错位。

专项复核还发现旧 finalizer 会反序列化 future-held-out payload。现已增加
`finalize_a3_v3_generation_partition()`：future 最终化只核对 descriptor 自哈希、冻结 recipe
binding 及 online/offline 文件 SHA，不调用 payload `from_dict()`；manifest 明确
`future_held_out_payload_read_count=0` 且 integrity verification 不等于 held-out consumption。
development 仍可在最终化时做语义重验。修复后定向回归为 `64 passed`，D5 全量为
`875 passed, 2 warnings in 119.98s`，零失败。

当前 readiness 是 `source_generation_request_ready_generation_only`，不是执行 ACK。104 个正式
episode、source manifest、训练和 future 一次性评估均未执行。模型、shadow、assist、相机命令、
runtime、production、control 和中心 ID 写权限保持 false；在线仍不得读取 truth ID。
...
四段意图窗口、两类困难混淆任务和控制项。当前四段窗口均为 2 秒，每段独立要求至少 24 个唯一
...
接线。当前 readiness 为 `source_generation_request_ready_generation_only`，其中
`producer_adapter_complete=true`、request ready=true，但执行授权仍为 false。main 取得单独执行授权后才能生成三
...
样本。D5 于 2026-08-01 使用严格 lazy loader、显式保留 seed 1000-1019 和公共 corpus audit
完成复核。manifest 与 corpus audit SHA-256 分别为 `bccbdad4...35a4` 和
`85db29f8...f74e`，来源/完整性研究门全部通过。

训练结构门仍失败。train 102,610 个样本中 `hold=0`，高空侦察相机没有 `search_sector`；
13 个失败原因形成三项确定性补采请求。全量 ACK 和匿名 observation key 均覆盖
159,487/159,487，但物理匿名观测帧和离线 outcome 不可用。来源合格、ACK 完整和匿名键完整
均不能覆盖训练动作缺失，也不能授予模型、相机、分配、接管或控制权限。BC/PPO/assist 未
启动，`global_track_id` 未创建或改写。D5 全量回归为
`770 passed, 2 warnings in 102.24s`，零失败。

## 2026-07-28 A3 训练语料覆盖复核

前一阶段的逆平方根权重和逐动作模型门能够识别 99:1 多数类假高准确率，但仍缺少训练前的
语料结构治理。权重只能改变已有样本的损失贡献，无法补足 `hold`、少数动作和侦察相机示范。
训练入口此前也没有统一输出需要补采的动作、场景和相机角色。

D5 已增加公共 corpus audit/planner。审计完整扫描 descriptor 与实际 episode，只统计 train
split 中通过身份隔离、有限特征和完整性检查的唯一策略输入。统计维度包括动作意图、
拦截机/高空侦察机角色、场景、seed、意图与角色组合，以及场景、意图与角色组合。每个单元
同时记录样本、episode 和 seed 数量。

默认开发下限为每动作意图 `4 sample / 2 episode / 2 seed`，每相机角色 `8 / 2 / 2`，
每动作与角色组合 `2 / 2 / 2`。调用方可增加必须覆盖的场景版本。任一层不足都失败关闭。
`hold=0` 和侦察角色为 0 有独立原因码。该阈值只说明语料具备最低结构，不能证明统计充分、
模型泛化或运行可用。

同一 episode 内，相机、快照时间、候选动作集合和候选特征相同的输入按指纹识别为复制。
复制项不进入 coverage，并使训练语料失败。完整 episode 重复、split seed 交叉、reserved
seed 混入、非有限特征、truth/actor 字段和未知相机角色同样拒绝。复制、过采样和逆平方根
重加权均不能改变覆盖状态。

补采 planner 按场景、动作和角色稳定排序，输出 `AV-CORPUS-NNN` 请求及新增唯一样本、
episode 和新训练 seed 数量。污染、缺 seed 注册证据和合同错误列为先行整改项。输入 episode
遍历顺序改变时，报告与内容 SHA-256 保持一致。

行为克隆 cache 已升级为 v2，manifest 和 data audit 同时绑定语料审计。训练在模型初始化前
要求审计有效且通过。历史 v1 cache 可读取基础数据，但因缺少审计而不能训练。正式入口先保存
独立审计文件。审计器不具有正式候选、assist、相机、分配、接管、控制或全局编号写权限，
对重算摘要后的提权载荷也失败关闭。

2026-07-28 的验证使用小型合成开发 fixture。专项 `11 passed`，D5 全量
`755 passed, 2 warnings in 123.86s`。正向 fixture 只有 2 个训练 seed。100 episode、
...
旧正式行为克隆的总体精确动作准确率为 `0.955978`，但 `reacquire` 占 92.16%，
`observe_target` 召回为 0，`hold` 无正样本，侦察相机精确动作准确率约 `0.621823`。该结果
不能支持模型准入。当前主要问题是训练分布和角色覆盖，不是候选动作推理速度。

D5 已在 development-only 缓存训练链加入有界逆平方根意图权重。权重由完整 train split
冻结并用于训练和最佳验证轮次选择。无正样本动作保持 unavailable，不补零、不伪造。
行为克隆报告 v2 增加动作签名、宏平均与每动作召回、真实/预测分布、拦截/侦察相机分层、
置信度校准、训练边界分布外比例和诊断回退原因。

模型预检查与运行权限分离。开发默认门先检查 train split 每动作真实正样本，再检查 test
split 每动作召回、宏平均召回、两类相机角色、期望校准误差和分布外比例。逐动作指标对
零分母保持 unavailable。99:1 fixture 中，多数动作预测达到 0.99 总体精确动作准确率，仍因
`observe_target` 召回 0 与 `hold/search_sector` 无正样本被拒绝。输出固定
`assist=false`，主动视觉、分配和控制权限均为 false。零检测帧继续按
`reacquire + coverage=false` 处理，不提供 `hold` 正样本或锁定收益。

本轮没有运行正式 900-cell 或大写盘实验，也没有生成新模型权重。2026-07-20 的旧指标仍是
当前模型证据。D5 全量回归为 `744 passed, 2 warnings in 111.52s`。下一步由独立 producer
补充真实 `hold/observe_target/recon/FOV` 和边界动作，在 clean/frozen 谱系上重训，再用至少
20 个明确未见、非 synthetic seed 做同配置 A3/R0 成对非退化。每 episode 的安全、可见率和
重捕获均不得退化。完成上述条件前，bundle 不得自授 assist、相机命令、分配、接管或控制权限。

## 2026-07-27 A3 主动视觉采用证据复核

D5 已补齐 A3 的模块级采用证据组装和严格验证。实现直接复用
`ActiveVisionDecisionV1`、`ActiveVisionRuntimeAckV1`、`ActiveVisionCameraFeedbackV1` 与
`ActiveVisionCameraState`，并结构化读取 main 的 `CameraObservationCommand`、
`runtime.camera_command_ack` 和 `CameraRuntimeState`。D5 没有另建 ACK、相机状态或命令
路由，也没有获得相机控制权限。

证据链把模型求值、建议、确定性投影、命令下发、ACK、相机反馈、姿态与版本生效及后续物理
观测分开。验证器根据命令前姿态和最终动作重算期望方位、俯仰、视场模式，检查 ACK 后反馈的
最后接受版本和计划/联盟/通信版本。后续匿名轨迹以
`resource_id/camera_id:local_id` 命名，逐帧保留双时间戳和本地绑定；D5 只引用中心给出的
`global_track_id`。只有非模拟的完整采用链、可用的关联/覆盖结果和同键独立规则 R0 窗口同时
存在，才允许将记录交给 D6 做收益审计。

本轮补充 observation-frame v2，关闭“相机已处理图像但零检测被误记为无观测”的 D5 模块
合同缺口。历史 v1 仍要求非空匿名轨迹且序列化口径不变；v2 明确保存
`processed_zero_detections`、中心航迹只读清单和来源序号。有分配目标时，零检测帧只产生
`reacquire` 和覆盖率 0；没有分配目标时结果不可用。该合同不读取 truth/actor ID，不创建或
改写 `global_track_id`，也不授予相机、分配、接管或控制权限。main 正式 writer 和 clean/
frozen 重跑原为跨模块 P1；当前 scalable 3D 开发 writer/runtime 已接入，正式证据仍开放。

main 现为每台相机生成 truth-free frame event，只有零检测走 `sensor.camera_empty_frame`；
事件携带相机/资源、双时间戳、scan index 和三类版本。A3/R0 按时间和版本绑定，观测触发命令
后保留 0.25 秒证据尾窗，通信丢包/抖动使用独立随机流。scalable 3D 全量为
`352 passed, 1 warning`。
默认通信退化 seeds `1000-1019` 开发复跑为 candidate=492、pairable=488、unpairable=4、
coverage=`99.18699%`；329 个 v2 零检测为 `reacquire/coverage=false`，159 个 v1 为
`locked`，empty rejected=0、权限全 false。零丢包/零抖动对照为 `500/500`、coverage=100%。
新结果均为 dirty-worktree、`formal_evidence=false`，seed 未证明 unseen，不能证明收益或授权。

本轮增加 `ActiveVisionA3RuleArmTrace` 和两级公开组装 API。规则 trace 不含候选模型
provenance，只接受学习关闭的确定性规则决定、独立命令、运行 ACK 和相机反馈。它可在规则
episode 结束后序列化，在另一进程中重建并结合匿名帧形成 R0 窗口。唯一配对 API 拒绝重复 R0、
跨键/相机/版本、同日志复用、窗口时长不一致、双时间戳缺失和不完整运行反馈。

本轮再增加逐候选只读 API `attempt_active_vision_a3_pairing()`。它把现有严格配对器的结果
整理为 `ActiveVisionA3PairingDisposition`，每条记录只有一个稳定主原因，并保留底层诊断码。
候选窗口缺失时不从 `None` 推断具体断点。新增
`ActiveVisionA3CandidateStageEvidence` 后，调用方只有在明确声明运行/观测事件清单完整，并
提供与 trace 和来源日志绑定的命令、ACK、反馈、匿名观测及窗口装配信息时，v2 disposition
才细分 ACK 缺失/未确认、命令过期/时序不匹配、反馈缺失、匿名观测缺失/不完整和物理窗口
缺失/不完整。失败结果不携带 paired evidence，成功结果也只开放 D6 审计输入。

持久化 disposition 现在可通过 `from_mapping()` 或公开 validator 严格复载。顶层字段与 JSON
类型必须精确，摘要必须由内容重算一致。pairable 记录的 paired evidence 由原 validator 完整
重建，权限篡改会被拒绝；unpairable 记录不得夹带 paired evidence。复载通过不能独立证明
reason 的因果真实性，只能说明保存的制品未被篡改且内部约束一致。v2 会递归复载阶段证据并
重算细分原因，未知原因或重算摘要后替换成另一已知原因同样拒绝。旧 v1 保持严格复载。

历史冻结批次完成状态隔离的候选与规则 episode，并对 20 个受控开发 seed 的 536 条候选逐条保存
disposition。536/536 可严格复载，其中 152 条可配对、384 条不可配对；384 条主原因均为
...
D6 按完整候选分母复核后得到 `a3_auditable_pair_count=0`。合法 unpairable 存在时，完整批次
的实际采用、物理窗口、同键 R0 和收益计数均为 `unavailable`，所有权限为 `false`。152 条
可配对子集不能替代完整批次。

main 已用同配置 seeds `1000-1019` 和当前 candidate-stage sidecar 做不落盘全量重跑。
536/536 候选有阶段证据，152 条 pairable、384 条 unpairable，完整可审计 seed 仍为 `0/20`。
344 条同时为匿名观测缺失和物理窗口确认缺失；其余 40 条因观测清单不完整保持空细因，计为
物理窗口缺失细因未解析。D6 聚合 evidenced=344、unresolved=40、
`detail_completeness=false`，每 seed unresolved=2。部分清单门控避免了将这 40 条越界归为
物理窗口不完整。ACK 缺失、确认缺失、命令过期、时序错配和相机反馈缺失均为 0。开发摘要
SHA-256 为 `1ba6040e7c3e7e3b9e7d5506dfd20cf3539ce12c5aac13cca7f02799f0cd99ef`，并标记
`formal_evidence=false`、`source_worktree_clean=false` 和
`persisted_full_pair_inventory=false`。旧持久化记录不追溯
改写。后续 P1 是 clean/frozen v2 全清单持久化、默认通信退化缺失的因果核验和正式未见策略
成对非退化。

当前 A3 专项测试为 `84 passed in 1.38s`，D5 完整回归为
`739 passed, 2 warnings in 97.98s`，无测试失败。新增测试固定了历史 v1、v2 零检测、中心
目标约束、同源混合窗口、篡改拒绝、零覆盖和全部权限关闭。正向 case 是软件 fixture，
20 seed 使用测试
策略替身，不是 AirSim、实机、未见 seed 或模型收益证据。A3 assist、G1、分配、接管、导引、控制及
`global_track_id` 修改权限保持关闭。

## 2026-07-27 G1 v5 正式证据复核

D5 在 clean commit `8d5e02ec989259ce3d39e1e4ad6a90dd0d8d5b54` 上完成最终 runtime
`b0708e718b374e5bb52db41c7bd2f994e340a2b009cfd348881a5f9d549baffe` 的正式证据链。
development manifest SHA-256 为
`7d459ed855cf74b810fa1f79ed0327efd39eb4be4409451266da3f3a95387ce0`，权重 SHA-256 为
...
candidate recall 均为 1.0，false merge 为 0，CPU P95 约 0.913 ms。paired-shadow 覆盖
900 帧、74024 条边，模型 edge/cluster F1 均为 1.0，最高单特征 AUC 为 0.720073。lineage
SHA-256 为
`83e105290f3e624f267d92ceaf050d32291bd5bbbabf98580846cd31498b1af1`，包含 900 records/900
unique UIDs。

D6 external audit v2 为 pass，文件 SHA-256 为
`cbd6c72b2d9e7b78bf3aa36f975e6627250d2bf18de5a0b0ebc2c8f6cf760cd6`，内容 SHA-256 为
`334cf662e49c735931019ff358be1894d1358f1b4a5a868759eee41d3d282d15`。D5 生产 assembler
随后生成 v5，manifest SHA-256 为
`b431d066362005868374d038eb93a83b773c03715a53d8a9dfd0da21784f317d`。D6 post-assembly v2
同样为 pass，内容 SHA-256 为
`17dda42d06b4be1d21ff8f1f8baecc320fd49b532be06a9f9f6b304341763e1d`。

strict loader 和 shadow loader 均通过。assist 请求按
`bundle_g1_assist_authority_not_granted` 失败关闭。模型晋级、G1 辅助、默认路径变更、分配、
故障接管和控制六项权限全部为 false，在线 G1 未启用。真实相机泛化、中心
`global_track_id` binding 正确性和物理闭环结果继续显式 unavailable；它们分别保留为真实相机
回放、离线身份真值连接和跨模块闭环 P1。

## 2026-07-26 v5 paired lineage P0 复核（历史，已闭环）

main 指出的断点成立：此前 v5 软件结构没有把逐 episode paired lineage 作为输入和打包实物，
因此无法满足 D6 post-assembly v2。D5 现已把 lineage 纳入生产 assembler、manifest、
admission report v2、`SHA256SUMS` 和 strict loader。

正式合同要求 JSONL 每行含非空且唯一的 `episode_uid`，记录数和唯一数均为 900。paired report
使用 `schema_version/filename/record_count/sha256`；D6 external audit v2 的
`candidate.paired_lineage` 使用 `available/sha256/record_count/unique_episode_uid_count`；
manifest 和 admission report 分别保存四项和三项绑定字段。旧 `file` 字段、缺失、篡改、重复
UID 和计数不一致均失败关闭。

当前 runtime SHA 为 `b0708e71...baffe`。assembler 专项 `69 passed`，lineage 联合专项
`86 passed`，D5 全量 `655 passed, 1 warning`。本节记录当时尚未运行正式重训、外审或 v5
装配的状态。main 已于 2026-07-27 按 development -> held-out -> paired-shadow/lineage ->
registry -> D6 external audit v2 -> v5 -> D6 post-assembly v2 顺序完成重建。

## 2026-07-26 六权限合同 v2 与版本治理复核（lineage 修复前）

D5 已将六项运行权限集中为 `d5.tracklet-g1-authority-contract.v2`。assembler 只接受精确六
字段、严格布尔且全部为 `false` 的 D6 authority；旧四字段、多余或缺失字段、拼写错误、未知
schema、非布尔值、任一授权值和仅有原因说明的输入均失败关闭。

新 admitted bundle 使用 `d5.tracklet-model-bundle.v5`，准入报告使用
`d5.tracklet-g1-admission-report.v2`。v5 在 manifest 和准入报告中绑定同一权限合同、D6
审计文件 SHA-256 与内容 SHA-256。
公开严格加载器每次重新读取打包的 D6 审计，验证三方字段完全一致。证据通过状态使用
`g1_evidence_eligible_not_authorized`，不会转化为分配、故障接管、控制、默认路径或 G1 在线
辅助权限。影子加载保持可用；辅助请求因 `g1_assist_granted=false` 被明确阻断。

该中间实现的 runtime implementation SHA-256 为
`fe116fd50975e4adc63354a591bbf88d5da0700b43c557dde569658d67e11c91`。
专项测试 `70 passed, 1 warning`，D5 全量 `636 passed, 1 warning`。代码合同已关闭，但该中间
阶段按任务边界没有重训、held-out、paired-shadow、D6 external audit v2 或 v5 装配。当时要求
main 形成 clean commit 后重建完整证据链，旧 runtime 证据不得转移；该重建已于 2026-07-27
完成。该实现后来因缺 paired lineage 被判定不能生成 D6
可受理 v5，现已由上述 P0 修复取代。

旧 `d5.tracklet-model-bundle.v4` 和旧 `d5.tracklet-g1-admission-report.v1` 保持历史语义；
严格加载路径不会将其解释成 v5/v2。新装配只接受
`d6.d5-g1-external-audit.v2`，input spec 和 consumer contract 分别继续使用各自 v1。

## 2026-07-26 旧 D6 audit v1 与 v4 装配复核（修复前记录）

当时输出目录名带 `v2` 后缀，但 JSON 顶层 schema 实际为
`d6.d5-g1-external-audit.v1`。该旧审计通过自身门限，JSON SHA-256 为
`24c8b0cd...9ad7d`，内容 SHA-256 为
`f17acecf...35f`。审计引用的 9 项 D5 文件均通过独立哈希复核，blocker 为空，模型晋级、G1
辅助、默认路径、分配、故障接管和控制六类权限全部为 `false`。

当时 runtime 正式 assembler 使用上述旧 audit v1 输入执行原子装配，以
`d6_authority_fields_mismatch` 失败关闭。该审计的 authority 对象比冻结 assembler schema
增加分配和故障接管两项字段。没有创建 v4、没有执行 loader 探针，也没有生成 post-assembly
handoff。该结果保留 D6 的完整权限表达，不把新增字段投影为旧合同。

该次失败触发了版本化合同修复。runtime 实现摘要随之变化，该中间阶段要求重新形成全部实现绑定
证据，不能沿用 `55066382...b8ea` 的旧 audit v1 结论。最终 runtime 的重证据、external audit
v2、v5 和 post-assembly v2 已于 2026-07-27 完成。

## 2026-07-26 clean R0 与 G1 复核（历史 5506 runtime）

main 在 clean commit `64cb865b...2b05` 完成 20-seed 几何候选图 R0。正式制品覆盖
`2670` 帧、`16842` 个匿名节点和 `4658` 条图边；离线评分为 `4642` 真边、`16` 假边和
`4645` 个真值合格同目标对。precision/recall/F1 为
`0.996565/0.999354/0.997958`，hard violation 为 `0`。该运行没有调用 G1，只评价当前时序、
投影、协方差、来源链接和稀疏几何门产生的候选图。

正式 writer 随后使用冻结 composite/formal/supplemental 输入、原 seed 和 robust-v2 超参数
重训。权重仍为 `7fb5db8b...ca71`，新 manifest `db908b05...1d14` 原生绑定该阶段 runtime
`55066382...b8ea`。这不是对旧 manifest 的重写。

该阶段 runtime held-out 正式通过 `20/900/45`，总体 precision/recall/F1 和候选召回均为
`1.0`，P95 推理时延约 `0.872 ms`。paired-shadow 的 5 类真值无关扰动最低边/簇 F1 均为
`1.0`，最高单特征 AUC 为 `0.720073`。在线真值、同相机候选边和全局编号创建或换绑均为 `0`。

证据保存在
`/tmp/MSM-d5-g1-current-runtime-retrain-64cb865-20260726/`，current-runtime registry 状态为
`evidence_chain_closed_shadow_only`。后续旧 audit v1 通过自身门限；历史 v4 因 authority
schema 不一致未生成。本节当时未形成的 external audit v2 和新 v5 已由 2026-07-27 正式证据
取代。确定性规则继续默认，所有身份、分配和控制权限保持关闭。正式流水线专项
`46 passed`，clean D5 全量 `600 passed, 1 warning in 97.84s`。
...
main 已完成上述 clean 20-seed R0，并形成 edge truth 与零 hard violation 证据。R0 不包含 G1。
合同修复前的 runtime development bundle 已严格加载并完成 shadow 评估；旧 v4 仍因谱系不匹配
而失败关闭。截至该段记录时，该 runtime 的 external audit v2 尚未通过，默认规则和所有门限
不变；最终 runtime 的 external audit v2 已于 2026-07-27 通过。

## 2026-07-26 异步相机状态复核

### 根因

统一三维在线输入按相机异步发布。旧 `Scalable3DTerminalAdapter.process()` 只把本次调用内的
状态更新批次送入图。5v5 seed 1000 的旧运行中，4 次发布只有 1 个相机批次，因此跨调用状态
无法同图。main 后续独立核验还确认，本次短输入的 6 条在线观测全部是离线标注虚警。旧运行的
零边同时受“异步状态未同图”和“输入没有真实目标共同中心支持”影响，不能单独归因于评分器。

### 实现

D5 增加按资源和相机隔离的活跃快照。新实测批次作为图锚点，其他相机的最近匿名实测航迹和对应
外参只有在量测时间、到达时间、外参年龄、missed-frame 和 TTL 全部有效时才参与。默认 TTL 为
`1.0 s`，容量为 `256`。复用状态保留双时间戳和全部观测/外参协方差，不预测、不改编号。

OOSM 不覆盖缓存。重复量测继续拒绝。缺外参、过期、超窗、容量淘汰和重复节点均失败关闭。
没有本次实测锚点时不发布历史缓存图。stream 和 episode reset 已覆盖；性能诊断只输出固定
数量，不含业务 ID。几何门、规则/G1 评分、同相机互斥、中心 Hungarian 绑定和
`global_track_id` 所有权均未改变。

### 结果

异步同目标 fixture 由修复前第二次调用的 `1 node / 0 edge` 变为 `2 nodes / 1 edge`；异步
不同目标仍为 `2 nodes / 0 edge`。规则回退和模型评分都通过。adapter 专项为 `48 passed`，
D5 全量为 `598 passed, 1 warning in 97.36s`。

5v5 seed `1000`、`2.2 s` 规则短复跑把每次节点数从旧运行的 `[1,1,1,2,1]` 提升为
`[1,1,2,2,2]`，发布时刻 `1.25/1.75/1.95 s` 出现双相机节点，两次发布复用跨调用匿名航迹。
在线 6 条 `vision_bbox` 的离线 sidecar 标签全部为
`disposition=known_false_alarm/truth_entity_id=null`，时间与相机分布为
`0.7/0.8 CAM-INT-0002`、`1.1 CAM-INT-0001`、`1.6 两相机` 和
`1.8 CAM-INT-0002`。`support_by_node` 没有共同中心 `GlobalTrack`，预筛选正确保留 0 边，
在线真值使用为 0。

该短结果只关闭异步节点同图和虚警失败关闭子项。后续 clean R0 已补充真实目标候选边和几何门
正式证据，但没有把该指标解释为 G1 收益。中心只读 binding 仍需继续验收；截至该段记录时，该
runtime 的 development、held-out、paired-shadow 和 shadow-only registry 已闭合到 D5-owned
范围，下一门是 D6 独立外审。该外审及后续 v5、post-assembly v2 已于 2026-07-27 完成。全过程
不调整门限，不把 truth 写入在线路径。

### G1 边界

D6 已在 clean evaluator commit `107cf0756d7b75cd6bf1456d1f1aa940fec6a63c` 完成既有 G1 v4
正式 post-assembly audit。结果 `status=pass`、blocker 为空，content SHA-256 为
`3738444168138584c7ec3eb895d123178092176ec751a5b455e575b177a2d852`，覆盖 `20/900/45`，
在线真值、同相机互斥违规和中心 ID 改写均为 0。该结果只证明装配完整性，不授予模型晋级、
默认、在线 G1、身份、分配或控制权限。

该阶段来源合同变更使当时 runtime SHA 变为 `55066382...b8ea`，旧 v4 绑定的是
`408e71fe...f4fe`。严格加载以 `bundle_implementation_runtime_mismatch` 失败关闭。
该 runtime 的 development、held-out、paired-shadow 和 shadow-only registry 已完成；
规则路径继续默认。在当时的六权限版本治理完成后，需要针对新 runtime 重建证据，由 D6 输出
external audit v2，再装配新 v5 并复审；这条链已于 2026-07-27 闭环。
...
## 2026-07-26 G1 evidence assembler 复核（历史 v4 合同）
...
G1 来源摘要已包含 assembler，当时为 `41381db3...94b07`。测试确认改变 assembler 会改变实现
摘要。旧 development bundle 未绑定该文件，严格加载返回 `implementation_runtime_mismatch`，
不增加兼容白名单。

正向 fixture 已由公开 strict loader/runtime 加载，说明软件合同可执行。它不代表该历史模型获准。
post-assembler D6 audit 文件/内容 SHA-256 为 `98bf9e02...c8ed` /
`40a42af0...b90d`，绑定当时实现摘要 `41381db3...94b07`。实际
...
A3 evidence assembler 在该历史 v4 阶段仍未实现。2026-07-27 已补齐 D5 采用证据软件合同，
但 A3 production writer 和公开 assist loader 仍保持失败关闭。后续
...
泛化缺口。该历史阶段缺少的 A3 evidence assembler 已由 2026-07-27 首节的软件合同补齐，
真实运行接线、收益证据和授权仍未完成。
