...
短/长逐帧业务、最终 binding 和操作数哈希与冻结记录一致；online truth use 和 `global_track_id` mutation 为 0。当前源码 D5 全量 `544 passed in 155.17s`。配对归一化增长没有稳定下降，当前源码也尚未完成三种子系统复跑，因此超线性规模成本、D6 操作数聚合和正式性能准入继续列 P1。本轮没有 AirSim 或在线学习权限变化。
