...
该结果关闭异步节点同图和虚警失败关闭子项，不评价真实目标候选边、几何门或 G1 收益。后续由
main 构造真实目标共同可见的 truth-isolated 场景，可使用 recon cue 或延长时长；truth 只离线
评分。先运行规则边并验收几何与中心只读 binding，再按当前源码重新装配 G1 并交 D6 复审。
全过程不调整门限，不把 truth 写入在线路径。
