...
| PN/Pure Pursuit/PNG 多 seed 对照 | D7 已有 PN、Pure Pursuit、`png_vm`、`png_ttc` comparison rows 和 calibration summary helper；main 已完成默认 `png_vm` 混合闭环 2v2 10-seed。 | 现有 10-seed 不是四种导引律的受控 A/B 对照；仍需同一 seed/几何/阈值下运行 PN、Pure Pursuit、`png_vm`、`png_ttc`，D7 只保持 rows/summary 字段稳定，不用对照结果绕过 gate。 | P1 |
...
| P1 | 在已完成默认 `png_vm` 混合闭环 10-seed 基线上，复现 seed 3/10 timeout，并完成 PN/Pure Pursuit/PNG 受控对照与 visual gate/range/closing speed calibration。 | D6 报告按 seed/pair 区分 collision、timeout、实际 `vision_terminal`、raw gate allowed 和 aggregate switch；同场景输出四导引律对照、阈值版本、闭合速度/距离误差、D4/D5/D3 阻断和 `summarize_guidance_calibration()` advisory。 |
