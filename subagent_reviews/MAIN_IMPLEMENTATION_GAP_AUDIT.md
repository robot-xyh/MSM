...
本轮没有新增运行级 P0。D3、D4、D5 的开发诊断能力已补齐，D6 正式运行准备度已关闭
调用方自签事实进入正式结论的路径。G1、A1、A2、A3、C1、F1 仍未取得运行授权，正式
学习 episode 仍为 0。
...
1. **D3 已能量化离散分配边界。** 新增 A1 冻结帧动作裕量校准，复用原规则代价、
   ...
   采用或收益证据。
2. **D4 已定位实际区域策略的主拒绝原因。** 独立校准集覆盖 20 个 seed、420 个样本。
   两次复跑均得到 76 个安全非零输出和 344 个资源不可行主分类；360 个样本包含资源
   不可行原因，其中 16 个仍保留其他安全动作。候选实现谱系与当前代码不一致，因此
   `actual_model_nonzero_development_evidence_available=false`。该结果只能描述历史候选
   行为，不能作为 A2 准入、二级接管或系统收益证据。
3. **D5 已阻断类别不平衡造成的伪高准确率。** 行为克隆使用有上限的逆平方根意图权重，
   验证损失固定复用训练权重，并报告逐动作精确率、召回率、F1、宏平均、相机角色、
   置信度校准和分布外比例。99:1 夹具的总体精确动作准确率为 0.99，但少数动作召回为
   0，模型前置检查仍失败关闭。训练集缺类、零分母、非法候选索引和非有限特征均显式
   拒绝；零检测继续输出 `reacquire` 和 coverage=false。
4. **D6 准备度信任根已收紧。** readiness v2 不再接受调用方提供 gate facts，也不再
   信任十类通用 `d6.learning-run-*-evidence.v1` 自签文件。当前只对既有 canonical
   seed split 严格审计链提供受信 adapter；适配器逐文件校验相对路径、外层/原始文件
   SHA-256、固定目录布局和内部 schema，并由既有 D6 auditor 重算冻结种子事实。模型
   来源、实际采用、运行确认、物理窗口、同键 R0、成对非退化、真值使用、有限状态和
   外部权限九类门暂时保持 unavailable。
5. **资源门继续阻断正式大批量运行。** 当前文件系统可用空间约 13 GiB，低于固定
   20 GiB 保护线。磁盘状态只改变 execution startability，不改变模型或证据结论；未
   启动 900-cell 或完整多 seed 写盘，也未降低保护线。
6. **共享工作区回归通过。** D3 为 `571 passed, 1 skipped`，D4 为
   `689 passed, 1 warning`，D5 为 `744 passed, 2 warnings`，D6 为
   `1124 passed, 1 warning`，可扩展三维为 `352 passed, 1 warning`，跨模块合同为
   `8 passed, 1 warning`。skip 为可选 OR-Tools；warning 为既有 Matplotlib
   `Axes3D` 和显卡管理接口提示。

仍开放的 P1：

1. D3 需在与调参隔离的冻结 20-seed 输入上形成可辨识 binding 变化，并补齐计划发布、
   运行确认、D7 命令谱系、物理窗口和同键 R0 非退化。
2. D4 需重新生成与当前实现谱系一致的候选，完成独立时延测量、实际安全采用、D3 后继
   计划、ACK、物理窗口和 D6 成对审计。
3. D5 需补充真实 `hold`、少数动作和侦察相机示范，并在冻结谱系及至少 20 个未见
   非合成 seed 上完成成对非退化。
4. D6 需按既有 producer/auditor schema 逐项增加剩余九类受信 adapter。适配器形成前，
   六个学习变体的 `formal_evidence_readiness` 必须保持 unavailable。
5. main 需获得不少于 20 GiB 的可用输出空间或经核验的外部归档挂载点，随后才能启动
   正式分片和完整多 seed 运行。
