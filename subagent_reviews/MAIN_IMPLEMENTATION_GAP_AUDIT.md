...
4. **D5 异步跨相机节点断点已关闭。** 有界快照保留匿名局部航迹、量测时刻、到达
   ...
   重复和容量超限失败关闭。D5 全量为 `598 passed, 1 warning`，scalable D5/D7 消费
   主栈为 `76 passed, 1 warning`。原 nominal 5v5、seed 1000、2.2 秒短测只有 6 条
   视觉虚警，双相机零边是正确拒绝，不能评价真实目标几何门。
5. **main 已形成真实目标共同可见的开发校准输入。**
   `d5_crossview_visibility_calibration_v1` 的 5v5、2 个侦察节点、seed 1000、
   12 秒运行状态有限，在线真值读取为 0，相机命令 `791/791` 接受。离线侧车确认
   667 条目标视觉观测覆盖 5 个目标、虚警为 0。D5 累计形成 294 条候选边、247 条
   几何边、139 次跨调用相机复用和 530 次中心航迹绑定。该结果来自开发工作树，只证明
   正向输入链可达，不构成边分类精度、错误合并率或 G1 收益证据。

当前最短 P1 路径是：

1. 在 clean commit 上对跨视角可见性配置运行至少 20 个保留 seed，离线统计规则 R0 的
   候选边召回、错误边、错误聚类、中心绑定正确率和运行时延。
2. 按当前 D5 实现重新装配不可变 G1 bundle，由 D6 重做 post-assembly audit；未通过前
   不运行学习组。
3. 使用相同 seed、外生配置和随机日程运行 R0/G1 paired evaluation。G1 必须同时满足
   模型实际加载、候选边大于 0、无 fallback、错误合并不恶化和全部安全计数为 0。
