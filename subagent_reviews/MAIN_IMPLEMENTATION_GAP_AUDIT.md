...
- shard 0-1：各 45/45 完成、压缩、双端复核。
- 全作用域：90/900，尚不具备正式 R0 总体结论。
...
2. shard 0-1 覆盖 seed 1000/1001、9 类场景和 5/20/50/100/200 五档规模。90/90 有限状态，
   在线真值使用总数为 0。
3. D6 只读复核基础正式准入、矩阵正式准入和 D1-D2 代次合同均为 90/90。D4 过时建议
   和无效建议均为 0。
4. 两个分片原始合计 2,988,529,684 字节，压缩后合计 290,018,789 字节。shard 1
   压缩包 SHA-256 为
   `519234f124a36a46b0467d9d99aef628dbb99a489de2574656fc4d96544ca1e9`。
5. 源目录和压缩包再次逐文件相等，均未删除。`run-shard` 的 20 GiB 保护线保持启用。

### 开放 P1

1. 完整 ID Switch 指标为 0/90 available，生产者均声明
   `producer_declared_id_switch_count_unavailable`。不可用项未补零。
2. 100 对 100 和 200 对 200 累计平均实时倍率为 0.463 和 0.194，部署实时性仍未关闭。
3. 继续长期运行 shard 2 至 19 前，需明确是否允许在双端复核后人工移除对应原始分片。
4. 全部分片恢复、确定性合并和 D6 900-cell targeted/full posterior 尚未执行。
...
- 20 GiB 运行保护线：保持不变。
- 正式 900-cell：仍关闭；clean execution plan 和 shard 0-1 已完成，当前缺少后续 18 个
  分片及逐片源目录移除授权。
...
1. 顺序执行剩余 18 个分片；每片完成后压缩并使用 `verify-shard --source` 独立复核。
