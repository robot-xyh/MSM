...
- 20 GiB 运行保护线：保持不变。
- 正式 900-cell：仍关闭；当前缺少新 clean execution plan 和逐片源目录移除授权。
...
1. 从当前代码冻结新的 clean source 和 900-cell execution plan。
2. 顺序执行 20 个分片；每片完成后压缩并使用 `verify-shard --source` 独立复核。
3. 只有用户明确授权后，才能人工移除已经逐文件验证的原始分片。当前历史正式目录保持
   原状。
4. 合并前恢复全部 20 个分片并重新执行存储校验，再运行
