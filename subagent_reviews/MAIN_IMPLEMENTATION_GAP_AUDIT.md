...
- shard 0：45/45 完成、压缩、双端复核。
- 全作用域：45/900，尚不具备正式 R0 总体结论。
...
2. shard 0 覆盖 seed 1000、9 类场景和 5/20/50/100/200 五档规模。45/45 有限状态，
   在线真值使用总数为 0。
3. D6 只读复核基础正式准入、矩阵正式准入和 D1-D2 代次合同均为 45/45。D4 过时建议
   和无效建议均为 0。
4. 分片包含 2027 个文件，原始 1,503,424,928 字节，压缩后 144,187,839 字节。
   压缩包 SHA-256 为
   `9f0b8c426ea6f04d34f18414f646e2444d4f2548ba90bfa83c4adfe1fd60ee13`。
5. 源目录和压缩包再次逐文件相等，均未删除。`run-shard` 的 20 GiB 保护线保持启用。

### 开放 P1

1. 完整 ID Switch 指标为 0/45 available，生产者均声明
   `producer_declared_id_switch_count_unavailable`。不可用项未补零。
2. 100 对 100 和 200 对 200 平均实时倍率为 0.458 和 0.196，部署实时性仍未关闭。
3. 继续运行 shard 1 至 19 前，需明确是否允许在双端复核后人工移除对应原始分片。
...
- 20 GiB 运行保护线：保持不变。
- 正式 900-cell：仍关闭；clean execution plan 和 shard 0 已完成，当前缺少后续 19 个
  分片及逐片源目录移除授权。
...
1. 顺序执行剩余 19 个分片；每片完成后压缩并使用 `verify-shard --source` 独立复核。
...
- 正式 900-cell：仍关闭；D4 低层 formal 门已恢复为 `6/6`。分片压缩链已验证，当前
  直接阻断为新的 clean execution plan 尚未生成，且逐片源目录移除尚未获得授权。
