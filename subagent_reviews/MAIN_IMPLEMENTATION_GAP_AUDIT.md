...
**最新 D1 状态（2026-07-24）**：PSD-safe V3 正式矩阵已完成 13 组 pair、26 个 episode。
向量化协方差限制通过 D6 预注册准入；独立 seed 和长时增长子缺口关闭。系统实时、严格精度、
AirSim 和目标硬件证据仍为 P1。以下 V3 记录优先于早期单 seed clean 记录。
...
V3 candidate 的下一可分离热点是 D1 扫描输入，short/long 累计均值为
`1.220624/6.572076 s`；D2 关联 long 均值为 `5.815163 s`。后续先剖析扫描输入的数据组织、
复制和固定滞后重放成本，再以同一矩阵复核 D2 关联。不得通过降低量测频率、缩短滞后窗口或
删减协方差字段获得表面性能收益。
