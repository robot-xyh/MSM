...
## 2026-08-10 D5长距离时序关联与D6回灌

### 当前判断

- 新增运行级P0：无。在线真值隔离、中心持有`global_track_id`、实测/预测证据分离继续保持。
- D5有状态时序几何关联已接入main长距离ComputerVision episode；每相机状态独立，reset后
  不复用。
- 0.25秒内的缺测只形成带协方差的非授权外推；单帧换绑不生效，连续两帧且满足原门限后才
  确认。D7 PNG公式和控制路径未修改。
- main已自动调用D6生成逐episode CSV、聚合JSON、中文Markdown和PNG，D6保持只读且无
  控制权限。
- 冻结v2单seed基线仍失败关闭：1934条可评分关联，准确率0.9979317477，身份切换0，短缺口
  3，长期重发现48，几何绑定切换7，交叉可评分3/31；重复分配、在线真值和全局编号改写均为0。

### 已闭合

1. main不再直接把逐帧瞬时匈牙利结果当成稳定绑定。发现、提示、完成和准确率只消费D5
   `measured_assignments`。
2. `active_bindings`和短时外推记录明确不可授权，运行时输出绑定事件、掉检外推、双时间戳、
   预测年龄和像素协方差增长。
3. 运行时同时保留逐帧瞬时换绑数与两帧确认后的时序换绑数；超过保持窗的过期才进入有效
   中断口径。
4. 空关联、空事件和空掉检CSV保留固定表头；D6对历史无表头空文件标记不可用，不抛异常，
   也不把缺失证据写成零。
5. 接口回归为AirSim长距专项23通过、D5全量904通过、D6专项7通过、D6全量1425通过并
   跳过16项。Matplotlib三维坐标轴环境告警为既有告警。

### 开放P1

1. 新时序关联器尚未在真实AirSim v3长距episode上复跑。冻结v2基线只能作为改造前对照，
   不能证明3次短缺口或7次绑定切换已经消除。
2. `crossing_calibration_v1`已通过2、5、20目标几何预检，但实际成像、共同可见和关联事件仍需
   真实复跑；预检不能计入实际交叉窗口分母。
3. 至少10个未见seed的真实AirSim批量校准尚未启动。P1验收仍需统计有效中断、确认换绑、
   关联错误、交叉可评分比例及失败原因分布。
4. 当前`GlobalTrack`仍是合成D1/D2夹具；真实雷达误差、漏报、云台滞后和图像检测误差尚未
   进入本专项。

专项证据见`subagent_reviews/MAIN_D5_LONG_RANGE_TEMPORAL_INTEGRATION_REPORT_20260810.md`。

## 2026-08-03 终极目标完成度复核

### 当前判断

- 新增运行级 P0：无；规则主线和失败回退仍可用。
- 可扩展三维主模块当前回归：`522 passed, 1 warning`。
- D3/D4/D5 来源生成：`728/728` 已关闭；D6 载荷独立审计已通过，当前代际训练仍为开放 P1。
- D1 质量：单种子暖机召回 `0.8183`，低于建议目标 `0.95`。
- D2 质量：200 规模正式样本 `67/90` 可计算严格身份指标，64 个 episode 累计 562 次
  ID Switch，身份连续性未闭合。
- 总线：D1-D3 仍存在同进程直接对象传递，D1 航迹摘要未完整携带双时间戳。
- 正式 R0：`450/900`；G1/A1/A2/A3/C1/F1 最终同键对照、20-seed 五米物理闭环、最终
  Word 报告均未完成。

### 开放 P1

1. **学习来源字段完整性**：D6 已独立复核 D3/D4/D5 的 episode/sample、split、时间戳、
   真值隔离、身份所有权和文件 SHA-256。D3/D4 来源未提供协方差，D3 未提供显式六维状态，
   D5 未提供显式 bbox/local-track geometry；模块 owner 需在训练前决定是否升级来源合同。
2. **D1/D2 规模质量**：使用冻结的多传感器密集场景和至少 20 个未见 seed，校准 D1 召回、
   协方差一致性和 D2 身份连续性。当前结果不能写成达到目标。
3. **统一总线闭环**：补齐 D1 航迹发布的双时间戳，消除 D1-D3 只依赖进程内对象传递的
   运行路径，并保持总线版本、序号、计划版本和身份承诺一致。
4. **当前代际学习候选**：A1 v3、A2 v8、A3 v3 分别完成训练、模型卡、权重哈希、验证、
   一次性保留评价和 D6 模型审计。训练、shadow、assist 和控制必须分开授权。
5. **规则和组合验收**：补齐 R0 shard 10-19；随后形成 R0/G1/A1/A2/A3/C1/F1 同 seed、
   同外生输入对照和至少 20 个未见 seed 的五米物理闭环。
6. **性能与交付**：关闭 200 规模实时倍率、长期增长、显存和阶段耗时归因；输出最终三维
   图、GIF/MP4、中文 Markdown 和 Word 报告。

逐项证据见
`research_modules/scalable_3d_simulation/docs/SCALABLE_3D_GOAL_COMPLETION_AUDIT_20260803_CN.md`。

## 2026-08-03 D3-D5 来源生成收口

### 当前判断

- 新增运行级 P0：无。
- D3 A1 v3 来源：`300/300` 已最终封装，3086 帧、300 个唯一 seed。
- D4 A2 v8 来源：`324/324` 已最终封装。
- D5 A3 v3 来源：`104/104` 已最终封装，development/future-held-out 为 `72/32`。
- 三模块合计 `728/728`；训练、运行、控制、正式 seed 读取和 future-held-out 模型消费均为 0。
- D6 载荷独立来源审计已通过；训练和模型准入仍未授权。

### 已闭合

1. 原“D3/D4/D5 producer 已就绪但正式来源未生成”的 P1 状态已关闭。D3、D4、D5 分别在
   clean source commit `e7c438c`、`6737b44`、`64dfc08` 上完成 generation-only 生成。
2. D5 在旧运行的序列 97、`seed 24097` 因 observe/hold 无共同几何失败关闭。main 仅在
   冻结采集窗口内保持“侦察相机到当前 D3 计划内中心航迹”的 truth-free 提示绑定。目标
   退出当前计划或航迹集后立即重选，operational profile 不受影响。
3. D5 新运行严格覆盖序列 `0-103` 和 seed `24000-24103`。原失败 seed 已通过，全部 104
   个 episode 的在线真值、`global_track_id` 创建和改写聚合计数为 0。
4. D5 总来源清单为 `b3bbdc1b...ad7978`，development 和 future-held-out 清单分别为
   `20afdfe3...0a7f4`、`f9db3912...62489`。状态为 `source_generated_not_trained`。
5. 旧 D4/D5 失败目录继续保持 `failed_closed`，不恢复、不迁移、不与新来源合并。
6. main 生成 SHA-256 为 `341afff7...aca60092` 的外置预检输入。D6 在按模块兼容 D4
   `schema`、D3/D5 `schema_version` 后完成真实元数据预检，结果 SHA-256 为
   `2c051c5d...b31205f`。三模块状态均为 `metadata_ready`，阻断项、payload 打开、训练和
   运行权限均为 0。
7. main 在精确 audit-only 授权下调用 D6 读取冻结 inventory。v3 审计状态为
   `source_integrity_audit_passed_not_training_authorized`，D3/D4/D5 分别覆盖
   `300/324/104` episode 和 `3086/921/280968` 条语义记录；真值泄漏、切分泄漏和 blocker
   均为 0，机器报告 SHA-256 为 `8fa4f39c...bc7`。

### 开放 P1

1. **来源字段边界。** D3/D4 来源没有协方差，D3 没有显式六维状态，D5 没有显式
   bbox/local-track geometry。该边界不影响完整性审计结论，但会限制后续模型输入与诊断。
2. **模型质量与准入。** 新来源尚未训练、选模、调阈值或执行验证/future-held-out 模型消费。
   A1/A2/A3 学习候选仍不得进入辅助、分配、降级、相机命令、运行或控制链路。
3. **外部证据。** 本批为三维质点来源，不能替代 AirSim、真实相机、硬件在环和实飞验证。

详细清单见
`research_modules/scalable_3d_simulation/docs/SCALABLE_3D_D3_D5_SOURCE_GENERATION_COMPLETION_20260803_CN.md`。
...
12. main 跨模块预生成门已核对 658 个保护 seed、728 个新分配 seed 和全部来源 SHA-256。
    D3/D4/D5 计划和 producer adapter 均为 `3/3` 完整。clean `f0819e0` 预检状态为
    `blocked_by_source_generation_request`，专项 `9 passed`；执行命令为空，所有权限为
    false。
...
1. **A1 正式来源。** adapter 与 A1 v3 writer 已通过 smoke，但 300 episode 生成请求、正式
   分区目录、manifest 和 D6 来源审计尚未获批或执行。
2. **A2 正式来源。** 区域拓扑、通信、供需、正反转移、困难负样本和 v8 writer 已接通，
   但 324 个 TRAIN episode 尚未获批生成；TRAIN 冻结 actor 后才能申请新 validation/test。
3. **A3 正式来源。** 四段意图窗口、相机角色、五类困难混淆和配额 writer 已接通，但
   104 episode 三 split 来源尚未获批生成；future-held-out 继续一次性失败关闭。
...
- 新的来源独立 A3 质点 corpus：100/100 episode 已生成并最终封装。
- D5 仿真研究来源门：通过；训练覆盖门：失败关闭。
- D6 对实际 finalized root 的独立来源审计：12/12 通过。
...
6. main 从 clean `4a8c117` 使用新 seed `21000-21099` 完成 100 个 episode。9 类场景、
   5 档规模和 45 个场景-规模单元均覆盖，在线真值使用、checkpoint 恢复和正式 seed 重叠
   均为 0。最终语料含 159487 个样本。
7. D6 对实际 finalized root 的 302 个制品独立审计。100 个来源身份均为 clean
   `4a8c117`，在线 truth/Actor/Object 标识均为 0，训练/验证/测试 seed 互斥，12 项检查
   全部通过。
8. D5 全样本检查确认运行 ACK 为 159487/159487 accepted，匿名观测键为
   159487/159487 且重复为 0。物理匿名观测帧和离线 outcome 仍不可用，因此这些计数不构成
   策略收益或物理非退化证据。

### 开放 P1

1. D5 训练门确认结构覆盖不足。训练集 102610 个样本中 `hold=0`；侦察相机的
   `search_sector=0`。现有大量 `reacquire` 样本不能替代缺失动作。
2. 下一批只补采 `hold/interceptor`、`hold/recon` 和 `search_sector/recon`。每项至少使用
   2 个新训练 seed，形成 2 个新 episode 和 2 个样本；禁止复制、过采样或重加权后声称
   覆盖通过。
3. 补采数据必须继续使用单一 `scalable_3d_point_mass_runtime` 来源，排除验证、测试和正式
   seed；D5 全样本门与 D6 独立审计都通过前，不得启动训练。
4. 来源完整性不等于模型效果。训练覆盖闭合后，A3 仍需来源独立评价、实际运行确认、
   唯一同键 R0 和物理非退化；此前所有运行权限保持 false。
...
- main 导出接线：`1fe66ff`
- D6 独立审计：`4df42ab`
- A3 语料计划：`4a8c117`，schedule SHA-256
  `9c45caac96879289b84608e9779ec913a9d8cd2040e20322cf8944ad06eb4e51`
- A3 数据集 manifest：
  `bccbdad42a71b130720469bb4e99dd1dd99e29a9b33af036679b9d64b0fe35a4`
- D5 语料审计：`85db29f86d924a437259a478e2fb182c220d3469c8f8a0c4374820e61e6ef74e`
...
- 新增运行级 P0：无。
- D3-only 来源数据：100/100 episode、292 帧、seed 20000-20099，完整可用。
- D3 v2 预注册总体机器门：通过，但状态为 `not_admitted`。
- D6 外部独立审计：通过，只确认离线完整性和机器门。
- 正式 seed 1000-1019：读取数 0。
- 运行、辅助、分配、计划、控制、物理、正式和生产权限：全部关闭。

### 已闭合

1. main 从 generation plan、summary、checkpoint、progress、seed registry 和 D3 dataset
   manifest 核对来源。生成提交为 clean `fc7a1c2`，100 个 episode 全部有限，在线真值
   使用为 0，D3-only 组件清单固定，未拼接首次失败的 60 项。
2. 冻结来源覆盖 5、20、50、100、200 五档和 10 个场景规模单元。数据集包含 292 帧，
   train/validation/test 为 178/57/57 帧，split episode 为 60/20/20；schedule SHA-256
   为 `468bddc8...1009`。
3. D3 v2 在合同、bundle、归一化、教师、门限和安全投影不变时只运行一次。总体安全
   换绑 `13/110=11.82%`，教师完全匹配 `8/110=7.27%`，负类 exact-R0
   `182/182=100%`，通过预注册的 5%、2% 和 99% 门限。
4. 94 个拒绝帧的矩阵和绑定均恢复 R0。重复资源、硬禁边、M-to-N 原子性、版本、规则
   矩阵突变和模型 assignment/plan/runtime 输出均为 0。
5. D6 外部审计不导入 D3 evaluator、aggregate、模型或 solver。审计器独立解析固定
   21 列、292 行 CSV 并与 JSONL 逐行闭合，复算 split hash、全部规则矩阵摘要，以及
   R0/candidate/effective 各 21637 条边的索引、资源容量、动作掩码和需求完整性。
6. D6 新增 CSV 与 checksum 联合篡改、错误数据集绑定、自报安全计数为 0、边越界、
   矩阵摘要冒充、评价真值字段和 split hash 冒充等失败关闭负例。专项 `18 passed`，
   D6 全量 `1348 passed, 1 warning`。

### 开放 P1

1. **子组泛化**：test 子组安全换绑为 `2/25`，教师完全匹配为 `0/25`。合同只预注册
   292 帧总体门，不能在结果产生后新增子组门限；该事实仍阻止把总体通过解释为充分泛化。
2. **正式评价**：seed 1000-1019 尚未读取。后续需要新冻结的一次性正式合同、完整正式
   R0 来源和独立 D6 后验，不得用本批结果调参后重复评价。
3. **运行和物理证据**：本批没有 D3 计划发布、运行 ACK、D7 控制、完整物理窗口或同键
   R0 非退化结果。当前候选不得进入默认分配或控制路径。
4. **正式父矩阵**：R0 当前仍为 450/900，shard 10-19 受存储保护线限制。D3 来源独立
   机器门通过不替代剩余正式 R0、归档合并和 D6 targeted/full posterior。

### 证据

- D3 结果：
  `research_modules/d3_assignment_planner/results/a1_source_independent_evaluation_v2_20260731/`
- D6 外部审计：
  `research_modules/d6_evaluation_metrics/reports/D3_A1_SOURCE_INDEPENDENT_V2_EXTERNAL_AUDIT_20260731/`
- main 汇总：
  `research_modules/scalable_3d_simulation/docs/SCALABLE_3D_D3_A1_SOURCE_INDEPENDENT_EVALUATION_20260731_CN.md`

## 2026-07-31 正式 R0 新批次进度
...
1. execution plan 绑定 5700 个父清单单元、900 个 R0 单元、20 个分片和正式 seed
   1000 至 1019，SHA-256 为
   `b922ff5f95864345efa583da7256935694e5c675529989a659716522a0d7590e`。
2. shard 0-9 覆盖 seed 1000-1009、9 类场景和 5/20/50/100/200 五档规模。450/450 有限状态，
   在线真值使用总数为 0。
...
1. main 增加正式分片存储校验，使用与 `merge-r0` 一致的静态计划、连续进度、单元制品、
   checkpoint 完成状态和摘要检查。
2. main 增加确定性 PAX tar 与单线程 Zstandard 压缩。manifest 同时绑定 execution plan、
   parent plan、源提交、分片编号、单元清单以及三个分片控制文件摘要。
3. 创建前后重复检查源目录；压缩流逐文件校验。源变化、压缩损坏、错 execution plan、
   路径跳转、非普通文件和恢复后语义不一致均失败关闭。
4. 工具没有删除入口。归档和恢复采用临时目录与原子发布，只清理工具创建的失败临时目录。
5. 历史完整分片 17 的原始大小为 1,086,483,308 字节，压缩后为 113,628,123 字节，
...
- 验证报告：
  `research_modules/scalable_3d_simulation/docs/SCALABLE_3D_FORMAL_SHARD_ARCHIVE_VALIDATION_20260731_CN.md`
- 历史分片 17 压缩包 SHA-256：
  `bbf98e30a9af38c22ee4100b7ff129d85422d14f49a0f369cccca19e34df040c`
- 文件树 SHA-256：
...
- 正式 900-cell：仍关闭；D4 低层 formal 门已恢复，正式新批次当前 90/900。逐片源目录
  移除尚未获得授权，剩余 18 个分片和最终 D6 后验未完成。
...
4. **存储执行**：`/dev/shm` 总容量仍低于 20 GiB。根文件系统可使用逐片压缩降低驻留
   空间，但必须在每片复核后取得原始分片移除授权；不能用降低 20 GiB 保护线代替。
...
## 2026-07-30 D3 来源独立数据故障围栏导出（历史状态）
...
2. clean commit `fc7a1c2` 已重新生成 100/100，main 已核对真值隔离、seed 隔离、
   有限状态和制品摘要。
3. D3 v2 一次性评价和 D6 独立重算已于 2026-07-31 完成，最新指标和限制见本文首节。
4. A1 正式 holdout、assist、分配、计划发布、控制和物理权限仍继续关闭。
...
| D7/main/D6 | `png_ttc` 受控覆盖 | tuned 2v2 10 seeds 为 20/20，not-expanding/TTC-out-of-range 已实测；2026-08-01 真实 AirSim seed 1 已完成 area-jump 与 bbox-clipping 各一次受控注入，两例均正确拒绝视觉控制、回退 `radar_pn`、保持期望/分配身份一致，配对和目标物理结果均为 2/2 | 单 seed 运行边界已关闭；仍需两类受控 case 的多 seed 复验。完整 suite 本轮未运行，不得把受控子集通过写成总体通过 |
...
| D6/main | 末端本地 ID Switch 的多流分组 | 2026-08-01 两个真实 2 对 2 受控 case 中，诊断值 2 均为各自资源/相机流内本地号变化；正式执行层保持 unavailable，未用诊断值补齐 | 当前实现只按 `global_track_id` 分组；需冻结 `(global_track_id, resource_id, camera_id/stream_id)` 口径，验证跨流稳定、单流切换、记录重排和缺流标识失败关闭后，才能用于 M 对 N 正式验收 |
