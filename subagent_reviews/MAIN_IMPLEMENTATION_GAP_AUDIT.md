...
1. **D6 独立来源审计。** 本轮授权只允许 D3/D4/D5 数据源生成，D6 尚未独立复算清单、
   seed/split、唯一性、双时间戳、协方差和身份隔离。该工作必须另行授权。
2. **模型质量与准入。** 新来源尚未训练、选模、调阈值或执行验证/future-held-out 消费。
