...
2. **D4 已具备当前谱系候选构建边界。** 历史候选在 20 个校准 seed、420 个样本上得到
   76 个安全非零输出，但谱系与当前实现不一致。新增构建器只读取 train，模型选择只读取
   validation；test、历史 calibration 和保留 seed 读取数固定为 0。manifest 绑定源码、
   数据集、拆分、配置、权重和训练摘要。当前共享工作区为 dirty，实测按
   `source_worktree_dirty` 拒绝生成实物，未伪造 clean-lineage 候选。
...
1. D3 需在与调参隔离的冻结 20-seed 输入上形成可辨识 binding 变化，并补齐计划发布、
   ...
   完整性，不替代这些运行证据。
2. D4 需在本轮代码提交后的独立 clean checkout 中生成并冻结当前谱系候选，再完成实际
   安全采用、D3 后继计划、owner/coalition ACK、物理窗口和 D6 成对审计。
...
5. main 需将临时目录中的正式 G1 原始制品归档到持久只读位置，并获得不少于 20 GiB
   的可用输出空间或经核验的外部挂载点，随后才能启动正式分片和完整多 seed 运行。
