...
## 2026-08-10 D5长距离配准评估 P1 部分关闭

### 已关闭范围

D6 已具备独立、只读的 v2/v3 episode 评估入口。加载、指标可用性、分相机汇总、结构门、实际
交叉门、CSV/JSON/中文报告/PNG 和 CLI 已实现。冻结 seed `20260810` 的事实得到复现：准确率
`0.9979317476732161`、`1934` 条可评分、错误 `4`、身份切换 `0`、短缺口 `3`、长期重发现
`48`、几何绑定切换 `7`、交叉可评分 `3/31`，三项安全计数均为 `0`。专项软件回归为
`7 passed, 1 warning`，D6 全量为 `1425 passed, 16 skipped, 1 warning in 129.29s`。真实 main v3
行结构和无表头空 CSV 均有回归；空文件保持不可用，不解释为零事件。

本项关闭 D6 对历史 v2 兼容、v3 接口可表示、缺字段不补零和 preflight/actual 隔离的实现缺口。
D6 没有修改 D5 或 AirSim runtime，也没有在线关联和控制权限。

### 未关闭范围

1. 真实 AirSim 尚未产出 v3 时序绑定与掉检事件。保持帧、同编号恢复、保持过期和绑定振荡只有
   合成夹具，仍属 P1。
2. 冻结单 seed 的有效短缺口中断为 3，实际交叉可评分仅 `3/31`，低于 `10` 和 `0.30` 两项门限。
3. 至少 10 个原始尺度 seeds 尚未运行。D6 不能从单 seed 或软件测试关闭多 seed P1。
4. 当前没有新的 D6-owned P0；P1 在真实 v3、多 seed 和 main 交叉模块复核完成前保持开放。
...
1. 完整 900-cell 分母、20 shard、merged index 和逐 episode artifact tree 的独立审计
   已实现。
2. 全量路径复用五项定向审计的低层 evaluator，没有复制第二套宽松后验逻辑。
3. 每项均从 online observations 与 summary 重算在线真值、有限状态、clean formal、
   matrix formal、D1/D2 generation、skip 和 pending。
4. 缺 cell、重复 progress、SHA 篡改和必需证据不可用均有直接失败关闭测试。
5. 完整 JSON、逐 cell CSV、中文报告、紧凑 JSON 和 `SHA256SUMS` 已生成。
6. 原五项定向审计入口和配置保持兼容。

### 结果与开放 P1

- R0 执行与审计范围 `900/900`，clean formal、matrix formal、generation verified 均为
  `900/900`。
- 严格通过 `872/900`。28 个失败 cell 均为高威胁 M 对 N 场景的
  `d4_fail_closed:collecting_member_acks`。D4/main 需处理成员确认收敛、lease/timeout
  和 episode 终止条件；D6 保持原 evaluator 判定。
- D2 ID switch 900 项均显式 unavailable。D2/main 需要提供独立离线真值配对制品，D6
  才能形成该指标分母。
- D4/D5 四项安全计数 700 项可用且为 0，200 项因 runtime 未发布而不可用。跨场景总计
  继续为 `null`。
- 完整父矩阵仍为 `900/5700`。G1、A1、A2、A3 同范围对照、因果收益和学习准入保持开放。

当前无新增 D6-owned P0。D6 全量审计代码与证据输出缺口已关闭；开放项属于跨模块运行
证据和后续学习对照。专项 `19 passed, 1 warning in 2.31s`，全量
...
1. 已增加统一 strict input/output/consumer schema，顶层未知字段、非有限值和内容摘要篡改在
   进入模块 validator 前拒绝。
2. 已按 A1、A2、A3 分别输出 availability、最高阶段、blocker、实际采用、物理窗口、同键 R0
   和收益审计输入。规范字段为 `benefit_auditable_count`，旧字段作为等值兼容别名；输入完整
   只写 `audit_input_available`，不声明正收益或非退化。
3. 已复用 A1 核心和 A3 的公共 strict validator。A2 使用 D4 公共数据对象和严格构造接口逐层
   重建，并要求 `to_dict()` 精确往返。候选、选择、计划、命令、ACK、权属回执、联盟确认、
   相机反馈、双时间戳、物理窗口、同键身份和权限按各自合同复核。
4. A2 已对齐真实 D4 `RegionResourceA2BenefitAuditInput`、batch 和 public validator。D6
   先索引旧安全采用记录，再按 wrapper 来源摘要查找唯一记录并调用带
   `safe_adoption_evidence` 参数的 validator。被引用旧记录不重复计数。
5. 已关闭 D4 投影后合法拒绝被误报为 `a2_d3_plan_stage_contradiction` 的 consumer 缺口。
   `safe_adoption_rejected` 在 preparation、投影和非空原因有效且后续执行证据全空时，实际
   采用数可审计为 0；物理窗口、R0、收益和权限均不开放。缺原因、字段篡改或夹带执行证据
   失败关闭。
6. D6 独立重算候选物理窗、场景/版本/规模/seed/逻辑窗口/时长、冻结外生摘要、episode/事件
   日志和唯一 R0；不信任 wrapper 汇总计数。
7. A3 增加跨 episode 检查，并修复对 trace 不存在 `comparison_identity` 属性的访问。现在由
   trace 已有字段显式重算窗口身份。候选和 R0 必须来自不同 episode；同 episode 多窗口可共享
   一个日志身份，摘要不得跨 episode，窗口不跨键复用，一个 R0 不重复配对。
8. 已增加 strict input v2 的 `a3_pairing_dispositions`，旧 v1 保持可读。输出/consumer 当前
   为 v4；v2 冻结完整分母语义，v3 增加阶段明细与输出复载，v4 增加候选观测结果清单。v1
   输入的 disposition inventory 明确 unavailable，不在原 schema 上静默改变语义。
9. 已复用 D5 `validate_active_vision_a3_pairing_disposition()`，并独立检查 trace 摘要唯一、
   pairable disposition 与顶层 A3 证据一一对应、内嵌/顶层证据相同、候选与 reason 计数守恒。
   输出候选、pairable、unpairable、覆盖率、reason 分布和两类完整性。
10. 合法 unpairable 允许进入完整原因清单，但 A3 实际采用、物理、同键 R0 和收益计数保持
   unavailable。`complete_model_evidence_claimed=false`，152 条 pairable 子集不能写成 536
   候选的完整模型证据。
11. 已增加显式多文件 loader，校验
   `scalable3d-learning-adoption-evidence-records-v1`、唯一 episode 标识和内容摘要。D6
   还要求 D4 安全采用来源 episode 与候选 execution arm 一致，D4/D5 候选和 R0 episode 文件
   均显式存在；不扫描目录，不把两个旧单臂记录自行升级为 pair。
12. synthetic、在线真值、outcome 泄漏、重复 comparison key、跨键配对、证据复用、摘要篡改
   和权限越界均有失败关闭测试。
13. 混合 A1/A2/A3 聚合不会用 A3 可用结果覆盖 A1/A2 unavailable。聚合计数只有三个变体对应
   计数都可用时才求和。
14. 已关闭仓库根目录导入布局缺口。A1/A2/A3 公共模块和 A2 子合同同时支持顶层包与
   `research_modules...` 路径；只在请求路径缺失时回退，内部依赖错误不被吞并。
15. main 的 A2 开发批次已完成 20/20 候选评估。可识别区域干预、实际采用和 A2/R0 收益审计
    均为 0，原因均为 `identifiable_regional_intervention_missing`。无操作没有被归因为学习
    采用。批次 SHA-256 为
    `ff3c10a089b6a94582451ae05d8a884af3a2bd7485acd4df0496442ea7e0ec55`。
16. main/D5 的 A3 开发批次已提供完整 disposition 分母：536 个候选、152 个 pairable、384 个
    unpairable，覆盖率 28.36%。全部不可配对原因均为
...
17. 已消费 D5 disposition v2 的 `candidate_stage_reason_codes` 和哈希绑定
    `candidate_stage_evidence`，输出顶层原因、阶段明细、顶层到明细矩阵及物理窗口明细完整性。
    D5 v1 只保留顶层原因并计入 unresolved；未知、重复、证据矛盾和计数不守恒均失败关闭。
18. 已关闭共享工作树中 D6 初始化阶段的 `NameError`。A3 inventory validator 入口在公开输出
    校验器之前定义，干净子进程可独立构造并复载 v3 输出，不依赖测试夹具导入顺序。
19. 当前 strict audit 专项为 `59 passed, 1 warning in 11.75s`；main A3 paired smoke 为
    `1 passed, 1 warning in 3.29s`。当前 D6 全量回归为
    `1101 passed, 1 warning in 101.09s`。此前 `paired_learning_adoption 5 passed`、scalable
    `345 passed, 1 warning`、cross-module `8 passed` 和 D6 全量
    `1093 passed, 1 warning in 98.33s` 均为 v3 修改前的冻结历史证据。
20. main 已用同配置 seeds 1000-1019 完成 A3 不落盘阶段开发探针。536/536 个候选有 stage
    evidence；152 条 pairable、384 条 unpairable，完整可审计 seed 为 0。多标签细分为匿名
    观测缺失 `344` 和物理窗口确认缺失 `344`。另 40 条 observation inventory 不完整，但
    stage reason 为空；D6 将其保留为 unresolved，不补写 physical-window incomplete。物理
    窗口缺失细分 scope/evidenced/unresolved 为 `384/344/40`，completeness 为 `false`，其余
    ACK、运行确认、命令过期、时序错配和相机反馈缺失均为 0。摘要 SHA-256 为
    `1ba6040e7c3e7e3b9e7d5506dfd20cf3539ce12c5aac13cca7f02799f0cd99ef`。由于
    `source_worktree_clean=false`、`formal_evidence=false`、
    `persisted_full_pair_inventory=false`，该探针
    只关闭“开发态断点完全未知”的诊断缺口，不关闭正式证据缺口。
21. 已关闭 D6 对 D5 v2 零检测语义不可见的 consumer 缺口。v4
    `observation_outcome_inventory` 分开记录候选窗口、普通轨迹帧、零检测帧、四类关联状态和
    分配目标覆盖率。零检测帧有分配目标时只能是 reacquire 和 0 覆盖，无分配目标时结果
    unavailable；locked、ambiguous、hold 或可见伪造失败关闭。可审计输入不会自动形成正收益、
    非退化或权限。
22. main 在 D5 v2 和 truth-free 帧事件接线后完成同 seeds 1000-1019 的第二次开发复跑。
    492 个候选中 488 个可配对、4 个不可配对，覆盖率 99.18699%；329 个零检测帧全部
    reacquire，159 个 v1 帧 locked，零检测帧 locked/ambiguous 为 0。4 个缺失来自默认 1%
    通信丢包，对应 seed 关闭丢包后全部配对。该结果未持久化完整逐候选清单，来源工作树不
    干净，不替换旧 536/152/384 冻结批次，也不形成模型收益或授权证据。
23. v4 strict 专项为 `64 passed, 1 warning in 11.79s`，D6 全量为
    `1106 passed, 1 warning in 100.94s`。warning 是既有 Matplotlib `Axes3D` 环境提示。

### 当前 P1 上游证据缺口

1. **A1 batch loader。** `isolated_intervention_batch` 新增的 candidate/selection inventory
   没有公共 strict loader。D6 以
   `a1_batch_public_strict_loader_unavailable` 阻断。
2. **A1 运行和配对来源。** lifecycle 只有 ACK 和物理窗口摘要，未序列化可独立复核的 runtime
   provenance、窗口载荷和 R0 comparison identity。D6 可确认摘要自洽，不能确认 synthetic
   排除或跨键配对。
3. **正式跨 episode pair 样本。** DTO、validator、D6 consumer 和 20-seed 开发批次消费均已
   闭合，main 也已逐 episode 持久化 `learning_adoption_evidence.json`。A2 开发批次没有可识别
   干预，A3 开发批次因候选物理窗口缺失不能形成完整收益输入。仍缺冻结
   `paired_exogenous_config_sha256` 后实际执行的候选/R0 双 episode 正式样本、结果指标和正式
   多 seed 汇总。新增阶段探针来自不干净工作树且未持久化完整逐候选清单，不能替代该正式输入。
4. **结果指标。** 当前新增合同只覆盖采用、物理窗和 R0 配对输入。真实结果指标、正收益和
   非退化尚未提供；D6 权限和两类结论字段保持 false。
5. **正式 disposition 全分母。** 开发批次的 536 条完整分母已经通过 v2 consumer；该接口
   GAP 已关闭。正式验收仍需为每个候选持久化一条原始 v2 disposition。开发探针的聚合阶段
   摘要不能替代完整记录；40 条空 stage reason 必须保持 unresolved，不能因上游 observation
   inventory 不完整而补造原因。只有 152 条 pairable 子集时继续按 v1 局部范围处理，
   inventory unavailable，不得据此关闭完整模型证据 GAP。
6. **第二次复跑正式化。** 492/488/4 和 159 locked/329 reacquire 当前只有开发汇总。main
   仍需在 clean source 下持久化 492 条 disposition、488 条 paired evidence 和 4 条通信丢包
   处置，并提供独立候选/R0 episode 与完整校验和。没有这些输入时，v4 只能证明软件口径，
   不能确认完整分母、未见 seed 或正收益。

本次没有新的 D6-owned P0。AirSim 集成计划已检查；代码不改变 AirSim settings、控制或运行
入口，因此不更新该文件。开发证据边界已同步到 D6 实验文档。

## 2026-07-27 D5 G1 正式外审与装配后审计 GAP 更新

### 已关闭

1. **正式 external audit v2 已完成。** 在 clean commit
   `8d5e02ec989259ce3d39e1e4ad6a90dd0d8d5b54` 上运行
   `d6.d5-g1-external-audit.v2`，结果为 `status=pass`、`blocker_codes=[]`。主 JSON
   文件/内容 SHA-256 为
...
2. **正式 paired lineage 已冻结。** 候选 lineage 为 900 条记录、900 个唯一
   `episode_uid`，文件 SHA-256 为
   `83e105290f3e624f267d92ceaf050d32291bd5bbbabf98580846cd31498b1af1`。
3. **正式 D5 v5 已由生产 assembler 生成。** manifest 文件 SHA-256 为
   `b431d066362005868374d038eb93a83b773c03715a53d8a9dfd0da21784f317d`。D5 strict
   loader 与 shadow loader 均通过，不再依赖 D6 手工构造正例。
4. **正式 post-assembly v2 已完成。** 输出
   `d6.d5-g1-post-assembly-audit.v2` 为 `status=pass`、`blocker_codes=[]`，内容
   SHA-256 为 `17dda42d06b4be1d21ff8f1f8baecc320fd49b532be06a9f9f6b304341763e1`，
   consumer 为 `d6.d5-g1-post-assembly-audit-consumer.v2`。
5. **授权边界已验证。** 模型晋级、G1 辅助、默认路径、分配、故障接管和控制六项权限始终为
   false。D5 assist 请求以 `bundle_g1_assist_authority_not_granted` 失败关闭。

由此关闭此前“正式 external audit v2、正式 v5、正式 post-assembly v2 待运行”的 P1 GAP。

### 当前 P1

1. **真实相机泛化证据缺失。** 当前输入只有合成 held-out 和 paired-shadow，真实相机、
   外参漂移、遮挡及在线检测误差的泛化状态为 explicit unavailable。
2. **中心绑定正确性证据缺失。** 输入没有将中心 `global_track_id` binding 结果与隔离的
   离线真值连接，绑定正确性为 explicit unavailable。
3. **物理闭环结果缺失。** 输入没有导引、控制或物理拦截记录，物理闭环结果为 explicit
   unavailable。

上述三项不得由当前模型边/簇性能或装配完整性推断。D6 审计是只读证据门，`pass` 不授予模型
晋级、辅助运行、默认路径、分配、故障接管或控制权限；该边界不是待放宽的缺口。

## 2026-07-26 D5 v5 生产装配正向复核

### P0 已关闭

1. **D5 生产装配与 D6 审计的正向兼容性已验证。** D6 测试直接调用 D5 公共生产写包器和
   `assemble_tracklet_g1_bundle()`，生成的 v5 再进入 D5 公共严格加载器和 D6
   post-assembly v2。正例不再依赖手工拼装 manifest。
2. **实际 v5 字段全部交叉一致。** 七文件布局、lineage 文件摘要、900 条记录和 900 个唯一
   episode UID、admission report 的 lineage 三字段、六项 false 权限、external audit
   文件/内容摘要和运行实现摘要全部通过。
3. **生产产物 lineage 负例已覆盖。** 篡改文件或删除文件分别触发摘要、计数、文件树和
   availability blocker，D6 保持失败关闭且不授予权限。
4. D5 与 D6 当前结构一致，未发现需要在 D6 owner 范围内兼容旧字段的缺口。审计门限、字段
   集合和 20/900/45 正式门均未放宽。
5. external 专项 `14 passed, 1 warning in 4.40s`，post-assembly 专项
   `55 passed, 1 warning in 4.93s`，D6 全量 `1042 passed, 1 warning in 91.36s`。

### 状态更新

正式 external audit v2、正式 v5 和正式 post-assembly v2 已于 2026-07-27 按顺序生成并通过。
本节 pytest 临时目录仍只证明软件兼容性，正式证据以本页顶部记录为准。

## 2026-07-26 D5 G1 审计版本治理 GAP 更新

### P0 已关闭

1. external audit 从四权限扩展到六权限后仍声明 v1，属于同 schema 双语义。D6 已将主输出升为
   `d6.d5-g1-external-audit.v2`。输入 spec 和 consumer 字段未变，分别保留 input v1 和
   consumer v1，并在结果中分开记录。
2. external audit v2 精确要求模型晋级、G1 辅助、默认路径、分配、故障接管和控制六项权限均为
   false。真实相机、中心 binding 和物理闭环三类缺失证据保持 unavailable，不由模型指标推断。
3. post-assembly 输出、输入、consumer 和 profile 已升为 v2。审计只接受
   `d5.tracklet-model-bundle.v5`、
   admission report v2、authority contract v2 和 external audit v2。
4. v5 的 paired lineage 是独立冻结制品。D6 校验文件 SHA-256、900 条记录和 900 个唯一
   episode UID，并与 paired 报告、external audit、manifest 和 admission report 交叉绑定。
5. 六权限、D6 文件/内容摘要、held-out、paired-shadow、lineage 和运行实现摘要均进入严格
   交叉绑定。旧 v4、audit v1、report v1、错误权限合同版本和任一混用组合失败关闭。
6. post-assembly 结果继续将六权限全部设为 false，并原样保留三类 unavailable evidence。
   审计通过不授予运行、装配、晋级或控制权限。
7. `/tmp/MSM-d5-g1-current-runtime-d6-external-audit-64cb865-20260726-v2/`
   已标记为 `rejected_transition_schema_v1`。目录名不能替代 JSON schema；该制品不得用于
   v5 装配。
8. 最新回归已增加真实 D5 生产装配正例和生产产物 lineage 篡改/缺失负例，结果见上节。

### 状态更新

1. 正式 external audit v2 已在新证据目录生成，旧 v1 内容哈希没有重绑定。
2. D5 已生成正式 bundle v5、report v2、authority contract v2 和 lineage。
3. D6 已完成正式 post-assembly v2；剩余证据 GAP 见本页顶部三项。

当前无 D6-owned P0。本模块不存在 `docs/AIRSIM_INTEGRATION_PLAN.md`；本项不改变 AirSim
接口，因此不新建。
...
9. 正式 R0 输入缺口已关闭。clean source commit `64cb865b...b05` 的批次 8834 项和 D6
   报告 3 项校验清单全部通过；dataset manifest 与 2670 条 sidecar 记录摘要绑定一致，
   20 个 seed、单一场景版本、完整标签和稳定帧坐标均满足 formal 门。
10. 正式 R0 覆盖 2670 帧、16842 个节点和 4658 条候选边。时间合格真值对 4645，保留
    真边/假边 4642/16；precision/recall/F1/false rate 为
    `0.9965650494/0.9993541442/0.9979576481/0.0034349506`。20-seed F1 均值
    `0.9976519241`，95% CI `[0.9953251507, 0.9995705026]`，硬违规为 0。
11. 合成专项 `12 passed, 1 warning`，D6 全量
   `1022 passed, 1 warning in 88.77s`，变更入口编译通过。

### 当前 P1

1. **正式 G1 候选图与 scoring 合同缺失。** R0 单臂正式输入已经闭合，但尚无同稳定坐标的
   G1 finalized dataset、sidecar 或 prediction sidecar。现有 dataset 没有 edge probabilities、
   冻结 threshold、
   selected edges 或 clusters。若要比较模型收益，需要独立 prediction sidecar，并绑定
   dataset、模型权重、实现和阈值摘要。D6 不从候选边反推模型输出。
2. **绑定和物理字段缺失。** cluster purity、中心 `global_track_id` binding correctness、
   control outcome 和 physical intercept outcome 仍 unavailable，需要各自显式输入合同。
3. **真实相机泛化未验证。** 外参漂移、时间偏差、漏检、虚警、遮挡和纹理退化下的候选图几何
   质量仍需 main 的真实或 AirSim 数据。

当前无新增 D6-owned P0。评估软件和失败关闭配对合同已闭合；开放项是正式输入生产、模型输出
sidecar 和真实相机证据。其中“正式输入生产”现只指 G1 及后续显式结果合同，R0 单臂输入已经
闭合。本项不改变 AirSim 接口，`AIRSIM_INTEGRATION_PLAN.md` 已检查且无需修改。
...
本节为旧 v1/v4 历史记录。其通过制品不满足当前 external audit v2 和 bundle v5 版本链。
...
3. 文件 SHA、内容 SHA、模型指纹、dataset/split/training-set、当次十文件实现和报告联合实现
...
8. D5 已在 clean worktree `fa3ec10` 发布 7fb5 robust-v2 正式 registry。D6 对 registry、
   bundle、held-out、paired-shadow、lineage 和十个运行时源文件重新计算摘要，当次实现摘要为
   `408e71fe...f4fe`。
...
16. main 已在 detached clean evaluator commit `107cf0756d7b75cd6bf1456d1f1aa940fec6a63c`
    上正式运行强化版审计。结果为 `pass`、`audit_passed=true`、blocker 为空，
    `tree_evidence.exact=true`；结果 JSON 文件/内容 SHA-256 为
    `12f457e2...8ea` / `37384441...d852`，三项输出校验均通过。
17. post-assembly 专项 `35 passed`、D6 全量 `1010 passed`。回归覆盖六类制品逐项篡改、
    额外未列文件、清单缺项/重复/路径逃逸、符号链接、bundle/外审权限误开、三份内容摘要错误、
    外审绑定不一致、原子写入和确定性。
18. clean commit `64cb865b...b05` 的当次 runtime 外审已完成。输入 JSON SHA-256 为
    `f98b42d3...23a5`；顶层、bundle、formal 和 registry 四套清单 24/2/2/3 项全部通过。
19. 当次十文件 runtime implementation SHA-256 为 `55066382...b8ea`，与输入、manifest 和
    held-out/paired 联合证据一致。九个 artifact 均来自当次批次，900 条 lineage 唯一，
    无历史证据混用；审计前后 80 个输入文件未变化。
20. 当次证据覆盖 20 seeds、900 episodes、45 cells、五类扰动。最低边/簇 F1 为
    `1.0/1.0`，最高单特征 AUC 为 `0.7200734257`，三类身份安全违规为 0。
21. 当次 runtime 外审按旧 v1 门限为 `pass`、blocker 为空。最终 JSON 文件/内容 SHA-256 为
    `24c8b0cd...ad7d` / `f17acecf...135f`，重复输出逐字节一致。
22. D6 输出已显式关闭 model promotion、G1 assist、default、control、assignment 和
    failover 权限；真实相机泛化、中心 binding 和物理闭环结果显式 unavailable。本轮没有
    运行 G1 episode，也没有装配新的 v4。
23. 当前外审专项 `14 passed, 1 warning in 4.39s`，D6 全量
    `1022 passed, 1 warning in 89.39s`；Python 编译、文档和差异检查通过。
...
3. **中心绑定与物理闭环证据缺失。** 当前只证明 `global_track_id` 零创建或换绑违规，没有
   中心 binding 结果与离线真值配对；输入也没有导引、控制或物理拦截记录。
4. **运行作用域未验证。** G1 正式执行后还需现有 `learning_scope_formal_audit` 做同键 R0
   配对。预准入审计不能替代运行证据。
5. **运行授权保持关闭。** 2026-07-27 正式 v5 已完成装配和两级审计，但两层 D6 审计中的六项
   权限均为 false，且没有显式 G1 运行作用域、无静默回退或同键 R0 运行证据。D6 `pass`
   不等于在线启用或默认路径变更。

当前无新增 D6-owned P0。2026-07-27 正式链已关闭 D6 外部审计输入、实现谱系、冻结门、装配
完整性、权限边界和报告缺口。上述 P1 属于候选图全链路泛化、真实相机、中心绑定、物理闭环和
正式运行作用域。
...
定向回归已经通过。正式 R0 已在后继 clean source `1e5ed8d` 上启动。以下为增量阶段
177/900 的历史记录；当前完整结论见文件顶部“正式 R0 全量后验审计 GAP”。
...
R0 当时已执行 177/900，但尚未完成。D6 当时继续保持旧正式结论 895/900，900/900 formal
acceptance 仍是开放验收项。

D6 的 skip 门没有放宽。此次修复通过来自 skip=0 和实际消费闭环；未来任何未经版本化完整
D2 输入摘要验证的 skip 仍不能进入 formal 守恒式。

### Clean-source 增量正式状态（全量完成前阶段记录，2026-07-30）
...
- 该阶段无新增 D6-owned P0。开放项是 main 完成其余 723 cell，并由 D6 对完整 900-cell
...
## 2026-07-31 D3 A1 来源独立 v2 外部审计 GAP

### 已关闭的 D6-owned 项

1. **D3 自报汇总缺少独立复算**：已新增纯标准库外部审计器，不导入 D3 evaluator、aggregate、
   模型或 solver。D3 aggregate 只作为声明核对。
2. **结果目录完整性未失败关闭**：已固定普通文件清单、禁止符号链接、验证 `SHA256SUMS`
   完整覆盖和摘要，并拒绝已存在输出目录。
3. **生成源和匿名数据集缺少统一审计**：已核对 100 episode、292 frame、seed
   20000-20099、60/20/20 episode、178/57/57 frame、D3-only export、有限状态、clean
   来源记录和在线真值使用为 0。
4. **真值与实体身份字段缺少独立扫描**：已逐行拒绝 truth、Actor、Object、全局航迹及其他
   身份字段；正式 seed 1000-1019 读取数为 0。
5. **逐帧计数和权限冒充缺少负例**：已覆盖文件篡改、缺文件、符号链接、身份字段、计数冒充、
   权限冒充、模型输出冒充和输出覆盖。
6. **CSV 只校验自带摘要，未与 JSONL 内容闭合**：已固定 21 列、292 行，独立解析身份、
   教师机会、三组边、换绑、正负类、OOD、拒绝原因和 fallback；CSV 内容被修改并同步重写
   `SHA256SUMS` 仍失败关闭。
7. **数据集可被替换且 split 自报未独立核验**：审计入口强制 dataset path 属于 generation
   root 固定子目录，并从逐帧 seed/episode/split inventory 独立重算 split hash。
8. **安全计数和规则矩阵摘要仍来自 D3 自报**：已按连续小端双精度字节重算每帧矩阵摘要；
   从动作掩码、目标需求和资源容量独立重算 R0、candidate、effective 三组边的索引、容量、
   硬禁边和原子性，再与自报字段闭合。机器门使用 independent effective 计数。
9. **评价 JSONL 未扫描身份字段**：评价行现执行与数据集一致的 truth/Actor/Object 身份扫描，
   仅允许零值审计字段 `online_truth_use_count`。

正式结果为安全换绑 `13/110`、教师完全匹配 `8/110`、负类 exact-R0 `182/182`、
fallback 矩阵/绑定 `94/94`。预注册总体门通过。安全违规和模型越权输出均为 0。
CSV `21×292` 全闭合；split hash 为 `f1380dd6...ca5`；三组各 21637 条选择边的独立安全
计数均为 0。

### 仍开放的 P1 证据

1. test 子组教师完全匹配为 `0/25`。该事实不改变预注册总体门，但表明未见子组泛化仍弱。
2. 正式 seed 1000-1019 尚未读取和评价。后续只能按新预注册合同一次性运行，不能使用本批
   数据调参后再重复评价。
3. 运行采用、D3 计划发布、控制、物理拦截和生产可用性均无本次证据，权限必须保持 false。
4. 正式 R0 父矩阵剩余范围及 A1 同键物理非退化证据仍由 main/D3 提供，D6 只做外部审计。
