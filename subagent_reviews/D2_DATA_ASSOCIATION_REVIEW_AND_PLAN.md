...
## 2026-07-31 正式 R0 半程评审

clean producer `80e55eb` 已完成 shard 0-9，共 `450/900` 个 R0 episode。D6 v12
evaluator `b6289c5` 的严格离线重聚合得到 `414/450 available`；可用项合计 893 次
ID Switch，169 个 episode 非零。36 个失败关闭项包括 27 个
`multiple_truth_targets_for_global_track` 和 9 个
`source_observation_outside_lineage_window`。在线 producer 继续为
`0/450 available`，没有从离线 truth sidecar 回填在线指标。

规模证据具有明显分层：5/20/50/100/200 五档的严格可用数依次为
`86/90、90/90、90/90、81/90、67/90`，可用项 ID Switch 合计依次为
`0、13、59、259、562`。一轨多真值失败在 100 规模出现 4 次、200 规模出现 23 次，
50 及以下未出现。100/200 规模贡献 821/893 次严格 ID Switch，说明高规模身份合并和
稳定性仍是 D2 P1，不能用 5v5 或 20v20 结果外推。

谱系窗外失败不呈规模单调性。4 次位于 5v5 延迟噪声场景，另 5 次位于 100v100 的
seed 1004，并分布在标称、编队分裂、多高度机动、中心失效和二级节点失效场景。该组
证据优先指向时间链、调度和 sidecar 匹配诊断，不支持直接扩大 `0.9 s` 窗口。

本轮评审不修改 D2 算法。后续候选必须先对 27 个一轨多真值和 9 个窗外 episode 做
逐帧因果回放，再以新 source/new execution plan 运行。严格不可用不是零；部分 ID
Switch 下界不能替代完整指标。若任何窗口或关联行为改变，现有 450-cell 只保留为
`80e55eb` 历史证据，不与新代码产物混合。

## 2026-07-24 D1 发布审计 v2 消费评审
...
内容审计；后续只按同一对象身份复用。强引用和 `is` 检查阻断裸 `id` 重用。等值不同
身份、畸形构造、子类、marker、自定义 equality 和可变 backing 均不能获得该信任。
原内建等值复用未改变。
...
D2-owned 审计缺口关闭。main 与 D6 随后在 clean 提交 `909669b` 完成真实 scalable
episode 的 v2 evidence/evaluation 原子持久化、聚合，以及以首个预留的未见 gate
seed 1100 开展的 A/B。

baseline 的 D2 航迹、D3 分配、strict IDSW、track continuity、coverage continuity
和 commitment coverage 分别为 `203/200/9/0.865/0.870/1.0`。candidate 的 D2 航迹和
D3 分配为 `201/197`；1787 条记录中 committed/uncommitted 为 `1714/73`，coverage
