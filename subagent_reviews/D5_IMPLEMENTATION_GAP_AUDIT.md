...
| 性能旁路接入 main/D6 | **P1 开放，非运行阻塞** | main 应在 episode 结束时读取固定大小快照并单独落盘，reset 后复核归零；不得污染 `TerminalAssociation` 或改变发布频率。 |
...
| 缺口 | 当前状态 | 权威证据与剩余边界 |
| --- | --- | --- |
| 200v200 D5 长时内部超线性成本 | **D5-owned 紧密范围已关闭** | 固定 10 秒日志重放中终端关联 `4.133 -> 2.776 s`，主动视觉 `37.431 -> 25.918 ms/轮`；116/116 输出一致。根因是稳态调用次数和帧内候选规模，未发现历史无界累积。 |
