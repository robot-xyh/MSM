...
D5 当前已经实现了“离线科研版”的核心安全边界和轻量数据合同：`GlobalTrack -> CameraModel -> image projection -> LocalVisualTrack -> TerminalAssociation`、友方声明模拟、`ReconImageCue` 辅助代价、`TerminalObservationBus` 跨视角摘要、5v5 ComputerVision 检测框 dry-run helper，以及禁止改写 `global_track_id` 的测试。

尚未实现的是完整工业/学术工具链接入：真实 ByteTrack/BoT-SORT/Deep SORT、ROS 2 tf2、OpenDroneID/MAVLink/DDS/AprilTag 协议栈、OpenCV calibration/solvePnP 标定链路、跨相机几何融合器，以及 AirSim stress 中真实调用 `TerminalAssociator` 的端到端路径。当前 `research_modules/airsim_runtime/d4d5_stress.py` 是 replay/stress 分析器：它使用 AirSim replay detection metadata 构造 `LocalVisualTrack`、手工生成 `TerminalAssociation`，再给 D4 仲裁；它没有调用 `TerminalAssociator.decide()`。

## 实现差距表

| 预期项 | 当前状态 | 证据文件 | 未实现原因 | 缺失条件 | 建议优先级 |
|---|---|---|---|---|---|
| AirSim `simGetDetections` 输入接入 | **部分实现**。D5 有 `simGetDetections` 风格 fixture 转换器；`d4d5_stress.py` 消费 replay 后的 `AirSimDetectionBox`，不是直接调用 AirSim API。 | `research_modules/d5_terminal_association/src/d5_terminal_association/airsim_cv_adapter.py`; `research_modules/airsim_runtime/d4d5_stress.py` | D5 被设计为离线模块，不直接依赖 AirSim 包；真实采集由 main/runtime 负责。 | 需要 main 确认 `simGetDetections` 字段到 `AirSimDetectionBox`/fixture 的稳定 schema，尤其 bbox、camera_id、object_id、timestamp。 | P0 |
| `LocalVisualTrack` 生成 | **已实现基础版**。可由 detection bbox 生成中心点、bbox、类别、质量、时间戳；也支持手工测试 fixture。 | `airsim_cv_adapter.py`; `tests/test_airsim_cv_5v5_evidence.py` | 未包含真实 MOT 历史维护，只读取或默认 `mot_history_length`。 | 需要帧间 tracker 缓存、检测器置信度定义、相机帧时间戳。 | P0 |
...
| `ReconImageCue` | **部分实现**。数据结构、scope、代价 bonus 和 used count 已有；新鲜度/frame 强校验主要靠调用方。 | `models.py`; `associator.py`; `tests/test_terminal_association.py`; `tests/test_airsim_cv_5v5_evidence.py` | 当前没有二级相机原始像素到拦截机相机平面的真实重投影链路。 | 需要二级相机 pose、目标三维估计、目标相机 pose、cue age policy、frame_id 校验。 | P1 |
...
| 跨视角目标关联 | **部分实现摘要层**。`TerminalObservationBus` 按已有 `assigned_global_track_id` 汇总多资源支持、local ID 命名空间和重复锁定风险。 | `observation_bus.py`; `models.py`; `tests/test_terminal_observation_bus.py` | 没有跨相机几何投影融合、时间对齐、姿态协方差、per-view cost。 | 需要 `CrossViewObservation`、每相机 `CameraModel`、D2 `GlobalTrack` 列表、measurement/arrival timestamp 和融合策略。 | P1 |
| 多个无人机看到不同目标集合 | **部分实现测试级覆盖**。UAV1 看到 G1/G2/G3、UAV2 看到 G2/G3/G4 的 bus grouping 已测试，单视角 G1/G4 保留。 | `tests/test_terminal_observation_bus.py`; `README.md` | 只是基于手工 `TerminalAssociation` 的摘要，不是由真实多相机投影自动推导。 | 需要多相机 `TerminalAssociator` 调用或跨视角几何融合器生成每视角候选。 | P1 |
| 重复锁定风险 | **已实现摘要信号**。多个资源 `locked` 同一 `global_track_id` 时输出 `duplicate_terminal_lock_risk=True`。 | `observation_bus.py`; `tests/test_terminal_observation_bus.py`; `tests/test_airsim_cv_5v5_evidence.py` | D5 不负责解除重复锁定，符合边界。 | 需要 D3/D4 消费该风险并生成新 plan/version 或仲裁动作。 | P0 |
...
| D4/D5 stress 是否真实调用 `TerminalAssociator` | **未实现**。`d4d5_stress.py` 未导入 `TerminalAssociator`；它手工构造 `TerminalAssociation`。 | `research_modules/airsim_runtime/d4d5_stress.py` | stress 当前目标是 D4/D5 evidence 和降级 case replay，不是完整 D5 几何关联端到端验证。 | 需要从 frame/replay 构造 `GlobalTrack[]`、`Assignment`、`CameraModel`、`LocalVisualTrack[]`、`IdentityClaim[]`，并调用 `TerminalAssociator.decide()`。 | P0 |
| AirSim replay 中真实 `TerminalObservationBus` 使用 | **已实现**。`d4d5_stress.py` 使用 bus 汇总观察和 cross-view associations。 | `d4d5_stress.py` | bus 输入的 association 是手工生成，非 `TerminalAssociator` 输出。 | 需要把 `TerminalAssociator` 输出接入 bus，替换或对照手工 association。 | P0 |
...
| `TerminalConsistencySummary` | **未实现代码类**。文档中定义建议字段；D4 stress 使用 D4 模块自己的 `TerminalAssociationSummary`。 | `docs/ALGORITHM_AND_IMPLEMENTATION.md`; `d4d5_stress.py` | 当前优先实现 bus/cross-view 和 D4 自有 summary，未新增 D5 summary dataclass。 | 需要 D4/D5 确定统一 summary schema，避免 D5 与 D4 重复建模。 | P1 |
...
1. **最高优先级缺口是 `d4d5_stress.py` 未调用 `TerminalAssociator`**。当前 stress 报告能验证 D4 仲裁链路和 D5 evidence 格式，但不能证明 D5 几何投影、马氏门控、友方 hold、cue bonus 在 AirSim replay 中真实工作。建议下一步在 stress 中增加一条 `TerminalAssociator.decide()` 路径，并保留当前手工 association 作为 case-control fixture。

2. **跨视角当前是摘要关联，不是几何融合**。`TerminalObservationBus` 能回答“哪些资源报告同一个全局 ID”，不能回答“不同相机中的 bbox 是否几何上一致支持同一目标”。要完成 UAV1 `{1,2,3}`、UAV2 `{2,3,4}` 的真实跨视角配准，需要 `CrossViewObservation`、每相机 `CameraModel`、每观测时间戳和跨相机投影代价。
