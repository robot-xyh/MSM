...
| YOLOv8 frame adapter | `YoloMotAdapter.process_frame()` 将图像帧或 mock detector 输出转为命名空间化 `LocalVisualTrack`；fallback/native MOT 状态按 `(resource_id, camera_id)` 隔离，原生 ByteTrack/BoT-SORT 不可用时 per-stream `IouFallbackTracker` 保持 deterministic local ID 连续性。metadata 标明 stream key/backend/scope，并提供单 stream/全 episode reset。 | 真实 AirSim frame stream 接入、部署参数和多 seed 标定仍由 main/runtime 完成；main 在 episode 边界调用 `reset_all_streams()`。 |
...
| P1 | D4 逐决策 evidence 接线验收。 | 在同一 decision tick 传递 stable/not-registered count、evidence timestamp/age、scope、threshold version 和冲突原因；验收 D4 不使用 stale/episode 聚合值代替实时证据，D5 不直接决定降级动作。 |
