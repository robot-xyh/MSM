...
## 2026-08-10 有状态时序几何关联 GAP 更新

| 项目 | 当前状态 | 证据与剩余限制 |
| --- | --- | --- |
| 有界短缺口预测 | **D5 代码项已完成** | 新增默认 0.25 秒 coast，按像面角速度预测中心和边界框，协方差按 `rate_sigma_px_s` 随 age 增长；0.06/0.10/0.17 秒专项通过，0.26 秒按要求过期。 |
| 非实测证据权限 | **D5 合同项已关闭** | predicted/lost 不进入 `measured_assignments`，只输出 coast/reacquire/hold；日志固定不授予 terminal authorization。 |
| 绑定迟滞 | **D5 代码项已完成** | 挑战者需当前实测、连续 2 帧且总代价优势达到 `min_lock_margin`；单帧、近似平局、成对交换和重复占用均保持 pending/held。 |
| 状态隔离与 reset | **D5 合同项已关闭** | 状态按资源/相机/流/本地编号隔离；支持时戳回退、本地 reset 和显式范围 reset。 |
| 在线身份隔离 | **保持关闭** | 专项向本地 metadata 注入 actor/object/truth 字段，输出未读取或透传；`GlobalTrack.global_track_id` 未创建、改写或归一化。 |
| main runtime 接线 | **接口项已关闭** | `long_range_cv_scan` 已按相机维护 episode 独立状态；只有 `measured_assignments` 驱动发现/提示/完成，coast 只记录且不授权；输出 temporal binding/dropout 两类 CSV。main-D6 联合专项 `23 passed`。 |
| 长距离 AirSim 短缺口 | **P1 仍开放** | 冻结 seed `20260810` 的 3 次短缺口来自旧 v2 D6 基线。新 v3 真实 AirSim 尚未复跑，不能由接线测试关闭。 |
| 长距离 AirSim 绑定切换 | **P1 仍开放** | 旧 v2 基线仍有 7 次绑定切换，交叉窗口仅 `3/31` 可评价；这些数值不是新时序算法结果。 |
| 多 seed 与真实误差 | **P1 仍开放** | 未完成不少于 10 个原始尺度 seed，也未注入真实位置、姿态、外参、时钟、漏检和虚警误差。 |

验证日期为 2026-08-10。专项 `13 passed in 1.17s`，D5 全量
`904 passed, 1 warning in 146.08s`。本次把“短缺口和绑定迟滞缺少 D5-owned 实现”更新为
“接口、单元测试和 main/D6 接线完成，v3 实景复跑与验收待完成”；P1 等级不下调。冻结旧
v2 报告仍为 `fail_closed`，准确率 `0.9979317477`、短缺口 3、绑定切换 7、交叉窗口
`3/31` 只作为对照。

## 2026-08-10 真实 AirSim 长距离 20 目标 GAP 复核（冻结旧 v2 基线）

| 项目 | 当前状态 | 证据与剩余限制 |
| --- | --- | --- |
| ComputerVision 运行链 | **真实接口证据已形成** | seed `20260810`，原始尺度 `coverage_safe` 20 秒 episode；20 个 `Quadrotor1` actor，50 米/秒、100 Hz，中心距离 2.8-3.2 千米、拦截相机约 500 米。只证明 AirSim 接口，不代表真实设备。 |
| 二维扫描和提示窗口 | **本轮接口项已关闭** | 中心使用合成 `GlobalTrack` 及协方差生成偏航/俯仰蛇形扫描；发现和五帧确认均为 `20/20`。拦截提示 `20/20`，观察及完成 `19/20`。 |
| 在线身份隔离 | **本场景已关闭并可审计** | 在线文件无 actor/object/truth ID；离线侧车独立评分。online truth use=0，`global_track_id` rewrite=0；中心轨迹为 `synthetic_d1_d2_fixture`，真实 D1/D2 误差未验证。 |
| 云台运动补偿 | **单 seed 复跑有效** | 世界视线速度跟踪的 1934 条可评分关联准确率为 `0.9979317477`，连续可见段 IDSW=0、纯度/连续性=`1.0/1.0`。结果只限一个 seed。 |
| `coverage_safe` 覆盖与跟踪总门 | **P1 未通过** | 发现/确认 `20/20`、观察/完成比例 `0.95/0.95`、准确率 `0.9979317477`和记录项通过相应子门。总门直接失败于聚合 MOT 短缺口中断 `3 > 0`。 |
| 短缺口连续性 | **P1 开放** | 连续可见段 IDSW 为 0，但仍有 3 次短缺口中断和 48 次长期重发现。需用有界 coast/预测处理短缺口，保留显式 dropout 证据，不得直接放宽门限。 |
| 交叉窗口标定 | **P1 开放** | 31 个预定交叉窗口只有 3 个可评价；可评价窗口 IDSW=0、纯度/连续性=`1.0/1.0`。样本覆盖不足。 |
| 几何绑定稳定性 | **P1 开放** | 原始尺度有 7 次 `local_track_id -> global_track_id` 几何绑定切换。需标定投影协方差、门限、绑定迟滞和交叉歧义处理，不得使用真值身份修正。 |
| 可视化取证 | **单场景记录项已关闭** | 原始尺度保存 98 幅原图和 98 幅在线标注图，19/20 条全局航迹形成双相机图像证据；必需记录缺失为 0。标注在线不使用 actor/object/truth ID。 |
| 真实光电可辨识性 | **P1 开放** | 中心相机 3 千米原始尺度截图接近 AirSim 渲染/像素极限。`simGetDetections` 返回框不等价于真实光电辨识，需真实小目标数据或保真合成数据验证。 |
| 2 倍网格诊断 | **诊断完成，不关闭主场景 GAP** | 双相机证据 `20/20`，2319 条可评分关联准确率 `0.9892194912`，IDSW=0，短缺口 3，几何绑定切换 42，总门仍失败。放大网格提高可见性，但大框重叠增加歧义；不得替代原始尺度。 |
| 真实误差与多 seed | **P1 开放** | 尚未注入真实位置、姿态、外参、时钟、漏检和虚警误差，也未形成多 seed 统计。当前单 seed 结果不能作为工程准入。 |
| 图学习关联 | **未执行，既有可选状态不变** | 本轮没有运行图神经网络，不构成 G1/GNN 准入或泛化证据。 |

验证日期为 2026-08-10；主场景样本为一个冻结 seed、20 个 actor 和一个 20 秒 episode。
原始尺度证据目录为
`research_modules/airsim_runtime/outputs/d5_cv_long_range_20target_visual_evidence_20260810/`；2 倍网格诊断位于
`research_modules/airsim_runtime/outputs/d5_cv_long_range_20target_scale2_visual_diagnostic_20260810/`。
本节关闭单场景二维扫描、提示窗口、在线身份隔离和记录完整性接口项，不关闭短漏检、交叉样本覆盖、
几何绑定稳定性、真实光电辨识、真实传感器误差、多 seed、真实设备性能或物理拦截 P1。

## 2026-08-10 不规则交叉目标窄视场扫描 GAP

| 项目 | 当前状态 | 证据与剩余限制 |
| --- | --- | --- |
| 不规则三维与交叉几何 | **理想场景基线已关闭** | 默认 20 目标初始径向跨度 390.0 米、高度跨度 33.6 米，全时段最小三维间距 30.0 米；A/B 参考像面交叉或明确近距交叉为 10/6 对。目标数保持参数化。 |
| 解析扫描状态机 | **理想场景基线已关闭** | 100 Hz 可见性时间轴、五帧确认、扫描恢复和 B 提示观察均已实现。关联按确认事件运行，不把 100 Hz 可见性计算冒充为 100 Hz 求解。 |
| 机械扫描漏区审计 | **本场景已关闭** | 180 度/秒对应 1.8 度采样步长，大于 A 的 0.621 度水平视场。标准 seed 实测中心发现率和完整链均为 0，结果未用真值补齐。 |
| 安全覆盖与两级关系链 | **理想场景已关闭** | 49.68 度/秒、20% 视场重叠模式在标准 seed 1.61 秒内完成 20/20。seed `20260810-20260819` 的 A/B/端到端准确率和完整链均为 1.0，IDSW、重复、未匹配、在线 truth 和全局 ID 改写均为 0。 |
| 观测与输出审计 | **D5-owned 基线已关闭** | 保留双时间戳、`1e-6 I` 协方差、100 Hz 双模式时间线、匿名观测、事件级完整代价、关系链、离线评分、10 张 PNG、76 帧 GIF 和中文报告。旧全时可见输出未覆盖。 |
| 真实窄视场鲁棒性 | **P1 继续开放** | 未加入漏检、虚警、遮挡、像素噪声、云台误差、相机位姿误差、时间偏差、通信丢包和本地轨迹中断；未运行 AirSim 或真实吊舱回放。20% 重叠与确认门限仍需在真实图像链路标定。 |
| 图学习关联 | **未执行，原可选项不变** | 交叉航迹和代价图只解释几何候选与匈牙利选择，不构成图神经网络准入证据，既有权重谱系门不变。 |

验证日期为 2026-08-10。新增扫描专项测试为 `7 passed`，标准输出目录为
`research_modules/d5_terminal_association/outputs/ideal_20_target_irregular_crossing_20260810/`。
本节只关闭无误差解析扫描、事件配准和产物审计基线，不关闭真实跨视角配准、检测、多目标
跟踪、M 对 N 联盟共同完成或物理拦截 P1。

## 2026-08-10 理想二十目标两级视觉配准 GAP

| 项目 | 当前状态 | 证据与剩余限制 |
| --- | --- | --- |
| A 侧全局航迹到匿名轨迹绑定 | **确定性理想基线已关闭** | 中心三维 `GlobalTrack` 按精确 A 相机内外参逐帧投影，最近 5 帧位置/位移完整代价矩阵经匈牙利算法绑定到 A 匿名本地轨迹。10 个 seed 的 A 侧准确率均为 `1.0`。 |
| A 已绑定航迹到 B 匿名轨迹绑定 | **确定性理想基线已关闭** | B 只提供匿名像素轨迹；中心使用精确 B 相机几何完成第二级投影和匈牙利匹配。10 个 seed 的 B 侧及端到端准确率均为 `1.0`，完整关系链比例均为 `1.0`。 |
| 规模与可审计输出 | **D5-owned 基线已关闭** | 目标数量由参数决定，不固定 20。标准 seed 共 76 个图像时刻；每级每帧导出完整 20×20 位置、位移和总成本，标准输出含 13 张 PNG、76 帧 GIF、三类轨迹 CSV、公开关系链和离线评分 sidecar。 |
| 身份与时间合同 | **本场景已关闭并保持回归** | seed `20260810-20260819` 的 `id_switch_count`、重复分配、未匹配、在线 truth 使用和 `global_track_id` 改写均为 `0`；量测/到达时间戳同时保留且在理想条件下相等，观测和航迹携带 `1e-6 I` 数值正则协方差。 |
| 全时段可见性 | **本场景已关闭** | A、B 两相机在 10 个 seed 中的全时段可见率均为 `1.0`。该结果来自云台逐帧精确指向目标群质心和无误差投影，不是探测器可见率。 |
| 真实跨视角鲁棒性 | **P1 继续开放** | 未加入漏检、虚警、遮挡、像素噪声、本地编号中断、位置/姿态误差、时钟偏差或通信时延；未运行 AirSim 或真实图像。仍需按单变量误差注入和真实 replay 标定正确/错误候选代价间隔、IDSW、完整链比例和恢复时间。 |
| 图学习关联 | **未执行，原可选研究项不变** | 三层 60 节点图只显示每行最低 3 条候选边和匈牙利选中边。没有加载或运行图神经网络，也没有绕过既有权重谱系校验。本结果不能作为 GNN 准入证据。 |

验证日期为 2026-08-10。专项测试为 `7 passed in 5.47s`；标准输出目录为
`research_modules/d5_terminal_association/outputs/ideal_20_target_two_stage_registration_20260810/`。
本节关闭的是理想条件下的两级算法与可审计输出基线，不关闭现有真实 AirSim、真实相机、
M 对 N 联盟共同完成、主动视觉收益或物理拦截 P1。
...
| 缺口 | 当前状态 | 证据与剩余边界 |
| --- | --- | --- |
| lineage 显式输入 | **D5-owned P0 已关闭** | `TrackletG1EvidenceInputs` 携带 lineage 路径和调用方冻结 SHA-256。 |
| JSONL 完整性 | **关闭并保持回归** | 每行必须为对象且含非空、唯一 `episode_uid`；空行、非法记录和重复 UID 均稳定拒绝。 |
| 正式计数 | **关闭并保持回归** | 实物、paired report、D6 candidate、manifest 和 admission report 均要求 `900/900`。 |
| v5 实物与清单 | **关闭并保持回归** | lineage 实际复制到 `evidence/`，manifest 登记四字段，`SHA256SUMS` 和 strict loader 每次覆盖。 |
...
| 缺口 | 当前状态 | 证据与剩余边界 |
| --- | --- | --- |
| 六权限 schema | **D5-owned 代码已关闭** | `d5.tracklet-g1-authority-contract.v2` 精确要求模型晋级、G1 辅助、默认路径、分配、故障接管、控制六字段均为严格布尔 `false`。 |
| 旧合同静默兼容 | **关闭并保持回归** | 缺失、多余、拼写错误、旧四字段、未知 schema、非布尔、任一 `true` 和 `reason` 替代权限均拒绝。 |
...
| 缺口 | 当前状态 | 证据与剩余边界 |
| --- | --- | --- |
| 异步批次只形成单相机图 | **D5-owned 已关闭** | `process()` 已把其他相机仍有效的匿名实测航迹和对应外参带入当前图；同步路径保持原语义。 |
| 双时间戳和协方差保留 | **关闭并保持回归** | 缓存复用原始 measurement/arrival timestamp、像素协方差、位置协方差和姿态协方差；不做隐式预测或伪协方差收缩。 |
| 快照有界性 | **关闭并保持回归** | 默认 TTL `1.0 s`、容量 `256`；missed-frame、量测/到达时间窗和外参年龄继续使用既有门。 |
| OOSM、重复量测和 reset | **关闭并保持回归** | OOSM 不覆盖快照，重复量测继续拒绝；stream/episode reset 分别清除局部/全部快照。 |
| 失效输入失败关闭 | **关闭并保持回归** | 缺外参、超时间窗、过期、容量淘汰和重复节点均剔除；没有当前实测锚点时不使用历史缓存发布新关联。 |
| 身份所有权 | **关闭并保持回归** | 在线 truth 使用和 `global_track_id` 改写为 0；缓存只保存 camera-local 匿名航迹，中心 ID 仍只读。 |
| 诊断可观测性 | **D5-owned 已关闭** | 已提供本次更新、实测相机、跨调用活跃相机、复用航迹及时间/外参/过期/OOSM/重复/容量排除等固定标量，不记录业务 ID。 |
| 异步同目标候选 | **fixture 范围关闭** | 规则和模型 fixture 均为 `2 nodes / 1 edge`；异步不同目标为 `2 nodes / 0 edge`。 |
...
| 缺口 | 当前状态 | 证据与剩余边界 |
| --- | --- | --- |
| clean source 重建 | **D5 producer 已关闭** | commit `d437744c...4ffb` 已重建 supplemental/composite、权重 `7fb5db8b...ca71`、manifest `0eff183f...da77`、20-seed held-out、paired-shadow 和 900 条 lineage。 |
...
| 单特征 limitation 误保留 | **关闭** | AUC `>=0.995` 才加入 blocker。7fb5 clean 输入 AUC=`0.720073`，不保留旧 blocker；当时冻结 registry 固定保留候选图未重建、D6 外审待完成和无在线权限三项。 |
...
| 项目 | 状态 | 证据与边界 |
| --- | --- | --- |
| 空网格重复探测 | **局部子项关闭** | 复用只读占用桶序列，按同一搜索半径直接检查占用桶对；500 组随机输入与旧整数偏移搜索的桶对集合完全一致。 |
| frozen replay 配对性能 | **通过描述性 A/B** | seeds `42000/42001/42002` 中位耗时分别下降 `15.34%/15.88%/18.27%`，三 seed 中位值均值下降 `16.45%`。 |
| binding 与几何语义 | **保持回归** | 三 seed 逐帧核心、最终 binding、操作数哈希各自唯一并与冻结记录相同；truth/ID mutation/降帧/降候选/门限变化/D7 gate 变化均为 0。 |
| D5 长时单次超线性成本 | **P1 开放** | 本轮删除一个图索引常数项热点，没有完成检测数、相机数、中心候选数、时长的正交多 seed 线性准入。 |
| D6 操作数与阶段耗时联合准入 | **P1 开放** | frozen replay 是 D5 单模块证据；仍需 D6 预注册完整系统报告，且需区分主机调度离群值。 |
| 主动视觉快照成本 | **P1 开放，仅形成 profiling 结论** | 三 seed 集成累计均值仍约 `4.184 s`。合成同规模剖析的快照构造/真值隔离审计约 `0.691/0.306 s`；缺少真实逐帧输入 replay，不能安全证明缓存等价，也不得跳过 truth-free 审计。 |
| AirSim/硬件实时性 | **P1 开放** | 本轮未启动 AirSim，也未测真实相机、检测器或机载处理器。 |

本轮没有新增 P0。结构化证据为 `results/scalable_3d_camera_overlap_ab_20260722.json`，中文报告为 `reports/D5_SCALABLE_3D_CAMERA_OVERLAP_AB_20260722.md`。
定向回归为 `52 passed`，D5 全量回归为 `545 passed in 129.59s`。

## 2026-07-22 f80b5bd 最终集成状态

| 缺口 | 当前状态 | 权威证据与剩余边界 |
| --- | --- | --- |
| 当前源码 200v200 三种子完整复跑 | **已关闭** | clean `8f86192 -> f80b5bd`，nominal 200v200、10.0 秒、seeds `42000-42002`。三个候选 episode 均 finite，online truth use=0，D1/D2/D3/D5/D7 最终数量相同。 |
| D5 逐条业务等价 | **已关闭并保持回归** | 视觉 binding 与 active-vision payload 逐条语义相同；最终 binding 为 `22/29/28`。D3 opaque `plan_id` 只按 occurrence/version 归一化，ACK 原始来源载荷 SHA-256 在归一化前验证，owner/version/coalition/`global_track_id`/command 均未忽略。 |
| 同时间戳中心预测复用边界 | **D5-owned 已关闭** | 只在单次 `process()` 内复用只读 center prediction，不跨调用、不减少检测或中心候选、不改变几何门、唯一绑定或中心 ID 所有权。每 seed 投影 DTO 缓存命中/未命中仍为 `68/48`、`71/48`、`70/48`。 |
| 终端关联累计阶段耗时 | **通过 clean 描述性对照** | 三 seed 均值 `2.545876 -> 1.974446 s`，约下降 `22.45%`。主动视觉 `4.174315 -> 4.183797 s`，约 `+0.23%`，基本持平。 |
| D5 长时单次超线性成本 | **P1 开放** | 累计阶段耗时下降不证明单次成本线性；既有短长归一化结果仍高于线性门。需正交控制检测数、活跃相机数、中心候选数和时长，并由 D6 做预注册多 seed 操作数/耗时联合准入。 |
| AirSim/硬件实时性 | **P1 开放** | 本轮为三维质点 clean 集成，不含真实图像、AirSim 帧率、变化外参或机载处理器证据。 |

本轮没有新增 P0。关闭的是“当前源码三种子完整复跑”和“跨提交逐条业务等价”子项，不关闭 D5 超线性规模成本、正式统计准入或真实相机泛化。

## 2026-07-22 中心预测工作区审计

| 缺口 | 当前状态 | 权威证据与剩余边界 |
| --- | --- | --- |
| 中心投影重复数组/对象物化 | **D5-owned 已关闭** | 每次矩阵构建只抽取一次中心轨迹数组，并按唯一量测时刻生成只读预测；短/长物化 `76/715 -> 23/116`，缓存不跨调用。 |
| 完整 10 秒路径收益 | **通过描述性配对门** | 五轮交替旧/新重放的长序列平均单次成本中位数 `26.078 -> 19.145 ms`，下降 `26.6%`；中心投影累计约 `0.706 -> 0.164 s`。不是缩小微基准。 |
| 固定大小诊断与业务等价 | **关闭并保持回归** | 33315 次局部比较、499505 个投影单元、472288 个 binding 单元全部保留；短/长业务、最终 binding、操作数哈希均与冻结记录一致，truth/ID mutation 为 0。 |
| 归一化超线性成本 | **P1 开放** | 独立五次候选为 `2.366x`；配对旧/新为 `2.418x/2.450x`，受短序列和主机热态影响且未达到线性门。需稳定环境、正交规模和多 seed。 |
| 当前源码三种子集成与 D6 聚合 | **集成已关闭，D6 正式聚合 P1 开放** | main 已在 `f80b5bd` 复跑 `42000-42002` 完整系统并完成逐条语义等价审计；D6 仍需在预注册正式报告中联合展示阶段耗时和 D5 快照操作数。 |

当前源码通过 `544 passed in 163.09s`。没有新增 P0，没有改变检测/相机频率、候选预算、几何/友方/身份/版本/唯一性门控、AirSim 或在线学习权限。
