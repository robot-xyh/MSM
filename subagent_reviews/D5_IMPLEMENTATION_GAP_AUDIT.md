...
尚未实现的是完整工业/学术工具链接入：真实 ByteTrack/BoT-SORT/Deep SORT、ROS 2 tf2、OpenDroneID/MAVLink/DDS/AprilTag 协议栈、OpenCV calibration/solvePnP 标定链路，以及跨相机几何融合器。当前 P0 已补齐 AirSim stress 中真实调用 `TerminalAssociator` 的端到端路径：`research_modules/airsim_runtime/d4d5_stress.py` 从 replay frame 构造 `GlobalTrack[]`、`Assignment`、`CameraModel`、`LocalVisualTrack[]` 和 `ReconImageCue[]`，再调用 `TerminalAssociator.decide()` 生成 `TerminalAssociation`，最后交给 `TerminalObservationBus` 和 D4 仲裁。
...
| `ReconImageCue` | **部分实现**。数据结构、scope、代价 bonus 和 used count 已有；stress 中已生成面向当前拦截机相机平面的 reprojected cue。 | `models.py`; `associator.py`; `d4d5_stress.py`; `tests/test_terminal_association.py`; `tests/test_airsim_cv_5v5_evidence.py` | 当前 cue 仍由 replay 真值/几何构造，不是从二级相机图像检测结果反投影再重投影。 | 需要二级相机 pose、二级图像检测、目标三维估计、目标相机 pose、cue age policy、frame_id 校验。 | P0 已完成 stress cue 输入；真实二级图像链路为 P1 |
...
| 多个无人机看到不同目标集合 | **部分实现**。UAV1 看到 G1/G2/G3、UAV2 看到 G2/G3/G4 的 bus grouping 已测试；stress 中每个拦截机视角都会独立调用 `TerminalAssociator.decide()`，再由 bus 汇总同一 global ID 的多视角支持。 | `tests/test_terminal_observation_bus.py`; `d4d5_stress.py`; `README.md` | 仍不是跨相机联合优化；每视角先独立关联，再做摘要级 cross-view grouping。 | 需要 `CrossViewObservation`、每相机 `CameraModel`、measurement/arrival timestamp 和跨相机联合代价。 | P0 已完成每视角真实 D5 调用；跨相机几何融合 P1 |
...
| AirSim replay 中真实 `TerminalObservationBus` 使用 | **已实现**。`d4d5_stress.py` 使用 bus 汇总观察和 cross-view associations；bus 输入已改为 `TerminalAssociator.decide()` 输出。 | `d4d5_stress.py`; `tests/test_blocks_runtime.py` | bus 仍是摘要层，不做分配和跨相机联合优化。 | 需要 D3/D4 消费 cross-view risk 和 future `TerminalConsistencySummary`。 | P0 |
