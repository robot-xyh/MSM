...
`20000-20099`、100 个 episode 和 292 帧；正式 seed `1000-1019` 读取数为 0。结果目录
固定文件齐全且 `SHA256SUMS` 全部通过，在线真值使用为 0，所有输入有限。

正类安全换绑 `13/110=11.82%` 通过 `>=1` 且 `>=5%`；正类教师完全匹配
`8/110=7.27%` 通过 `>=1` 且 `>=2%`；负类 exact-R0 `182/182=100%` 通过
`>=99%`。94 个拒绝帧的矩阵和绑定均恢复 R0。重复资源、硬禁边、M-to-N 原子性、版本、
