...
D3 当前已经完成中心化一对一资源-目标分配主线：SciPy Hungarian / `linear_sum_assignment`、无 SciPy 时的小规模 DP fallback、滚动重分配、迟滞、防 stale plan、版本化 `AssignmentPlan`、`resource_count/target_count` 和 `assignment_matrix_shape` 规模字段、可解释代价分解、D5 terminal feedback helper、D5 feedback metadata 写回下一轮输入、D4/main 选定 secondary node 后的 `secondary_plan_v2` owner/source/version DTO、D7 `AssignmentGuidanceBinding`、`AssignmentValiditySummary`、D6-compatible `AssignmentRecord` 导出，以及 synthetic AirSim dry-run adapter。2026-07-08 P1 准备小修已把 current-plan owner/version/source、replan/takeover reason、previous/supersede、迟滞决策、矩阵规模、cost gap 和 N/M mismatch replay 字段补入 `AssignmentRecord`，并新增 D5 terminal feedback calibration summary helper。2026-07-09 已补齐 EVAL P0-B baseline：资源状态细化、增强迟滞和 stale rejection release metadata、可解释 threat score baseline helper。

当前 D3 P0/P1 补齐状态已从“接口缺口”转为“真实 episode 校准”：D5 feedback writeback helper、feedback calibration summary helper、N/M mismatch replay summary、secondary takeover plan DTO/helper、D7 `AssignmentGuidanceBinding`、plan owner/version/source metadata、fallback DP、`AssignmentValiditySummary`、D6-compatible `AssignmentRecord` 导出、P0-B 资源状态细化、P0-B 增强迟滞和 P0-B threat baseline 均已完成并应保持回归。main runtime 已接入 D5 feedback writeback、center replan owner/version/source、secondary owner/version/source 记录、P1 D4/D5 calibration sweep 和 sweep 后自动生成的 D6 标准报告 bundle。D3-relevant P1 保留为真实多 seed/N 规模日志回灌、D5 feedback 权重阈值人工复核标定、完整动态威胁评估、增量分配、时间窗口硬约束和 OR-Tools Min Cost Flow 对照接口。D3 算法按输入数组长度运行，现有测试覆盖非 5v5 的 3 target / 2 resource 场景，5v5 只作为基准规模。

当前 D3 P0/P1 缺口清单：P0 为“无运行级 P0 blocker，P0-B baseline 已闭合并保持回归”；若后续发现资源状态、迟滞/stale rejection 或 threat baseline 在多 seed/N 规模回放中退化，再按 P0 backlog 处理，验收口径以 cost metadata、min dwell/switch penalty/release condition/stale reason、TTC/关键区接近/速度/协方差/目标状态进入 threat baseline 为准。P1 为真实多 seed AirSim/point-mass 校准、D5 feedback 权重/迟滞阈值人工复核标定、完整动态威胁评估、增量分配、时间窗口硬约束、OR-Tools Min Cost Flow 对照接口，以及持续合同回归。OR-Tools 在确认文档中采用 P1 对照/复杂约束升级口径，不是当前 P0 替换 Hungarian 主线；容量/备份资源/分组配额等更复杂约束仍不进入当前 P1 默认主线。
...
1. **P0 运行状态**: 无运行级 P0 blocker。Hungarian、fallback DP、版本化 `AssignmentPlan`、迟滞、stale plan 拒绝、D7 binding、D6 export、P0-B 资源状态细化、P0-B 增强迟滞/stale rejection 和 P0-B threat baseline 均已实现；继续用 D3 单元测试防退化。
