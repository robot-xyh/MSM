...
- AirSim SimpleFlight 真实控制已在 main/runtime 层接入 D7，正式 episode bus metrics 已能合并真实执行结果；默认 `png_vm` 混合闭环的首轮 2v2 10-seed 数据和 D6 报告已生成。D7 本地已补齐 calibration summary/advisory helper，D4 降级阻断、D4 secondary 非 `takeover_ready` 阻断、D5 locked、D5 registration/projection/Yolo-MOT 摘要、D3 owner/version 和 D4 allowed gate 已有单元测试覆盖。剩余 P1 风险集中在两次末端检测超时根因、PN/Pure Pursuit/`png_vm`/`png_ttc` 受控对照、视觉 gate/range/closing speed 阈值建议、3D/高度差与机动能力 benchmark，以及长期 D5 事件流稳定性。
...
- 2026-07-10 10-seed 已将该证据扩展为 18/20 collision success、2 次 `terminal_detection_timeout` 和 `terminal_switch_allowed_rate` 跨 seed 均值 0.0822。现阶段应把这批数据作为唯一真实 AirSim 导引基线：radar PN + `png_vm`，必要时 LOS fallback；Pure Pursuit 和 `png_ttc` 尚无真实 AirSim 同 seed 执行证据。下一步复现 seed 3/10 timeout，并通过 law selector 在相同 seed、几何、阈值和成功判据下做 PN/Pure Pursuit/`png_vm`/`png_ttc` 对照。aggregate reject 是逐帧计数，必须与 episode outcome 分开解释；D7 pair 级 2.113m 与 D6 episode 聚合 1.812m 也必须保持不同口径。
