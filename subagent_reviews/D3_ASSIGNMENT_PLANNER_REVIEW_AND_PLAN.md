...
100 episode、292 帧、seed `20000-20099`、test 教师完全匹配 `0/25`、94 帧 exact-R0
回退和全部权限 false 均重新闭合。既有 D6 外部审计状态已同步为“离线完整性与预注册机器
门确认、未准入”，不再列为待办。

test 25 个正类中，9 帧明确为 OOD 回退，16 帧为非 OOD 候选动作不匹配。22 帧同时被
安全投影拒绝，但没有投影前正确候选，因此投影不是零匹配的单独根因。逐边候选可达性、
逐边模型排序和需求槽映射未落入 v2 工件，对应归因保持 `unavailable`。

v3 只冻结开发来源需求：15 个场景规模单元、300 个 episode、300 个待分配全新 seed，
正类/负类/困难负类最低配额为 900/900/450。seed `0-99`、`1000-1019`、
`20000-20099` 禁止复用，其他 D3 登记值由 main 分配前合并。当前没有分配、生成、训练、
bundle 或权限变化。专项 `9 passed`，D3 全量 `677 passed, 1 skipped, 1 warning`。
...
结果状态为 `source_independent_evaluation_v2_gate_passed_not_admitted`。这项证据关闭
D3 的“来源独立评价未执行”缺口；D6 后续已完成离线复核。正式保留集、运行采用、版本化
ACK、物理窗口、同键 R0 非退化和 production admission 仍开放。所有运行、分配、计划、控制、物理
