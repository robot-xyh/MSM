...
### 2026-07-31 正式 R0 半程身份评估

- clean producer `80e55eb` 已完成 shard 0-9，共 `450/900` 个 R0 episode。当前证据覆盖
  5、20、50、100、200 五档规模、9 类场景和 seed 1000-1009，只代表正式矩阵半程，
  不能写成完整 20-seed 结论。
- D6 v12 evaluator `b6289c5` 从隔离的离线身份制品读取严格指标。`id_switch_count`
  在 `414/450` 个 episode 可用，可用项合计 893 次，其中 169 个 episode 非零；另有
  36 个 episode 失败关闭。
- 36 个不可用项只包含两类原因：27 个
  `multiple_truth_targets_for_global_track`，9 个
  `source_observation_outside_lineage_window`。不可用不等于零。35 个失败项虽有局部
  下界诊断，该下界也不能并入 893、不能替代严格指标。
- 在线 D2 producer 的 `id_switch_count` 仍为 `0/450 available`。这是在线不读取真值的
  正确合同，不是把离线结果回填到运行时。严格离线指标只用于 D6 后验评估。

| 目标规模 | 严格可用/总数 | 非零 episode | 可用项 ID Switch 合计 | 一轨多真值 | 谱系窗外 |
|---:|---:|---:|---:|---:|---:|
| 5 | 86/90 | 0 | 0 | 0 | 4 |
| 20 | 90/90 | 11 | 13 | 0 | 0 |
| 50 | 90/90 | 30 | 59 | 0 | 0 |
| 100 | 81/90 | 64 | 259 | 4 | 5 |
| 200 | 67/90 | 64 | 562 | 23 | 0 |

半程证据把 D2 的开放 P1 分成两项。第一项是 100/200 规模下规范航迹吸收多个真值来源，
其中 23/27 个一轨多真值失败位于 200 规模。第二项是场景/种子相关的谱系时间链问题；
9 个窗外失败不随规模单调增加，不能通过直接扩大 `0.9 s` 窗口解释或修复。后续若修改
关联、身份承诺、调度或谱系窗口，必须冻结新 source 和新 execution plan，并从 shard 0
重新运行；现有 450 个 episode 保持原始解释。
...
- 2026-07-23 模块回归为 `291 passed, 1 warning in 29.05s`，验收阈值为零失败。专项覆盖活动
...
- 新 candidate 输出 D2 航迹 201、D3 分配 197；1787 条记录中 1711 条 committed、
  ...
  `GT3D-000185`、`GT3D-000186`、`GT3D-000202` 的恢复被
  `source_observation_outside_recovery_publication_freshness_window` 正确阻断。
  未提交 source/candidate binding violation 均为 0，在线 truth use 为 0。
- 发布新鲜度门控解决了旧制品的 strict-unavailable 阻断。candidate strict IDSW 为 3，
  track continuity 为 `0.8266667`，coverage continuity 为 `0.8283333`，三项均可用；
  duplicate assignment 仍为 0。IDSW 虽由 9 降至 3，但 track continuity 和 coverage
  continuity 分别从 `0.865/0.870` 降至 `0.8266667/0.8283333`，D2 航迹和 D3 分配也
  分别少 2 条和 3 条。候选未满足联合非退化门槛，继续默认关闭。
