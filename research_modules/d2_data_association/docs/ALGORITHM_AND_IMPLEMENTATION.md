...
本轮没有修改算法。clean producer `80e55eb` 生成 450 个冻结 episode，D6 v12 evaluator
`b6289c5` 只在离线侧读取 `offline_identity` 和 `d6_truth_isolated` 制品。在线 D2
继续发布 `id_switch_count=None` 与 unavailable reason，离线 evaluator 则校验来源文件
哈希、观测真值 sidecar、逐帧谱系和身份承诺后计算严格指标。

严格判定分为三步：

1. 每条 D2 身份证据必须通过来源哈希、记录序列、双时间戳和在线真值隔离检查。
2. 每个可评分 `global_track_id` 在一帧内只能对应一个真值目标，来源观测必须落在冻结的
   `0.9 s` 谱系窗口内。
3. 只有完整转移宇宙通过上述检查，才计算严格 ID Switch 和连续性；任一阻断原因存在时，
   episode 级严格指标整体失败关闭。

半程结果为 `414/450` 严格可用、36 个失败关闭。可用项合计 893 次 ID Switch，169 个
episode 非零。失败项为 27 个一轨多真值和 9 个谱系窗外。35 个失败项仍能计算部分
ID Switch 下界，1 个没有可评分身份转移；这些诊断不具备完整分母，不能与 893 相加，
也不能作为严格指标进入准入比较。

后续算法任务先对失败 episode 重放候选边、连通分量、来源 claim、身份承诺和生命周期
转换。高规模问题优先验证一轨多真值是否来自错误合轨或歧义期间过早承诺；时间链问题
优先核对 measurement、arrival、state-valid、published 和 consume 时刻。任何门限、
窗口或关联策略变更都形成新的 source 和 execution plan，现有 450 个 episode 不重算、
不改标签、不后验解释。
