...
## 2026-07-31 正式 R0 shard 0-9 身份结果

### 证据范围

本节统计 clean producer `80e55eb` 的正式 R0 shard 0-9。共 450 个 episode，覆盖 9 类
场景、5 档规模和 seed 1000-1009。完整计划为 20 个 shard、900 个 episode，因此本节
只报告半程事实，不给出完整 20-seed 验收结论。严格重聚合由 D6 v12 evaluator
`b6289c5` 完成，输出目录为
`/tmp/msm-formal-r0-20260731-80e55eb/d6_strict_partial_450_b6289c5`。

在线 D2 producer 的真值指标在 `0/450` 个 episode 可用。离线严格评估在 `414/450`
个 episode 可用，可用项 ID Switch 合计 893 次，169 个 episode 非零。36 个 episode
失败关闭，其中 27 个为一轨多真值，9 个为来源观测超出谱系窗口。未将任何不可用值
写成零。

### 规模分布

| 目标规模 | episode | 严格可用 | 非零 | ID Switch 合计 | 最大值 | 一轨多真值 | 谱系窗外 |
|---:|---:|---:|---:|---:|---:|---:|---:|
| 5 | 90 | 86 | 0 | 0 | 0 | 0 | 4 |
| 20 | 90 | 90 | 11 | 13 | 3 | 0 | 0 |
| 50 | 90 | 90 | 30 | 59 | 7 | 0 | 0 |
| 100 | 90 | 81 | 64 | 259 | 18 | 4 | 5 |
| 200 | 90 | 67 | 64 | 562 | 48 | 23 | 0 |

100 和 200 规模贡献 821/893 次严格 ID Switch。27 个一轨多真值失败中有 23 个位于
200 规模、4 个位于 100 规模，50 及以下没有该失败。高规模身份稳定性和规范航迹错误
合并是明确的 P1，不能由总体平均值掩盖。

### 场景分布

下表将相同场景族的五档规模合并，每个场景族含 50 个 episode。

| 场景族 | 严格可用 | 非零 | ID Switch 合计 | 最大值 | 一轨多真值 | 谱系窗外 |
|---|---:|---:|---:|---:|---:|---:|
| 中心失效 | 48 | 17 | 54 | 8 | 1 | 1 |
| 通信退化 | 48 | 15 | 42 | 6 | 2 | 0 |
| 延迟噪声 | 46 | 33 | 486 | 48 | 0 | 4 |
| 密集交叉 | 44 | 21 | 95 | 25 | 6 | 0 |
| 多高度机动 | 45 | 17 | 38 | 6 | 4 | 1 |
| 编队分裂 | 44 | 16 | 41 | 6 | 5 | 1 |
| 高威胁 M 对 N | 47 | 17 | 42 | 6 | 3 | 0 |
| 标称 | 44 | 16 | 41 | 6 | 5 | 1 |
| 二级节点失效 | 48 | 17 | 54 | 8 | 1 | 1 |

延迟噪声场景贡献 486/893 次严格 ID Switch，但没有一轨多真值失败。它反映的是可评分
身份切换负担，不能与严格指标不可用混为一类。密集交叉的一轨多真值最多，共 6 个；
标称、编队分裂和多高度机动也出现相同失败，说明问题不只存在于场景名称带“密集”的
用例。

### Seed 分布

| Seed | 严格可用/45 | 非零 | ID Switch 合计 | 最大值 | 一轨多真值 | 谱系窗外 |
|---:|---:|---:|---:|---:|---:|---:|
| 1000 | 39/45 | 11 | 86 | 40 | 5 | 1 |
| 1001 | 34/45 | 5 | 57 | 34 | 11 | 0 |
| 1002 | 44/45 | 19 | 75 | 13 | 1 | 0 |
| 1003 | 41/45 | 20 | 71 | 29 | 4 | 0 |
| 1004 | 40/45 | 14 | 90 | 48 | 0 | 5 |
| 1005 | 43/45 | 19 | 81 | 29 | 1 | 1 |
| 1006 | 44/45 | 26 | 154 | 48 | 1 | 0 |
| 1007 | 44/45 | 19 | 92 | 28 | 0 | 1 |
| 1008 | 43/45 | 12 | 85 | 20 | 1 | 1 |
| 1009 | 42/45 | 24 | 102 | 40 | 3 | 0 |

seed 1001 集中了 11 个一轨多真值失败；seed 1004 集中了 5 个谱系窗外失败。另有 4 个
谱系窗外失败位于 5v5 延迟噪声场景的 seed 1000、1005、1007、1008。该分布不支持
“目标越多，谱系窗口越容易失败”的单一解释。后续应追踪时刻链和 sidecar 匹配，不能
先扩大 `0.9 s` 窗口。

### 部分诊断边界

36 个严格不可用 episode 中，35 个提供局部 ID Switch 下界，1 个因没有可评分身份
转移而连下界也不可用。下界只覆盖证据完整的若干转移，缺少严格指标所要求的完整转移
宇宙。它可以定位错误发生前后的帧，不能与 414 个严格可用 episode 的 893 次相加，
不能用于算法排名、均值、置信区间或准入结论。

### 后续验收

下一候选先复现 36 个失败 episode，并选择相同规模、场景和相邻 seed 的通过项作为
对照。关联、身份承诺、调度或 lineage window 一旦改变，必须冻结新的 source commit
和 execution plan，从 shard 0 重新运行。正式验收继续要求在线真值使用为 0、
`global_track_id` 非法改写为 0、严格不可用项不补零，并同时比较 ID Switch、连续性、
航迹数量、位置误差和 D2 墙钟。现有 450-cell 只保留为 `80e55eb` 的历史半程证据。
...
| v2 合同验证 | 3 | 3 |
| v2 完整内容审计 | 3 | 3 |
| v2 同身份复用 | 597 | `3*(200-1)` |
| 内建等值复用 | 0 | 0 |
| 上游 `global_track_id` 进入 Detection | 0 | 0 |

负向测试覆盖只读树中的 truth/actor/object/target/global-track 禁止键、等值不同身份根、
畸形精确构造、v2 子类、伪造 marker、自定义恒等映射和可变后端。禁止键和不合法合同
均失败关闭；等值不同身份根分别验证和审计。原内建容器回归保持“1 次完整审计 + 3 次
等值复用”。
