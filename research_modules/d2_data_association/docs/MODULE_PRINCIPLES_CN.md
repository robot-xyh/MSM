...
2026-07-31 的正式 R0 半程证据来自 clean producer `80e55eb` 的 shard 0-9，共
450 个 episode。矩阵总量为 900，当前只覆盖 seed 1000-1009，不能代表完整 20-seed
结论。D6 v12 evaluator `b6289c5` 校验离线身份制品后，严格 `id_switch_count` 在
414 个 episode 可用，可用项合计 893 次，其中 169 个 episode 非零。

D2 在线 producer 在全部 450 个 episode 中均声明该真值指标不可用。在线关联只消费
不含真值的观测、状态、协方差、时间戳和来源谱系；D6 在 episode 完成后才把独立真值
sidecar 与 D2 证据联接。在线不可用与离线严格可用是两个并行口径，不能用离线结果
回填在线消息。

严格评估的失败关闭规则保持不变。若一个 `global_track_id` 在同一证据范围内对应多个
真值目标，或来源观测落在冻结的谱系窗口之外，episode 的严格 ID Switch、连续性和
重复指标均记为不可用。当前 36 个不可用项中，27 个是一轨多真值，9 个是谱系窗外。
不可用值没有数值含义，不能按零参与均值。局部可评分转移生成的 ID Switch 下界只用于
定位问题，不能替代完整转移宇宙上的严格指标。

规模分布说明两类问题来源不同。一轨多真值在 100 规模出现 4 次、200 规模出现 23 次，
50 及以下未出现，指向高规模下的身份合并与延迟承诺 P1。谱系窗外在 5 规模的延迟噪声
场景出现 4 次，在 100 规模的 seed 1004 出现 5 次，未呈规模单调性，指向时间链和证据
生产 P1。扩大 lineage window 会改变评分和在线恢复语义，必须作为新 source 候选重新
执行，不能重解释现有制品。

## 最新增量 D1 发布审计 v2

D1 在一批全局航迹中发布三类批级诊断：关联审计、时延审计和传感器健康。D1 v1
候选把这些诊断改为共享只读对象后，D2 仍按每条航迹递归检查。D6 的 13 对正式评估中，
D1 融合阶段明显缩短，D2 关联阶段却分别增加 `53.44%` 和 `169.89%`，最终核心墙钟
没有达到 `5%` 的系统准入门。

D2 当前采用两层准入。第一层调用 D1 公布的精确
`d1.publication_audit_tree.v2` 递归验证函数，确认根和所有子节点均为不可变合同类型，
不存在可变后端、子类或不受支持的叶值。第二层继续执行 D2 原有的内容策略，检查
`truth`、AirSim actor/object/target 身份和 `global_track_id` 等禁止键。合同不可变只
说明内容不会在审计后变化，不能证明内容本身符合在线真值隔离要求。

同一个 v2 根首次通过两层检查后，D2 在本批保存对象强引用，并按 `id` 定位后再用
`is` 确认。后续相同对象不再递归扫描。等值但不同身份的对象不复用；畸形精确构造和
v2 子类失败关闭。旧内建容器仍沿用“首个等值代表完整审计后复用”的兼容路径。
...
5. **模块通过不等于算法准入**：2026-07-23 的确定性专项为 `32 passed`，完整 D2 为
   `291 passed, 1 warning in 29.05s`。配置谱系绑定后的 clean 提交 `ff88131` 已确认
   seed 1100 strict 指标可用、绑定违规为 0、在线真值使用为 0；候选仍因 D2/D3 数量及
   连续性退化被拒绝，seeds 1101/1102 继续停止。

## 三十五、租约因果与运动学保持原则（2026-07-23）

1. **租约参数不等于运动学算法**：gap 决定无新证据时的 soft release，hard 决定首次
   分量的绝对上限。两者都不提供状态修正。clean seed 1100 的五组扫描均得到 197 条
   D2 终态航迹，说明缩短 deadline 不能替代歧义期运动学处理。
2. **长短租约存在同一根因的两种表现**：长租约保留 tentative 航迹，但让它长期未提交；
   短租约更早释放，随后因缺少合格运动学更新进入 miss/drop。默认候选累计阻止 69 次
   hit/update，是连续性下降的首要 D2 原因。
3. **离散消费不得通过放宽新鲜度补偿**：三条租约 deadline 落在相邻 D2 frame 之间，
   下一消费时证据年龄达到 `0.9308153039 s`。超过 0.9 秒的恢复继续失败关闭。
4. **身份承诺与运动学支持必须分离**：下一候选可为已有成员提供保守、排列不敏感的
   运动学支持，但不能产生 observation-to-track 身份绑定、确认命中、建轨、改绑或
   committed 恢复。歧义子空间协方差不得小于纯预测协方差。
5. **下游只消费 committed 身份**：旧冻结 seed-1100 制品的 D3 第 2/3 版计划曾包含
   11/8 条未提交航迹。固定提交 `7e15dac9` 的同输入 clean 复验已证明新准入会把未提交
   航迹从 D3 assignment、D5 active vision 和 D7 guidance 中全部排除，并严格增加计划
   版本。D2 仍不能用未提交航迹维持分配数。
6. **运行配置必须有谱系证据**：权威 clean A/B 的 9 条 D2 发布均携带承诺
   schema/policy 和恢复配置快照。集成配置
   ...
   manifest 已验证 episode 内一致性并绑定原始 D2 JSONL。算法结论只能引用该实际配置，
   不能以模块默认值或运行后推断替代。

## 三十六、承诺准入与算法准入分离原则（2026-07-23）

1. **同输入复核先验证运行谱系**：本轮两臂来自固定提交
   `7e15dac9cdaf6743999dfe045a70676fd31a17d6`，均为 clean 工作树、nominal
   200v200、`recon_count=2`、`2.2 s`、seed 1100，场景配置 SHA-256 均为
   `20ef5248c8b45ff5aced9080c8d47e65a43aaba54f18ce824dc50fac7a52b840`。
   运行时配置只允许在被比较的质心候选开关上不同。
2. **D2 相同输出证明候选没有实际处理**：两臂 D2 在线记录 SHA-256 完全相同，终态
   航迹、IDSW、两项连续性和映射计数也完全相同。质心臂虽产生 46 个候选，但 30 个因
   OOSM 扫描、16 个因分量不平衡被拒绝，实际处理为 0。零差异不能解释为算法等价或
   有效，只能说明该输入没有进入 treatment。
3. **安全合同按在线发布物验收**：在 D2 首次发布 11 条未承诺航迹的 `t=1.0 s`，
   D3 第 2 版计划显式拒绝这 11 条航迹，强制重规划并绕过迟滞；旧绑定不再出现在新计划。
   第 3 版继续按显式状态拒绝未承诺目标。被拒绝 ID 与 D3 assignment、D5 active-vision
   command、D7 guidance command 的交集均为空。
4. **安全关闭不能覆盖性能退化**：两臂 strict IDSW 为 3，但 track continuity 和
   coverage continuity 仍只有 `0.8266666667/0.8283333333`，可用映射仍为 1491，
   未承诺映射仍为 76。显式准入防止错误身份继续执行，没有修复 D2 航迹连续性、映射
   可用性或 prediction-only hold 的运动学信息损失。
5. **P1 晋级仍由联合门槛控制**：结构歧义 hold 和质心候选继续默认关闭。下一候选需先
   在 seed 1100 形成非零、可归因的 treatment，并同时满足中心 ID 不改写、未承诺绑定
   零违规、航迹数、可用映射和 continuity 非退化。未满足前不启动 seeds 1101/1102。
6. **证据类型必须标明**：本轮是三维质点 200v200 全栈复核，不是 AirSim、硬件在环或
   实飞。AirSim 时钟、传感器漏检、遮挡、杂波和算力时延仍需单独标定。
