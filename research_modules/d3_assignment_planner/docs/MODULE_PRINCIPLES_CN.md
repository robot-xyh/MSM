...
> 状态基线：2026-07-26。本文只描述当前仓库已实现行为、已验证证据和明确保留的研究边界，不改变模块能力状态。

## 证据输入与版本谱系

A1 选择只消费匿名规划帧、冻结策略摘要和预注册范围。在线字段名只要携带真值身份、
仿真 Actor 身份或 Object 身份语义，即使写成 `target_truth_id` 或
`resource_actor_name`，也不能进入选择。D6 物理窗口中的离线真值制品摘要属于选择完成
后的来源证明；它不回流为代价、候选边或选择条件。

序列化 A1 候选必须保留前序、规则和处理计划的连续版本关系。规则和处理计划最多前进一个
版本；处理绑定发生变化时必须严格升版。内容摘要只能证明载荷一致，不能替代上述业务谱系
复算。2026-07-27 全量验证为 `562 passed, 1 skipped`，未产生新的运行权限或物理证据。

## 学习模型准入

D3 将模型读取权限与在线辅助权限分开。合法的旧版模型仍可在隔离 shadow 中复现预测，
...
`bundle_shadow_only`。D6 已有证据只覆盖同帧离线比较；运行采用、干预后物理结果和成对
...
滚动规划不能只检查上一计划中的可执行绑定。未分配目标也可能保留 incomplete
coalition、需求摘要和联盟版本。当当前 `TargetDemand` 从 `k=1` 变成 `k=3`，或从
`k=3` 降为 `k=1` 时，旧 coalition 已不再表示当前任务合同。即使旧目标没有 executable
assignment，也不能让成员迟滞继续保留该库存。

D3 先比较当前候选与上一计划的需求合同签名。签名包括需求资源数、主资源数、协同模式、
时间和波次模板、终端授权范围。上一 assignment 声明的需求数也参与检查。存在差异时，
上一计划的可保留性记为 false。规划器采用当前硬安全候选重新生成计划，并通过计划版本
撤销旧库存。迟滞仍可保护需求合同未变化且仍可行的 coalition。

重建后的计划继续经过最终库存规范化。目标 executable assignment 数不得超过当前需求；
完整 coalition 的成员数必须等于 executable assignment 数；不完整 coalition 不得发布
可执行成员；同一资源不得重复分配。审计字段记录不兼容目标、重建要求和实际采用状态，
便于 main 和 D6 区分普通成员换位与需求合同强制释放。

2026-07-25 的开发复验复现了 clean commit `32b3b40` 正式 R0 的原失败条件：
`high_threat_m_to_n`、200v200、seed 1000、2.0 秒。修复后
`GT3D-000021` 在 `t=1.0` 从空 `k=1` 库存重建为完整 `k=3` coalition，episode
有限状态为真，在线真值使用、过分配和需求摘要失配均为 0。D3 全量为
`464 passed, 1 skipped`。这属于当前工作树开发证据，新 clean commit 的 formal R0
重跑仍待 main 完成。

## 多周期学习残差影子评估

D3 的学习模型不直接输出目标与资源的配对结果。冻结行为克隆模型只对当前硬安全候选边
给出代价残差，形成
`C_effective = C_rule + alpha * tanh(delta_C)`。最终配对仍由原匈牙利算法或需求槽
匈牙利算法求解。可达性、容量、身份承诺、友方冲突、M-to-N 联盟完整性和计划版本均在
学习模型权限之外。

多周期评估为每个 seed 建立两个独立规划器。规则组使用原规则矩阵；处理组在隔离求解器中
使用冻结残差。两组在每个周期接收同一组匿名目标、资源、时间戳和外生事件，然后分别把
自己的当前计划作为下一周期前序计划。这一结构能够观察残差对迟滞、绑定保持和后续计划
谱系的累积影响，避免把若干互不关联的单帧比较写成动态结论。

2026-07-25 的固定保留种子影子样本包含 20 个保留种子和 6 类场景，共 620 个周期。行为克隆残差改变
580 个有效代价矩阵，并在 120 个周期形成不同绑定。匈牙利切换边界中 20/20 seed 均出现
绑定差异，说明模型干预已可辨识。资源重复、硬约束或谱系违规、旧版本采用和在线真值使用
均为 0。M-to-N 需求从 1 增至 3 时有 40 个周期进入分布外保护，处理矩阵逐元素恢复规则
矩阵。

处理组累计抖动为 200，规则组为 520；处理组按规则矩阵重评分的周期平均代价高 0.000707。
抖动降低与规则代价上升同时存在，当前没有运行确认、物理结果或可归因奖励，不能判断任务
收益。模型仍为 development/shadow-only，PPO、线上辅助、线上权限和运行计划发布保持关闭。

## 身份承诺先于分配

D3 只对身份已提交的全局航迹分配资源。`committed` 表示 D2 当前允许该航迹进入执行计划；
歧义保持期未提交和保持期后仍未提交均表示身份证据不足。该判定是硬准入条件，不是代价
权重。学习代价修正、威胁度和资源优势均不能恢复未提交边。

缺失状态和未知状态同样不形成候选边。main 显式传递 D2 状态并负责 hold/replan；D3 在新
...
ACK 的总线位置不能替代执行状态判断。当前 seed 41 重放中，首条 ACK 有可执行 binding，
末条 ACK 因航迹过期全部保持。D3 先用各 ACK 对应的发布时计划、D3 来源和 D7 来源完成
逐条验证，再选择首个 `guidance_command_present=true` 且 `held=false` 的 binding 建立结果
窗口。末条保持 ACK 仍是有效的失败关闭证据，但不能被解释为控制已采用样本。该取样规则
不改写状态时间，不放宽 D7 航迹时效门，也不改变比例导引公式。2026-07-22 当前全量回归
...
## 计划外性能诊断原则（2026-07-22）

性能诊断与战术计划分离。确定性操作数可记录输入规模、候选边、求解矩阵、迟滞访问、证据
复制和发布校验，但不能写入 AssignmentPlan 元数据、计划身份或控制合同。墙钟受机器负载、
缓存和进程调度影响，只能由 benchmark 或报告保存，不参与计划哈希和运行时决策。

性能归因优先比较冻结输入下的结构操作数。200×200 不只表示 40,000 个代价对，还应同时
说明候选边数量、连通分量形状、未分配虚拟列、计划证据条目和 previous-plan 重评分数量。
这样可以区分求解复杂度、可解释证据成本和多周期迟滞成本，避免把集成调用密度误判为单次
算法退化。

参考路径不得成为安全旁路。关闭匿名规划证据可以测量证据构造边界，但在线计划必须继续
保留证据。candidate signature 只在一次规划调用内复用。latest published execution
signature 由 planner-owned cache 跨帧保存并作为权威；caller previous 必须计算自身签名
完成一致性校验，不能提供或替换 latest。公共发布始终从待发布对象计算 candidate signature。

异常类型应保留模块语义。区域接管先检查 plan id/version，再检查 pending inventory，最后
执行通用 execution signature 校验。库存证据篡改返回区域专用错误；其他同 identity 执行
语义篡改返回 stale 错误。异常优先级不能削弱直接发布或 authority fence 的失败关闭门控。

默认路径与参考路径必须比较资源目标 binding、版本、规范业务哈希和计划号复用行为。任一
项不一致时，性能比较无效。单次微基准不能替代 clean episode、多 seed、运行时 ACK、D6
读取或物理结果验证。

## 冻结归因与集成复核分层原则（2026-07-22）

固定输入微基准和完整 episode 累计计时回答不同问题。冻结 200x200 benchmark 固定输入
SHA、候选边和结构操作数，用于定位一次规划调用中的成本边界。clean 10 秒复跑保留完整
模块调度、总线发布和运行确认，用于检查调用密度、累计墙钟和计划业务语义。两组数字不能
直接相减或互相替代。

commit `8f86192` 的 seed 42000-42002 均为 clean、finite，在线 truth 使用为 0。每组 D3
调用和计划 ACK 均为 10，计划业务摘要与旧 commit `3bac3ff` 一致。D3 assignment 累计均值
由 `3.348 s` 变为 `3.289 s`，约下降 1.8%。该变化处于集成调度噪声范围，只能判定为基本
持平。它不证明签名复用或 D1 快照优化带来 D3 性能收益，也不能触发算法晋级。

集成复核首先检查调用次数、版本、绑定、ACK、hold 和在线真值隔离，再解释墙钟。业务摘要
不变时，可确认跨模块优化没有改变 D3 执行语义；仍需 AirSim、物理拦截、长期内存和实时
预算证据才能形成系统能力结论。冻结 benchmark 的 `334.735 ms` 等原数字继续作为独立归因
基线保留。

## 独立运行计划身份原则（2026-07-22）

计划号用于运行内隔离，不是业务结果的确定性编号。两个全新的规划器即使接收相同 seed、
输入和时刻，也应建立不同的原始计划号；同一规划器的纯评估刷新必须保持原计划号和版本。
执行绑定、所有者、激活状态或联盟执行语义变化时，新计划严格升一版并指向上一已发布计划。
权威代际隔离可在执行签名不变时升版，但必须由专用 fence 记录说明，不能作为普通刷新。

跨运行等价审计采用“先验证链、后映射、再比较”。每个 episode 按权威事件顺序把计划号首次
出现映射为 `P0000/P0001/...`，并保留 version、parent token、owner 和 transition reason。
计划引用和由计划号派生的 binding/decision 标识使用同一映射；摘要在规范化后重算。
resource、target、全局航迹、节点、区域和 coalition 标识保持原值。版本、前序关系、stale、
owner、epoch、lease、commit、成员和角色均须精确一致。删除全部标识或只比较 assignment
数量不构成业务等价证明。

## 干预候选帧资格原则（2026-07-26）

学习干预的物理续跑必须先证明规划决策可辨识。规则组和处理组在同一匿名输入、同一前序
计划和同一时刻下运行；若处理矩阵虽改变但资源目标绑定不变，该帧不能作为后续物理差异的
共同起点。D3 只判断当前帧是否具备比较资格，不根据 seed、物理结果或事后成功率挑选样本。

资格由原始规划证据推导，不接受调用方填写的安全布尔。模型必须真实作用于至少一条
hard-safe 候选边，且无回退、分布外、超时、非有限值或元数据缺失。规则计划与处理计划都
必须满足资源唯一、成本有限、候选边允许、计划版本连续、前序计划有效、需求槽完整和
M-to-N 联盟全有或全无。最终绑定至少有一个资源、目标、联盟角色或波次发生变化。

证据包含规范摘要和完整内容 SHA-256。缺字段、额外字段、占位摘要、手工修改 eligibility、
摘要不一致和历史乱序均失败关闭。摘要用于完整性和可重复选择，不是签名，也不赋予模型
准入、在线计划发布、区域权威或飞行控制权限。

main 按每个 seed 提供帧对。`sequence_index` 与 `timestamp_s` 必须分别严格递增，一个规划
周期只允许一个时间戳；重复或逆序时间戳直接拒绝，不能用序号为同刻记录消歧。D3 在该合同
下返回按规划时间的首个合格帧。D7 共同检查点、后续续跑窗口和结果评价仍由 main 与 D6
管理。没有合格共同帧时结论保持“不可比较”，不得降低分布外门限、硬安全门或 M-to-N
原子性来寻找正样本。

## 单帧隔离重放原则（2026-07-26）

候选资格评估前，规则组和处理组必须由同一冻结规则输入在隔离规划器中分别计算。规则矩阵、
匿名航迹、资源、规划时刻、前序计划和强制重规划状态构成输入快照。规则组不得加载学习
助手；处理组只能使用通过 manifest SHA-256、policy version、v3 development 和
shadow-only 校验的冻结 bundle。求解器、需求槽、硬候选边和默认阈值继续复用线上实现。

前序计划发布到新建规划器只用于恢复该隔离实例的本地状态。规则和处理候选都以
`publish=False` 生成，不进入运行总线，不产生运行 ACK，也不取得 authority。单帧 DTO
保存两份完整规划证据、bundle 装载状态、输入谱系、资格证据和内容摘要。bundle 身份错误、
模型回退、分布外、超时、非有限值、旧版本、过期前序计划、矩阵不等价、标识顺序变化或
内容篡改均失败关闭。

匿名规划帧没有实验 seed。D3 不在该接口中增加 seed，也不据此声明保留集清单完整。main/D6
负责从外层 manifest 验证 seed `1000-1019`、split 身份、样本数量和逐 seed 时间顺序。D3
接口只回答一个规划周期能否成为检查点候选，不回答模型准入、物理 outcome、reward 或正式
实验是否完成。

## 20-seed 外层重放原则（2026-07-26）

seed 身份与匿名规划帧分层保存。`PlanningFrameEvidence` 本身不增加 seed；外层 manifest
负责把 seed `1000-1019` 与一组显式文件绑定。每个文件同时绑定文件 SHA-256、完整内容
SHA-256 和输入快照 SHA-256。runner 只读取清单列出的路径，不能通过目录枚举补齐缺帧。
源码 commit、clean/dirty 状态、bundle 身份、规划配置和成本权重也是输入谱系的一部分。

每个 seed 的帧序号和规划时间必须同时严格递增。runner 对每帧执行相同的单帧规则/处理
隔离重放，再由资格 selector 选择首个合格记录。没有合格记录属于可表达结果，状态为
`unavailable`。以最后一帧、最接近门限帧或绑定未变化帧替代首个合格帧会改变试验问题，
因此不允许。

可复现批量摘要只保存稳定语义。独立 planner 的计划号由 UUID 生成，推理耗时取自本机
墙钟；二者不参与跨目录确定性摘要。摘要保留输入、矩阵、动作掩码、资源目标绑定、bundle
身份、回退、安全计数和选择状态的规范哈希。这样既能追溯业务差异，又不会把无业务含义的
本地随机值误作实验差异。

输出在临时目录内完整生成，并在所有输入再次验哈希后一次发布。JSON、逐 seed CSV、中文
报告和校验清单缺一不可。结果始终属于检查点选择证据；计划发布、运行 ACK、分配权限、
控制权限、物理结果和奖励均为不可用。后续 main 可以消费选择结果，但必须另行完成 D7
检查点求交和物理运行。

## 匿名联盟身份重放原则（2026-07-26）

匿名目标标识和匿名联盟标识属于两个独立命名空间。规划帧匿名化会同时查看当前计划和前序
计划，再为联盟生成稳定 token。隔离规划器只看到匿名目标名；目标第一次形成联盟时，本地
命名规则可能生成不同字符串。该差异不代表资源分配变化，但联盟标识属于执行签名，不能从
严格比较中删除。

重放时只允许使用已通过文件 SHA、内容 SHA 和输入 manifest 绑定的记录规则计划恢复联盟
命名。恢复前必须验证记录计划、重放计划和前序计划的目标联盟库存、标识唯一性、assignment
引用、成员角色、需求摘要和 metadata 引用。前序计划已有联盟时，记录和重放两侧必须继续
使用原标识；只有前序计划中尚无联盟的目标，才能把本地新标识替换为记录匿名标识。重复
标识、目标库存差异、已有标识重写或任一引用冲突均失败关闭。

身份恢复不采用记录计划的资源成员、成本、版本、迟滞或决策结果覆盖重放结果。规则控制臂
恢复后仍须通过原完整执行签名比较；处理臂只恢复同一目标的联盟命名，成员和绑定继续由
...
## A1 阶段事实分离原则（2026-07-26）

学习评估、代价接受、离散计划变化、计划发布、运行确认和物理结果是不同事实。资格层只
回答同输入规划帧是否通过谱系、安全、版本、需求槽和联盟原子性检查。A1 选择层在其上增加
预注册边界，判断代价修正是否有界、规则成本下是否近似竞争、覆盖是否非退化，以及离散
binding 是否确实变化。后一层不得复制或放宽前一层资格逻辑。

预注册必须发生在评估前。模型制品摘要、seed 清单、序号与时间范围、代价差和绑定变化
上限进入内容摘要。在线 truth、目标真实身份、物理命中、后验 reward 和 D6 结果不能进入
selector。当前 formal 记录没有合格帧时，预注册也不能把任意帧变为可用；结果保持
`no_safe_discrete_intervention`。

计划发布必须由 main 总线实物证明。被选中的 `AssignmentPlan` 与 runtime payload 的计划
号、版本、schema、assignment、未分配目标、规模、求解器和 metadata 必须完全一致。选择
结果本身不能声明发布。运行确认必须复用既有 ACK 验证对象，并绑定同一发布序号和 payload
摘要；日志中出现控制命令不等于确认采用。

物理窗口按计划的全部执行 binding 判定。只有每个资源目标 binding 都有命令、已应用确认
和至少一项可用观察结果，`physical_window_available` 才能为 true。R0 配对还要求每个
binding 都有同 seed 基线证据。部分成员、缺窗口、缺 ACK 或未配对时，整份计划继续失败
关闭。物理窗口可用也不等于因果收益或模型晋级。

阶段证据采用单调谱系，不采用单调布尔补写。调用方每次用实际新增实物重新装配完整 DTO；
缺少后续实物时停留在最后一个已证明状态。候选和选择对象固定不允许预填发布、ACK 或物理
字段，防止事后报告把上游可用性扩写为下游事实。

## 隔离证据稳定投影原则（2026-07-27）

隔离 replay 的计划身份属于单次执行局部事实。新建 planner 会生成随机计划号，推理耗时也
受本机调度影响。将完整运行 DTO 原样写入跨运行 batch 会导致相同 manifest 的 checksum
变化。A1 batch 因此分为两层：内存层使用完整核心 candidate/selection DTO 进行严格判断；
写盘层保存去除局部随机身份后的稳定证据投影。

稳定投影不能删除业务身份。资源、全局航迹、联盟、成员角色、计划版本、规则/处理 binding
摘要、成本、需求缺口、安全状态、原因码、输入文件摘要、bundle 和预注册谱系必须保留。
只排除随机 plan id、由随机 plan id 影响的完整 plan payload 摘要、核心 candidate 运行内
内容摘要和墙钟推理时间。投影 schema 明确记录排除项，禁止把它解释为运行发布证据。

selector 仍消费核心 A1 candidate，而不是消费投影后的布尔值。每帧 candidate 由
`replay.rule_frame` 和 `replay.treatment_frame` 重新调用核心评估器生成，并与 replay
资格摘要核对。稳定 selection 只在核心 selector 已完成后生成；调用方不能提交
`selected=true`。

预注册是 batch 输入，不是命令行提示值。它必须使用完整内容摘要，精确覆盖清单 seed 和
全部帧范围，并绑定实际 state-dict。预注册、manifest、帧、bundle manifest 和权重文件
在输出前后均需保持不变。任一输入变化时 staging 输出被丢弃。

旧 batch v1 保持原函数、schema 和四文件输出。A1 是显式可选扩展模式。这样已有证据仍可
重放，新证据又能为 main 提供确定性的 paired-evaluation 检查点。两种模式都不发布计划、
不制造 ACK、物理窗口、奖励或生产权限。

## 区域提示权属单一性原则（2026-07-27）

一份 D3 `AssignmentPlan` 只有一组顶层执行权属。区域提示覆盖多个区域时，各约束必须给出
相同的权属层、权属节点、权属代次和租约截止时刻。D3 不采用“第一个区域优先”、最小代次、
最短租约或多数投票。四元组不唯一时，提示整体拒绝，规划回到未受提示影响的规则路径。

统一权属判断位于提示候选图生效之前，但不取代既有验证。来源计划号和版本、created/expiry、
逐区域租约、区域集合、资源守恒、跨区额度、保护资源和安全边仍须全部成立。处于 hold 且
没有活动 owner 的提示不能成为顶层执行权属。

权属字段只随形成严格后继的提示进入计划 metadata。owner 系列字段使用同一 layer/id，
`authority_epoch` 与 `lease_expires_at_s` 成对出现。拒绝提示、无提示计划和没有形成新
执行语义的提示不补写或刷新这两个字段。运行 ACK 可以据此逐字段核对 D4 来源权属，但 D3
metadata 仍只证明计划合同，不证明 ACK 已送达、联盟成员已确认或计划产生了物理效果。

authority epoch 和 lease 本身是同一执行计划上的权属审计信息，不单独触发计划升版。
资源目标绑定、联盟、owner 或其他执行签名发生变化时，仍由现有计划身份规则生成严格新
版本。相同 owner、相同执行签名的周期复核保持 evaluation refresh，并显式返回无后继；
无提示刷新保持原默认 owner 和原计划身份。

## 区域提示后继计划原则（2026-07-27）

区域约束参与候选图不等于后继计划已经发布。D3 先完成提示合法性、候选图、求解、迟滞和
身份承诺，再比较候选与来源计划的完整执行签名。只有签名变化时，候选才具备后继资格。

设来源计划为 \(P_s\)，最终候选为 \(P_c\)：

\[
\Delta_{\mathrm{exec}} =
\mathbf{1}\left[\operatorname{Sig}(P_c)\ne
\operatorname{Sig}(P_s)\right]
\]

当 \(\Delta_{\mathrm{exec}}=1\) 时，新计划必须使用不同 `plan_id`、严格递增版本，并把
`previous_plan_id` 指回 \(P_s\)。当 \(\Delta_{\mathrm{exec}}=0\) 时，D3 返回当前计划的
幂等评估刷新，同时把后继状态标为 `no_successor`。此时不得刷新租约、权属代次或将当前
身份提交给 D4 作为新计划。

`regional_hint_constraint_applied` 只回答提示约束是否进入候选求解。
`regional_hint_successor_plan_available` 回答是否存在可供 D4 接管、运行确认和物理审计
使用的严格后继。两个状态分开后，零动作建议、迟滞保留和相同绑定重算均不会伪造版本。

2026-07-27 验收覆盖不同资源规模的零动作提示、重复幂等消费和真实执行变化。区域提示
专项 `21 passed`，D3 全量 `547 passed, 1 skipped`（548 项）。该结果证明 D3 合同，
不代表 A2 已产生物理收益或获得运行权限。

随后完成的 A2 seed 1000-1019 重跑进一步验证了边界。20/20 seed 均有候选评估，但全部
候选的资源配额变化、hold、重规划请求和跨区转移均为空。可识别干预、实际采用和同键
A2/R0 收益审计均为 0/20。早期 18/20 采用表象来自同周期普通 D3 滚动重规划，不属于 A2
因果结果。

因此 `no_successor` 是当前正确输出，不是待修复的版本故障。A2 只有产生经过确定性投影后
仍非零的区域动作，才可继续进入严格后继、运行确认和物理窗口。输出证据 SHA-256 为
`ff3c10a089b6a94582451ae05d8a884af3a2bd7485acd4df0496442ea7e0ec55`。

## 区域保持与重规划原则（2026-07-27）

区域保持只能保留来源计划中仍满足硬安全条件的绑定。目标或资源触及 hold 区域时，D3
不允许新增和换绑。来源边已经硬不可行时，提示整体拒绝。继续执行一条已失效承诺会把
安全故障伪装成策略采用，因此该拒绝不能为提高 20-seed 通过率而降低。

重规划请求只要求重新计算。计划是否升版由最终执行签名决定。相同绑定、联盟、未分配库存
和权属的重算保持同一计划身份；请求本身不制造后继。无来源承诺区域的 hold 可以阻止新
目标进入执行分配，该库存变化能够形成严格后继。守恒跨区转移也可通过实际改变绑定形成
后继。

后继必须同时绑定 advisory、来源计划、计划所有者、权属代次和租约。模块夹具已覆盖上述
正负边界，专项 `25 passed`，D3 全量 `551 passed, 1 skipped`（552 项）。main 未落盘
探针的 15/20 只用于策略诊断；五个失败 seed 的原始拒绝原因继续保留，不改写正式 0/20
证据。

## A1 动作裕量原则（2026-07-27）

学习残差进入代价矩阵不等于能够改变离散分配。对目标 \(i\)，先在硬安全候选集合中找到
规则最低成本边 \(b\)，再定义其他候选边 \(j\) 的局部间隔：

\[
g_{ij}=C_{ij}^{rule}-C_{ib}^{rule}
\]

模型记录的单位修正为 \(u_{ij}=\tanh(\Delta C_{ij})\)。候选边相对最低成本边的方向优势为：

\[
d_{ij}=u_{ib}-u_{ij}
\]

只有 \(d_{ij}>0\) 时，增大 `alpha` 才可能使候选边相对更便宜。局部跨越阈值为
\(\alpha_{req}=g_{ij}/d_{ij}\)。由于两个边的单位修正均在 \([-1,1]\)，理论上的最大相对
优势是 \(2\alpha\)。局部间隔能被覆盖仍不保证全局换绑，资源互斥、未分配成本、M-to-N
需求槽、联盟原子性、迟滞和版本约束都会影响最终结果。实际 binding change 必须由原
Hungarian 求解器给出。

校准只能重放已通过源帧分布外、有限性和模型应用检查的记录残差。候选置信门高于已记录
置信度时必须回退规则矩阵；候选修正超过显式绝对上限时不能进入求解。硬拒绝边始终不参与
间隔统计和残差作用，身份未承诺目标、旧计划版本和不完整联盟不能因校准恢复。

冻结重放在校准入口重新校验内容摘要、矩阵形状、非有限值、目标资源清单和动作掩码。
空矩阵、没有硬安全候选动作或源帧已经发生换绑时不继续候选求解。非等量目标/资源矩阵
继续由原 Hungarian 或需求槽 Hungarian 处理，不做方阵补丁或固定规模假设。候选输出以
规则/处理绑定摘要及计划版本证明全局求解结果，局部临界值不直接等同于换绑。

动作裕量输出是开发诊断。即使某个候选 `alpha` 产生不同绑定，它也没有 seed 未见性、
运行确认、物理结果或人工授权。校准报告固定禁止计划发布、分配授权和控制授权，不能直接
改写 bundle 清单或线上默认值。

## A1 批次读取的证据边界（2026-07-28）

A1 隔离批次的写入成功不能代替消费端复核。公共读取器先锁定固定文件布局和校验和清单，
再验证每个 JSON 的精确字段与内容摘要。候选必须与旧批次中的同 seed、同序号帧逐项对应，
包括时间、输入路径、文件摘要、内容摘要、重放摘要和资格摘要。逐 seed 选择必须重新从
候选顺序中找到首个安全离散变化，计数和候选摘要序列必须守恒。

计划版本检查沿用 A1 核心语义。规则计划和处理计划相对前序版本只能保持或增加一次；当
版本合同有效且绑定发生变化时，处理计划必须为前序版本加一。读取器不恢复随机计划号，也
不制造运行计划载荷。稳定投影中的版本关系只用于发现篡改。

输出目录中的 `SHA256SUMS` 负责发现普通文件变化，内容寻址和跨文件关系负责发现攻击者
同步重算校验和后的语义篡改。两层校验缺一不可。在线 truth、Actor、Object 身份键在整个
JSON 树上继续禁用，路径只作为摘要绑定文本处理，不作为读取外部输入的指令。

读取通过只说明候选和选择批次可供离线审计。发布、运行确认、物理窗口、同键规则基线和
正式准入是后续独立证据。读取结果将这些阶段和生产分配、控制权限固定为 false，D6 不能
依据候选数、选择数或读取成功推断后续阶段成立。

## A2 后继的同输入规则基线原则（2026-07-28）

D4 区域建议进入 D3 后，计划严格升版只能证明执行语义发生变化，不能单独证明变化由 A2
造成。当前规划帧可能同时发生目标新增、资源失效、需求变化或普通滚动重规划。A2 归因必须
再提供一个相同输入、相同前序计划、没有 A2 区域提示的 R0 规则计划。

证据分别计算前序、R0 和候选后继的执行签名。R0 与前序不同表示普通周期重规划已经改变
执行语义；该变化只记录为背景。候选后继还必须区别于 R0，A2 证据只覆盖这两个计划之间的
增量。候选与 R0 相同，即使两者都相对前序升版，也不能形成 A2 归因。

区域动作本身也必须可执行。D4 实际模型诊断中的安全投影结果先归一化为区域配额、备用
资源数、hold、重规划请求和跨区转移，再与 D3 实际消费的 `RegionalPlanningHint` 逐项
比较。安全投影前的原始动作、被掩码动作、资源不可行动作和仅有请求但未改变执行签名的
动作不能形成后继证据。

候选身份由 candidate manifest 文件摘要、内容摘要、模型权重和源码身份共同确定。episode、
seed、frame 和输入快照摘要构成同输入比较键。任一身份或版本不一致均失败关闭。在线真值
不进入该记录。身份通过只表示来源可审计，不表示候选适配当前运行特征。

该证据止于离线影子计划结构。运行确认、owner 或 coalition ACK、物理窗口、D7 执行和
收益均是后续独立事实。loader 将这些状态和学习、分配、控制权限固定为 false，不能因
合同夹具能够构造非零开发输出而提前开放。

当前 current-lineage 候选在 5v5、2 区域的 3/3 次预检和 200v200、8 区域的 2/2 次
预检中均触发 `feature_ood`，非回退模型执行为 0。该候选不能进入正式 20-seed。D4 必须
先依据实际运行特征和动作课程生成 clean-lineage、runtime-compatible 的新
development/shadow 候选。D3 完成身份复核后，由 main 先做非正式预检；只有出现非回退
模型执行且安全投影继续通过，才能冻结候选并生成正式 successor 证据。

## 来源承诺与增量跨区原则（2026-07-29）

区域提示绑定来源计划后，跨区资源分为来源计划已经发布的精确 target-resource 基线边
\(B_r\)，以及本轮 transfer allowance 从未承诺资源中选择的新增集合 \(A_r\)。route
\(r\) 上的候选跨区边满足：

\[
X_r \subseteq B_r \cup A_r,\qquad
|X_r \setminus B_r| \leq allowance_r
\]

基线资源只允许原边，不能把“已经跨区”解释为对整个目标区域开放。基线边仍须通过当前
硬安全门；失效时拒绝提示并回退规则规划。新增集合继续受配额守恒、备用资源、route
容量、资源唯一性和 Hungarian 求解约束。该定义只消除来源承诺的重复计数，不降低跨区
安全约束。

连续策略输出只有映射到 D3 可执行语义后才可形成后继。`reconnaissance_priority` 当前
没有区域搜索任务、资源资格、量化死区或成本映射，约 `1e-4` 的浮点差异不进入执行签名。
reserve ratio 是新增转移的安全余量；没有具体备用 roster 时，它也不单独产生计划版本。

## 权限作用域与同身份刷新原则（2026-07-29）

计划身份同时约束资源绑定、联盟关系和执行权限。owner、authority epoch 与 lease 任一变化
都属于执行语义变化，不能留在相同 `plan_id/version` 内。区域后继进入下一评估周期时，
只有完整执行签名不变且原租约仍有效，才允许保留计划身份。

无新提示不代表权限清空。D3 从当前有效后继精确复制既有 owner、epoch、lease 和 assignment
