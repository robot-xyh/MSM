...
> 状态基线：2026-07-31。本文只描述当前仓库已实现行为、已验证证据和明确保留的研究边界，不改变模块能力状态。
...
当前状态是“来源独立机器门通过、未准入”。评价不产生分配计划或运行权限，也不改变默认
规则加匈牙利主线。runtime、assist、assignment、plan、control、physical、formal 和
production 权限全部为 false；D6 独立复核及后续人工决策仍未完成。2026-07-31 模块
全量回归为 `668 passed, 1 skipped, 1 warning`，没有因评价结果改变在线算法或权限。
