...
诊断器只接受固定 v2 结果目录、固定合同、冻结 development bundle、D6 外部审计、main
状态报告、v3 数据请求和 seed 排除注册表。它先重算四个结果文件及 `SHA256SUMS`，再核对
合同、bundle manifest/state/tree 三摘要。任何文件缺失、额外文件、符号链接、摘要变化、
CSV/JSONL 行不一致、非有限值、在线身份键、权限为真或正式 seed 出现均立即停止。

逐帧复算保留原有三种动作：规则基线 (R_0)、投影前候选 (A_c) 和投影后有效动作
(A_e)。教师动作记为 (A_t)。诊断量为：

\[
d_c=|A_c\triangle A_t|,\qquad
d_e=|A_e\triangle A_t|,\qquad
m_c=\mathbb{1}[A_c=A_t],\qquad
m_e=\mathbb{1}[A_e=A_t].
\]

分层维度包括 train/validation/test、场景、配置规模、正负类、候选可用状态、OOD、拒绝
原因、匿名目标和资源数量、目标资源基数关系、需求槽总数、教师换绑数量以及候选教师差异。
每一层只汇总冻结逐帧记录，不重新求解或调用模型。

### 排他路径与不可用项

对教师正类且 (m_e=0) 的帧，排他可观测路径按以下顺序生成：

1. `feature_ood_rule_fallback`：记录明确标记 OOD，模型残差被置零并回退规则路径；
2. `candidate_selection_mismatch_non_ood`：非 OOD，且投影前候选已不等于教师；
3. `projection_rejected_teacher_exact_candidate`：投影前精确匹配教师，但投影拒绝；
4. `other_observable_or_unavailable`：现有字段仍不能说明。

test 正类得到 9、16、0、0 帧。投影拒绝与前两类可以共现，本批为 22 帧；共现计数不进入
排他分母。由于 v2 不保存候选边列表、逐边残差排名和需求槽映射，第二类只能证明“候选结果
不一致”，不能继续拆成候选不可达、模型排序错误或动作槽结构差异。这三项均写
`available=false` 和原因，不用零值代替缺失证据。

### v3 请求合同

v3 request 使用独立 schema，定义 15 个场景规模单元和 300 个 episode。每个 cell 请求
20 个 episode、至少 60 个正类帧、60 个负类帧和 30 个困难负类帧，总最低值为
900/900/450。动作变化覆盖保持 R0、单目标释放重绑、双目标交换、多目标环换、目标增删、
资源失效恢复、M-to-N 需求升降和主备角色变化。

未来帧必须增加匿名候选边可达性、教师边是否位于动作掩码、逐边残差排序、动作掩码摘要、
匿名需求槽结构和投影前后原因。seed registry 当前只列排除集合和请求数量，
`assigned_seed_values=[]`。main 必须先绑定所有 D3 已登记 seed 的规范快照，再分配 300 个
全新 seed；缺少快照、存在重叠或跨 split 复用时停止。该合同没有生成、训练、选模、阈值、
运行或控制权限。
...
评价前状态为 `evaluator_v2_ready_evaluation_not_run`。2026-07-31 已完成唯一一次官方
执行，当前状态为 `source_independent_evaluation_v2_gate_passed_not_admitted`。

### 实际评价结果

评价读取 `20000-20099` 共 100 个 seed、100 个 episode 和 292 帧；train、validation、
test 来源子组分别为 178、57、57 帧。三个标签仅用于分组，不参与训练、模型选择、
归一化重拟合或阈值调整。数据帧 SHA-256 为
`1568a69e8d93d3357c6cf53f4a416f5083b7cf1f90a33fa0ccc0d0a7ed47d972`。

预注册机器门的实际值如下：

- 正类安全换绑 `13/110=11.82%`，通过至少 1 帧且不低于 5% 的门限；
- 正类教师完全匹配 `8/110=7.27%`，通过至少 1 帧且不低于 2% 的门限；
- 负类 exact-R0 `182/182=100%`，通过不低于 99% 的门限；
- 94 个失败关闭帧的矩阵和绑定均 exact-R0；
- 重复资源、硬禁边、M-to-N 原子性、版本、规则矩阵突变、模型 assignment/plan/runtime
  输出均为 0；
- 在线真值使用、训练 seed 重叠、正式 seed 重叠和正式 seed 读取均为 0。

27 帧被标记为特征分布外。94 个投影拒绝帧中，拒绝原因可叠加：绑定变化上限 65 帧、
规则成本差上限 53 帧、相对规则成本差上限 6 帧、特征分布外 27 帧。该分布说明安全外壳
实际参与了候选裁剪，不能把候选矩阵变化直接视为可执行计划变化。

结果目录包含固定 JSONL、CSV、聚合 JSON、中文报告和校验清单，全部 SHA-256 校验通过。
机器门通过只证明冻结候选在该来源集合上满足预注册离线条件。D6 后续已独立确认结果
完整性和总体机器门，但正式保留集、运行 ACK、物理窗口和生产准入仍不在结论内。
2026-08-01 D3 全量回归为 `677 passed, 1 skipped, 1 warning`，冻结评价与只读诊断入口
语法检查通过。
...
main 使用 clean commit `8f86192` 运行 200v200、10 秒、seed 42000-42002。每组 episode
执行 10 次 `module.d3_assignment`，发布 10 份计划并收到 10 次计划 ACK。三组均满足
`repository_dirty=false`、有限状态和在线 truth 使用计数为 0。

### 55.2 累计计时

三组 D3 assignment 累计墙钟为 `3.437013/3.318859/3.110420 s`，均值
`3.288764 s`。旧 clean commit `3bac3ff` 对应值为
`3.434595/3.427859/3.180729 s`，均值 `3.347728 s`。均值变化约 `-1.8%`，且 seed
42000 略有增加，说明差异不能归因于固定算法边界。

### 55.3 语义核对

新旧两次复跑的 D3 调用数和计划 ACK 均逐 seed 相等。binding ACK、control applied 和
hold 摘要分别保持为 `1983/1983/199`、`1977/1977/0`、`1980/1980/400`。该结果确认 D1
快照优化没有改变 D3 计划发布、绑定应用或保持决策。D3 规则代价、Hungarian、需求槽、迟滞、
版本和跨模块 binding 没有因本次集成复跑调整。
