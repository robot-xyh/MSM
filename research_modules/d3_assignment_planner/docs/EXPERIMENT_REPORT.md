...
实时上限，也不能单独解释 clean 10 秒 seed 42000-42002 的累计时间变化。main 仍需在隔离
clean worktree 复测同一 episode 的调用密度、输入规模、累计阶段墙钟、计划 ACK、输出字节
和内存。D3 全量中的两项 `global_track_stale` 失败属于既有 main/D7 时序链路，本试验没有
放宽 stale 门控。
