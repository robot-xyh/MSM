...
`20000-20099` 共 100 个 seed、100 个 episode 和 292 帧，覆盖 5、20、50、100、200
五档规模与 10 个场景单元。来源子组帧数为 train 178、validation 57、test 57；三组均
不用于训练、选模或调参。正式 seed `1000-1019` 的读取数为 0。
...
机器门整体为 true。94 个投影拒绝帧的矩阵和绑定均与 R0 完全一致。重复资源、硬禁边、
M-to-N 原子性、版本、规则矩阵突变，以及模型直接输出 assignment、plan、runtime 的计数
均为 0。分布外帧为 27。拒绝原因允许一帧多项，计数为绑定变化上限 65、规则成本差上限
53、相对规则成本差上限 6、特征分布外 27。

在线真值字段数和生成在线真值使用数均为 0；100 个来源 seed 与训练 seed、正式保留 seed
均无重叠。五个固定结果文件齐全，`SHA256SUMS` 全部通过。主要文件摘要为：

| 文件 | SHA-256 |
| --- | --- |
| `aggregate.json` | `9d44819c50fe7a7b4aa96f1ac99b665235a8e7e7fba0e84b3fbe6ef282a6e3b1` |
| `per_frame_evaluation.jsonl` | `88377f35a874423af3fa55b9b183ec5b8a883f85f4929ff12a66f96d07a2076b` |
| `per_frame_evaluation.csv` | `a84bda095cfcecd0d6b6c5649600dbdbbc91b5a4b82ca4166b272337834bf9c2` |
| 中文评价报告 | `4934c616ce91ac2f9462c5b8718187a7c7781e06b10bd848bdd24748f01b470a` |
...
v1 先完成只读预检。合同 SHA、源码树 SHA、bundle manifest/state/tree 三摘要、100 个
生成单元、292 帧、178/57/57 分组、在线真值使用 0 和有限状态均通过。官方命令只运行
一次，在逐帧输入检查处返回：
...
结果目录未创建，`SHA256SUMS` 和聚合指标不存在。v1 最终状态为
`FAIL_CLOSED / EVALUATION_NOT_COMPLETED`。该状态不是模型机器门未通过，不能据此填写正类
安全换绑、教师精确匹配、负类 exact-R0、分布外或拒绝统计。

main 的后续纯结构统计表明，匿名资源数始终等于配置资源数。匿名目标是在线 D1/D2 航迹，
数量随观测变化。`delayed_noisy-200v200` 为 176 至 207，
`evasive_multilevel-100v100` 为 96 至 103，`nominal-5v5` 为 4 至 5。规则成本矩阵行数
与匿名目标数一致，列数与匿名资源数一致。v1 的配置目标数等式不符合该数据语义。

v2 只修正这条等式。配置目标数改为 `configured_scenario_target_count`，匿名航迹数允许
低于、等于或高于配置值。资源数、帧内矩阵和候选形状、有限值、匿名身份、硬安全边、
多机需求完整性、R0 恢复、门限和权限均保持原要求。v1 文件和失败记录未修改。

软件验证结果：

- v2 新增结构测试：`9 passed`；
- v1/v2 评价器专项：`26 passed`；
- D3 全量：`649 passed, 1 skipped`，跳过项为可选 OR-Tools；
- 既有 Matplotlib `Axes3D` 告警不影响本项。
...
本轮没有运行 v2 评价，没有读取正式 seed `1000-1019`，没有修改模型、归一化、教师、
安全投影、门限或权限。

## 来源独立评价工具验证（2026-07-30，历史）

本轮没有运行来源独立数据，也没有生成模型性能结果。工作内容是冻结评价合同、只读加载器、
流式数据校验、单帧安全投影、聚合机器门和结果写盘测试。

冻结候选摘要如下：

| 对象 | SHA-256 |
| --- | --- |
| bundle manifest | `ec9f93d668e1aa319f65fcda0d73adb0527f316a2d1880e93e88697b6468ad3d` |
| state-dict | `c185823bd9a4cf5363d17854385aeb74c340c8ac384327281d224a1097eb8206` |
| bundle tree | `de7b627df9782d7d2577687f30d02d4faeeaf577ecc557c2b8d91dd6e7115dd9` |

专项测试共 17 项，覆盖合同加载、冻结 bundle 只读加载、权重篡改、期望摘要错误、seed
重叠、权限越权、生成真值使用、来源摘要、流式数据读取、test 子组、结果文件与校验和、
机器门正例、两类空分母、失败关闭不一致、重复结果目录、非法模式和源码篡改。专项结果为
`17 passed`。D3 全量收集 641 项，结果为 `640 passed, 1 skipped`；跳过项是可选
OR-Tools。既有 Matplotlib `Axes3D` 环境告警不影响本评价器。

该结果只证明评价软件能按预注册口径失败关闭。新来源 `20000-20099` 尚未送入评价器，
因此正类安全换绑、教师完全匹配、负类 exact-R0、OOD 和拒绝分布均没有实际数值。正式
seed `1000-1019` 仍为 `not_read_not_evaluated`。模型权重、特征、归一化和阈值未改变；
运行、分配、计划、控制、物理和正式准入权限全部为 false。
...
物理结果和同键 R0 非退化审计能力，但当前没有实际 D6 A1 formal-scope 审计输出，且该
审计器明确不授予
...
使用 main 100-cell 预检的相同场景、规模、seed 和 2 秒时长，在内存中复现 D3 发布并
比较同身份载荷。
...
| 成员、角色或联盟变化 | 0/48 |
| owner、epoch 或 lease 变化 | 0/48 |
| 完整载荷摘要相同 | 0/48 |
| assignment 仅顺序变化 | 33/48 |
| D4 摘要错配 | 90 |
| D4 交叉绑定拒绝 | 90 |

重复组按规模为 5v5 20 个、20v20 17 个、50v50 8 个、100v100 2 个、200v200
1 个。32 组第二轮为联盟成员迟滞保持，16 组为同分配 unchanged。

200v200 seed1017 的两轮时刻为 0.95 和 1.00 秒，均为版本 1，198 条执行绑定集合
相同。完整载荷有 990 个叶级差异，其中 metadata 977 个；D4 分别记录 37 次摘要错配
和交叉绑定拒绝。

D3 已增加权威发布判定及合同测试。上述表格记录 v3 修复前证据，不能代表当前 main
runtime 状态。

main 接入合同后运行 v4 开发态批次：
...
100v100 seed1010、200v200 seed1013 和 seed1017 的原失败均未复现。v4 证明 main
按 D3 身份语义完成了开发态接线，P0 在开发态关闭。该批次使用未冻结工作树、2 秒三维
质点 episode 和 `/dev/shm` 临时输出，尚未形成 clean commit、正式结果清单或完整正式
R0，因此不能写成正式 R0、AirSim 或物理拦截验收。专项完整判断见
`../reports/D3_PLAN_IDENTITY_PAYLOAD_AUDIT_20260730_CN.md`。

main 接线后，两个 D3 集成测试的旧假设不再成立：旧测试以最新评估对象校验首次权威
载荷，并期待同身份刷新生成第二个运行 ACK。测试已改为按首次权威载荷校验，并断言刷新
