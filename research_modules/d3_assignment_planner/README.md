...
Main/runtime has connected D5 feedback writeback, center replan owner/version/source recording, secondary owner/version/source recording, and the P1 D4/D5 calibration sweep. The sweep now produces a D6 standard calibration report bundle, so D3 provides replay/calibration summaries and current-plan evidence for those episode-level records; D3 consumes that bundle through exported plan/record/evidence helpers and does not own D6 storage. Remaining D3 work is to feed real multi-seed episodes across N scales into those helpers, tune D5 feedback weights/thresholds with D6 assignment records, calibrate hard-window inputs/reject reasons, and calibrate the full P1 dynamic threat model beyond the P0-C baseline.
