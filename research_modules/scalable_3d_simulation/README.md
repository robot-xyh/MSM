...
帧和 `300` 个唯一 seed；D4 已最终封装 `324/324`。两者的训练、正式留出消费、运行、控制
和身份写入计数均为 0。D3 与 D4 的已封装目录保持只读，不因 D5 修复重复生成。

D5 首轮运行在序列 97、`seed 24097` 失败关闭。失败前已持久化 97 个 episode descriptor，
其中 development 72 个、future-held-out 25 个。失败原因是
`hold_vs_observe_gimbal_busy_boundary` 没有形成合格边界对。该目录永久保持
`failed_closed`，不得恢复、迁移或作为正式来源。

开发集同构条目 `seed 24007` 显示，旧处理在 observe/hold 两个窗口间只有 1 个共同中心
航迹，边界能否形成依赖侦察提示的偶然重叠。main 现仅在带冻结采集窗口的 profile 中保持
“相机到当前 D3 计划内中心航迹”的提示绑定；航迹退出当前计划或当前航迹集后立即重选，
