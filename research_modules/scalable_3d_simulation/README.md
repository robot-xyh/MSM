...
## 2026-08-03 正式来源生成失败关闭处理

main 在 clean commit `e7c438c` 上获得仅允许 D3、D4、D5 写入来源数据的授权。D3 已完成
`300/300` 并最终封装；共 `3086` 帧、`300` 个唯一 seed，正式留出读取、训练、运行和控制
计数均为 0。D4 在 `seed 28203` 暴露在线航迹假设数可能超过配置物理目标数的问题。旧验证
错误地把 `scenario.task_count` 当作在线任务容量；D4 owner 已改为仅把该字段用于来源和
报告，同时继续强制任务编号、中心航迹编号和区域引用唯一有效。

D5 在 `seed 24013` 暴露跨相机角色边界签名问题。原 main 适配器把计划版本和联盟版本写入
通信等价哈希，不同时间窗即使通信健康状态相同也无法配对。适配器现只用版本化 schema 和
真实通信健康状态计算该等价签名；计划版本和联盟版本仍保留在在线特征及原有安全门控中。
原失败 seed 回放得到 `69` 个主动视觉帧和 `3586` 个合格样本，两类要求边界均形成，在线
真值使用为 0。

D4、D5 的旧输出均保留 `failed_closed` 记录，禁止恢复或作为正式来源。修复后必须形成新
...
`3/3` 完整、生产器适配 `3/3` 完整、来源生成请求 `3/3` 就绪。共享工作区尚有待提交变更，
因此状态为 `blocked_by_dirty_generation_worktree`；唯一 blocker 是
`generation_worktree_dirty`。main 执行授权、正式来源生成、训练、验证消费、运行和控制仍为
`false`。

| 模块 | 当前软件证据 | 来源请求 SHA-256 | 正式来源 |
| --- | --- | --- | --- |
| D3 A1 v3 | 300/300 dirty 开发探针通过；逐 episode 固定要求 `9/3/3/2`，实际最低 `9/3/3/3` | `b5685b61acff9f0f1bde504ecc27d17621e0161c8adad95d1789e4d46b74c42f` | 0/300 |
| D4 A2 v8 | 324/324 配方、972 帧通过；sequence 0/1 跨进程恢复通过 | `e93e7a79a3bfae055721fc21d9ba1591228c3d46f662922de2c04713076fe808` | 0/324 |
| D5 A3 v3 | 104/104 配方通过；5 个探针 693 帧；双调用完成数 `1 -> 2` | `707fcb6233f393cbe35cb5b4f6ad264682918d3435476afdfc8f94ec4dc644bf` | 0/104 |

D3 的动态增删、匿名资源事件、真实规则成本 near-tie 和稳定量测窗口均由当前三维状态机
实际调用。15 个 cell 的 300 个 episode 全部通过；失败、探针错误、在线真值使用、身份
创建或改写、重复帧、正式 seed 读取和 R0 shard 10-19 读取均为 0。该结果状态为
`exploratory_dirty_pass`，`readiness_eligible=false`，不能替代正式来源生成。D4 的区域
拓扑、通信、供需边界和 writer 恢复合同已覆盖全部 324 个 TRAIN
配方。D5 的四类意图、两类相机角色、五类困难混淆和 future-held-out 隔离合同已覆盖全部
104 个配方。三模块均未使用在线真实编号，D5 未创建或改写 `global_track_id`。

本轮更新后的 main 来源适配、生成和预检专项为 `24 passed`。上一轮专项回归为 main
前置链 `25 passed`、三维世界/传感器/配方链 `89 passed`，scalable
全量为 `509 passed`；D3、D4、D5 全量分别通过，D4 为 `1013 passed`，D5 为
`877 passed`。全量回归同时修复了旧 generation plan 中元组写入 JSON 后变为列表所造成的
resume 误报；修复只规范化计划表示，不改变 episode 内容。下一步是在分批提交后的干净
commit 重新生成预检，再签发仅允许数据集写入的 main 授权。正式来源按 D3、D4、D5 顺序
限量续跑；训练、验证读取和运行准入必须另行授权。

## 2026-08-01 阶段收口（历史阶段）
