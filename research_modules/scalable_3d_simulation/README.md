...
20 个分片中的 shard 0 和 1 均完成 45/45，覆盖 seed 1000/1001、9 类场景和五档规模。

90 个单元均为有限状态，在线真值使用为 0。D6 离线复核的基础正式准入、矩阵正式准入和
D1-D2 代次合同均为 90/90；D4 过时建议和无效建议为 0。完整 ID Switch 指标为
0/90 available，原因均为生产者显式声明不可用，未补零。100 对 100 和 200 对 200 的
累计平均实时倍率分别为 0.463 和 0.194。

两个分片原始大小合计 2,988,529,684 字节，确定性压缩后合计 290,018,789 字节，占比
9.70%。压缩包和源目录双端复核通过并同时保留。当前总进度为 90/900；继续长期逐片执行
前仍需取得移除已验证原始分片的明确授权。累计阶段结果见
[`docs/SCALABLE_3D_FORMAL_R0_80E55EB_SHARDS0_1_20260731_CN.md`](docs/SCALABLE_3D_FORMAL_R0_80E55EB_SHARDS0_1_20260731_CN.md)。
