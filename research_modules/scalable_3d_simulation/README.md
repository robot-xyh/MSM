...
| --- | --- | --- | --- |
| D3 A1 v3 | 15/15 首配方满足 `3/3/2` 并实际 stage；在线真值使用 0 | `11008b9240bf0219aa14c5fb724332d0061469ab85e048721af5f349804b68a6` | 0/300 |
| D4 A2 v8 | 324/324 配方、972 帧通过；sequence 0/1 跨进程恢复通过 | `18b595057197dda06b8b2a1ec2a357f1f4d652d2512752be83db2f1e979df1e2` | 0/324 |
| D5 A3 v3 | 104/104 配方通过；5 个探针 693 帧；双调用完成数 `1 -> 2` | `157166b8188ded72d7b317161242b904914f083686cdaf76cd4f3a92c94f80b3` | 0/104 |

D3 的动态增删、匿名资源事件、真实规则成本 near-tie 和稳定量测窗口均由当前三维状态机
实际调用。D4 的区域拓扑、通信、供需边界和 writer 恢复合同已覆盖全部 324 个 TRAIN
配方。D5 的四类意图、两类相机角色、五类困难混淆和 future-held-out 隔离合同已覆盖全部
104 个配方。三模块均未使用在线真实编号，D5 未创建或改写 `global_track_id`。

专项回归为 main 前置链 `25 passed`、三维世界/传感器/配方链 `89 passed`，scalable
