...
20 个分片中的 shard 0 已完成 45/45，覆盖 seed 1000、9 类场景和五档规模。

45 个单元均为有限状态，在线真值使用为 0。D6 离线复核的基础正式准入、矩阵正式准入和
D1-D2 代次合同均为 45/45；D4 过时建议和无效建议为 0。完整 ID Switch 指标为
0/45 available，原因均为生产者显式声明不可用，未补零。100 对 100 和 200 对 200 的
平均实时倍率分别为 0.458 和 0.196。

分片原始大小 1,503,424,928 字节，确定性压缩后为 144,187,839 字节，占比 9.59%。
压缩包和源目录双端复核通过并同时保留。当前总进度为 45/900；继续逐片执行前需取得
移除已验证原始分片的明确授权。阶段结果见
[`docs/SCALABLE_3D_FORMAL_R0_80E55EB_SHARD0_20260731_CN.md`](docs/SCALABLE_3D_FORMAL_R0_80E55EB_SHARD0_20260731_CN.md)。
