...
2026-07-20 的 main 集成回归为 **36/36 passed**。其中 5v5、seed 7、1.2 秒场景形成
...
当前尚未完成六维速度估计统计校准、长时多 seed 或 200-camera 图构建。D5 默认使用几何
规则回退；现有图网络只有训练管线 smoke，没有独立验证 checkpoint。D6 规模评估、D5
模型验收和 D3/区域/主动视觉学习策略验收仍是后续工作。上表是稀疏分配优化前的历史短测，
不能代表当前代码的最终性能；D1/D2 校准完成后将按相同 seed 和时长重新生成规模曲线。
