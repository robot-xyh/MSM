...
本轮关闭了四个模块级软件缺口：D3 隔离批次缺少公共严格加载器，D4 无法构建并复核当前
实现谱系候选，D5 行为克隆缺少训练语料结构门，D6 无法从原始制品重算 G1 模型来源。
D4 当前谱系实物已在 clean checkout 生成；这些能力使候选生成、加载和审计边界更清楚，
但没有形成新的运行授权。
...
升级均失败关闭。共享工作区曾按预期返回 `source_worktree_dirty`，没有绕过检查。main
提交后，D4 从独立 clean checkout `b0d498d9...` 完成构建和 `review-only`。

候选 manifest 文件 SHA-256 为 `7cc10ad7...de64`，权重为
`fd1b9c4c...0047`，数据集、split 和 source identity 分别为
`7e17aba7...2d7f0`、`b413fa81...0c16` 和 `b81780ce...dfdf`。train 包含 60 个 seed、
180 个样本，validation 包含 20 个 seed、60 个样本。固定门限开发诊断得到 train
168/180、validation 54/60 个安全非零实际模型动作；其余样本与基线相同，资源不可行、
门控回退、身份错配和非有限输出均为 0。训练和验证属于已见开发分布。A2 admission、
assist、authority、assignment、takeover、coalition commit、control、actual adoption
和 benefit 全部为 false。
...
1. 冻结 D4 当前候选身份，在至少 20 个严格未见 seed 上形成非零实际模型干预、D3
   严格后继计划、runtime/owner/coalition ACK、确认后物理窗口和独立同键 R0。
...
5. 将 G1 原始证据树归档到持久只读位置。当前约 32 GiB 可用空间已高于 20 GiB 启动
   保护线，但仍需逐分片核验预计输出容量。
6. 正式未见种子、模型权限、运行确认和成对非退化条件满足后再启动 900-cell 和完整
   多 seed；不得仅因磁盘恢复而启动，保护线、身份门和权限门不降低。
