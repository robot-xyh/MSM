...
离线三态标签和相机几何治理完成后，严格身份阻断收敛到雷达扫描间多真值谱系。main 对
baseline `488dc39` 与 D1 v1 candidate `d967c96` 运行了同配置 clean A/B。两端均为
200 对 200、2.2 秒、`recon_count=2`、seeds 1000/1001/1002，逐 seed 配置哈希相同。

| seed | ambiguous | strict identity | D2 航迹 | D3 分配 | 雷达抑制 |
| ---: | ---: | --- | ---: | ---: | ---: |
| 1000 | `2 -> 0` | unavailable -> available，候选 IDSW 3 | `201 -> 200` | `200 -> 198` | `22/1962 = 1.12%` |
| 1001 | `0 -> 0` | IDSW `9 -> 7`，continuity `0.869444 -> 0.814444` | `202 -> 194` | `200 -> 190` | `130/1966 = 6.61%` |
| 1002 | `2 -> 0` | unavailable -> available，候选 IDSW 4 | `200 -> 197` | `200 -> 193` | `78/1958 = 3.98%` |

候选把严格身份指标可用 seed 从 `1/3` 增至 `3/3`，但三个 seed 的 D2 航迹和 D3 分配均
下降，seed 1001 continuity 下降约 `0.055`。三组状态有限、来源干净、在线真值使用为 0，
目标与已知虚警标签数相同。该结果不满足“身份改善不得以航迹和下游可用性退化换取”的晋级
条件，v1 被拒绝。

提交 `8f17c5d` 将候选设为默认关闭。同配置三 seed 的业务指标全部恢复 baseline，跨构建审计
`3/3 passed=True` 且 `normalized_online_payloads_equal=True`。早先遗漏
`--recon-count 2` 的 8 侦察机结果只作 stress 诊断。该诊断中的 `200x199` 门内图存在
2 个 free row、1 个 free column 和可保持匹配基数的替代路径，说明 v1 只检查已匹配行强连通
分量的边界不完整；三维雷达量测只有距离、方位和俯仰，零径向速度是未观测占位。

严格身份 P1 未关闭。下一候选需覆盖最大匹配 allowed-edge 图中的交替环、free-row 和
free-column 路径，并联合验收身份可用性、D1/D2 航迹、D3 分配、连续性、抑制、birth 与
recall。被拒绝 v1 不再运行 10 秒或 20-seed。机器证据见
`SCALABLE_3D_RADAR_ASSIGNMENT_CANDIDATE_REVIEW_20260723.json`。
...
当前优化已在 detached clean 提交
`5263e2b343dc4b96d239f77ef09437eb132f9efb` 上完成 seed `1000-1019` 的
nominal 200 对 200、10 秒规则全栈顺序运行。每组固定
`OPENBLAS_NUM_THREADS=1` 和 `OMP_NUM_THREADS=1`。20/20 episode 状态有限，
在线真值使用总数为 0，D1-D2 后验代次审计通过，进程资源记录齐全。候选原始制品和
D6 输出约 9.0 GiB，不提交仓库。

性能参考为已有 detached clean `0d2da25` 同场景、同 seed 的 20 个 episode。当前
`5263e2b` 与参考逐 seed 执行直接跨构建审计。20/20 的规范在线载荷、真值状态与标签、
D3 计划谱系、D4 内容地址和运行确认均相同。该比较允许评价实现性能变化，不评价算法效果。

| 指标 | `0d2da25` 均值 | `5263e2b` 均值 | 配对变化 | 95% seed bootstrap 区间 | 改善 seed |
| --- | ---: | ---: | ---: | ---: | ---: |
| 核心墙钟/s | 96.391 | 86.099 | -10.63% | [-11.71%, -9.61%] | 20/20 |
| 实时倍率 | 0.1039 | 0.1163 | +11.98% | [+10.68%, +13.38%] | 20/20 |
| module stack/s | 84.903 | 74.952 | -11.67% | [-12.71%, -10.68%] | 20/20 |
| 在线日志/MiB | 212.645 | 212.645 | 0 | [0, 0] | 0/20 |

候选核心墙钟的 P50/P95/范围为 `86.063/89.648/79.761-91.152 s`。实时倍率
P50/P95/范围为 `0.1162/0.1227/0.1097-0.1254`。当前结果仍低于 1.0 实时目标，
平均吞吐差距约为 8.6 倍。20 个候选进程的外部总耗时均值/范围为
`118.749/109.38-125.58 s`，峰值驻留内存均值约 2.40 GiB、范围约
2.08-2.60 GiB。旧 20-seed 参考没有外部进程资源文件，因此不做 20-seed 峰值内存配对。
可比较的 `4ac3bb2 -> 5263e2b` 三 seed 外部总耗时平均下降 3.66%，峰值内存变化
+0.04%。候选结束后处理均值/范围为 `30.150/27.173-32.275 s`，其中运行计划结果联接、
离线身份、在线总线身份视图和离线一致性均值分别为
`9.300/8.865/7.539/3.304 s`。

| 阶段 | 参考均值/s | 候选均值/s | 配对变化 | 95% seed bootstrap 区间 | 候选 P50/P95/ms |
| --- | ---: | ---: | ---: | ---: | ---: |
| D1 扫描输入 | 12.418 | 9.671 | -22.06% | [-23.36%, -20.73%] | 1.443/135.454 |
| D1 融合 | 51.649 | 43.774 | -15.15% | [-16.61%, -13.76%] | 34.474/233.488 |
| D2 关联 | 5.492 | 5.139 | -6.41% | [-7.79%, -5.04%] | 113.603/142.627 |
| D3 分配 | 2.448 | 2.499 | +2.46% | [-1.02%, +6.13%] | 230.755/348.911 |
| D5 主动视觉 | 3.107 | 3.182 | +2.69% | [-0.18%, +5.46%] | 32.890/37.574 |
| D5 终端配准 | 1.185 | 1.163 | -1.84% | [-5.28%, +1.81%] | 10.739/15.895 |
| D7 导引 | 3.638 | 3.859 | +6.24% | [+2.96%, +9.80%] | 19.572/22.520 |
| main publication bus | 4.439 | 4.633 | +4.44% | [+2.20%, +6.72%] | 5.404/61.566 |

D1 扫描输入、D1 融合和 D2 关联在 20/20 seed 上均下降，证明三项局部优化已转化为
全栈收益。D1 两阶段仍占候选核心墙钟约 62%，是下一轮主瓶颈。D3 分配、D5 主动视觉和
D5 终端配准的变化区间跨过零，暂不判定稳定变化。D7 与 publication bus 的增长区间高于
零；两者业务输出与参考相同，下一轮应从 DTO 物化、哈希和日志成本归因，不能修改控制公式
或降低发布频率来换取墙钟。

D6 使用 5,000 次固定随机数 bootstrap 聚合 20 个不同 seed。20/20 clean provenance、
schema 和 generation integrity 通过，failure reason 为空；D1/D2 航迹数均值为
`203.45/203.50`，D3 计划覆盖率均值为 `0.989606`，95% 区间为
`[0.987150, 0.991817]`。这些航迹数量包含重复或虚假航迹可能性，不能直接解释为 200 个
目标全部稳定识别。

严格 `id_switch_count` 在 20/20 episode 上仍为 unavailable，原因是 producer 未形成
可唯一解释的 `global_track_id -> truth_target_id` 全时序映射。D6 不从航迹数量、名称、
距离或部分下界补算严格值。

D6 子模块使用现有公共 API 对每个 producer 制品重新计算来源与输出哈希，并重新构建
episode record。三层 manifest、重建记录一致性和在线真值隔离均为 20/20。部分身份证据
在 20/20 可用：映射覆盖为 `178531/181110=98.5760%`，完整帧覆盖为
`103/959=10.7404%`，相邻转换覆盖为 `1149/187800=0.6118%`。保守 ID Switch 下界在
19 个 episode 可用，合计为 199/15,215 个锚点区间；另一个 episode 因没有可评估身份转换
保持不可用。该下界没有回填 strict ID Switch，没有生成上界，也没有进入控制链路。

D1 的 20 个 episode 均为 partial。创新平方、归一化创新平方和创新门限覆盖率均值为
`3.385237/1.146517/0.991315`；位置均方根误差、估计误差平方和归一化估计误差平方因
`d2_lineage_mapping_missing` 保持 unavailable。严格身份映射缺口因此同时影响 D2 身份
评价和 D1 部分真值精度指标。

学习 bundle 均未加载，正式实验矩阵 episode 数为 0，10 秒
场景没有五米接近事件。因此本轮关闭当前实现的 20-seed 规则性能与语义等价校准，不关闭
实时、严格身份、学习算法比较或物理拦截验收。

机器摘要为
`SCALABLE_3D_20SEED_PERFORMANCE_CALIBRATION_20260723.json`。候选核心制品清单、
参考核心制品清单、20 个跨构建审计清单和 D6 aggregate 的 SHA-256 分别为
`6207841d...08e4`、`8479cdee...08da`、`dc3908a3...e3b1` 和
...
4. D5 已关闭 history gauge、匿名审计和 singleton binding 的局部重复成本。下一步正交
   ...
   不能减少视觉帧或放宽几何门限。
5. D7 和 publication bus 出现小幅稳定增长。下一轮先以固定输入 profiler 区分业务计算、
   DTO 物化、哈希和日志成本，PNG/PN 控制公式与安全门控保持不变。
6. D3 冻结输入归因已经建立。下一轮优先处理 D1/D2/D7 和写出边界，不因墙钟噪声修改规则
   代价、迟滞或匈牙利主线。
7. 规则全栈达到可接受吞吐且严格身份可评估后，再运行 D4 故障、D5 跨视角压力和 D7 五米
   物理闭环，不将本轮
   性能校准写成拦截效果验收。
8. 同一 clean `0d2da25` 参考和 `5263e2b` 候选的 seed `1000-1019` 长时规则批次均已完成。
   下一轮不重复运行相同 R0 nominal 校准；应先关闭吞吐与严格身份热点，再由正式矩阵 runner
   运行有冻结 metadata 和学习 bundle 的配对单元。保留 seed 继续禁止回流训练。
...
- 第九轮候选：`outputs/scalable_3d_optimized_clean_5263e2b_20260723/`
- 第九轮 D6：`outputs/scalable_3d_optimized_clean_5263e2b_20260723/d6_aggregate_10p0s_20seed/`
