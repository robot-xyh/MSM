...
2026 年 7 月 24 日完成 D1 扫描输入同提交正式矩阵。`candidate_v2` 在 short 10 seed 中
9 个更快，扫描输入累计墙钟平均下降 `5.3601%`；long 3 seed 全部更快，平均下降
`5.1425%`。业务语义、有限状态、在线真值隔离、实现身份、核心墙钟和内存门全部通过。
候选最低实时因子仍为 `0.1434`，因此只关闭扫描输入数据组织的准入项，不关闭 200 对 200
实时容量。详细结果见 `SCALABLE_3D_D1_SCAN_INPUT_MULTISEED_REVIEW_CN.md`。

2026 年 7 月 25 日完成 D1 关联稀疏预筛同提交正式矩阵。候选将非雷达精确求解减少
`86.636767%`，13/13 对业务语义和逐模态精确门内计数保持一致，但 short D1 fusion 仅改善
`0.228437%`，short core 仅改善 `0.091096%`，候选只在 `7/10` 个 short seed 更快；
long D1 fusion 改善 `0.713776%`。五个预注册性能门失败，D6 判定 `reject`，默认继续
`disabled_v1`。候选最低实时因子为 `0.206273`，因此该局部优化也没有关闭 200 对 200
实时容量。后续先分别画像逐 pair 下界计算、EO fail-open 和 D1 其余阶段，再决定是否转向
更粗粒度候选生成；冻结目标处理器上的长时容量仍需单独验证，不能降低本轮门限。
