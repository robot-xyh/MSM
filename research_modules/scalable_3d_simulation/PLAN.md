...
1. [x] 在 generation-only 授权下完成 D3 `300/300`，最终清单记录 `3086` 帧和 `300`
   个唯一 seed；正式留出读取、训练、运行、控制和身份写入计数均为 0。
2. [x] 完成 D4 `324/324` 并最终封装。物理目标数仅作为来源和报告元数据；在线任务编号、
   中心航迹编号、版本、epoch、lease 和联盟门控继续严格校验。训练、运行、降级和控制
   权限均为 0。
3. [x] 修复 D5 `seed 24013` 的通信等价签名问题。计划版本和联盟版本仍保留在在线特征及
   安全门控中，不再作为跨时间窗通信健康等价条件。
4. [x] D5 在序列 97、`seed 24097` 因 observe/hold 窗口没有共同几何而失败关闭。旧目录
   已持久化 97 个 descriptor，但永久禁止恢复、迁移和正式使用。
5. [x] main 仅为冻结采集窗口增加 truth-free 侦察提示保持。提示必须属于当前 D3 计划和
   当前中心航迹；目标失效即重选，operational profile 不缓存。开发集同构验证的共同航迹
   `1 -> 4`、有效边界候选 `240 -> 960`、在线真值使用 0。D5 owner 确认模块合同无需修改。
...
10. [x] 在独立开发流程中构建行为克隆缓存，并执行一次固定配置训练及逐动作、逐角色、
    特征边界分布外和校准评价。模型因 `observe_target`、`search_sector` 召回均为 0、
    宏召回不足和校准误差过高而失败关闭。真正场景分布外、同键 A3/R0、AirSim、真实相机
    和物理动作结果仍未形成，模型不进入辅助或控制路径。
...
2. [x] shard 0-9 均完成 45/45，覆盖 seed 1000-1009、9 类场景和
   5/20/50/100/200 五档；checkpoint 完成、孤立单元恢复 0、在线真值使用 0。
3. [x] D6 v12 修复严格身份指标接线，并对 450 个既有 episode 完成派生重聚合。
   严格 ID Switch 为 414/450 available；36 个不可用项保留原始失败原因且未补零。
4. [x] shard 0-9 完成确定性压缩和 `--source` 双端复核。原始合计
   14,859,178,318 字节，压缩后合计 1,419,786,552 字节；源目录和压缩包均保留。
5. [x] D2 已对 36 个严格身份指标失败关闭样本生成正式只读因果包：27 个一轨多真值
   episode 对应 38 个事件，9 个谱系超窗 episode 对应 518 个事件。450 个 episode、
   10 个分片归档载荷和 76 个派生文件均完成哈希复核；D6 继续保留 unavailable，不推断
   身份或补写 0。后续开放 P1 是使用几何、协方差、运动和来源一致性设计在线无真值
   缓解候选，并绑定新的 clean producer/execution plan 从 shard 0 重新验证。
...
5. [x] 从 clean commit `fc7a1c2` 重新生成 100/100。结果为 292 帧，在线真值使用和
   非有限状态均为 0；schedule、seed registry、manifest、frames 和 generation evidence
   摘要闭合，且未拼接首次 60 项。
6. [x] D3 使用冻结 v2 合同运行唯一一次来源独立评价。未训练、选模、重拟合归一化或
   调整阈值；安全换绑 `13/110`、教师完全匹配 `8/110`、负类 exact-R0 `182/182`，
   预注册总体机器门通过但未准入。
7. [x] D6 不调用 D3 高层评价器，独立复算 generation、dataset、JSONL、CSV、split hash、
   规则矩阵摘要和三组选择边安全性。CSV `21x292` 全闭合，三组各 21637 条边的安全违规
   均为 0，D3 聚合声明与 D6 复算一致。
8. [ ] test 子组教师完全匹配为 `0/25`。在 main 另行冻结正式评价合同前，不读取 seed
   `1000-1019`；运行采用、计划发布、控制、物理和生产权限继续为 false。
...
## 14. D1 光电关联风险留出执行（2026-07-31）

### 已完成

1. [x] 统一三维运行时增加默认关闭的 D1 关联风险影子开关，只在开启时发布原始证据和
   版本化分类，不改变 D1 后验、D2 输入语义或 `global_track_id`。
2. [x] calibration CLI 区分 `development` 与 `held_out`。留出模式强制要求持久化在线分类，
   并验证在线分类与离线冻结 profile 的逐条一致性。
3. [x] 从 D2 只读因果诊断提取相机导致的多真值事件。非相机阻断样本显式排除，不将不可用
   严格身份指标按零处理。
4. [x] 完成 seeds 2000 至 2019 的 nominal 100v100 与 200v200，共 40 个 2.0 秒 episode；
   40/40 状态有限，在线真值使用为 0，D2 诊断完成 40/40。
5. [x] 36 个可评估 case 共 1,015 条原始证据。在线分类与离线复算 `1015/1015` 一致；
   11/13 个故障事件命中，对照告警 0/25。
6. [x] 完成 100v100 seed 2006、200v200 seed 2001 的控制/影子业务等价复核；剔除旁路字段后
   D1、D2、身份语义和 truth NPZ 相同。
7. [x] 证据包区分 `held_out_offline_shadow_validation`，保存输入摘要、CSV、JSON 和中文报告。

### 判定与保持开放

1. [ ] v2 的事件召回 `0.8461538462` 低于冻结门限 `0.90`，性能门未通过。继续
   default-off、shadow、`evidence_only`，不得增加 D2 enforcement。
2. [ ] 本留出集不得用于调参。下一候选必须使用新的 development 数据形成，并用新的独立
   留出集验收。
3. [ ] 增加跨扫描稳定性和 event/source 去重合同；seed 2010 已观察到同一因果事件对应两条
   正证据。
4. [ ] 在 AirSim 相机标定、真实时延和目标平台算力条件下复验旁路采集，不把质点短 episode
   结果外推到工程性能。
