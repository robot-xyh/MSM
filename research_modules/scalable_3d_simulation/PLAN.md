...
下一算法决策仍为开放 P1。候选进入新的 clean seed 1100 前，必须先确定拒绝路径的发布态
替换语义，以及是否能定义可复现的固定滞后 OOSM 修正。方案不得利用在线真值，不得改变
hit/lineage/source support，不得放宽身份承诺、版本、时间戳或绑定门。只有新的真实匿名冻结
扫描产生自然、非零且可审计 treatment，并通过状态一致性和下游可用性门，才恢复
seeds 1101/1102。
