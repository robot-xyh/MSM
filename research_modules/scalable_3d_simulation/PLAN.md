...
2. [x] shard 0 完成 45/45，覆盖 seed 1000、9 类场景和 5/20/50/100/200 五档；
   checkpoint 完成、孤立单元恢复 0、在线真值使用 0。
3. [x] D6 离线复核基础正式准入、矩阵正式准入和 D1-D2 代次合同均为 45/45；
   D4 过时建议和无效建议为 0。
4. [x] shard 0 完成确定性压缩和 `--source` 双端复核。原始 1,503,424,928 字节，
   压缩后 144,187,839 字节；源目录和压缩包均保留。
5. [ ] 保持完整 ID Switch 可用性缺口。当前 0/45 available，生产者明确声明不可用，
   不能补零或将其写成 0 次切换。
6. [ ] 获得明确授权后，人工移除已验证的原始 shard 0，再按同样流程运行 shard 1 至 19。
   未获授权前不继续累积无法释放的正式原始分片。
7. [ ] 全部 20 个归档恢复并逐片复核后执行 `merge-r0 --write-d6-report`，由 D6 完成
   targeted/full posterior。当前进度为 45/900。
...
10. [ ] 在新的 clean source 上冻结 execution plan，按 20 个分片执行
    `run -> pack -> verify`。只有获得明确源分片移除授权后，才可释放每个已复核分片
    的原始占用；全部归档恢复并复核后执行 `merge-r0 --write-d6-report`。
