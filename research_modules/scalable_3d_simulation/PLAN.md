...
5. development 快速治理基准已覆盖 20/50/100/200 四档、每档 5 seed、每例 136 帧。
   ...
   时延 0.25 秒，在线真值使用 0。
6. 单 seed、2.2 秒全栈质点烟测在尾部合并前后分别用时 95.41 秒和 60.21 秒，200 规模
   实时倍率由 0.0231 提高到 0.0365。D2 尾部调用由 31 次降为 1 次；当前主要瓶颈为 D1
   融合 35.12 秒和 D3 三次分配 7.33 秒。
7. 当前权威回归为 D1 `151`、D2 `189`、D6 `521`、scalable main `111` 项通过；其余模块
   沿用上一轮已记录回归，未因本批治理改动调整算法。

### 边界与后续

快速治理基准关闭了“账本无上限”和“没有四档多 seed 容量证据”的开发期缺口。它使用
确定性治理 fixture，不能代替完整传感器融合精度、身份连续性、物理拦截或 AirSim 证据。
正式验收仍需在干净提交复跑同一 manifest，并增加完整点质量测、多 seed 长 episode、真实
时钟偏差、遮挡、杂波和通信退化。D1 小扫描触发全后验重算、D3 200 规模分配时延和 D5/D7
完整闭环仍是 P1。学习策略在独立非退化评审前继续保持 shadow/fail-closed。
