...
D1-D2 结构歧义侧车、有界保活和身份承诺 v2 已完成原子接线，默认关闭。D1 只发布完整
允许边分量证据，不提交身份；main 锁存并在下一次成功 D2 调用一次消费。D2 对已绑定成员
执行 prediction-only 租约，显式区分 `committed`、hold 未承诺和 hold 后未承诺。未承诺
记录不发布来源绑定，也不进入 D3 当前分配窗口。D6 已能独立重算承诺覆盖、恢复水位、
overflow 和绑定违规。D2、D6、scalable 3D 当前回归分别为
`286/598/145 passed`。

detached clean `909669b` 的 seed 1100 同构建门槛仍未通过。候选消费 46 个侧车，
形成 `1714/69/4` 条 committed/hold/after-hold 记录，全记录承诺覆盖为 `0.959149`，
未承诺来源和候选绑定违规为 0。三个恢复航迹使用 `measurement_timestamp=1.2 s` 的新
原始雷达证据，但评分帧为 `2.130815 s`，`0.930815 s` 年龄超过固定 `0.9 s` 窗口。
严格 ID Switch 和 continuity 仍不可用；D2 航迹 `203 -> 201`、D3 分配
`200 -> 197`、可用映射 `1566 -> 1491`。seeds 1101/1102 和扩大矩阵继续停止。

下一轮不调整默认规则路径，也不扩大 `0.9 s` 评分窗口。若继续该候选，执行顺序为：
先让恢复承诺同时满足“量测晚于 hold 水位”和“在当前发布帧仍处于谱系新鲜窗口”，不满足
时保持未承诺；再标定 gap/hard lease 对 track availability 和 D3 分配的影响；最后从
同一 clean 提交重跑 seed 1100。严格指标仍不可用或 D2/D3 可用性继续下降时，该路线保持
研究性并不进入主线。
