...
所有场景由版本化 JSON 配置、`scalable3d-catalog-v1` 场景目录和固定 seed 驱动。故障
计划在对应 D3/D4 运行时端口接入前保持“配置存在、尚未执行”的状态标记。
...
- 阶段 1-3 已完成，并由原有 26 项环境测试覆盖。
- D1、D2、D3、D4、D5、D7 的 scalable 3D 模块入口已接入 main-owned
  `IntegratedScalableModuleStack`；当前 main 集成测试总计 30 项通过。
- 5v5 规则闭环和 200v200 的 0.25 秒雷达烟测已通过。后者形成 200 条中心航迹、200 项
  分配和 200 路三维导引命令，候选边为 6400/40000；该短时结果不能替代长时多 seed。
- 单一二级节点覆盖全部活动区域时，中心失效后的 D3 新版本计划和 D4 接管已闭合。多二级
  区域 owner 以及二级再次失效后的分布式 D3 计划尚未闭合，当前按安全合同停止 D7。
...
- 5/20/50/100/200 的 0.25 秒雷达短测实时因子依次约为 8.54、2.32、0.61、0.28、
  ...
  性能瓶颈。分阶段耗时已进入 episode 诊断和 `stage_timings.csv`。
- 下一阶段先优化 D3 稀疏代价构造和求解路径，并补齐区域化 fallback 计划；D5 同步增加
  相机覆盖索引和候选相机对预算。之后由 D6 完成规模化离线评估，再开展学习训练和 20 个
  未见 seed 验收。
