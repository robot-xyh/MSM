...
2. [x] D4 在 `seed 28203` 失败关闭。D4 owner 已确认 18 个配置目标可形成 19 条唯一在线
   航迹假设，并修复把物理目标数误作在线任务上限的验证合同；唯一性、版本、epoch、lease
   和联盟门控保持不变。
...
6. [ ] 获得绑定新 commit 的 `AUTHORIZE D3 D4 D5 SOURCE GENERATION ONLY` 授权后，使用
   ...
   说明，不重复生成。
7. [ ] 三模块来源均最终化后交 D6 独立审计。D6 通过前不申请训练、held-out 消费、运行或
   控制授权。
