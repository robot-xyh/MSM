...
当前开发工作树增加 main-owned D1→D2 待处理 posterior 锁存。锁存只跨调度 tick 保存真实
后验，在 D2 消费后清除；D7 继续使用后验真实有效时刻执行 0.75 秒过期门。seed 7 的
调度遗漏已恢复，seed 41 在无新后验时继续 fail-closed。该变更通过可扩展模块 123 项回归，
但需在下一 clean commit 后重新生成性能制品，不能沿用 `f80b5bd` 的计时作为新代码证据。
