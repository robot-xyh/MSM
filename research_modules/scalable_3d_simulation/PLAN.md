...
## 10. D2 陈旧观测治理与物理窗复核（2026-07-22）

### 已完成

1. D2 对 D1 后验携带的传感器命名空间和不透明 `latest_observation_id` 做新鲜度判定。
   重复观测不再进入关联、状态更新或确认命中；暂定航迹连续两次没有新证据后删除。
   统计合并还要求共享在线谱系，并同时通过三维位置和速度马氏门。
2. main 的 `modules.d2.associated_tracks` 已持久化
   `d2-observation-evidence-governance-v1`。记录包含新鲜/重复数量、时间冲突、合并事件、
   受抑制起始、暂定删除累计值和 D2 中心身份所有权，不包含仿真目标真实编号。
3. active-risk 5v5 seed 1005 的 2.2 秒复现中，活动航迹数由
   `5,6,6,5,5,5,5,5,5,5` 收敛到 5 条。陈旧观测隔离 9 次，删除暂定伪航迹 1 条，
   最终五条航迹均有唯一离线身份映射，在线真值使用为 0。
4. 脏工作树开发复跑覆盖 seed `1000-1019`，随后已在 detached clean 提交
   `0fa7c00c3514c4fa87a17953ab66fdfb73489b0b` 上完成同配置正式复验。物理清单 v2 记录
   `repository_dirty=false`、20 个同提交源 episode 和 0 个脏源。D6 的计划消费、导引血缘、
   物理窗、D4 降级采用、配对物理比较和非退化证据均为 `20/20`；D4 区域采用为
   `188/188`，两臂各写入 1960 条 D7 命令，全部 SHA-256 校验通过。
5. 当前回归为 D1 `136`、D2 `168`、D3 `419 passed/1 skipped`、D4 `508`、D5 `534`、
   D6 `507`、D7 `213`、scalable main `110` 项通过。

### 边界与后续

本次结果关闭 seed 1005 导致 active-risk 物理窗缺失的代码级问题，并完成 clean-tree
20-seed 复验。两臂在 1 秒计划有效窗内均无 5 米成功，平均最近距离约 3822.36 米；该窗口
只证明计划消费、导引执行、D4 采用和 D6 结果链路完整，不证明拦截有效或降级带来收益。
D2 的 observation claim 当前在单 episode 内不淘汰；20/50/100/200 规模长时容量、合法目标
误抑制率、近邻召回、整帧乱序量测处理和真实 AirSim 阈值标定继续列为 P1。不得用本结果
宣称 200 对 200 完整验收或学习策略获得在线权限。
