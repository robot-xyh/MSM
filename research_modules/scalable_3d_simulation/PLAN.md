...
第一轮规则全栈性能收敛已完成。D1 使用增量后验检查点和每扫描公共审计快照，D3 对规划证据
去重并只复制候选边迟滞明细。两项模块内 A/B 均保持确定性语义。D1 和 D3 分别提交为
`492979e` 与 `f1c8041`。

main 已从 detached clean 提交 `492979e` 运行 20/50/100/200 四档、每档 5 seed 的 2.2 秒
规则全栈。20/20 状态有限，在线真值使用为 0；平均实时倍率为
`1.362/0.491/0.185/0.068`。200 规模 D1/D2/D3 平均累计时间为
`12.103/7.555/0.870 s`。系统实时 P1 尚未关闭。

当前执行顺序调整为：

1. D1 继续剖析扫描级一对一关联和发布物化，保持逐扫描语义哈希与固定滞后边界。
2. D2 剖析运行期关联和 finalize，保持中心身份、声明账本、生命周期和 ID Switch 口径。
3. main 复跑同一四档五 seed，并增加 200 规模长时内存测试。
