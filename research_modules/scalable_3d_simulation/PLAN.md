...
D1-D2 结构歧义侧车和有界保活已按上述方向完成原子接线，默认关闭。D1 只发布完整允许边
分量证据，不提交身份；main 锁存并在下一次成功 D2 调用一次消费；D2 对已绑定成员执行
prediction-only 租约，阻止 hit、miss、birth、rebind 和重复合并。合法延迟证据的年龄上限
显式覆盖雷达延迟、D1 最大缓冲驻留和一个 D2 关联周期。D1、D2、scalable 3D 回归分别为
`237/271/144 passed`。

detached clean `9cd2a79` 的未见 seed 1100 同构建门槛未通过。候选消费 46 个侧车，接受
33 个分量事件并阻止 `69/69/4` 次 hit/miss/birth，但 D2 航迹 `203 -> 201`、D3 分配
`200 -> 197`、可用映射 `1566 -> 1492`。3 个受评分映射因保活后来源观测超出 0.9 秒
谱系评分窗口，严格 ID Switch 和 continuity 由 baseline 可用变为 unavailable。该结果
不是身份改善证据，seeds 1101/1102 和扩大矩阵已经停止。

下一轮不调整默认规则路径。若继续该候选，执行顺序为：先定义歧义保活帧的可评分谱系合同，
保证不把候选观测硬分配给某个身份；再标定 gap/hard lease 对 track availability 和 D3
分配的影响；最后从同一 clean 提交重跑首 seed。严格指标仍不可用或 D2/D3 可用性继续下降时，
该路线保持研究性并不进入主线。
