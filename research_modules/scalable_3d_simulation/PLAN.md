...
4. 脏工作树开发复跑覆盖 seed `1000-1019`。D6 的计划消费、导引血缘、物理窗、D4
   降级采用、配对物理比较和非退化证据均为 `20/20`；D4 区域采用为 `188/188`，两臂
   各写入 1960 条 D7 命令。反事实和因果证据仍为 `0/20`，生产运行确认仍不可用。
5. 当前回归为 D1 `136`、D2 `168`、D3 `419 passed/1 skipped`、D4 `508`、D5 `534`、
   D6 `507`、D7 `213`、scalable main `110` 项通过。

### 边界与后续

本次结果关闭 seed 1005 导致 active-risk 物理窗缺失的代码级问题，只是脏工作树开发证据。
在形成提交后仍需使用同一配置执行 clean-tree 20-seed 复验。D2 的 observation claim 当前在
单 episode 内不淘汰；20/50/100/200 规模长时容量、合法目标误抑制率、近邻召回、整帧乱序
量测处理和真实 AirSim 阈值标定继续列为 P1。不得用本结果宣称 200 对 200 完整验收或学习
策略获得在线权限。
