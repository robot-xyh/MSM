...
第三轮规则全栈性能收敛已完成单 seed 长时复测。D1 使用增量后验检查点、固定滞后后缀复用、
每扫描公共审计快照和非雷达扫描量测模型缓存；D2 对受信诊断元数据执行一次审计并只投影关联
所需字段；D3 对规划证据去重；D5 复用投影矩阵和不可变索引。模块内 A/B 均保持确定性语义。
...
detached clean 提交 `3bac3ff` 的 seed 42000 长时对照已完成。2.2 秒和 10 秒核心墙钟为
`18.611/172.214 s`，实时倍率为 `0.118/0.058`，峰值驻留内存为 `1.002/2.981 GiB`。
相对提交 `c0460e0` 的同配置 10 秒基线，核心墙钟下降 34.6%，D1 融合下降 36.5%，D2 常规
关联下降 76.8%。长短单位时间成本增长由 2.668 倍降至 2.036 倍，仍未达到实时或线性增长。
seed 42000-42002 的三组 10 秒运行也已完成，核心墙钟均值 172.097 秒、峰值内存均值
3.055 GiB，在线真值和 D1/D2 overflow 均为 0。

当前执行顺序调整为：

1. D1/main 分离滤波、GlobalTrack 物化和全量快照持久化成本，评估同融合时刻合并与轻量
   heartbeat/lineage sidecar，保持逐扫描融合、状态变化和谱系语义。
2. D5 剖析终端图构建和历史缓存，解释 10 秒场景单次调用成本 2.696 倍增长。
3. 三组 10 秒 seed 已确认 D3 分配约 3.35 秒和约 3 GiB 峰值稳定存在。下一步用冻结输入
   分离算法、证据生成和序列化成本，并补稳定窗口周期 P50/P95/max。
4. 吞吐收敛后再扩展 D4 故障、D5 跨视角和 D7 五米接近的长时多 seed 验收。
...
下一步进入发布边界和冻结输入热点治理，不直接声明 P1 关闭。详细结果
