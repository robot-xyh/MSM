...
第二轮规则全栈性能收敛已完成。D1 使用增量后验检查点、每扫描公共审计快照和非雷达扫描
量测模型缓存；D2 删除重复真值元数据预扫描并缓存有限键规范化；D3 对规划证据去重并只复制
候选边迟滞明细。模块内 A/B 均保持确定性语义。
...
1. D1 剖析创新求解、数值雅可比和发布物化，保持逐扫描语义哈希与固定滞后边界。
2. main 剖析模块发布总线、模块栈结束收束和日志序列化，不删除审计字段。
3. 增加 200 规模 10 秒以上长时运行，记录周期 P50/P95/max、检查点内存和生命周期。
...
当前实施顺序为：D1/D2/D3/D5 分别关闭自身的历史或载荷重复，main 分模块提交；随后在
新的 detached clean worktree 复跑 2.2 秒和 10 秒相同 seed，由 D6 单独消费两组产物。
候选 pair 需保持真值隔离、计划版本、中心身份所有权和输出语义；若单位仿真时间或内存
仍明显超线性，继续归因，不直接扩大到多 seed。
...
8. 当前权威回归为 D1 `151`、D2 `189`、D6 `521`、scalable main `111` 项通过；其余模块
