...
- 阶段 1-3 已完成，世界、传感器、真值隔离和集成合同由当前 39 项测试覆盖。
- D1、D2、D3、D4、D5、D7 的 scalable 3D 模块入口已接入 main-owned
  `IntegratedScalableModuleStack`；当前 main 集成测试总计 39 项通过。
...
- D3 稀疏代价构造、D5 候选相机对预算、D4 区域建议和 D6 离线规模评估主链已经接入。
  ...
  D4 区域策略的训练与 paired shadow；同时由 D5 owner 实现主动视觉策略。正式结论至少
  使用 20 个未见 seed。D1/D2 仍需在同批次完成 NIS/NEES、门控率和高机动 coverage 标定。
