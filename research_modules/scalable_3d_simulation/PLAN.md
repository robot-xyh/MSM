...
冻结 current-lineage 候选只通过来源和制品校验，不具运行分布兼容性。不得启动正式
20-seed A2/R0、不得降低 OOD 门限、不得把影子投影记为实际采用，也不得据此生成运行
确认、物理窗口或收益。

### 下一执行

1. [ ] D4 组合 900-episode/1798-frame 运行数据与 100-episode/300-frame 动作课程。
2. [ ] 对两个来源共同使用的数字 seed 0 至 99 做全局原子 60/20/20 切分，不沿用任一
   原切分；seed 1000 至 1019 完全排除。
3. [ ] 清单绑定两个来源数据集 SHA-256、切分 SHA-256、特征范围、动作库存和适用域。
4. [ ] 首个候选限定 8 区域几何；未覆盖的 2 区域边几何继续规则回退。
5. [ ] 在本轮 D3/D4/D6/main 变更提交后，从独立 clean checkout 训练新的
   development/shadow 候选。
6. [ ] main 使用非正式 seed 重跑预检。出现分布内、非回退模型执行且安全投影通过后，
   才进入候选冻结和正式 20-seed 准备。
