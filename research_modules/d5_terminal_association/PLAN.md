...
## 2026-08-10 有状态时序几何关联实施状态

状态：**D5 接口、单元测试和 main runtime 接线完成；v3 真实 AirSim 复跑与多 seed 验收待执行**。

- [x] 保持无状态 `associate_tracks_to_detections_geometrically()` API 兼容，新增导出的
  `TemporalGeometricAssociator`、配置、结果、绑定事件和预测记录类型。
- [x] 按资源、相机、流和本地轨迹编号隔离状态；显式保留量测时间、到达时间、候选间隔和
  当前中心航迹只读引用。
- [x] 实现默认 0.25 秒有界 coast。按最近 `bearing_rate` 预测中心和边界框，协方差使用现有
  `rate_sigma_px_s` 随预测时长平方增长。
- [x] `predicted/lost` 仅输出 `coast/reacquire/hold`，不进入实测分配，不产生锁定、提示或
  控制许可。
- [x] 实现绑定迟滞：挑战者必须为当前实测证据、连续两帧且总代价优势不小于
  `min_lock_margin`；单帧、近似平局、成对交换和重复绑定保持 `pending/held`。
- [x] 实现 0.25 秒内原绑定立即恢复、超时过期、量测/到达时间回退清理、本地轨迹 reset 和
  资源/相机/流范围的显式 reset。
- [x] 完成 13 项专项测试和 D5 全量回归；结果为 `13 passed` 与
  `904 passed, 1 warning in 146.08s`。
- [x] main 已在 `long_range_cv_scan` 按相机建立 episode 独立状态。只有
  `measured_assignments` 触发发现、提示和完成，`coasted_records` 仅记录且不授权；新增
  `temporal_binding_events.csv` 与 `dropout_events.csv`。main-D6 联合专项为 `23 passed`。
- [ ] 使用新时序接线运行 v3 真实 AirSim 原始尺度单 seed。冻结 seed `20260810` 的旧 v2 D6
  报告仍为 `fail_closed`，其准确率 `0.9979317477`、短缺口 3、绑定切换 7 和交叉窗口
  `3/31` 只能作为复跑对照，不能写成新算法结果。
- [ ] main/D6 在不少于 10 个原始尺度 seed 中核对短缺口中断、长期重发现、绑定切换和交叉窗口
  可评价率。单元测试不得替代该项验收。

本轮 D5 owner 没有修改检测器、图神经网络、D7 导引或 main runtime；runtime 接线由 main
完成，现有门限未放宽。AirSim P1 只有在 v3 复跑满足短缺口中断 0、连续身份切换 0、重复分配 0、在线真值使用 0、
全局编号改写 0 且关联准确率不低于 0.95 时才能关闭。

## 2026-08-10 真实 AirSim 长距离 20 目标取证复核与后续计划（冻结旧 v2 基线）

状态：**二维扫描和延长提示窗口已运行；发现、提示、关联和记录项通过，总门因短缺口中断未通过**。

- [x] 在 AirSim ComputerVision 中运行原始尺度 `coverage_safe` 20 秒 episode。seed 为
  `20260810`，使用 20 个 `Quadrotor1` actor、50 米/秒目标速度和 100 Hz 逻辑更新，中心
  2.8-3.2 千米距离和约 500 米拦截相机距离。
- [x] 中心按合成 `GlobalTrack` 及协方差执行偏航/俯仰二维蛇形扫描；拦截相机对中心确认目标
  执行二维提示。`coverage_safe` 累计发现和五帧确认均为 `20/20`，提示 `20/20`，
  观察和完成均为 `19/20`。
- [x] 保留在线/离线身份分流。在线观测无 actor、object 或真值身份；在线真值使用和
  `global_track_id` 改写均为 0，中心航迹来源为 `synthetic_d1_d2_fixture`。
- [x] 运动补偿后，1934 条可评分关联的准确率为 `0.9979317477`，连续可见段身份切换为
  0，纯度/连续性为 `1.0/1.0`。
- [x] 记录完整性门通过：保存 98 幅原始相机图和 98 幅在线标注图，19/20 条全局航迹形成
  双相机图像证据。标注不使用 actor/object/truth ID，真值只进入离线混淆矩阵。
- [ ] P1：消除 3 次短缺口中断。当前多目标跟踪门要求短缺口中断为 0，因此不能用身份切换为
  0 或长期重发现 48 次来宣告连续性闭合。
- [ ] P1：提高交叉窗口可评价率。31 个预定窗口只有 3 个形成有效双目标时间样本；这 3 个窗口
  的身份切换为 0、纯度/连续性为 `1.0/1.0`，不能外推到其余 28 个窗口。
- [ ] P1：标定并消除 7 次几何绑定切换。可评分关联准确率 `0.9979317477` 已超过 `0.95` 门限，
  但逐帧准确率不能替代绑定稳定性。
- [ ] P1：区分检测接口返回框与真实光电辨识能力。中心相机 3 千米原始尺度截图接近 AirSim
  渲染/像素极限，需要真实小目标数据或保真合成检测证据。
- [ ] P1：在同一冻结场景分层注入真实位置、姿态、外参、时间、漏检和虚警误差，并运行多 seed；
  现有 `GlobalTrack` 是合成 D1/D2 夹具。

主场景发现、确认、提示比例、关联准确率和记录项均通过相应子门。多目标跟踪要求身份切换和
短缺口中断均为 0，实测为 `0/3`，因此专项总门继续失败关闭。7 次几何绑定切换保留为 P1。

2 倍网格仅作可视化诊断：双相机 `20/20`，2319 条可评分关联准确率 `0.9892194912`，短缺口
仍为 3，几何绑定切换增至 42，总门仍失败。后续不得用放大网格替代原始尺度验收。本轮没有
运行图神经网络，不改变既有 G1 权限和确定性默认路径。

## 2026-08-10 不规则交叉目标窄视场扫描

状态：**理想解析场景已实现并完成 10-seed 验收**。

- [x] 用独立 runner 构造参数化的不规则三维前后错列目标，保持全时段最小三维间距不小于
  25 米，并在 A、B 参考像面形成至少 6 对交叉或明确近距交叉。
- [x] 实现 A 的 100 Hz 解析扫描与五帧确认状态机，以及 B 对 A 已绑定中心航迹的最近转角
  提示扫描。关联仍按确认事件执行，不伪称 100 Hz 连续求解。
- [x] 对比 180 度/秒机械扫描和带 20% 视场重叠的 49.68 度/秒安全覆盖扫描，保留机械模式
  实际漏区，不用真值补齐观测。
- [x] 标准 seed 达到径向跨度 390.0 米、最小三维间距 30.0 米、A/B 投影交叉 10/6 对；
  安全覆盖模式在 1.61 秒完成 20/20 两级关系链。10 个 seed 全部满足准确率和完整链 1.0，
  身份切换、重复/未匹配、在线 truth 使用和全局编号改写均为 0。
- [x] 导出双时间戳、协方差、扫描时间线、匿名观测、事件级完整代价、两模式指标、图表、
  GIF 和中文报告，保留旧全时可见基线输出。

下一步仍按单变量方式加入云台指向误差、位姿误差、时间偏差、漏检、虚警和本地轨迹中断，
分别标定机械覆盖、提示扫描和两级匹配的失效边界。真实检测和 AirSim 接线继续作为独立验证，
不由本次理想结果关闭；现有图学习权重谱系和准入门保持不变。

## 2026-08-10 理想两级配准基线

状态：**已实现并完成 10-seed 验收**。

- 已实现参数化三维质点场景、A 侧全局航迹到匿名轨迹的第一级匈牙利匹配，以及 A 已绑定航迹
  到 B 匿名轨迹的第二级匈牙利匹配。
- 已实现最近五帧位置与帧间位移代价、完整候选代价审计、三层稀疏展示图和在线/离线身份数据
  分离。展示图不执行图神经网络。
- 已实现 CSV/JSON、13 张 PNG、76 帧 GIF 和中文报告生成。目标数、seed、时长和输出目录均可
  参数化。
- 2026-08-10 对 `20260810-20260819` 共 10 个 seed 验收：两级和端到端准确率均为 1.0，
  `id_switch_count=0`、重复分配为 0、未匹配为 0、完整关系链比例为 1.0、在线 truth 使用为 0、
  全局编号改写为 0、双相机全时段可见率为 1.0。专项测试 `7 passed`。

下一步按单变量方式加入像素噪声、位置/姿态误差、时间偏差、漏检、虚警、遮挡和本地轨迹
中断，标定正确匹配与最近错误候选的代价间隔。确定性基线形成可复现失效样本后，才进入冻结
数据上的图匹配对照；现有图神经网络权重谱系校验不得绕过。AirSim 接口和运行计划本轮不变。
