...
def test_production_writer_and_runtime_block_hand_built_g1_admission(
    tmp_path: Path,
) -> None:
    ...
    with pytest.raises(ValueError, match="evidence assembler is unavailable"):
        write_tracklet_model_bundle(
            ...
            g1_admission_report=object(),  # type: ignore[arg-type]
        )
    ...
    assert runtime.available is False
    assert runtime.failure_reason == "bundle_checksums_fields_mismatch"
