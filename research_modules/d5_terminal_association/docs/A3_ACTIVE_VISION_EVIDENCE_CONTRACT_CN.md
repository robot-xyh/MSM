...
A3 采用证据从模型已产生动作开始，不能反向证明模型训练语料完整。D5 在行为克隆入口前增加
独立语料审计。审计只统计 train split 的唯一策略输入，并按动作意图、拦截机/高空侦察机、
场景、seed 及其组合同时记录样本、episode 和 seed 覆盖。validation、test 和 reserved seed
不得贡献训练覆盖。

默认 development 结构门要求每个动作意图至少 `4 sample / 2 episode / 2 seed`，每个相机
角色至少 `8 / 2 / 2`，每个动作与角色组合至少 `2 / 2 / 2`。缺 `hold`、少数动作不足、
侦察相机缺失、重复 episode、同 episode 策略输入复制、seed split 污染、非有限特征、
truth/actor 字段或未知相机角色均失败关闭。

输出补采请求固定绑定场景、动作意图和相机角色，并给出需要新增的唯一样本、episode 和训练
seed。复制、过采样和逆平方根重加权不会改变缺口。行为克隆 cache v2 嵌入审计及内容
SHA-256；旧 v1 cache 因缺少审计不能进入训练。审计通过也只允许 development-only 训练，
不产生正式候选、未见非合成证据、运行 ACK、动作结果或任何权限。

2026-07-28 的小型合成 fixture 专项为 `11 passed`，D5 全量为
`755 passed, 2 warnings in 123.86s`。历史模型的 `hold=0`、`observe_target` 召回 0 和
侦察相机约 `0.621823` 仍未修复。100 episode、1200 sample 补充课程为合成数据；正式
非合成 producer、20 个独立未见非合成 seed 和运行结果谱系继续开放。
