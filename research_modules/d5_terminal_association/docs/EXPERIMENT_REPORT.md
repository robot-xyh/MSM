...
当前没有真实目标跨视角边、几何门、中心 binding 或 G1 收益的新证据。下一步由 main 构造能
产生真实目标共同可见观测的 truth-isolated 场景，可使用 recon cue 或更长时长；truth 继续只
用于离线评分。先运行规则边，再按当前源码重新装配 G1 并交 D6 复审。确定性几何规则继续作为
默认路径，不修改现有门限。
