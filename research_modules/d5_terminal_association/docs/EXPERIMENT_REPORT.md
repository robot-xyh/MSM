...
## 2026-08-10 有状态时序几何关联单元验证

### 结论

D5 有状态包装层通过专项和全量单元测试。默认 0.25 秒 coast 能在 0.06、0.10 和 0.17 秒量测
缺口内输出平移预测框及增长协方差，并在原本地编号和原中心航迹关系恢复后立即标记 recovered。
0.26 秒缺口会清理历史。预测和丢失输入没有进入 `measured_assignments`，日志中的控制许可均为
false。

挑战者验证覆盖单帧 pending、连续两帧 confirmed、近似平局 held 和两条轨迹成对交换 held。
量测时戳回退会先输出 expired，再以当前帧重新建立初始绑定。相同本地编号在不同相机和不同流
中保持独立；范围 reset 不影响其他相机流。向本地轨迹 metadata 注入 actor、object 和 truth
字段后，时序输出未读取或透传这些值，输入 `GlobalTrack.global_track_id` 保持不变。
...
| --- | --- |
| 日期 | 2026-08-10 |
| 场景 | 确定性针孔相机与合成像素量测单元序列 |
| seed | 不使用随机 seed |
| coast 门限 | 0.25 秒 |
| 挑战者确认 | 连续 2 个不同实测帧 |
| 默认代价优势 | `min_lock_margin=3.0` |
| 专项结果 | `13 passed in 1.17s` |
| main-D6 接线专项 | `23 passed` |
| D5 全量结果 | `904 passed, 1 warning in 146.08s` |

唯一警告来自既有 Matplotlib 环境无法导入 `Axes3D`，与本次时序关联逻辑无关。测试命令为：

```bash
pytest -q research_modules/d5_terminal_association/tests/test_temporal_geometry.py
pytest -q research_modules/d5_terminal_association/tests
```

### 限制

main 已完成 `long_range_cv_scan` 接线和联合接口测试。每台相机状态独立，发现、提示和完成只
消费 `measured_assignments`；coast 只进入日志且不授权。该轮没有运行采用新接线的 v3 真实
AirSim，也没有形成新的 seed 级性能样本。

现有 seed `20260810` 的 D6 离线结果仍是旧 v2 冻结基线：准确率 `0.9979317477`、短缺口 3、
绑定切换 7、交叉窗口可评价 `3/31`，报告保持 `fail_closed`。这些数值不能写成新时序算法的
结果。v3 单 seed、10-seed、检测漏检分布、姿态和外参误差、时钟偏差及虚警仍待验证。

## 2026-08-10 真实 AirSim 长距离 20 目标取证试验（冻结旧 v2 基线）

本次使用 AirSim ComputerVision 和 seed `20260810` 重新运行原始尺度
`coverage_safe`。episode 包含 20 个 `Quadrotor1` actor，目标以 50 米/秒运动，
运行 20 秒，逻辑更新率为 100 Hz。目标距中心约 2.8-3.2 千米，拦截相机距目标群
约 500 米。中心执行偏航/俯仰二维扫描，拦截相机按中心确认结果执行二维提示。

| 指标 | 实测 | 门限或判断 |
| --- | ---: | --- |
| 中心累计发现 / 五帧确认 | `20/20` / `20/20` | 发现门通过；本 seed 全部确认 |
| 拦截提示 / 观察 / 完成 | `20/20` / `19/20` / `19/20` | 观察和完成比例 0.95，通过 0.90 门限 |
| 可评分几何关联 | `0.9979317477`，1934 条 | 通过 0.95 门限 |
| 连续可见段身份切换 | `0` | 通过零切换门限 |
| 短缺口中断 / 长期重发现 | `3` / `48` | 短缺口要求为 0，未通过 |
| 轨迹纯度 / 连续性 | `1.0` / `1.0` | 通过 `0.95/0.90` 门限 |
| 可评价交叉窗口 | `3/31` | 28 个窗口不可评价 |
| 交叉身份切换 | `0` | 只适用于 3 个可评价窗口 |
| 几何绑定切换 | `7` | 继续标定 |

发现、提示比例、关联准确率以及运行记录子门通过。专项总门未通过的直接原因是短缺口中断
为 3，而门限为 0。7 次几何绑定切换继续作为 P1 记录。取证包含 98 幅原始相机图和 98 幅
在线配准标注图，19/20 条全局航迹形成中心与拦截相机双视角图像证据。

在线检测和标注只包含匿名本地轨迹、框、双时间戳、协方差和中心只读航迹投影，不含 actor 名、
`object_id` 或真值身份。真值只进入离线混淆矩阵和指标评分；在线真值使用和 `global_track_id` 改写
均为 0。
中心轨迹来源为 `synthetic_d1_d2_fixture`，本次没有检验真实 D1/D2 传感器误差。

原始尺度中心相机在约 3 千米的截图仍接近 AirSim 渲染和像素极限。
`simGetDetections` 返回检测框不等价于真实光电设备已完成目标辨识。三次短漏检、交叉窗口
可评价率 `3/31`、7 次几何绑定切换和真实传感器误差仍未闭合。结果属于单 seed 接口仿真，
没有运行图神经网络。
...
诊断场景仅将目标网格放大到 2 倍，其余主要条件与取证场景保持一致。中心发现/确认和拦截观察/完成
均为 `20/20`，双相机图像证据为 `20/20`。2319 条可评分关联准确率为 `0.9892194912`，
连续可见段身份切换为 0。短缺口仍为 3，几何绑定切换为 42，专项总门仍失败。

诊断结果表明，目标放大可以改善目标的图像可见性和提示覆盖，同时会因检测框重叠增加几何歧义。
该场景不是原始尺度验收，不能替代主场景或实际目标尺度。诊断证据位于
`research_modules/airsim_runtime/outputs/d5_cv_long_range_20target_scale2_visual_diagnostic_20260810/`。

## 2026-08-10 不规则交叉目标窄视场扫描

本次实验保留旧全时可见基线，另建不规则三维扫描场景。默认 20 个目标位于距 A 约
2.8-3.2 千米的搜索扇区中央，初始径向跨度 390.0 米，高度跨度 33.6 米，全时段最小三维间距
30.0 米。A/B 参考像面分别统计到 10/6 对线段相交或图宽 2.5% 内的明确近距交叉。B 位于目标
群后方 500 米并随质心平移，只对 A 已确认航迹执行提示观察。

100 Hz 解析扫描对比两个模式。180 度/秒机械模式相邻光轴间隔 1.8 度，超过 A 的 0.621 度
水平视场。标准 seed 的目标落入采样间隙，15 秒内中心发现率和完整链均为 0。49.68 度/秒
安全覆盖模式保留 20% 视场重叠，中心发现、B 提示观测和完整链均为 1.0，完成时间 1.61 秒，
实测重访间隔中位数 0.91 秒。关联只在连续五帧、0.05 秒确认完成后执行。

2026-08-10 对 seed `20260810-20260819` 运行 10 组安全覆盖实验。10/10 的 A 侧、B 侧和
端到端准确率及完整链均为 1.0；身份切换、重复分配、未匹配、在线 truth 使用和全局编号改写
均为 0。标准输出共 24 个文件，其中包括 10 张 PNG、76 帧 GIF、两模式扫描时间线、匿名观测、
事件代价、关系链、离线 truth 和中文报告。

详细结果位于 `outputs/ideal_20_target_irregular_crossing_20260810/`。本次使用无误差质点、精确
针孔模型和精确提示，不含漏检、虚警、遮挡、位姿误差、时钟误差或通信丢包。机械漏区是当前
采样相位、视场和角速度的解析结果；安全覆盖 20/20 不能作为 AirSim、实飞或真实吊舱发现率。
图表只解释匈牙利候选关系，没有运行图神经网络。

## 2026-08-10 理想二十目标两级配准

本次实验使用 20 个三维质点目标、固定中心相机 A 和运动拦截相机 B。A 持有中心三维航迹与
A 匿名像素轨迹，B 只提供匿名像素轨迹。中心侧依次完成“全局航迹到 A 本地轨迹”和“A 已
绑定航迹到 B 本地轨迹”两次匈牙利匹配，形成公开关系链。场景时长 15 秒，动力学周期 0.1 秒，
图像周期 0.2 秒，共 76 帧；目标速度 3.5-4.7 米/秒，B 速度 14 米/秒。

2026-08-10 运行 seed `20260810-20260819`。10/10 seed 的 A 侧准确率、B 侧准确率、端到端
准确率、完整链比例和双相机全时段可见率均为 1.0；身份切换、重复分配、未匹配、在线 truth
使用和全局编号改写均为 0。标准 seed 输出 13 张步骤图、76 帧 GIF、两级各 30,400 条候选
代价和 1,520 条公开关联记录。专项测试为 `7 passed in 5.47s`。

详细报告及原始产物位于
`outputs/ideal_20_target_two_stage_registration_20260810/`。该实验没有使用 AirSim、真实图像
或图神经网络，也没有加入检测、位置、姿态、时钟和通信误差。结果只作为后续误差注入和学习
方法对照的确定性基线，不能作为真实跨视角配准准确率或实飞能力结论。
...
本轮验证训练语料审计和补采 planner 能否阻断类别、相机角色和 seed 覆盖不足，重点检查
复制样本、重加权和历史缓存不能绕过训练前门。测试没有启动 AirSim，没有运行 900-cell
正式语料，也没有生成新模型权重。

### 开发 fixture

正向 fixture 使用 4 个合成 episode：train 为 seed 10、11，validation 为 seed 20，test
为 seed 30。每个 episode 有 12 个规则示范。train 共 24 个唯一样本，四类动作在两个
训练 seed 中均有覆盖；拦截机和高空侦察机各有 12 个样本。该 fixture 满足默认结构门，但
`synthetic_fixture=true`，正式候选、非合成未见 seed 证据和全部运行权限保持
unavailable/false。
...
| 用例 | 预期结果 |
| --- | --- |
| 删除 `hold` 和压低少数动作 | 拒绝训练，输出对应动作与两类相机的补采请求 |
| 重复完整 episode | 重复份不增加覆盖，语料失败关闭 |
| 同一 episode 内复制 transition | raw train 样本为 25，合格唯一样本仍为 24，复制计数为 1 |
| 删除侦察相机样本 | 侦察角色覆盖为 0，四类侦察动作均生成补采请求 |
| train 与 validation 共用 seed | 报告 split 污染和训练/评估 seed 交叉 |
| train 使用显式 reserved seed | 报告保留 seed 污染，样本不进入有效训练覆盖 |
| 候选特征含 NaN | 排除该样本并拒绝训练 |
| transition 含 truth actor 字段 | 排除该样本，在线 truth 使用仍记录为 0 |
| episode 输入顺序反转 | 审计内容和 SHA-256 完全一致 |
| 旧 v1 cache 或重新哈希提权 | 在模型初始化前拒绝 |
...
两条警告来自既有 Matplotlib `Axes3D` 多版本环境和 NVML 初始化失败，不涉及本轮审计逻辑。
验收要求为零测试失败、复制样本不增加覆盖、污染 seed 不进入训练、补采请求顺序确定、任何
正式候选或运行权限均不能由审计授予。本轮满足上述软件验收。

### 证据边界

正向 fixture 只证明结构门和缓存接线可运行。历史正式行为克隆证据仍为 `hold=0`、
`observe_target` 召回 0，侦察相机精确动作准确率约 `0.621823`。已有 100 episode、
1200 sample 补充课程为合成数据，不能替代非合成训练语料。当前未完成新审计下的正式
producer、20 个独立未见非合成 seed、运行 ACK 与动作结果谱系，也没有新的 A3/R0 成对
非退化结果。正式候选和全部运行权限继续不可用。
...
正式流水线在 clean commit
`8d5e02ec989259ce3d39e1e4ad6a90dd0d8d5b54` 上执行，证据根目录为
`/tmp/MSM-d5-g1-formal-evidence-8d5e02e-20260727`。输入和实现绑定如下。

| 项目 | SHA-256 |
| --- | --- |
| runtime implementation | `b0708e718b374e5bb52db41c7bd2f994e340a2b009cfd348881a5f9d549baffe` |
| development manifest | `7d459ed855cf74b810fa1f79ed0327efd39eb4be4409451266da3f3a95387ce0` |
| weights | `7fb5db8b6099ca4da5706a3bec53ff7cd634e8bd267c036ce3ee4ee4bf71ca71` |
| paired lineage | `83e105290f3e624f267d92ceaf050d32291bd5bbbabf98580846cd31498b1af1` |
...
| episode | 900 |
| 场景规模单元 | 45 |
| held-out precision | 1.0 |
| held-out recall | 1.0 |
| held-out F1 | 1.0 |
| candidate recall | 1.0 |
| false merge | 0 |
| CPU P95 推理时延 | 0.9130288 ms |
| paired-shadow 帧数 | 900 |
| paired-shadow 边数 | 74024 |
| 模型 edge F1 | 1.0 |
| 模型 cluster F1 | 1.0 |
| 最高单特征 AUC | 0.720073 |
| lineage 记录数 / 唯一 UID 数 | 900 / 900 |

这些数值来自冻结合成 tracklet 图。最高单特征 AUC 低于审计捷径门限，但 paired-shadow 的扰动
保持候选图拓扑固定；结果不能代表真实相机发生外参漂移、遮挡和漏检后重新构图的性能。

### 外审与装配

| 阶段 | 结果 |
| --- | --- |
| D6 external audit | `d6.d5-g1-external-audit.v2`，`pass`，blocker 为空 |
| external audit 文件 SHA-256 | `cbd6c72b2d9e7b78bf3aa36f975e6627250d2bf18de5a0b0ebc2c8f6cf760cd6` |
| external audit 内容 SHA-256 | `334cf662e49c735931019ff358be1894d1358f1b4a5a868759eee41d3d282d15` |
| D5 生产装配 | `d5.tracklet-model-bundle.v5` |
| v5 manifest SHA-256 | `b431d066362005868374d038eb93a83b773c03715a53d8a9dfd0da21784f317d` |
| D6 post-assembly | `d6.d5-g1-post-assembly-audit.v2`，`pass`，blocker 为空 |
| post-assembly 内容 SHA-256 | `17dda42d06b4be1d21ff8f1f8baecc320fd49b532be06a9f9f6b304341763e1d` |
| strict loader | 通过 |
| shadow loader | 通过 |
| assist loader | 失败关闭：`bundle_g1_assist_authority_not_granted` |

两级 D6 审计与生产装配关闭了此前“正式 external audit v2、v5 和 post-assembly v2 待生成”的
证据缺口。装配状态为 `g1_evidence_eligible_not_authorized`。模型晋级、G1 辅助、默认路径
变更、分配、故障接管和控制六项权限全部为 `false`，在线 G1 没有启用。

### 保留限制

| 项目 | D6 状态 | 原因 |
| --- | --- | --- |
| 真实相机泛化 | `unavailable` | 输入只含合成 held-out 与 paired-shadow |
| 中心 `global_track_id` binding 正确性 | `unavailable` | 没有中心 binding 结果与离线真值的连接记录 |
| 物理闭环结果 | `unavailable` | 没有导引、控制或物理拦截记录 |

上述三项不能用 0 填充，也不能由合成指标外推。确定性几何规则继续作为默认路径；后续 P1 分别
由真实相机回放、离线身份真值连接和 main/D6/D7 物理闭环提供证据。

## 2026-07-26 v5 paired lineage P0 回归（历史，已由正式证据闭环）

| 项目 | 结果 |
| --- | --- |
| lineage 输入 | 显式路径与调用方冻结 SHA-256 |
| v5 内路径 | `evidence/paired_episode_lineage.jsonl` |
| manifest 字段 | `filename/sha256/record_count/unique_episode_uid_count` |
| admission report v2 字段 | lineage SHA、记录数、唯一 UID 数 |
| 正式计数门 | `900/900` |
| paired report 字段 | `schema_version/filename/record_count/sha256` |
| assembler 专项 | `69 passed in 1.93s` |
| lineage 联合专项 | `86 passed in 4.32s` |
| 模型流水线 | `20 passed, 1 warning in 2.27s` |
| D5 全量 | `655 passed, 1 warning in 100.57s` |
| runtime SHA-256 | `b0708e71...baffe` |

正向 fixture 使用 D5 生产 assembler 生成 v5，并验证 lineage 实物、manifest 四字段、admission
report 三字段、`SHA256SUMS` 和 strict/runtime loader。负例覆盖缺失文件、调用方哈希不符、空行、
非法 JSON、非对象、空 UID、重复 UID、899 条记录、旧 `file` 字段、paired/D6 摘要或计数不一致、
manifest/report 缺字段和装配后篡改。

本节只记录 2026-07-26 的代码与 fixture 阶段。正式重训、external audit v2、v5 装配和
post-assembly v2 已于 2026-07-27 完成；没有运行在线 G1 episode。六项运行权限仍全部关闭，
确定性几何规则继续默认。

## 2026-07-26 六权限合同回归（lineage 修复前）

### 代码结果

| 项目 | 结果 |
| --- | --- |
| 权限合同 schema | `d5.tracklet-g1-authority-contract.v2` |
| admitted bundle schema | `d5.tracklet-model-bundle.v5` |
| admission report schema | `d5.tracklet-g1-admission-report.v2` |
| D6 external audit schema | `d6.d5-g1-external-audit.v2` |
| D6 input / consumer schema | 各自保持 v1 |
| 权限字段 | 6 项精确字段 |
| 合法值 | 六项均为布尔 `false` |
| v5 admission 状态 | `g1_evidence_eligible_not_authorized` |
| 影子加载 | 代码 fixture 可用 |
| G1 辅助请求 | `bundle_g1_assist_authority_not_granted` |
| 中间 runtime SHA-256 | `fe116fd5...1c91` |
| assembler/loader 专项 | `70 passed, 1 warning in 2.78s` |
| D5 全量 | `636 passed, 1 warning in 106.54s` |

正例覆盖 v5 原子装配、严格加载、manifest/report/打包审计交叉绑定和影子加载。负例覆盖六个
权限逐项为 `true`、逐项非布尔、字段缺失、多余、拼写错误、旧四字段结构、未知 schema、
空原因、仅原因字段、bundle v4/report v1/audit v1 混用以及装配后审计篡改。所有负例均拒绝，
没有放宽任何模型、几何或身份门限。

该阶段只验证代码合同。当时没有重训，没有运行 `20 seeds / 900 episodes / 45 cells` held-out，
没有运行 paired-shadow、D6 external audit、正式 v5 装配或 G1 episode。该中间 runtime 后续又
因 lineage P0 修复而变化，相关指标不能转移到当前 `b0708e71...baffe`。

### 当时限制

截至该段记录时，没有绑定新 runtime 的 development bundle、registry、D6 外审和
post-assembly audit。上述正式证据与 v5 已于 2026-07-27 完成；默认确定性几何规则仍启用，模型
晋级、G1 辅助、默认路径变更、分配、故障接管和控制权限仍全部关闭。AirSim 接口未变化，该阶段
没有新增仿真结果。

## 2026-07-26 旧 D6 audit v1 与 v4 装配（修复前记录）
...
| --- | --- |
| 带 `v2` 后缀的目录清单 | 全部通过 |
| JSON 顶层 schema | `d6.d5-g1-external-audit.v1` |
| D6 JSON SHA-256 | `24c8b0cd...9ad7d` |
| D6 内容 SHA-256 | `f17acecf...35f` |
| 审计状态 | `pass` |
| blocker | `[]` |
| D5 引用文件 | 9/9 SHA-256 一致 |
| 六类权限 | 全部 `false` |

### 装配结果

修复前的正式 assembler 对 current-runtime development bundle、held-out、paired-shadow 和
旧 D6 audit v1 执行
原子装配。返回码为 `2`，稳定拒绝码为 `d6_authority_fields_mismatch`。v4 输出目录没有创建。

失败发生在 authority schema 严格集合比较。该旧 audit 含六个权限布尔字段，当时 runtime
assembler 含四个。两个新增字段本身为 `false`，但当前合同不允许忽略未知字段。该失败不表示
模型指标、权重、held-out、paired-shadow 或 D6 审计结果失败。

失败记录 SHA-256 为 `196174e5955fc751e615f080fa4ad6178849ee0dbe4ea031977c05b29fced482`，
保存在 `/tmp/MSM-d5-g1-current-runtime-v4-64cb865-20260726-failed/`。由于没有 v4，strict
loader、runtime loader、post-assembly handoff 和 G1 episode 均未运行。确定性规则继续默认。

## 2026-07-26 clean R0 与 G1 证据（历史 5506 runtime）

### 几何候选图

main 在 clean commit `64cb865b9933d45b13878019c0e1a21a8fbb2b05` 上完成 20-seed R0。
...
该结果评价几何候选图。流程在匿名图冻结后使用离线 truth 统计边级计数，没有调用 G1 模型，
因此不能表示 G1 相对规则路径的收益，也不授予身份、分配或控制权限。

### 冻结输入

该阶段 clean 源码严格复载既有 held-out corpus：

| 项目 | 结果 |
| --- | --- |
| seed / episode / cell | `20 / 900 / 45` |
| 节点 / 候选边 | `13344 / 74024` |
| 在线真值特征 | 0 |
| 同相机候选边 | 0 |
| 全局编号创建或换绑 | 0 |
| manifest 文件 SHA-256 | `a91d5540...196f` |
| manifest 内容 SHA-256 | `06e7b92c...69ba` |
| config SHA-256 | `528bdd1b...ad18` |
| evaluator lineage SHA-256 | `19f35ebb...a63eb` |

冻结模型权重 SHA-256 为 `7fb5db8b...ca71`，温度和决策阈值保持
`0.6541651703102319` 和 `0.8964798917983628`。本轮没有修改任何输入。

### 正式重训

旧 development bundle 绑定 runtime `408e71fe...f4fe`，不能在该阶段 runtime
`55066382...b8ea` 下作为证据。D5 使用正式 writer、原始冻结输入、原 seed 和 robust-v2
超参数重新训练，没有修改阈值、数据、真值边界或旧 manifest。

| 制品 | SHA-256 |
| --- | --- |
| 新模型权重 | `7fb5db8b...ca71` |
| 新模型 manifest | `db908b05...1d14` |
| bundle checksums | `2fe079ed...856` |
| 训练 admission 内容 | `dc520c9e...531` |

新权重与历史候选完全一致。manifest 由 writer 原生生成并绑定该阶段 runtime，不属于旧权重的
手工重新标记。
...
| 错误合并率 | `0.0` |
| 期望校准误差 | `0.0000347` |
| CPU P95 推理时延 | `0.872 ms` |
| 在线真值 / 同相机边 / 全局编号创建换绑 | `0 / 0 / 0` |

所有 45 个场景规模单元通过冻结门。温度、决策阈值和权重未在 held-out 上重新选择或更新。
...
最高单特征 AUC 为 `0.720073`，低于外审建议门限 `0.98`。另行执行的 9 类故障注入探针均正确
回退到确定性几何规则，探针回退成功率为 `1.0`；该数值不表示正常模型推理发生回退。扰动发生
在几何门之后并保持候选图拓扑固定，因此该结果不能替代真实相机外参漂移、遮挡和检测误差下的
重新建图验证。
...
`evidence_chain_closed_shadow_only`，registry audit evidence SHA-256 为
`c18f4149...4477`。D6 输入清单 SHA-256 为 `f98b42d3...3a5`，顶层
`SHA256SUMS` 文件 SHA-256 为 `0a3b8e39...b36e`。

该阶段未调用 D6，也未装配当时的 v4。该证据可进入当时的 D6 独立外审，但不授予模型晋级、在线 G1、
默认路径、全局身份、分配或控制权限。确定性几何规则继续作为默认路径。

验证命令覆盖正式 writer、held-out、paired-shadow 和 registry，专项结果为
`46 passed in 3.40s`。clean D5 全量结果为
`600 passed, 1 warning in 97.84s`；warning 为 PyTorch NVML 初始化提示，不影响 CPU 指标。
...
上述软件覆盖合同已由 main 的 clean 20-seed R0 形成正式几何候选图证据。截至该段记录时，该
runtime 的 development bundle、held-out、paired-shadow 和 shadow-only registry 已完成，剩余
工作是 D6 独立 external audit，以及审计通过后的历史 v4 装配和装配后审计。最终 runtime 的
external audit v2、v5 和 post-assembly v2 已于 2026-07-27 完成。

该历史 G1 v4 严格加载返回 `bundle_implementation_runtime_mismatch`。本节不授予 G1、默认、
身份、分配或控制权限。
...
同一套投影 fixture 在一次调用中输入两个相机时，关联图为 `2 nodes / 1 edge`。改为两个相机
分别调用时，旧实现第二次仍为 `1 node / 0 edge`。局部航迹、外参和评分器均有效，差异来自旧
`process()` 只选择本次 adapted batches。

### 单元结果

| 场景 | 节点 | 边 | 结果 |
| --- | ---: | ---: | --- |
| 异步同目标，规则评分 | 2 | 1 | 形成跨视角候选 |
| 异步同目标，模型评分 fixture | 2 | 1 | `model_scored` |
| 异步不同目标 | 2 | 0 | 几何条件未通过 |
| 量测时间超窗 | 1 | 0 | 缓存相机剔除 |
| 到达时间超窗 | 1 | 0 | 缓存相机剔除 |
| TTL 过期 | 1 | 0 | 旧快照删除 |
| OOSM 到达 | 0 | 0 | 不覆盖当前快照 |
| 缺外参 | 1 | 0 | 缺外参相机不入图 |
| stream/episode reset | 仅保留重置后状态 | 0 | 旧快照清空 |

adapter 专项为 `48 passed`。2026-07-26 D5 全量测试为
`598 passed, 1 warning in 97.36s`，零失败。warning 是既有 PyTorch NVML 初始化提示。

### 在线短输入审计

输入为 5v5 seed `1000`、时长 `2.2 s` 的统一三维在线短冒烟，规则路径输出位于
`/tmp/MSM-d5-active-snapshot-rule-seed1000`。该路径只作临时复测，不是正式长期制品。

| 指标 | 修复前 G1 短冒烟 | 当前规则复跑 |
| --- | ---: | ---: |
| D5 发布次数 | 5 | 5 |
| 输入相机批次总数 | 6 | 6 |
| 每次图节点 | 1, 1, 1, 2, 1 | 1, 1, 2, 2, 2 |
| 累计图节点 | 6 | 8 |
| 双相机节点发布时刻 | 1.75 s | 1.25、1.75、1.95 s |
| 跨调用活跃相机复用次数 | 无字段 | 2 |
| 每次复用匿名航迹数 | 无字段 | 1, 1 |
| 在线 `vision_bbox` | 本次未复核 | 6 |
| 离线 `known_false_alarm` | 本次未复核 | 6/6 |
| 离线非空 `truth_entity_id` | 本次未复核 | 0/6 |
| 共同中心 `support_by_node` | 本次未复核 | 0 |
| 累计图边 | 0 | 0 |
| 在线真值使用 | 0 | 0 |

6 条在线观测的量测时刻和来源分别为：`0.7/0.8 s CAM-INT-0002`、
`1.1 s CAM-INT-0001`、`1.6 s 两相机`、`1.8 s CAM-INT-0002`。离线 truth sidecar 将其全部
标为 `disposition=known_false_alarm`、`truth_entity_id=null`。这些标签只用于本次事后核验，
没有进入在线适配、图构造、门控或评分。

节点增加证明异步快照进入了同一张图。三个双相机图的 `support_by_node` 均没有共同中心
`GlobalTrack`，稀疏预筛选正确保留零边。该结果证明异步节点同图路径生效和虚警失败关闭，
不能用于判断真实目标跨视角候选边、几何门或 G1 收益，也不能表述为跨视角 binding 完成。

### G1 接口探针

实际 `7fb5db8b...ca71` 权重在合成异步同目标图上得到 `2 nodes / 1 edge`、
`scoring_status=model_scored` 和边概率 `0.9999935627`，最终形成一个双节点簇并只读绑定
`GT-0001`。该探针手工加载权重，没有通过正式 v4 严格准入，只用于确认新增图输入与评分器接口
兼容。

### D6 装配后审计

D6 clean evaluator commit `107cf0756d7b75cd6bf1456d1f1aa940fec6a63c` 已对既有 G1 v4
完成正式 post-assembly audit。输出目录为
...
| status | `pass` |
| blocker | `[]` |
| content SHA-256 | `3738444168138584c7ec3eb895d123178092176ec751a5b455e575b177a2d852` |
| 未见 seed | 20 |
| episode | 900 |
| 场景规模单元 | 45 |
| 在线真值使用 | 0 |
...
身份、分配或控制权限。该阶段来源合同修改后，当时运行时实现摘要为 `55066382...b8ea`，v4 绑定
摘要为 `408e71fe...f4fe`。公开严格加载器返回
`available=false/failure_reason=bundle_implementation_runtime_mismatch`。current-runtime
development、held-out、paired-shadow 和 shadow-only registry 已在后续正式流程中形成；
后续旧 D6 audit v1 已通过自身门限，但当时拟装配的 v4 因 authority schema 不一致未能生成。

### 结论

跨调用相机状态丢失已在 D5-owned 代码和回归范围内关闭，本短输入同时验证了虚警失败关闭。
后续 clean R0 已提供真实目标候选边和几何门证据，current-runtime shadow 评估也已完成。
新 v5 证据重建和装配后审计已于 2026-07-27 完成。中心 binding 的完整业务覆盖和真实相机
泛化仍待后续。确定性几何规则继续作为默认路径，不修改现有门限。

## 2026-07-26 G1 v4 正式装配（历史制品）
...
该历史 v4 的 G1 实现摘要已纳入 `tracklet_g1_evidence_assembler.py`，当时为
...
`40a42af015211d5e721584053e052a893e31aa35b7393195530a5d3d2dc9b90d`，当时实现摘要为
