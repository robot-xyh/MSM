...
逐帧投影和匈牙利算法给出当前帧的最小总代价关系，但短时漏检或两条像面轨迹交叉时，单帧
最优关系可能跳变。D5 在无状态几何结果外增加有界时序层。状态键由资源、相机、图像流和本地
轨迹编号共同组成，相同本地编号在不同相机或不同视频流中互不影响。全局航迹编号仍由中心持有，
D5 只保存引用关系。

最后一次实测中心为 $\mathbf{u}_0$，像面角速度为 $\dot{\mathbf{u}}$，漏检时长为
$\Delta t$。0.25 秒窗口内采用

$$
\hat{\mathbf{u}}(t)=\mathbf{u}_0+\dot{\mathbf{u}}\Delta t,
\qquad
\mathbf{R}_{\mathrm{pred}}=\mathbf{R}_{\mathrm{cam}}+
(\sigma_{\dot{u}}\Delta t)^2\mathbf{I}.
$$

边界框按同一像面位移平移，框宽和框高保持不变。$\sigma_{\dot{u}}$ 复用
`AssociationConfig.rate_sigma_px_s`，默认 40 像素/秒。预测时长越长，像素协方差越大；超过
0.25 秒后历史直接过期。预测和丢失记录只用于解释 coast 与重新捕获过程，不能形成末端锁定或
控制许可。

设当前本地轨迹原绑定为 $g_i$，瞬时几何结果提出挑战者 $g_j$，两者对同一本地量测的总代价
分别为 $C_i$ 和 $C_j$。只有满足
...
且 $g_j$ 在连续两个不同实测帧中胜出，才确认新的引用关系。$m_{\min}$ 使用既有
`min_lock_margin`，默认 3.0。单帧变化标记为 pending，优势不足标记为 held。两条本地轨迹在
同一帧正好交换原全局引用时，系统将其视为交叉歧义并保持原关系，不靠本地编号重排全局编号。

2026-08-10 已完成确定性接口和单元测试。13 项专项覆盖三档短缺口、过期、挑战者迟滞、成对
交换、时戳回退、多相机/多流隔离和真值不变式；D5 全量回归为 904 项通过。main 已按相机将
该接口接入长距离 runtime，只有实测关系参与发现、提示和完成，coast 只记录且不授权；main-D6
联合专项为 23 项通过。接线测试不等于真实 AirSim 性能验证，v3 单 seed 复跑和多 seed 标定
仍未完成，不能据此关闭现有长距离视觉 P1。

冻结 seed `20260810` 的 D6 报告属于接线前旧 v2 基线，仍为 `fail_closed`。其中准确率
`0.9979317477`、短缺口 3、绑定切换 7、交叉窗口可评价 `3/31`，只用于后续 v3 对照，不作为
新时序算法的性能结果。

## 真实 AirSim 长距离冻结旧 v2 证据的解释边界

2026-08-10 的冻结旧 v2 取证来自 AirSim ComputerVision 原始尺度 `coverage_safe` 场景。
episode 时长 20 秒，seed 为 `20260810`，20 个 `Quadrotor1` actor 以 50 米/秒运动，
逻辑更新率为 100 Hz。中心相机距目标约 2.8-3.2 千米，拦截相机距目标群约 500 米。中心用
带协方差的合成 `GlobalTrack` 计算偏航和俯仰搜索边界，并执行二维蛇形扫描；拦截相机只处理
中心已确认目标的二维提示。

第一轮二维扫描已经发现 20/20 个目标并完成 19/19 个提示，但直接在像素平面预测匿名轨迹，
云台转动使静态背景坐标系持续变化，最终产生 55 次身份切换。修正复跑把检测中心转换为 NED
世界坐标视线后再做速度预测和匹配，连续可见段身份切换降为 0。该对比说明相机运动补偿是
长距离窄视场跟踪的必要条件，不说明跟踪链已经闭合。

`coverage_safe` 累计发现和连续五帧确认均为 `20/20`，拦截相机获得 20 条提示，观察和完成
均为 `19/20`。1934 条可评分几何关联的准确率为 `0.9979317477`。匿名多目标跟踪在连续
可见段的身份切换为 0，轨迹纯度和连续性均为 1.0；仍有 3 次短缺口中断和 48 次长期重发现。
31 个预定交叉窗口只有 3 个可评价，这些窗口的身份切换为 0、纯度和连续性均为 1.0。另有
7 次几何绑定切换。

发现、确认、提示比例、关联准确率和记录完整性通过相应子门。多目标跟踪门要求短缺口中断为 0，
实测为 3，这是专项总门未通过的直接原因。7 次几何绑定切换作为 P1 继续标定。取证包保存
98 幅原始相机图和 98 幅在线标注图，19/20 条全局航迹形成双相机图像证据。

在线观测只携带匿名本地轨迹、像素框、双时间戳和协方差，不含 actor、object 或真值身份；
逐检测身份映射只进入离线评分文件。在线真值使用和 `global_track_id` 改写均为 0。中心航迹
来自 `synthetic_d1_d2_fixture`，所以该结果不能写成真实 D1/D2 融合已经通过。中心相机在约
3 千米的原始尺度图像接近 AirSim 渲染和像素极限，检测接口返回框不等价于真实光电辨识。

2 倍网格诊断将双相机证据提高到 `20/20`，但关联准确率降为 `0.9892194912`，几何绑定切换
增至 42，短缺口仍为 3，总门仍失败。它只用于说明目标尺度对可见性和重叠歧义的影响，不替代
原始尺度主场景。短漏检、交叉窗口覆盖、几何绑定稳定性、真实误差注入和多 seed 继续为 P1；
本轮没有运行图神经网络。

## 窄视场扫描与事件配准

窄视场相机不能同时看到目标群。D5 将连续搜索和身份配准分开处理：云台先在 45 度扇区内
按固定角速度扫描，目标连续进入视场 5 个 100 Hz 采样后才形成确认事件；匈牙利匹配只在确认
事件发生时运行。这样既保留机械扫描的真实漏区，也避免把 100 Hz 可见性计算误写成 100 Hz
关联求解。

中心 A 确认一条匿名轨迹后，保留中心只读全局编号及其三维状态。B 只接收这些已确认航迹的
提示，按当前云台到候选方向的最小转角逐目标观察，再用 B 匿名像素轨迹完成第二级匹配。
标准理想场景表明，光轴步长大于水平视场时可以完全跨过目标；把步长限制为视场宽度的 80%
后可形成连续覆盖。该结论只说明扫描采样和视场几何，不代表真实检测器已达到相同发现率。

## 理想两级视觉配准基线

2026-08-10 新增的理想基线把跨相机关系分成两个可审计步骤。第一级使用中心节点已有三维航迹、
A 相机内外参和图像时刻，把每条全局航迹投影到 A 图像，与 A 的匿名像素轨迹做一对一匹配。
第二级只处理第一级已经绑定的航迹，把这些三维状态投影到 B 图像，再与 B 匿名轨迹做一对一
匹配。B 不提供目标三维位置，D5 也不从本地编号生成全局编号。

每一级都保留完整 N×N 代价矩阵。最近五帧的位置差用于判断目标处于哪里，帧间像素位移差
用于判断短时运动是否一致。匈牙利算法在完整矩阵上求全局一对一最小代价；三层候选图只把
每行代价最低的三条边画出来解释候选关系，不改变求解范围，也不代表执行了图神经网络。

10 个理想 seed 的完整链准确率为 1.0，身份切换为 0。该结论建立在精确内参、精确位姿、无
噪声、无漏检和所有目标持续可见的条件上。它验证算法链路与数据边界，不证明真实小目标检测、
外参同步、遮挡恢复或跨相机学习特征已经解决。
...
seed。绑定同时固定全局登记表的内容哈希和文件哈希，避免修改登记表后重算自哈希绕过审计。
每个 allocation 还固定 owner、候选版本、生命周期、用途、允许的离线操作和协议来源文件。

train、validation 和 future-held-out 以 whole-episode seed 为最小单元。一个 seed 只能生成一个
split 的一个 episode，不能按 frame 或 sample 拆分，也不能跨 split 复制。schedule 使用
seed 派生的场景扰动产生不同观测历史，在线策略看不到 truth identity。中心提供的
`global_track_id` 仍是只读引用，D5 不创建、不改写、不换绑。

来源计划把 104 个 seed 逐项展开为 episode。每个 episode 安排四段意图窗口，每段计划 24 个
唯一样本；角色采用两种互补模式交替，使 8 个“意图×相机角色”单元在每个 split 达到协议
...
置信阈值。一次访问后，无论通过或失败都禁止再次读取。该原则把最终泛化评估与开发迭代分开。
...
新来源 seed 已由 main 在全局登记表中分配，协议文件本身仍保持空值。train、validation、
future held-out 三组必须互斥，并与 v2 全 split/test 的 `22100-22199`、正式保留的
`1000-1019` 零重叠。最低总
...
主动视觉示范必须由相机运行状态和确定性规则自然产生。D5 不接受外部直接指定 `hold` 或
`search_sector` 标签。现有相机合同包含云台是否可转动和当前动作完成时刻。拦截相机与侦察
相机在云台不可用，或前一动作尚未完成时，规则保持当前姿态和视场并输出 `hold`。该动作不含
目标引用和扫描扇区，不能被解释为重新分配。

侦察相机的搜索示范使用有界 cue-loss 条件。中心计划、中心航迹和三类版本继续保留，只在
指定时段缺少该相机对已分配目标的可用投影证据。通信健康且扫描扇区可用时，规则输出
`search_sector`。这种处理模拟观测提示暂时丢失，不删除中心身份，也不从 truth、actor、
object ID 恢复身份。

角色由 `resource_id` 中唯一的拦截或侦察标记确定。main 还需传入相机状态时间、云台姿态、
角速度、限位、速率上限、视场模式、`slew_available` 和 `action_in_progress_until`。所有
AssignmentPlan、coalition 和 communication 版本必须与调用期望一致。旧版本、通信异常和扫描
冲突仍走原有失败关闭路径，不能为补齐类别而绕过。
...
类别权重只能调整已有示范对损失函数的贡献，不能提供缺失动作的信息。D5 因此把训练语料
覆盖门置于行为克隆之前。语料门只统计 train split 中通过身份隔离、数据完整性和有限特征
检查的唯一策略输入，验证集、测试集和保留评估 seed 不参与训练覆盖。

覆盖需要同时满足样本、episode 和 seed 三个层次。默认开发阈值为：每个动作意图至少
4 个唯一样本、2 个 episode 和 2 个 seed；拦截机与高空侦察机每类至少 8 个唯一样本、
2 个 episode 和 2 个 seed；每个动作与相机角色组合至少 2 个唯一样本、2 个 episode 和
2 个 seed。样本数说明训练量，episode 数约束场景过程独立性，seed 数约束随机条件覆盖。
三者必须同时满足，任一单项不能替代其他两项。该门是开发数据结构下限，不代表独立同分布，
也不代表模型性能达到系统要求。

同一 episode 内，相机、快照时间、候选动作集合和候选特征完全相同的策略输入视为复制。
复制项被排除并使语料失败关闭。重复 episode、seed 分割交叉、保留 seed 混入、缺少
`hold`、少数动作不足、侦察相机缺失、未知相机角色、非有限特征和 truth/actor 字段同样
失败关闭。复制、过采样、逆平方根重加权或修改样本权重均不会增加覆盖计数。

补采计划按场景、动作意图和相机角色生成。每个请求同时给出至少还需增加的唯一样本、
episode 和新训练 seed，并要求从绑定的训练 seed 注册表中选择新 seed。请求编号和排序
确定，输入顺序变化不改变结果。合成 fixture 可以验证审计软件，但不能形成正式候选、
非合成未见 seed 证据、运行 ACK 或动作结果。

行为克隆 cache v2 将语料审计及其内容摘要写入 manifest。训练在模型初始化前重新验证
schema、内容摘要、权限字段和结构门；旧 v1 cache 因无审计而失败关闭。语料门通过最多允许
development-only 训练。正式候选、assist、主动视觉、相机命令、分配、接管、控制及中心
`global_track_id` 写权限始终保持 unavailable/false。

2026-07-28 的软件验证使用 2 个训练 seed 的合成小样本，专项 `11 passed`，D5 全量
`755 passed, 2 warnings in 123.86s`。该结果不改变历史正式语料缺口：`hold=0`、
`observe_target` 召回 0，侦察相机精确动作准确率约 `0.621823`。现有 100 episode、
1200 sample 补充课程为合成数据。非合成正式训练语料、20 个独立未见非合成 seed、
运行 ACK/结果和正式权限均未形成。

## 主动视觉模型前置原则

主动视觉行为克隆只能从已有安全候选动作中学习规则选择，不能绕过版本、身份、相机限位和
确定性投影门。旧正式数据中 `reacquire` 占 92.16%，总体精确动作准确率约 0.956，但
`observe_target` 召回为 0，`hold` 没有正样本。总体准确率主要反映多数动作，不能单独证明
模型具备观察、保持或侦察相机泛化能力。

开发训练现按所选意图使用有界逆平方根权重。样本数为 \(n_c\) 的动作类权重满足

\[
w_c \propto \min\left(w_{\max},\sqrt{\frac{N}{n_c}}\right),\qquad
w_{\max}=8,
\]

并归一化到训练样本平均权重为 1。该方法降低多数动作对梯度和最佳轮次选择的支配程度。
权重只作用于真实存在的示范。缺失动作没有可学习信息，必须保持不可用；补零、复制空标签或
把运行失败帧改成 `hold` 都会制造虚假确定性。

模型评估采用分层口径。宏平均召回和每动作召回约束少数动作；相机角色分层用于识别拦截相机
与高空侦察相机分布差异；置信度分箱给出期望校准误差；所有候选特征只要有一项超出训练边界
及其容差，该决策样本即记为分布外。动作不一致、低置信和分布外计数只用于建议规则回退，
运行时仍由既有确定性安全壳执行回退。

开发模型预检查不能授予权限。每个动作必须在 train split 有真实正样本，并在 test split
达到最低召回；两类相机角色、校准和
分布外指标都要可用且满足开发阈值，才允许进入后续正式 paired-shadow 候选阶段。99:1 回归
证明，即使总体精确动作准确率为 0.99，只要少数动作召回为 0 或 `hold` 无正样本，预检查仍
失败关闭。零检测观测也不提供 `hold` 正样本；有分配目标时其物理语义固定为
`reacquire` 和覆盖为 false。

2026-07-27 只完成软件实现和单元回归，D5 全量为
`744 passed, 2 warnings in 111.52s`。未运行新的正式 900-cell 训练，也未形成未见 seed
模型指标。旧 2026-07-20 指标继续作为当前证据。正式晋级还需要独立数据生产、冻结谱系、
至少 20 个明确未见且非 synthetic seed 的 A3/R0 成对非退化，以及每 episode 的安全、
可见率和重捕获无退化。模型始终不得读取在线 truth ID、改写 `global_track_id` 或自授
主动视觉、分配和控制权限。
...
保证同一次调用到达。旧实现只使用本次调用的批次，因此异步输入会退化成连续的单相机图，后续
几何门、规则评分器和图模型都没有可评分的跨相机节点对。

D5 现在保存每个相机流最近一次有效实测状态。快照键为资源和相机组成的局部命名空间，值包含
匿名局部航迹、量测时间、到达时间、像素协方差、相机位置与姿态、位置协方差和姿态协方差。
新批次到达后，本次实测相机作为图锚点；其他相机的快照只有在时间、外参、漏帧和有效期条件
同时成立时才参与当前图。缓存状态保持原量测，不在 D5 内做运动外推。

设当前锚点集合为 \(\mathcal A_k\)，某缓存相机状态为 \(s_i\)。其进入图的时间条件为

\[
\exists a\in\mathcal A_k:
|t_i^{m}-t_a^{m}|\leq \Delta t_m,\qquad
|t_i^{a}-t_a^{a}|\leq \Delta t_a ,
\]

其中 \(t^m\) 为量测时间，\(t^a\) 为到达时间。当前默认
\(\Delta t_m=0.35\,\mathrm{s}\)，\(\Delta t_a=1.0\,\mathrm{s}\)。相机外参时间与局部航迹量测
时间还必须满足既有外参年龄门。快照总寿命默认取量测时间窗、到达时间窗和外参年龄上限的最大值，
当前为 \(1.0\,\mathrm{s}\)。missed-frame 超过既有上限时删除相机状态。

这一级缓存只解决“异步状态能否进入同一张图”。它不保证节点之间一定形成边。候选边仍需通过
视场、时间、射线、重投影、像素马氏距离、协方差和中心投影等既有门控。真实短冒烟已经出现两个
相机节点和相机对索引，但缺少共同中心投影支持，候选边仍为零。该结果表明合图断点已经修复，
下一步应补齐真实输入的中心预测覆盖，而不应降低几何阈值。

快照状态有三个安全边界。第一，OOSM 只记录为忽略，不回写较新的相机状态。第二，缓存没有本次
实测锚点时不自行发布关联，避免历史状态循环形成新证据。第三，绑定仍只引用中心传入的
`global_track_id`；缓存、规则评分和图模型都没有创建、改写或换绑全局身份的权限。

缓存最多保留 256 个相机流。超过容量时按到达时间、量测时间和流键确定性淘汰最旧状态。
`reset_stream()` 清除指定流，`reset_episode()` 清除全部流。固定标量诊断记录当前批次、活跃
相机、复用航迹和各类排除数量，不输出相机或航迹业务 ID。

2026-07-26 单元回归证明异步同目标可形成 `2` 节点、`1` 边，异步不同目标为 `2` 节点、
`0` 边；D5 全量测试为 `598 passed, 1 warning in 97.36s`。等价 5v5 seed `1000`、
...
`3738444168138584c7ec3eb895d123178092176ec751a5b455e575b177a2d852`，覆盖 20 个未见 seed、
900 个 episode 和 45 个场景规模单元。在线真值使用、同相机互斥违规和中心全局航迹标识改写
均为零。
...
G1 v4 装配器才有资格尝试生成辅助模型 bundle。

第一阶段不相信报告中的 `pass` 字样。五份输入均由调用方给出冻结 SHA-256；held-out 和 paired
还要复算不含自身摘要字段的规范化内容哈希。装配器把 bundle、语料、held-out、paired 和逐帧
lineage 的哈希及数量逐项交叉核对，并确认所有 G1、辅助、默认路径和控制权限为 false。输出目录
已存在、输入在装配期间变化或清单缺项时，不发布 registry。

单特征捷径采用固定判据。最高单特征 AUC 达到或超过 0.995 时保留 blocker，低于该值时不沿用
旧报告结论。共享全局航迹计数只有在非零分层实际存在候选边且与标签近确定性时才形成独立
blocker。clean `d437744c...4ffb` 的 7fb5 输入 AUC 为 0.720073，非零共享计数分层为空。
因此只保留固定候选图未重建、D6 外部审计待完成和无在线权限三项限制。
...
旧冻结模型在合成保留集上存在两个直接问题。检测框尺度变化率差的单特征 AUC 为 0.997340，
遮挡重现代理的边/簇 F1 只有 0.563264/0.572845。新开发候选先在数据生成端为每个相机量测加入
独立、标签无关的尺度与运动误差，再在训练端把原图和三类困难特征视图共同优化。三类视图分别
模拟遮挡后重现、相似运动干扰和独立检测框尺度抖动。

困难视图的随机种子只由匿名 episode UID、视图名称和固定命名空间生成。变换函数不接收 truth
或边标签，不增加或删除候选边，也不修改原图。模型仍只学习已通过几何门的候选边概率，不能绕过
同相机互斥、受约束聚类或中心 Hungarian 绑定。
