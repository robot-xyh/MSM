...
逐帧业务及最终 binding 哈希与冻结记录一致，online truth use 和 `global_track_id` mutation 为 0。配对 10 秒平均单次成本中位数下降 `26.6%`，但归一化增长仍约 `2.37-2.45x`，所以“删除重复工作”已经实现，“200v200 规模线性/实时性”仍未证明。当前源码三种子系统复跑、D6 操作数聚合和正式计时准入继续开放。
