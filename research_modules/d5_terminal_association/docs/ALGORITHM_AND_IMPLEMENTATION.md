...
原无状态函数继续负责中心航迹投影、像素马氏门控、总代价计算和匈牙利选择。新增
`TemporalGeometricAssociator.associate()` 接收同一组 `GlobalTrack`、`LocalVisualTrack`、
`CameraModel`、量测时间和到达时间，并增加资源、相机和流标识。返回结果包含：

1. `instantaneous_result`：未经过时序迟滞的原始逐帧几何结果；
2. `measured_assignments`：本帧经迟滞确认的实测关系，方向仍为全局航迹到本地轨迹；
3. `active_bindings`：当前相机流中本地轨迹对中心航迹的只读引用；
4. `coasted_records`：预测中心、预测框、协方差和预测时长；
5. `binding_events` 与 `candidate_margins`：状态变化及其代价依据。

该包装层没有改变 `associate_tracks_to_detections_geometrically()` 的签名和返回值。main 可以先
记录瞬时结果，再使用 `measured_assignments` 作为稳定后的在线视觉证据。
...
当前实测帧
    ...
    |
    +-- 与原绑定一致 ----------------------------> continued
    |       `-- 0.25 秒内曾短时缺失 ------------> recovered
    |
    +-- 提出其他全局航迹
    |       +-- 单帧 ----------------------------> pending
    |       +-- 优势不足/成对交换/重复占用 ------> held
    |       `-- 连续两帧且优势达到门限 ----------> confirmed
    |
    +-- predicted/lost/本帧无量测 --------------> coast/reacquire/hold
    `-- 超过 0.25 秒或时戳回退 -----------------> expired/reset
```

预测采用常像面角速度。若最近实测框为 $(x_1,y_1,x_2,y_2)$，预测位移为
$(\Delta u,\Delta v)$，则四个边界同时加上该位移，框尺寸不外推。预测像素协方差在相机量测
协方差上增加 $(\sigma_{rate}\Delta t)^2 I$。结果元数据明确记录增长量、采用的速度标准差、
coast 上限和 `non_authorizing_evidence=true`。

挑战者间隔按同一本地轨迹比较：

$$
M=C_{incumbent}-C_{challenger}.
$$

$M$ 小于 `min_lock_margin` 时清除挑战计数并保持原绑定。满足门限后，第一个不同帧只记录
pending；第二个不同实测帧才确认。完全相同的 `frame_id` 不重复计数。若两个已绑定本地轨迹在
同一帧互换目标，二者均进入 held，避免交叉段按瞬时像素顺序交换身份。

### 清理与隔离

内部状态键为 `(resource_id, camera_id, stream_id, local_track_id)`。量测时间或到达时间回退会
清空对应相机流；本地轨迹带 reset 标志时只清理该轨迹；`reset()` 可按资源、相机和流筛选。
超过 0.25 秒未形成可连续使用的历史时输出 expired。所有事件只携带调用者提供的匿名本地编号
和中心航迹只读编号，不读取 `metadata` 中的 actor、object 或 truth 字段。

### 当前证据边界

2026-08-10 的专项测试共 13 项，D5 全量回归为
`904 passed, 1 warning in 146.08s`。该结果证明时序状态机、输出合同和失败关闭不变式通过
单元测试。main 已在 `long_range_cv_scan` 为每台相机建立 episode 独立状态，且只允许
`measured_assignments` 驱动发现、提示和完成；预测记录只进入日志。运行输出增加
`temporal_binding_events.csv` 和 `dropout_events.csv`，main-D6 联合专项为 `23 passed`。

上述结果证明接口已接线，不证明新算法已经通过真实 AirSim。冻结 seed `20260810` 的 D6
离线报告仍是旧 v2 基线，准确率 `0.9979317477`、短缺口 3、绑定切换 7、交叉窗口可评价
`3/31`，状态仍为 `fail_closed`。采用新接线的 v3 单 seed 和多 seed 统计仍待验证。

## 真实 AirSim 长距离旧 v2 运行链与当前失效点

main 在 ComputerVision Blocks 中重新执行原始尺度 `coverage_safe`。运行使用 seed
`20260810`、20 个目标、50 米/秒速度、20 秒观察窗口和 100 Hz 逻辑更新。中心相机距目标约
2.8-3.2 千米，拦截相机距目标群约 500 米。中心二维扫描
边界由带协方差的合成 `GlobalTrack` 计算，拦截相机按已确认航迹的偏航和俯仰提示依次观察。

在线观测和评分保持分流：

```text
合成 GlobalTrack + 协方差 -> 二维扫描边界 -> AirSim 匿名检测
    -> 像素中心反投影为 NED 世界视线 -> 世界视线速度预测
    -> 匿名轨迹一对一匹配 -> 几何绑定候选
    -> 离线真值侧车计算身份切换、短缺口、重发现和交叉窗口指标
```

第一轮直接在像素坐标中预测本地轨迹。二维覆盖达到 `20/20`、提示完成 `19/19`，但云台运动
改变了像素运动基准，产生 55 次身份切换。修正复跑把相机姿态纳入视线转换，匹配代价同时保留
检测框尺度及尺度变化。连续可见段和长期重发现分开计数，避免把超出保持窗口的重新出现都算成
在线身份切换。

取证复跑累计发现和连续确认均为 `20/20`，拦截提示为 `20/20`，观察和完成均为
`19/20`。1934 个可评分关联的准确率为 `0.9979317477`。连续可见段身份切换为 0，轨迹纯度和
连续性均为 1.0；短缺口中断为 3，长期重发现为 48。31 个预定交叉窗口只有 3 个具有足够的
双目标时间样本，可评价窗口均为身份切换 0、纯度和连续性 1.0。几何绑定另有 7 次切换。

当前门控不因指标改善而放宽。发现、确认、提示观察/完成比例、关联准确率和记录项均通过
相应子门。多目标跟踪要求连续可见段身份切换为 0、短缺口中断为 0、纯度不低于 0.95、连续性
不低于 0.90，并至少有一个可评价交叉窗口。实测短缺口中断为 3，这是专项总门失败关闭的直接
原因。7 次几何绑定切换作为 P1 记录。

在线检测和标注不含 actor 名、`object_id` 或真值身份，真值只用于离线混淆矩阵；在线真值使用和
`global_track_id` 改写均为 0。取证包保存 98 幅原图和 98 幅标注图，19/20 条全局航迹形成双相机
图像证据。中心航迹来自 `synthetic_d1_d2_fixture`。

2 倍网格诊断的双相机证据为 `20/20`，2319 条可评分关联准确率为 `0.9892194912`，几何
绑定切换为 42，短缺口仍为 3，总门仍失败。放大网格仅用于分析可见性与框重叠歧义，不能替代
原始尺度主场景。中心相机 3 千米原始图接近 AirSim 渲染/像素极限，`simGetDetections` 框不能作为
真实光电辨识证据。本轮没有修改 D5 模块算法代码，也没有运行图神经网络。真实传感器误差、更多
可评价交叉窗口、短漏检恢复、几何绑定稳定性和多 seed 继续为 P1。

## 不规则交叉目标窄视场扫描

独立入口 `scripts/run_ideal_irregular_crossing_demo.py` 在旧全时可见基线之外构造解析扫描场景。
默认 20 个质点按 10 对生成，每对有小幅相向横向速度，并用距离和高度错层保持三维分离。
中心 A 固定；B 与目标群质心保持 500 米后方偏置。动力学状态按 0.1 秒定义，扫描可见性在
0.01 秒时间轴上用常速度解析传播，图表和 GIF 按 0.1 秒抽样。

A 相机为 2600×2160、水平视场 0.621 度，B 相机为 1920×1080、水平视场 2.750979 度。
两者使用针孔投影。机械模式每个扫描采样移动 1.8 度；安全覆盖模式每个采样移动 0.4968 度，
相邻视场保留 20% 重叠。目标进入视场后，云台保持 5 个采样形成 0.05 秒确认窗口。B 只处理
A 已绑定的中心航迹提示，按最小转角顺序转向并确认。

确认窗口沿用最近五帧位置与帧间位移代价。每次事件对所有尚可用中心航迹和当前匿名轨迹构造
完整矩阵，再调用匈牙利算法求一对一绑定。扫描时间线、匿名像素观测、完整矩阵各项、选中项、
双时间戳和 `1e-6 I` 协方差均可审计；离线身份映射只在求解完成后评分。投影交叉定义为折线段
相交，或最小线段距离不超过图像宽度 2.5%，并同时记录对应目标的最小三维间距。

标准 seed 的径向跨度为 390.0 米，最小三维间距为 30.0 米，A/B 交叉或近距交叉为 10/6 对。
机械模式实际发现率为 0；安全覆盖模式在 1.61 秒完成 20/20 链。10 个 seed 的安全覆盖两级
准确率和完整链均为 1.0，所有身份合同违规计数为 0。该结果不含检测、位姿、时间和通信误差，
也没有执行图神经网络。

## 两级时间窗匈牙利配准

独立演示入口为 `scripts/run_ideal_registration_demo.py`。核心实现将在线帧和
`OfflineIdentityTruth` 分开返回。在线帧只含中心只读 `global_track_id`、三维状态、相机参数、
匿名本地编号、像素位置、双时间戳和协方差；两级求解函数不接受 truth 映射。离线评分器在
求解完成后单独读取 sidecar。

对全局航迹 `i` 和匿名轨迹 `j`，最近 `W=5` 帧代价为：

```text
C_ij = mean(||u_hat_i - u_j||^2 / 20^2)
     + 0.25 * mean(||delta_u_hat_i - delta_u_j||^2 / 10^2)
```

首帧只计算位置项；不足五帧时使用已有窗口。`build_temporal_cost_matrix()` 向量化生成位置项、
位移项和总成本，`solve_complete_assignment()` 调用
`scipy.optimize.linear_sum_assignment`。第一级的行是中心全局航迹，列是 A 匿名轨迹；第二级
的行仍保留同一中心全局航迹，列是 B 匿名轨迹，并在输出中带上第一级 A 绑定。两个阶段的
逐帧完整代价、候选排序和选中标志均写入 CSV。

目标状态使用现有 `integrate_point_masses()` 推进，相机使用现有针孔投影、北东地到光学坐标
旋转和协方差传播函数。A、B 本地编号各自随机置乱并在 episode 内稳定。`candidate_columns()`
只选择每行三个最低成本候选用于三层图显示，实际算法始终求解完整矩阵，不加载图神经网络
权重。

10-seed 验收覆盖 20 目标、76 个图像时刻和每级每帧 400 个候选。结果为两级及端到端准确率
1.0、`id_switch_count=0`、重复/未匹配为 0、完整关系链比例 1.0、全时段可见率 1.0、在线
truth 使用和全局编号改写为 0。无误差条件下正确项成本为 0，这不是后续真实链路门限；误差
校准必须重新统计正确项和最近错误项的间隔。
