...
实现采用四项等价收敛。第一，为主动视觉快照建立相机、投影证据和相机分配目标索引，替代循环内重复线性查找。第二，终端绑定复用稀疏图建图阶段已经计算的中心投影距离矩阵，避免对同一局部轨迹重复投影。第三，D2 航迹 DTO 按适配实际读取的状态、协方差、航迹时间戳、版本和 ID 生成内容指纹，任一变化即重建，episode reset 清空；相机观测双时间戳不进入该缓存。第四，匿名局部轨迹编号使用序号集合，避免重复字符串扫描。
