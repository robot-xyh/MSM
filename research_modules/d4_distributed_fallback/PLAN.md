...
4. 对 1-5 帧 `ambiguous/reacquire` 做同 plan/同 ID 的成对 dropout 验收：frame 1..3 只能保留 binding，不能单独授权 terminal PNG；frame 4+ 必须 `terminal_consistent=false`。friend/duplicate/resource/global-track/mismatch/stale-plan 在任意帧都必须立即 fail closed，并保持错误绑定为 0。
