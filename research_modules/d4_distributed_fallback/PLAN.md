...
- 只读审计正式 R0 的 28 个 `collecting_member_acks` 失败和同场景 72 个通过样本，
  逐项比较 D3/D4 计划身份、联盟 required/acked/missing 成员、ACK 到达时刻、epoch、
  lease、通信拒绝、pending/dropped 统计和 episode 终止时刻。
- 将 28 项归为 16 项 D3-D4 计划代次错位、11 项当前代次 ACK 晚到但未触发重评、
  1 项重发与 2 秒时域不足。没有发现成员集合变化、lease 过期、完整 ACK 被拒绝或
  D4 状态机再次执行后无法提交。
- 复核同场景通过样本，发现另有 35 个通过样本使用旧 D4 v1 提交状态对应最后 D3 v2。
  现有通过条件因此不能证明当前计划完成联盟仲裁。
- 冻结源码和当前源码的正常中心多成员逐 ACK 定向回归均为 4/4；当前 D4 全量回归为
  903/903。D4 代码未改，正式 R0 制品未改、未重跑。

### main 所属修复

1. D3 发布新计划后，必须基于该计划重新运行 D4，并要求最终 D3/D4 的
   `plan_id/plan_version/epoch` 一致。该项是当前跨模块 P0。
2. 当前计划仍有 `proposed/collecting_acks` 联盟时，有效 ACK 到达必须触发 D4 重评，
   不以中心失效为前提。
3. 对未确认成员增加有界定向重发；保持原 required 集合、epoch、lease、版本和成员
   身份，不允许以超时或部分 ACK 代替提交。
4. 在 episode 终止点增加有上限的通信/模块排空，或至少再运行一次模块栈；未闭合时
   输出明确的 `missing_required_acks`。
5. 保存逐消息 queued/dropped/delivered/pending/retry 记录，并由 D6 验证最后 D4
   决策绑定最后 D3 计划。

### 重跑边界

main 修复先复现 `00802`、`00815`、`00827` 和 `00881`，再检查本次识别的 63 个
受影响样本和 100 个 `high_threat_m_to_n` 样本。定向复现只能作为预检。发布顺序、
最终联盟状态和 D7 门控语义发生变化后，必须由新的 clean source 完整生成并独立审计
900 项 R0；不得拼接旧制品。
...
- 冻结 M16N20 来源数据和外部规则标签。32 个来源 episode、63 帧来自 seed 3008-3039；
  训练 0-99、正式 holdout 1000-1019、设计 pilot 3000-3007 与评价 seed 两两隔离。
- 新增 D4-owned 只读评价入口。它校验来源、标签、候选和配置哈希，只加载冻结 v4 actor
  与 v5 calibrator，不修改候选，不拟合模型，不调整固定 0.60 门或 split。
- 标签语义分层完成。63 帧均有离线确定性规则标签，其中 train/validation 各有 1 个经
  现有投影和干预不变量检查的安全转移动作。在线 D4 recommendation 不作为教师标签。
- 旧候选 TRAIN/VALIDATION 的 425 帧形成 251 个唯一可观测键；新数据 63 帧形成 41 个
  唯一键。精确键交集和记录重合数均为 0。
- 冻结 actor 在外部数据上产生 16 个相对 R0 的可执行差异，但没有一个与两个规则安全
  正动作签名一致。actor-derived 正类为 0，v5 得分 min/mean/max 均为 0，固定门通过
  0/63，负类误接收 0/63，规则回退 63/63。
- D4 本次读取 train/validation/test payload 为 43/10/10，所有 fit、selection 和
  threshold 调整计数为 0。main 先前只读检查 test 10 条的事实已单列。正式 holdout、
  pilot 和源在线 recommendation payload 读取均为 0。
- 评价前后 v4/v5 候选树摘要一致。持久化产物包含逐帧记录、输入完整性、可观测键重合、
  分 split 指标、制品哈希和中文报告；评价与既有 v5 专项合计 18/18、D4 全量
  843/843 通过。
- 评价输出路径现在拒绝等于或位于来源、标签、v4 候选、v5 候选任一冻结输入树之下，
  防止评价写出污染冻结输入。正常外部输出目录保持可用。
- D6 已完成独立只读复核。复算得到样本 43/10/10、规则安全正动作 1/1/0、
  actor-derived 正类 0/0/0、63 个得分均为 0、0.60 门通过 0、负类误接收 0、
  回退 63/63、旧/新唯一键 251/41 且重合 0、正式 holdout 读取 0，与 D4 一致。

### 当前 P1

1. **已部分关闭：来源独立输入不可用。** 当前外部输入与旧开发集精确可观测键零重合，
   seed、来源提交、M16N20 配置和数据哈希均已冻结。
2. **已部分关闭：来源独立负类拒绝不可评价。** 63 个 actor-derived 负类均未越过
   0.60 门，负类特异度为 1.0；这只覆盖本批外部负类。
3. **未关闭：来源独立正类召回。** 规则层有 2 个安全正动作，冻结 actor 没有输出匹配
   动作。按 v5 正类定义，positive denominator unavailable，不能把规则正动作数写成
   actor 正类召回。
4. **未关闭：候选准入。** D6 外部独立复核已完成，但 actor-derived 正类分母仍不可用。
   正式 holdout、runtime preflight、D3 successor、D7 权限、物理窗口和收益均未完成。
   v5 保持未注册、准入关闭和规则回退。

### 下一步

1. D6 对本批制品的独立重算已经完成。D4 不根据 test 或审计结果修改当前候选、数据
   split 或固定 0.60 门。
2. 当前冻结 v5 到此停止晋级。若后续另立候选，必须在不读取正式 holdout 的前提下，使
   冻结 actor 在新的来源独立 development 数据中产生可复核的 actor-derived 正类，再
   由独立审计判断正类分母是否足够。不得复制规则正类或降低 0.60 门。
3. 在正类分母和 main 单独授权均成立前，不运行正式 holdout、runtime
   preflight、D3 successor 或 D7 权限测试，不接入 AirSim 和生产总线。
...
- v3 已从 clean commit `4ba2c8a...4114` 构建、review 并逐字节登记；固定 manifest、
  模型和 gate 为 `7978aec0...ada2`、`ace5df6d...7f52d`、
  `77972834...6872`，全部权限 false。
- main 从 clean commit `83b8869...0226` 完成 5v5/2-region seed 2000、
  20v20/8-region seed 2001 和 200v200/8-region seed 2002 的单 seed development
  preflight。每组 2.2 秒、3 帧。
- 验收条件固定为帧数至少 2、分布内比例至少 0.80、模型评价至少 1 帧。20v20 和
  200v200 均为 3/3 分布内、3 次原始推理、3 次门应用、3 次动作一致和 3 次候选许可，
...
- 冻结 current-lineage 候选的只读加载、seed 注册、逐帧影子推理、确定性投影、内容寻址
  verifier 和逐特征 OOD 诊断。
- 将冻结候选的 8 个原始文件逐字节登记到受控 `model_registry/`，并通过真实登记路径加载
  测试。clean clone 不再依赖被忽略的 `outputs/`。
- 拒绝 train/validation/test/calibration/reserved seed 重叠、旧 registry、重复帧、
  时间/计划/epoch 回退、身份变化、非有限输出和制品篡改。
- main 完成 5v5/2 区域 3 帧与 200v200/8 区域 2 帧预检。5/5 均 OOD，模型执行 0，
  在线真值使用 0。

### 阻断结论

当前候选只完成可信加载和影子适配，不具运行分布兼容性。正式 20-seed 不得启动，不能放宽
5% OOD 余量，也不能把影子投影记为实际采用。900-episode 运行数据与 100-episode 动作
课程的 union 可覆盖默认 8 区域主要特征，但冻结候选只训练了动作课程。

### 下一候选

1. 以 `(source scenario, numeric seed)` 登记两套来源，再对数字 seed 0-99 做统一
   60/20/20 原子分割，不采用各自原 split。
2. 清单分别绑定运行数据集 `b06d741b...6158` 和课程数据集 `7e17aba7...2d7f0` 的
   SHA-256、特征范围及动作库存；seed 1000-1019 完全排除。
3. 适用域先限定 8 区域和已覆盖转移几何。2 区域边距离仍 OOD，继续规则回退。
4. D4 代码提交后在独立 clean checkout 构建新 development/shadow 候选，并先运行非正式
   seed 兼容性预检。出现真实模型动作后，main 才注册正式 20-seed。

本轮不重训、不改门限、不生成新候选权重。
验证日期 2026-07-28：专项 **17/17**、D4 全量 **706/706** 通过。
