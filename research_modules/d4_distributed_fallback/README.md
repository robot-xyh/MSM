...
- 二级节点生命周期摘要：`SecondaryNodeLifecycleSummary` 输出 `heartbeat`、`lease_epoch`、`coverage_cell`、`video_cue_freshness_s`、`cue_freshness_s`、`gimbal_pointing_ok`、`secondary_coverage_ratio`、`cross_view_support_count`、`secondary_capability_class`、`link_stale` 和 `secondary_available`，供 D4 仲裁与 D6 日志审计。
...
- D6 兼容事件：`D4ArbitrationAdapter` 输出 `EventRecord` kwargs，metadata 含 `degradation_mode`、`selected_coordinator`、`coverage_cell`、`trigger_reason`、`trigger_timestamp`、`decision_timestamp`、`review_label`、secondary takeover plan lifecycle、`active_plan_owner` 和 D5 二级覆盖/未配准诊断字段，并保留 `d4_degradation_mode` 兼容 D4 原始枚举。
...
- 机动高空侦察节点随拦截机出动但不拦截；它用 D1/D2 `GlobalTrack` 或雷达 cue 指向目标簇，正常时给局部拦截群提供图像/cross-view 证据，中心失效或主动降级硬条件满足时才可作为二级协调节点。仅有侦察图像、云台指向正常或 coverage ratio > 0 不会自动改变 action。
...
- 剩余 P1：真实 AirSim/Blocks D4/D5 stress 多 seed 校准，机动高空侦察节点 heartbeat/link/cue freshness、gimbal pointing、coverage ratio、plan activation delay、D5 peer evidence 合流和 D6 长期聚合口径。
