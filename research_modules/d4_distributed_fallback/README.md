...
**2026-07-21 保留 seed 同配对干预合同**：新增 `region_resource_paired_intervention.py` 和只读验证 CLI，冻结 `d4-region-resource-paired-intervention-spec-v1`、arm evidence v1 与 manifest v1。规范严格限定 seed 1000-1019；每个 seed 只能有一个确定性规则 control 和一个隔离 candidate treatment，两个 arm 必须重复绑定相同 scenario/version、配置、初始状态、通信 schedule、故障 schedule 和区域快照 lineage SHA256，同时绑定候选 bundle/model、推理阈值、确定性 projector、区域 authority、coalition/epoch/lease/ACK fence、运行确认和奖励证据 schema。treatment 候选只有在 bundle/阈值/数值检查通过，并经 owner/version/epoch/lease、fault fence、联盟 ACK、邻接、容量、备用和资源守恒投影后，才可标记为“隔离 arm 下一周期安全采用”；任何失败均保持或回退确定性规则。该标记不是 `runtime_advisory_applied_ack`，在线 `PPO/assist/authority=false`，`rule_fallback=true`。manifest 拒绝缺 arm、跨 arm 实际快照不同、输入或 schedule 不同、哈希篡改、真值 key 和非有限值。新增专项 **20/20 passed**，相关安全回归 **132/132 passed**，D4 全量 **469/469 passed**。本轮没有运行正式 20-seed episode，也没有接入 D6 outcome sidecar；observed outcome、paired non-degradation、counterfactual、causal 和性能声明均为 unavailable。
...
- `d4_distributed_fallback/region_resource_paired_intervention.py`：冻结保留 seed 的 control/treatment 同输入合同、隔离 arm 安全采用证据和完整 manifest；复用规则策略、确定性投影、运行确认/奖励 schema 与 paired evaluator，但不生成线上 ACK 或结果标签。
...
- 2026-07-21 全样本准入阶段为 397/397 项通过，新增专项 10/10；加入运行时确认和区域奖励合同后，当前 D4 全量为 469/469。正式和补充数据的模块内准入状态为 complete，但 D6 外部带外 SHA256 复核、真实运行时 ACK/outcome、可归因 reward、同 seed paired shadow、真实链路、误降级率、恢复时间和物理任务连续性仍开放。机器可读准入固定禁止把规则教师标签、后投影 recommendation 或低损失写成运行策略能力、applied ACK 或 assist 资格。
