...
- D4 模块内已闭合的 P1：连续 readiness、逐决策注册证据审计、pending/active transition timing、source/lease epoch/lease expiry strictness、D2 online truth 指标可用性门控，以及 heartbeat/link/cue/gimbal/能力回落到 distributed 的负例；边沿初始化和 truth-unavailable 回归已补齐，当前模块测试为 95 项通过。
