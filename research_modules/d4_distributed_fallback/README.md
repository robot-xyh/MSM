...
- `d4_distributed_fallback/region_resource.py`：truth-free 区域资源快照、规则建议、确定性安全投影、下一周期 advisory contract/一次性消费门、reward、整 scenario/seed 划分和 paired shadow 指标。
...
- 2026-07-20 当前 D4 全量测试为 350/350 项通过；其中 23 项区域 authority 合同测试覆盖五档 region/task/resource 元数据及原子/fencing 边界，原 32 项区域资源建议/学习管线测试覆盖 3/5/8/32 变长图、守恒、断边/分区、中心/多二级/distributed owner、旧 epoch/过期 lease/缺 ACK/fault fence、BC、PPO、bundle SHA、OOD/timeout/低置信/非有限回退和 shadow 不改正式 verdict；新增 15 个消费合同 case 覆盖 ID/有效期/序列化、重复消费、旧 snapshot/plan/epoch、非 projected、守恒、未知/非邻接 transfer、edge partition/unavailable、ACK/fault 和 `k>1` committed protection。2026-07-15 的 280/280、区域合同阶段 303/303、建议管线阶段 335/335 和更早 278/278 保留为历史证据。新增 15 项是纯 Python 接口/合同测试；未运行新 AirSim episode，也未形成正式多 seed 证据，真实链路、学习模型 20 个未见 seed 验收、误降级率、恢复时间和物理任务连续性仍开放。
