...
**2026-07-21 保留 seed 同配对干预合同**：`region_resource_paired_intervention.py` 已形成 specification、arm evidence、manifest、只读验证 CLI，以及面向冻结 development bundle 的隔离候选加载器和配对执行器。规范严格限定 seed 1000-1019；每个 seed 只能有一个确定性规则 control 和一个隔离 candidate treatment，两个 arm 必须重复绑定相同 scenario/version、配置、初始状态、通信 schedule、故障 schedule 和区域快照 lineage SHA256。加载器只接受 `region_resource_bc_900_20260720/bundle` 的冻结身份，逐次核对 manifest `dad2adbe...c05c9`、权重 `3da0360b...d5f62` 和训练清单 `ff3081c8...30dc6`，并复核 development/shadow-only 准入字段；载入或推理期间任一文件变化、分布外、超时、低置信、非有限输出或投影异常都记录稳定拒绝原因并执行规则回退。原始学习候选仍须通过 owner/version/epoch/lease、fault fence、联盟 ACK、邻接、容量、备用和资源守恒投影，才能标记为“隔离 arm 下一周期安全采用”。该标记不是 `runtime_advisory_applied_ack`，`PPO/assist/authority=false`，`rule_fallback=true`。专项现为 **26/26 passed**，D4 全量 **475/475 passed**；实际冻结文件读取前后 SHA 不变。本轮没有运行 20-seed 正式 episode，也没有接入 D6 outcome sidecar；observed outcome、paired non-degradation、counterfactual、causal 和性能声明仍为 unavailable。
...
- 2026-07-21 全样本准入阶段为 397/397 项通过，新增专项 10/10；加入运行时确认、区域奖励合同和冻结 bundle 隔离加载回归后，当前 D4 全量为 475/475。正式和补充数据的模块内准入状态为 complete，但 D6 外部带外 SHA256 复核、真实运行时 ACK/outcome、可归因 reward、20-seed 同 seed paired outcome、真实链路、误降级率、恢复时间和物理任务连续性仍开放。机器可读准入固定禁止把规则教师标签、后投影 recommendation、隔离采用或低损失写成运行策略能力、applied ACK 或 assist 资格。
