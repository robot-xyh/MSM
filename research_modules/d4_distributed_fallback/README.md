...
当前权威输入为 `research_modules/scalable_3d_simulation/outputs/reserved_seed_interventions_nominal_5v5_1000_1019_formal_7891296`，源提交 `78912963b67fe86ee9a8d29186b18a9dd60c460c`，`SHA256SUMS` 文件 SHA256 为 `821f15035e628d8db86f13c22d93f8e05142c5f00aae9118974a74bdc98b72bc`，manifest SHA256 为 `d6ef23b28add92e9a24a185ea72a7275e341bd796a2e11930c4d5f46b19a883c`。D4 独立只读复核确认 20/20 source clean 且 finite、在线 truth 使用数为 0，40/40 arm 均为 evidence v2，20/20 treatment candidate 被评估。confidence min/mean/max=`0.508892953/0.563426384/0.569492280`，在保持不变的 `minimum_confidence=0.6` 下通过 0/20；OOD、latency、finite、failure gate 各通过 20/20。v2 candidate latency min/mean/P95/max=`1.103510/1.977479/2.264415/2.703312 ms`，20/20 通过 `50 ms` 上限。safe adopted 为 0/20，规则回退 20/20；明确 `candidate_low_confidence` 和兼容 generic reason 均各出现 20 次。旧 v1 latency 只属于历史运行，不纳入或覆盖本次 v2 统计。bundle manifest 仍声明 `confidence_head_uncalibrated`；`formal_twenty_seed_performance_completed=false`，`PPO/assist/authority=false`、`rule_fallback=true`，因此该 nominal 5v5 结果只支持独立校准/重训计划，不证明候选有效或降级策略效果。配对专项 **33/33 passed**，D4 全量 **482/482 passed**；D6 outcome sidecar、paired non-degradation、counterfactual 和 causal 继续 unavailable。
...
- 2026-07-21 全样本准入阶段为 397/397 项通过，新增专项 10/10；加入运行时确认、区域奖励合同、冻结 bundle 隔离加载和候选门诊断回归后，当前 D4 全量为 482/482。正式和补充数据的模块内准入状态为 complete，但 D6 外部带外 SHA256 复核、真实运行时 ACK/outcome、可归因 reward、20-seed 同 seed paired outcome、真实链路、误降级率、恢复时间和物理任务连续性仍开放。机器可读准入固定禁止把规则教师标签、后投影 recommendation、隔离采用或低损失写成运行策略能力、applied ACK 或 assist 资格。
