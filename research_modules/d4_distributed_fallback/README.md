...
**2026-07-21 保留 seed 配对候选门诊断**：`RegionResourcePairedArmEvidence` 已升级为 `d4-region-resource-paired-arm-evidence-v2`。新证据除 aggregate `candidate_thresholds_passed` 外，还持久化 candidate confidence、冻结的 `minimum_confidence`、OOD 状态、candidate latency 与 latency limit、finite 状态，以及 confidence/OOD/latency/finite/external-failure 五项 gate 结果。executor 对已考虑候选至少输出 `candidate_low_confidence`、`candidate_ood_rejected`、`candidate_inference_timeout`、`candidate_output_nonfinite` 中对应的明确拒绝码；旧 `candidate_threshold_or_finite_gate_rejected` 仅作为兼容汇总码保留，不能单独解释拒绝。v1 reader 先按旧字段集合和旧 manifest content ID 验证，再迁移为 v2 且令新增诊断显式 unavailable；冻结 bundle、权重、manifest 和正式 v1 artifact 均未改写。

权威输入为 `research_modules/scalable_3d_simulation/outputs/reserved_seed_interventions_nominal_5v5_1000_1019_formal_6d5bfea`，场景为 nominal 5v5、seed 1000-1019、20 个 treatment record。2026-07-21 main 只读复算及 D4 不写盘诊断得到 confidence min/mean/max=`0.508893/0.563426/0.569492`，OOD 20/20、finite 20/20 通过；正式 v1 记录 latency P95/max=`35.608/42.302 ms`，20/20 小于等于冻结上限 `50 ms`。由于 20/20 confidence 均低于未改动的 `minimum_confidence=0.6`，明确诊断均为 `candidate_low_confidence`，全部继续规则回退；这不把候选重分类为有效。bundle manifest 仍声明 `confidence_head_uncalibrated`，后续只能在不使用保留 seed 调阈值的独立校准集上评估 reliability/ECE/Brier、校准或重训 confidence head，再以同一 0.6 门复验。`PPO/assist/authority=false`、`rule_fallback=true` 不变。配对专项 **33/33 passed**，D4 全量 **482/482 passed**；正式记录仍无 D6 outcome sidecar，paired non-degradation、counterfactual、causal 和性能优越性声明继续 unavailable。
