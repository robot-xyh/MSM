...
- 2026-07-21 全样本准入阶段为 397/397 项通过，新增专项 10/10；加入运行时确认和区域奖励合同后，当前 D4 全量为 449/449。正式和补充数据的模块内准入状态为 complete，但 D6 外部带外 SHA256 复核、真实运行时 ACK/outcome、可归因 reward、同 seed paired shadow、真实链路、误降级率、恢复时间和物理任务连续性仍开放。机器可读准入固定禁止把规则教师标签、后投影 recommendation 或低损失写成运行策略能力、applied ACK 或 assist 资格。
