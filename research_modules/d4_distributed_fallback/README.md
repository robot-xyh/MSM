...
- 当前 D4 全量测试为 198 项通过。除两个批量 replay 外，逐 tick episode adapter 和七场景验收矩阵已关闭 main 接入前的 D4 状态接口缺口；真实 AirSim 多 seed 的链路注入、secondary-interceptor/peer split、误降级率、恢复时间和物理任务连续性仍由 main/D6 开放。
