...
- 证据传递审计：D5/D6 episode 汇总已有 stable registration/not-registered 计数，但本批 1300 条 D4 capability input 中这两个字段均为 `null`；D4 通过 `cross_view_association_count=2..5` 判定 registered。该回退路径有效，但 main/D5 仍应把逐决策 stable/not-registered 摘要送入 D4，便于区分“瞬时 association”与“稳定注册”。
- 保持不变：轻量 CBBA 仍是完全无中心保底基线；未接入 MIT CBBA、CA-CBBA、独立 auction 或 contract-net。
- 剩余 P1：校准二级 network full-view 的时间稳定窗口与 coverage-cell 聚合语义，补齐逐决策 stable/not-registered 摘要，验证 `takeover_ready -> pending_secondary_plan -> secondary_plan_active` 的持续闭环和 plan/lease 有效性，并统计 activation delay、回落原因和恢复双轨窗口；继续覆盖 heartbeat/link/cue 退化、center-secondary/secondary-interceptor/peer split 网络分区、D5 peer evidence 合流和二级接管必要性。误降级标定采用同 seed 正常/故障成对场景，统计 false/missed degradation、动作混淆和 dwell/release 抖动，不通过降低 coverage/full-view/readiness 门限改善表面接管率。当前 D4 字段已足以表达 readiness，剩余工作不是 D4 视觉注册实现。
