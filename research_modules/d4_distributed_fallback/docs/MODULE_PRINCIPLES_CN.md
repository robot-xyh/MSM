...
readiness v3 的适用域是 8-region 区域图。clean development preflight 使用至少 2 帧、
分布内比例至少 0.80、至少 1 帧模型评价作为兼容性门。20v20 seed 2001 和 200v200
seed 2002 各产生 3 帧；两组均为 3/3 分布内、3/3 原始推理、3/3 运行门应用、
3/3 动作一致和 3/3 候选许可，规则回退为 0。该结果闭合单 seed 8-region 运行兼容性。
...
开始候选与同键规则基线的独立 episode 比较。当前没有多 seed 非退化、时延、收益、实际
接管或正式 holdout 证据，全部权限保持 false。
...
但不能从 6 个正例 frame 推导多 seed 稳定性、收益或正式运行准入；全部权限仍为 false。
