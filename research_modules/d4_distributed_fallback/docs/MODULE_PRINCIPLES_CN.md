...
在线帧只从真实 `RegionResourceSnapshot` 及同快照算法结果生成。构造器重新执行确定性
规则策略得到 R0，再重新投影匿名候选，并与 main 提交的实际结果逐字段比较。冻结 recipe
只在离线侧规定应采集的拓扑、通信、供需、类别和资源数；它不能向在线侧注入动作。场景
名称不参与分类，因此把场景命名为 `safe_forward_transfer` 不能形成正类。

### 拓扑与状态

快照中区域 tuple 的位置固定为匿名区域索引。真实边根据方向属性展开，再与冻结规范边表
比较。8/12 区域环的每个相邻关系必须同时形成正、反两个有向边；3x3 网格和 16 区域网状
...
正类要求实际 projected transfer 非空、方向正确、总资源数与 recipe 相等且无拒绝。困难
负类要求指定数量的匿名候选真实存在、实际投影为空，并由通信分区/不可用、容量、机动、
源区余量或权属代次事实形成拒绝原因。projector 对资源数的部分裁剪无法满足 v8 候选与
投影 action key 完全相同的约束，因此失败关闭。

在线来源中出现对象真值、`global_track_id` 或离线标签键会在 DTO 转换前拒绝。输出仍使用
全 false 权限。该构造器只为 writer 准备 TRAIN 证据，不参与区域分配、主动/被动降级、
联盟提交或控制。专项 9/9、D4 全量 964/964 通过；main 接线与 324 个真实 episode 仍待完成。

## A2 v8 数据集写出原则（2026-08-01）

### 生成与写出分离

main producer 负责产生区域拓扑、通信变化、供需状态、确定性 R0 和独立离线标签。D4
writer 只负责验证和封装这些结果。该边界防止 writer 根据目标类别修补在线帧，也防止把
单元测试构造误当成真实场景生成。构造时固定的 clean commit、场景版本和配置摘要会在每个
episode 再核对；脏工作树或不同来源元数据不能进入同一数据集。

### 顺序与内容寻址

冻结 registry 的第 (i) 项是 writer 唯一允许接收的下一项。提交 seed、拓扑、供需、
通信或目标类别与该项不一致时，既有严格 loader 拒绝；跳号、重复 episode、重复 seed、
重复 frame 和不连续 frame index 由 writer 先行拒绝。在线帧和离线标签逐帧通过在线规范
内容 SHA-256 绑定，文件再由 SHA-256 写入 manifest。三层绑定分别约束记录、文件和完整
episode inventory。

schedule 同时原样保存 topology、communication condition、requested target class、正类
转移数或困难负类候选数以及 seed/episode 映射。通信条件由帧内逐边状态和先分区后恢复的
时序验证，转移类别与数量由离线标签和 projected transfer 验证。来源场景名称不参与这些
判据，不能用名称声明代替真实处理证据。

### 两阶段收口

stage 阶段只在 dataset 目标目录同级的隐藏暂存目录保存 `online/` 与 `labels/` 文件。
324 项全部完成后才构造 generation schedule 和 dataset manifest。finalizer 先使用暂存
目录和临时 schedule 调用完整严格 loader，再发布 dataset 与外部 schedule，随后从最终
路径再次加载。dataset root 最终只含 manifest 和 324 对数据文件，诊断日志不得混入。

这一接口关闭模块 writer 缺口，不关闭 main recipe adapter。当前没有 324 个真实 episode，
没有训练、模型或运行准入；validation/test、训练/注册/运行计数和全部权限保持为空或零。
受控专项 8/8、D4 全量 955/955 只证明合同写出与读取闭环。
...
区域转移学习失败要区分“动作结果”“流水线阶段”和“特征原因”。动作结果直接比较同帧
确定性 R0、候选原始转移和安全投影后转移。流水线阶段再判断失败发生在激活、边选择、
资源数解码、投影或不变量检查。特征原因需要逐区域供需、完整有向拓扑、节点/边特征和
通信状态。前两层可由冻结逐帧记录复算，第三层缺字段时必须保持 unavailable。

冻结 v7 外部评价共有 45 个行为失败帧。42 个规则正类帧没有原始 actor 激活，属于激活
阶段未响应；3 个规则负类帧选择错误边，并在不违反可执行约束的情况下通过投影，属于边
选择错误。阶段级归因为 45/45。记录只保存在线可观测键摘要，没有保存形成该摘要的图
特征；供需差、完整邻接关系和通信状态无法复原，因此特征级归因为 0/45。

确定性投影解决容量、配额、owner、版本、epoch、lease 和安全不变量，不承担学习语义
正确性判断。一条错误边可能满足所有可执行约束并通过投影。v7 的三次虚假转移没有触发
投影拒绝或不变量失败，这说明安全外壳按原职责工作，也说明后续数据必须增加困难负类和
边方向监督，不能把零安全违规解释为调度决策正确。

### 来源独立性

seed 不重合只证明数值来源隔离。特征独立还需比较不含 seed、episode、标签和真值的在线
图可观测键。冻结 v4 TRAIN+VALIDATION 与 v7 外部数据的精确键交集为 0；v7 训练来源 B
完整特征没有进入原评价器，因此全部训练来源的精确重合状态仍不可用。D6 的独立低层重算
确认同一冻结输入可复现，不补足缺失的训练来源特征。

### v8 请求原则

v8 先冻结数据需求，再生成训练数据。请求覆盖 8、9、12、16 区域有向拓扑；每种拓扑
组合三类供需、三类通信和安全正向、安全反向、困难无转移负类。正向和反向由拓扑中预先
定义的有向边方向确定，不能由 `region-001` 与 `region-002` 的数字大小解释。困难负类
应包含高转移评分但无安全动作、错误方向、错误边、源区余量不足、旧 owner/version/
epoch/lease 和通信分区等情况。每个组合的三个重复分别指定 1、2、3 个资源。正类要求
对应数量形成安全转移；困难负类保留同样数量的候选资源，但投影后标签必须严格无转移。

新来源在线载荷必须携带双时间戳、逐区域供需、完整有向边、通信状态和完整安全外壳过程
字段。真值只允许在隔离的离线标签中出现；在线训练视图和运行视图不得含 truth、actor 或
object identity。v8 训练、置信校准、注册和运行时接线属于后续独立阶段，本轮权限全部
关闭。
...
其中 \(e\) 为中心权威 epoch，\(t_l\) 为计划租约到期时刻，\(e_r\) 为区域最大
epoch，\(t_{r,l}\) 为区域最早租约到期时刻。main 现在从 D3 首次权威计划直接发布
这四项。D4 不再需要从计划版本推测 epoch，也不能用后到诊断消息延长租约。

同一 `plan_id/version` 的 no-op 评价重评必须满足 \(G'=G\)。它可以记录建议被拒绝、
无后继等诊断，但不构成新计划、接管或联盟采用。执行语义或代次变化必须产生新的计划
身份并重新经过 D4 的版本、epoch、lease、摘要和成员确认门。

2026-07-31 的真实 main 5v5 集成专项验证了四项逐值保持、单次权威发布和无 adoption
ACK。该测试确认接口语义，没有改变 D4 原子提交条件，也没有替代新的多 seed 独立
审计。

## 2026-07-30 高威胁开发批次 v4 原理复核

D4 的联盟生命周期以当前 D3 计划为边界，不以单帧 D2 航迹是否出现为边界。当前计划
仍有效时，main 可向 D4 提供计划期保存的最后可信航迹均值、协方差和更新时间。D4
继续保留任务、区域归属和联盟确认状态，同时以增大的量测年龄表达证据陈旧。新计划、
显式撤销或 lease 到期后，旧计划证据不再具有执行权限。

计划证据延续不等于身份证据延续。D4 负责确认当前计划下哪些成员共同提交，D7 仍需
核对 D2 当前身份承诺和 D3 绑定。D2 临时缺轨时，D4 可以维持已提交联盟的因果记录，
D7 必须 hold。该分工避免任务静默消失，也防止仅凭历史位置继续导引。

v4 的 100 项开发预检中，28 个 episode 触发过计划期航迹 fallback，全部保持当前计划
对齐和联盟执行闭合。三个 v3 原失败样本均闭合，权威计划摘要冲突为零。D4 的原子
提交、不完整 ACK 失败关闭和严格内容摘要校验没有变化。

D6 独立审计确认计划 ID/版本对齐 100/100、644 个当前多成员联盟目标闭合，195838 条
通信处置在 100/100 episode 中 available/verified。D3 区域 epoch 与 lease 对照字段
均为 0/100 available，当前只能保留为开放 P1，不能由计划 ID/版本结果代替。该证据
属于 dirty development，formal R0 未运行。

## 2026-07-30 高威胁开发批次原理复核

以下内容记录 v3 的历史根因，现由 v4 开发证据完成闭环。

最新 100 项开发批次继续支持原子提交判据。seed 1010 和 1013 在必要成员 ACK 齐全后
均转为 `committed`；seed 1017 在同一计划身份出现不同载荷摘要时拒绝旧摘要 ACK。
因此 D4 不应放宽计划摘要、版本、epoch 或 lease 约束。

三个终态未闭合均发生在当前计划目标从 D2 输出消失之后。D4 快照不再包含对应 task，
而 D3 当前计划继续要求该联盟。该现象属于快照任务覆盖和计划生命周期合同，不改变
D4 联盟状态机原理。当前应通过显式 hold/tombstone、计划撤销或新版本重规划处理，
不能把临时缺轨解释为联盟自然取消。

## 2026-07-30 正式 R0 联盟确认原理复核

多成员联盟的执行许可仍采用原子提交。设必要成员集合为 \(R\)，当前有效确认为 \(A\)，
计划身份为 \(p=(plan\_id,version,epoch)\)，租约截止时刻为 \(t_l\)。只有

\[
R\subseteq A,\qquad p_{ack}=p_{plan}=p_{d4},\qquad t<t_l
\]

同时成立，联盟才可从 `collecting_acks` 转为 `committed`。缺任一必要确认、旧 epoch、
旧版本、错误成员或过期 lease 时保持失败关闭。`collecting_acks` 表示当前快照证据
尚未闭合，不能解释为成功；有效 ACK 到达后仍需再次执行状态机才能形成新提交决策。

正式 R0 的 28 个失败没有违反上述判据。16 个 episode 的最后 D3 计划与最后 D4 决策
属于不同代次；11 个 episode 的 ACK 在最后一次 D4 计算后到达但没有触发新快照；1 个
episode 的确认在丢包、重发和 2 秒终止窗口内未闭合。全部未提交联盟的 required 集合
保持稳定，lease 到期时间为 5.7-5.9 秒，晚于 2 秒终止点。对齐通过样本中，完整 ACK
在最后 D4 决策前到达后均能正常提交。

因此 D4 原子提交算法不需要放宽。修复对象是 main 的计划发布顺序、ACK 事件重评、
有界重发和终止排空。修复后仍必须逐项校验必要成员、计划、epoch、lease 和身份。
本轮诊断完成，代码未变，正式结果未重跑。

## 2026-07-30 v7 来源独立评价原理

### 冻结评价

来源独立评价把模型构建和模型评价分开。候选参数、训练审计、来源绑定和推理阈值在评价
前冻结。外部 train、validation、test 都只用于计算指标，不参与参数拟合、checkpoint
选择、阈值调整或置信校准。评价器对候选、原始来源、标签导出、数据集和冻结 v4 来源
分别计算树摘要，并在评价后重新计算。任一摘要变化都终止评价。

每帧先用同一快照重新计算确定性规则基线 \(R_0\)，再运行冻结 v7：

\[
d_\theta(G)=\{\hat g,\hat e,\hat q\},\qquad
\hat R=\operatorname{Project}(R_0\oplus d_\theta).
\]

\(\hat g\) 是 actor 原始残差激活，\(\hat e\) 是有向边，\(\hat q\) 是资源数。
“原始激活”只说明网络选择了边；“实际转移变化”要求 raw transfer 相对 R0 的边或数量
确实改变。资源数预测为 0 时，两者可能不同，因此评价同时记录两个指标。

### 动作判定

规则正类表示目标安全动作的可执行签名不同于 R0。精确正动作要求投影后签名与目标完全
一致，同时满足以下条件：

\[
\mathrm{ExactPositive}=
\mathrm{TargetDiff}\land
(\hat S=S_{target})\land
\mathrm{InvariantOK}\land
\neg\mathrm{ProjectionReject}\land
\mathrm{NodeTuplePreserved}.
\]

规则负类的正确结果是投影后完整回到 R0。方向错误、边错误和资源数量错误分开统计。
raw 节点动作必须逐字段继承 R0，比较范围覆盖 `RegionResourceAction` 的全部字段，包括
资源配额增量和 reasons。投影器为满足转移守恒而改变配额属于 projected action，另行
记录，不算学习模型改写节点。

actor-derived 正类分母只统计相对 R0 形成可执行差异、通过不变量、没有投影拒绝且完整
继承 R0 raw 节点动作的帧。分母为 0 时比率记为 `unavailable`，不能用 0 代替。

### 评价结果

新来源包含 128 帧和 42 个规则正类。train 的 actor 原始激活为 10，实际转移变化为 3，
但 3 帧均为规则负类，exact 正动作为 0/24。validation 和 test 的原始激活、实际转移
变化及 exact 正动作均为 0；负类完整保持 R0 为 11/11 和 9/9。三个划分均没有投影拒绝、
不变量失败或 R0 raw 节点字段偏差。

该结果说明安全外壳继续有效，学习残差没有在独立 validation/test 上形成正类行为。
评价失败关闭。候选不进入置信校准、注册、准入、分配、降级、联盟或控制路径，在线继续
使用确定性 R0。

## 2026-07-30 v7 规则节点与转移残差原理

### 动作分解

对区域快照 \(G\)，确定性规则策略先生成 R0：

\[
R_0(G)=\{a_i^0,\tau_e^0\},
\]

其中 \(a_i^0\) 是完整 `RegionResourceAction`，包含 `resource_quota_delta`、储备
比例、侦察优先级、hold、request-replan、owner、plan version、epoch、lease 和
reasons；\(\tau_e^0\) 是 R0 在有向边 \(e\) 上的转移资源数。v7 不学习
\(a_i^0\)，raw action tuple 直接复用 R0，只学习残差激活 \(g(G)\)、有向边得分
\(s_e(G)\) 和资源数 \(q_e(G)\)。

帧门关闭时，组合动作保持 R0：

\[
\tau_e=\tau_e^0,\qquad g(G)<0.
\]

帧门打开时，只选得分最高的一条有向边 \(e^\*\)，用预测的绝对资源数覆盖该边：

\[
e^\*=\arg\max_e s_e(G),\qquad
\tau_{e^\*}=\operatorname{round}(q_{e^\*}),\qquad
\tau_e=\tau_e^0\;(e\ne e^\*).
\]

资源数为 0 时表示删除 R0 在该边上的转移。该表示可同时表达“R0 无转移时增加一条边”
和“R0 已转移 3 个资源时改为 1 个”。节点 action 在投影前逐字段复制 R0；投影器根据
最终转移重新计算资源配额增量，保证总量守恒。

### 分头监督

边残差标签定义为目标转移数与 R0 转移数不同：

\[
y_e=\mathbf{1}(\tau_e^{t}\ne\tau_e^0).
\]

两个开发来源的正样本每帧均只有一条残差边。总损失只包含转移残差：
...
\(L_{\mathrm{edge\_act}}\) 对所有有向边做显式二分类；\(L_{\mathrm{rank}}\) 要求正边得分
高于同帧其他边 0.5；\(L_{\mathrm{count}}\) 只在正残差边比较资源数。正帧和负帧使用
独立帧门损失。负帧损失直接推动 \(g(G)<0\)，避免边级正样本权重把整帧全部激活。
正负帧、正零边和两个来源的权重均由合并 TRAIN 推导。

### 选模与安全门

VALIDATION 不更新参数或类别权重，只比较 checkpoint。优先级固定为：

1. M16N24 开发门是否通过；
2. 投影后 exact 正动作数；
3. 正确有向残差边数；
4. 负类 exact R0 数；
5. 零不变量失败、零虚假转移和零投影拒绝；
6. 固定 TRAIN 权重下的验证损失和较早 epoch。

M16N24 开发门要求 VALIDATION exact 正动作大于 0、actor 原始残差激活和实际
transfer change 均大于 0、负类 exact R0 至少 8/11、投影拒绝为 0、不变量失败为
0、R0 完整 action tuple 偏差为 0。当前最佳 checkpoint 为 epoch 137，对应
2/9、6/6、9/11、0、0、0。旧域 VALIDATION 为 13/15 个 exact 正动作和
58/60 个负类 exact R0。

### 证据边界

v7 只使用 v4 和 M16N24 的 TRAIN/VALIDATION。M16N24 TEST 17 帧、5216-5279、
1000-1019 和 3008-3039 均不参与拟合、选模或门限。当前验证集参与 checkpoint，
因此结果属于开发证据。M16N24 TRAIN 正类命中只有 1/24，帧激活仍需新来源检验。

候选没有置信校准器，不应用 0.60 门，也没有运行加载或执行权限。确定性规则回退仍是
唯一允许的在线路径。来源独立评价已按冻结候选、全新数据和只读执行原则完成，结果见
本文件页首；评价失败关闭。

## 2026-07-30 v6 来源独立评价原理

### 动作口径

对同一快照，记确定性规则基线、外部目标和 v6 经投影后的可执行签名为
\(s_0,s_t,s_a\)。外部规则正类满足 \(s_t\ne s_0\)，并通过既有备用资源、邻接、
总量守恒和 authority 不变量。正动作精确命中要求：

\[
y_{\mathrm{exact}}=
\mathbf{1}(s_t\ne s_0)
\land\mathbf{1}(s_a=s_t)
\land\mathbf{1}(\text{actor 动作安全})
\land\mathbf{1}(\text{无投影拒绝}) .
\]

actor-derived 正类分母采用更宽的“安全可执行差异”口径：

\[
n_{\mathrm{actor+}}=
\sum_i \mathbf{1}(s_{a,i}\ne s_{0,i})
\land\mathbf{1}(\text{actor 动作安全})
\land\mathbf{1}(\text{无投影拒绝}) .
\]

这个分母用于判断 actor 是否在外部输入上形成了可进一步校准的正输出。分母为 0 时，
条件命中率没有定义，系统记录 `unavailable`，不写成 0。

### 错误分解

目标和预测转移均按 `(source region, target region, edge id)` 建立有向键。相同键表示
正确有向边；相同键但资源数不同记为数量错误；源和目标互换记为方向错误；其他预测边记
为错误边。规则负类上出现的预测转移单独记为虚假转移。该分解在投影后执行，同时保留
actor 原始转移数，避免把投影删除后的零动作误判为模型没有输出。

### 来源独立性

在线可观测键只由节点特征张量、边特征张量、边索引及其形状、类型和值构成。seed、
episode、来源身份、目标标签和真值不进入键。冻结 v4 TRAIN+VALIDATION 有 251 个唯一
键，M16N24 外部数据有 94 个，精确交集为 0。外部 train、validation、test 只是本批
评价数据的划分，三类均未参与 v6 actor 训练。

### 结果与边界

外部规则正类在 train/validation/test 为 24/9/9。v6 原始和投影转移均为 0，正确有向
边和精确正动作均为 0；负类 exact R0 为 61/65、9/11、7/8。不变量失败为 6/6/3，
投影拒绝为 0。actor-derived 正类分母为 0，因此没有置信校准正类库存。

v6 的 `confidence_head` 未校准。本次评价不读取其数值，不应用 0.60 门，也不产生准入
决定。候选和输入树前后摘要一致，模型拟合、checkpoint 更新、阈值拟合、注册和权限
授予计数均为 0。确定性规则回退仍是唯一允许的运行路径。

## 2026-07-29 v6 转移动作学习原理

### 问题

设边 \(e\) 的教师转移数量为 \(n_e\)，可转移资源为 \(m_e\)。v4 将目标编码为
\(\operatorname{atanh}(n_e/m_e)\)，并用单个连续边输出同时决定是否激活、方向和数量。
TRAIN 只有 72 条正边，零边有 3,848 条。即使使用截断权重，零边的有效损失质量仍为
正边的 1.670 倍。连续均方误差没有显式约束正确 source-target 边应高于同帧其他边。

### 分头监督

v6 保留共享图消息传递，增加边激活得分 \(a_e\)，边数量头输出 \(q_e\)。总损失为：

\[
L =
w_s L_{\mathrm{node}}
+ L_{\mathrm{BCE}}(a_e,y_e)
+0.25L_{\mathrm{mag}}(q_e,n_e)
+0.50L_{\mathrm{rank}}
+0.25L_{\mathrm{count}} .
\]

\(y_e=1\) 表示教师在有向边上存在转移。正边权重由 TRAIN 的
\(3848/72=53.444\) 推导。排序损失要求正边激活得分至少比同帧零边高 0.25。数量损失
在正边上比较由 \(\tanh(q_e)m_e\) 得到的资源数与教师资源数。帧正样本权重只作用于
节点动作，避免与正边权重叠加。

推理时，\(a_e<0\) 的边固定为不转移；\(a_e\ge0\) 的边才进入数量解码。输出仍为共享
`GraphPolicyOutput`，随后经过既有确定性投影。学习模型不能绕过备用资源、邻接、容量、
owner、plan version、epoch、lease、ACK、联盟或 fault fence。

### 选模

VALIDATION 只用于 checkpoint 选择。选模先要求模型在正负两类上均有命中，并产生至少
一个可执行转移动作；随后依次最大化投影后 exact 正动作、正确有向边和负类
no-transfer，再最小化 no-transfer 偏置、投影拒绝、不变量失败和固定 TRAIN 权重下的
VALIDATION loss。全 no-transfer 模型即使均方误差更低，也排在非退化模型之后。

### 边界

v6 的 TRAIN/VALIDATION exact 正动作命中为 58/60 和 13/15，投影拒绝为 0。负类
no-transfer 为 255/290 和 55/60，尚低于 v4。该结果只证明分头监督可训练且动作数量不再
被激活损失推高，不证明来源独立泛化。固定 0.60 门、规则回退和全部权限关闭保持不变。

## 2026-07-29 来源独立评价原理

### 证据分层

本次评价把规则标签、actor 输出、置信门和生产准入分成四层。设同一快照的确定性规则
可执行签名为 \(s_0\)，外部规则目标签名为 \(s_t\)，冻结 actor 经确定性投影后的签名为
\(s_a\)。规则安全正动作要求 \(s_t \ne s_0\)，且目标通过既有投影和干预不变量。用
逻辑指示量表示为：
...
\mathbf{1}(s_t \ne s_0)
\land
\mathbf{1}(\text{目标通过投影与干预不变量}) .
\]

actor-derived 正类还要求 actor 输出同一签名并通过安全检查：
...
\mathbf{1}(s_a=s_t)
\land
\mathbf{1}(\text{actor 动作通过干预不变量}) .
\]

规则层存在正动作，只说明该快照上存在一个可安全表达的区域转移。只有 actor 输出同一
可执行签名，才进入 v5 正类分母。置信门为
\(g=\mathbf{1}(c\ge0.60)\)，其中 \(c\) 是冻结 11 近邻校准器的输出。生产准入还需要
候选登记、正式评价、运行预检和权限审查。本次生产准入固定为 false。

### 来源独立性

输入键只绑定节点特征、边特征、边索引及其形状、类型和值，不包含 seed、episode、
来源身份、目标身份、规则标签或未来结果。冻结候选原 TRAIN/VALIDATION 共 425 帧，
形成 251 个唯一输入键。M16N20 外部数据的 63 帧形成 41 个唯一键。两集合精确交集为
0，新记录与旧键精确重合数也为 0。

键零重合说明外部评分没有复用旧开发输入。它不自动建立正类泛化能力。正类召回的分母
必须是 \(y_{\mathrm{actor}}=1\) 的样本数。本次规则层有 2 个安全正动作，冻结 actor
匹配数为 0，因此正类分母不可用，正类召回记为 unavailable。

### 读取与权限

训练 seed 0-99、正式 holdout seed 1000-1019、设计 pilot seed 3000-3007 和评价 seed
3008-3039 两两隔离。D4 只读评价实际读取 train/validation/test 为 43/10/10 帧，fit、
weight fit、threshold fit、hyperparameter fit 和 candidate selection 均为 0。main
此前对同一 test 10 条记录做过一次只读预检，该事实独立记录；正式 holdout 读取为 0。

所有帧都输出 `candidate_authorized=false` 和 `rule_fallback_used=true`。即使未来某帧
分数超过 0.60，未注册和准入关闭仍会阻止生产使用。当前 v5 的 registered、D3、D7、
接管、联盟和控制权限均为 false。

### 评价结果

冻结 actor 在外部 63 帧中产生 16 个相对 R0 的可执行差异，但都没有匹配两个规则安全
正动作。v5 得分最小值、均值、中位数、95 分位和最大值均为 0。0.60 门通过为 0/63，
负类误接收为 0/63，规则回退为 63/63。

该结果支持“当前外部负类被固定门拒绝”的有限结论。它不支持“候选通过独立验证”、
“正类召回合格”或“可进入生产”。正式 holdout、runtime preflight、D3 successor、
D7 权限、物理窗口和 AirSim 收益均未运行。

D6 已对同一冻结制品完成独立复核。样本 43/10/10、规则安全正动作 1/1/0、
actor-derived 正类 0/0/0、63 个零分、固定门通过 0、负类误接收 0、回退 63/63、
旧/新唯一键 251/41 且重合 0、正式 holdout 读取 0，均与 D4 一致。独立复核确认了
本批负类和读取边界，没有补出 actor-derived 正类，因此生产准入仍关闭。
...
越界值夹到边界只能解决域外判断，不能保证模型越过固定置信门或形成动作。该修正的实测
置信度为 0.481511，投影后转移数为 0，因此不具备验收作用。

新夹具从 TRAIN 的模型可见图域定义。对可转移图 \(g\)，先计算其到逐特征训练域中心的
归一化距离 \(d(g)\)，再按 \(d(g)\) 和模型可见图指纹确定顺序。代表必须满足

\[
\operatorname{OOD}_{0.05}(g)=0,\quad
p_\theta(g)\geq0.60,\quad
P(a_\theta(g))\neq R_0(g),\quad
P(a_\theta(g))\neq a_{\mathrm{source}}(g).
\]

筛选只使用 TRAIN 在线快照、模型输出和确定性安全投影。target、reward、validation、
test、seed、episode 和来源身份不参与。选定后的 4 区域图固化为版本化常量，并绑定
节点特征、边特征、边索引、shape、dtype 和架构的 SHA-256 指纹。构建时不再搜索样本。
固定夹具在本轮只读模型上的置信度为 0.602367，投影形成 1 条安全转移；该数值只用于
development build 门，不能解释为正式收益。

该指纹与 TRAIN 模型输入键完全相同，因此夹具检验的是训练域内模型、序列化、OOD、
置信门和安全投影能否联通。它不检验未见输入上的泛化能力，也不构成独立 validation 或
formal holdout。夹具显式关闭独立泛化证据和正式验证声明。置信裕量定义为

\[
m_c=p_\theta(g)-0.60.
\]

本轮 \(m_c\approx0.002367\)。该薄裕量对模型权重、数值环境和重新训练结果敏感，只能
作为失败关闭的 smoke 门，不能据此开放 assist、登记或生产权限。
