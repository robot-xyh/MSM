...
arm evidence v2 保存 confidence/最小置信门、OOD、latency/limit、finite 和逐项 gate，并分别使用 low-confidence/OOD/timeout/nonfinite 稳定拒绝码；旧 generic reason 只保留兼容。v1 reader 在验证旧 content ID 后迁移，未知新字段保持 unavailable，不反向改写冻结记录。当前权威 `formal_7891296` 绑定源提交 `78912963b67fe86ee9a8d29186b18a9dd60c460c`，其 `SHA256SUMS` 与 manifest SHA256 分别为 `821f1503...72bc`、`d6ef23b2...883c`。独立只读复核得到 20/20 source clean 且 finite、truth 使用数 0；40/40 arm 为 evidence v2，20/20 候选被评估。confidence min/mean/max 为 `0.508892953/0.563426384/0.569492280`，在保持不变的 `minimum_confidence=0.6` 下通过 0/20；OOD、latency、finite、failure gate 各通过 20/20，v2 latency min/mean/P95/max 为 `1.103510/1.977479/2.264415/2.703312 ms`，均低于 `50 ms`。safe adopted 0/20、规则回退 20/20，明确 low-confidence 与兼容 generic reason 各 20 次；旧 v1 latency 只作历史证据，不与 v2 混称。manifest 的 `confidence_head_uncalibrated` 仍有效，且 `formal_twenty_seed_performance_completed=false`；该诊断只能驱动独立校准/重训计划，不能降低门、宣称候选或降级策略有效，不能开放 PPO/assist/authority。专项 33/33、D4 全量 482/482；D6 outcome sidecar、非退化、反事实和因果证据仍不可用。
