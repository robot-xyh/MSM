...
**保留 seed 配对实验增量**：D4 已形成规则区域调度与候选学习建议的正式实验边界。seed 固定为 1000-1019，每个 seed 具有一个规则 control arm 和一个隔离 treatment arm；场景配置、初始状态、通信时序、故障时序和区域快照 lineage 必须逐 SHA 相同。control 始终执行确定性规则。treatment 候选经过 bundle/阈值检查后，复用原 owner/version/epoch/lease、fault fence、联盟 ACK、邻接、容量、备用和资源守恒投影；只有投影和下一周期消费均通过，才记录隔离 arm 安全采用。安全采用不等于线上运行确认，不能授予计划所有权或导引权限。配对 manifest 还核对两个 arm 的实际快照 payload SHA，缺 arm、schedule 不同、旧 generation/lease、缺 ACK、真值 key、哈希篡改和非有限值均失败关闭。新增专项 20/20、D4 全量 469/469。正式 20-seed episode 和 D6 outcome sidecar 尚未执行，因此 outcome、非退化、反事实和因果证据均不可用。
...
- 2026-07-21 全样本准入阶段为 **397/397 项通过**，验收阈值为零失败；加入运行时确认和区域奖励合同时为 **449/449**，当前 D4 全量为 **469/469**。全样本审计专项 10/10，覆盖正常数据、非有限值、规范切分错误、配额不守恒、非法转移、旧 epoch/lease/version、generation 回退、真值泄漏和文件篡改。正式 900 episode 与补充 100 episode 的模块内全样本状态均为 complete；D6 外部复核和真实 ACK/outcome/reward/paired-shadow 仍 pending。该结果不包含新的 AirSim 或真实网络样本，历史阶段计数保持不变。
