...
- 2026-07-21 全样本准入阶段为 **397/397 项通过**；加入运行时确认和区域奖励合同时为 **449/449**，当前 D4 全量为 **482/482**，验收阈值均为零失败；
...
`tests/test_canonical_seed_split.py` 新增 12 项。正例覆盖 100 个 seed 的 D3 兼容 60/20/20 映射、BC 显式切换和源数据零修改；负例覆盖 schema/policy 变化、content/assignment 哈希篡改、registry 或 dataset 漏/多 seed、保留 seed 和源 registry SHA 不匹配。共享切分专项 12/12，当时 D4 全量为 381/381；当前全量为 482/482，新增/修改 Python 入口编译通过。
...
权威正式输入为 `reserved_seed_interventions_nominal_5v5_1000_1019_formal_6d5bfea`。20 个 treatment record 的只读诊断为 confidence min/mean/max `0.508893/0.563426/0.569492`，OOD 与 finite 各 20/20 通过；正式记录 latency P95/max `35.608/42.302 ms`，20/20 不超过 `50 ms`。默认 `minimum_confidence=0.6` 不变，confidence 通过数为 0/20，因此全部规则回退且明确原因仅为 low confidence。冻结 manifest 同时声明 confidence head uncalibrated；本结果只支持在独立 calibration split 上评估 ECE/Brier/reliability 并校准或重训 head，不支持用保留 seed 降阈值。正式记录没有 D6 outcome sidecar，因此不支持候选有效、优于规则、非退化或因果收益结论。
