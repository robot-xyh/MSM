...
审计固定读取来源提交 `1e5ed8ddcf27f375e922a447decfbd875d21bfdf` 的 900 项
正式 R0 制品，不写回 episode。对 28 个失败和同场景 72 个通过样本，按总线序号重建
以下时序：

1. 最后 D3 `AssignmentPlan` 的计划标识和版本；
2. 最后 D4 区域决策的 owner、计划、版本、epoch 和 lease；
3. 各联盟的 required、acked、missing 成员集合；
4. 计划广播与成员 ACK 的发送、到达和绑定计划；
5. 通信拒绝、丢弃、pending 和 episode 终止点。

28 个失败包含 38 个未提交联盟。23 个联盟位于 16 个计划代次错位 episode；15 个联盟
位于 12 个计划对齐 episode。对齐失败中有 14 个必要 ACK 在最后 D4 决策后到达，偏移
最小值、中位数和最大值为 0.000067、0.011493 和 0.082041 秒。唯一没有当前代次 ACK
交付证据的样本为 `00815__r0__high_threat_m_to_n__5v5__seed_1015`。

### 状态机复核

冻结源码和当前源码均用正常中心、三成员逐次 ACK、旧版本拒绝和租约约束做定向回归，
结果各为 4/4。第三个有效必要成员 ACK 被消费后，状态可从 `collecting_acks` 转为
`committed`。当前 D4 全量回归为 903/903。没有新增 D4 本地补丁。

### main 接口要求

main 应在 D3 新计划生成后再次构造 D4 快照，使最终发布计划身份一致；当前联盟存在未收齐
确认时，收到同代次有效 ACK 即触发重评。重发机制只能定向请求缺失成员，并设置固定次数
和超时预算。episode 结束时应增加有界排空，同时记录消息标识、主题、源、目的、
queued、dropped、delivered、pending 和 retry generation。

这些改动不能修改 required 成员集合，不能延长既有 lease，不能接受旧 epoch/版本，
也不能把超时或 `collecting_acks` 视为成功。本轮只完成诊断；代码未变，正式结果未
重跑。main 修复会改变正式业务语义，需从新 clean source 整体重跑 900 项。
