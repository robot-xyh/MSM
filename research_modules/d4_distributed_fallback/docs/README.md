...
0.60 门下 validation 仍有 1 条负类且不一致样本越门。TRAIN-only 在线图指纹审计进一步
发现 229 个图键中有 10 个同输入异标签冲突键，共 22 条记录；validation 假阳性与 train
两个正例的模型输入张量完全相同。指纹只使用三个模型输入张量、shape、dtype 和架构，
不使用节点或边身份元数据。该数据不可辨识，流程以内容寻址 blocker 失败关闭。专项
25/25、D4 全量 808/808 通过。当前没有 clean v4 候选或登记，D3 successor、D6 审计和
收益仍未完成。原理与实现细节见本目录两份主文档及 `../PLAN.md`。
