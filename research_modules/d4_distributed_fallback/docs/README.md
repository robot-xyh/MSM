...
2026-07-21 文档状态已包含保留 seed 配对干预合同、冻结候选隔离加载器和正式 evidence v2。权威 `formal_7891296` 记录包含 nominal 5v5 seed 1000-1019 的 20 个 clean/finite source、40 个 arm 和 20 个候选门诊断；truth 使用数为 0，候选因 confidence 0/20 通过 `0.6` 门而 safe adopted 0/20、规则回退 20/20。配对专项 33/33、D4 全量 482/482；该记录没有 D6 outcome sidecar，不能解释为候选或降级策略效果。详细边界见算法文档的“同 seed 配对干预”、模块计划 0.0 节和实验报告 4.12 节。
