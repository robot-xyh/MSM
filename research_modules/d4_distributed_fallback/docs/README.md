...
`0/100` 仍是历史制品结果，新的 v5 批量 D6 独立审计尚未运行。

2026-07-30 完成高威胁开发批次 v4 复核。5、20、50、100、200 规模各 20 个种子中，
D3-D4 当前计划对齐和联盟执行闭合均为 100/100；28 个 episode 使用过计划期航迹
fallback，三个 v3 原失败样本均闭合，权威计划摘要冲突为零。D4 协议代码和严格 ACK
校验未改。D6 独立审计确认计划 ID/版本对齐 100/100、644 个当前多成员联盟目标闭合，
195838 条通信处置在 100/100 episode 中 available/verified；D3 区域 epoch 与 lease
对照字段均为 0/100 available，属于开放 P1。该批次为 dirty development，formal R0
未运行。v3 根因与 v4 验收记录见
`../reports/HIGH_THREAT_PRECHECK_V3_COALITION_DIAGNOSTIC_20260730.md`。

2026-07-30 新增高威胁开发批次联盟闭合诊断。5、20、50、100、200 规模各 20 个
种子中，当前计划联盟闭合为 97/100。三个失败均由当前 D3 assignment 目标从 D2 输出
消失后被 main 从 D4 快照静默删除；其中一个样本还存在同一计划身份不同载荷摘要。
D4 保持失败关闭，模块代码未改。专项报告见
`../reports/HIGH_THREAT_PRECHECK_V3_COALITION_DIAGNOSTIC_20260730.md`。

2026-07-30 新增正式 R0 联盟确认后验诊断。D4 只读检查 28 个
`collecting_member_acks` 失败和同场景 72 个通过样本：16 项为 D3-D4 计划代次
错位，11 项为当前 ACK 晚到但中心正常路径未重评，1 项为重发与 2 秒终止尾窗不足。
没有发现 D4 状态机、required 集合、epoch 或 lease 的本地缺口。D4 代码未变，正式
R0 未重跑；当前全量测试 903/903。专项报告见
`../reports/FORMAL_R0_COALITION_ACK_DIAGNOSTIC_20260730.md`。main 修复后必须从新
clean source 整体重跑 900 项，不能修补或拼接旧结果。

2026-07-29 新增 v6 转移动作学习开发候选。TRAIN/VALIDATION 分别有 350/75 帧；
TRAIN 含 60/290 个正负动作和 72/3848 条正/零边。v6 将边激活与转移数量分头建模，
增加有向边排序和资源数监督，并保持原 `GraphPolicyOutput`、确定性投影和安全合同。

固定构建的 TRAIN/VALIDATION exact 正动作和正确有向边为 58/60、13/15，负类基线动作
保持为 255/290、55/60，投影拒绝为 0。两次构建逐文件一致。该结果没有超过 v4 的负类
保持，也没有来源独立正类证据。v6 继续未注册、准入关闭、规则回退，全部生产、D3 和
D7 权限为 false。TEST、seed 3008-3039 和正式 holdout seed 1000-1019 未用于训练或
选模。详细报告见
`../reports/D4_V6_EDGE_TRANSFER_DEVELOPMENT_CANDIDATE_20260729.md`；模块原理、实现、
计划、实验报告和 D4 GAP 已同步。v6 专项 12/12、D4 全量 855/855 通过。
