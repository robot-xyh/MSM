...
离线评分新增五项目标等权指标：航迹关系精度、每目标平均纯度和完整度、正确形成独立纯净目标簇的比例、涉及身份混合的目标数，以及具备双视角机会但未形成关系的目标数。分母只包含至少被两台相机看到的真实目标。真实编号在匿名关联结果形成后才读取，不进入候选、图网络特征或聚类。

本轮只在责任区/视场稀疏路径上调整图网络概率阈值、几何与图网络融合权重、未匹配代价。硬几何门控、模型权重、匈牙利一一约束和多帧确认均未修改。36组同批回放诊断选择了阈值0.05、融合权重0.25、未匹配代价0.85。该参数在20目标/30机场景将关系精度提高到0.9736并消除身份混合，但在20目标/8机和40目标/50机上明显降低完整度，因此不进入默认路径。

完整证据位于`../outputs/terminal_gnn_diagnostic_selection_20260819_v2/`。参数是在报告使用的同一批回放上择优，属于诊断结果，不是独立留出验证。
...
当前35项本目录测试通过。测试覆盖统一回放清单相对路径、SHA256失败关闭、在线真值污染拒绝、`full/sector_fov`相机对策略、审计/详细输出决策一致性、测试种子训练隔离、模型bundle摘要校验、固定代价融合和图网络不绕过硬几何门。
