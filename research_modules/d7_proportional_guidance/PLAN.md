...
排他成本分段”完成复核。整栈 `+6.243%` 的 seed bootstrap 区间排除零，但 D7 源码
完全相同，固定 replay 的变化只有 `+0.626%`，其 95% 区间
`[-1.828%, +3.178%]` 包含零。命令、世界加速度、pair 状态、模式和拒绝原因均严格
一致。本轮不修改代码、公式、门控、阈值或调用频率。

下一轮仍由 main 改善计时证据，而不是调整 D7 业务算法：

1. 分开记录 `_guidance_inputs()`、D4 permission 组织、D7 `command_batch()`、
   世界数组复制和 publication；
2. 性能复测保存完整 pair 输入 sidecar 和必要状态快照；
3. 记录处理器频率、温度、上下文切换和同机负载，区分缓存与调度因素；
4. 只有冻结输入差异区间排除零，且命令、状态和拒绝原因严格相等时，才评估
   命令对象物化或批处理循环的局部无语义优化。

当前 200 对实时 P1 仍为系统级开放项。D7 未出现新的 P0/P1 代码 blocker。固定 replay
中的末端视觉命令只有 120 条，不能据此关闭真实视觉吞吐或 AirSim 平台标定。

## 2026-07-22 三维导引性能审计完成

已按“源码同一性 -> 调用与规模 -> 历史输出语义 -> 冻结输入状态 -> 分配与序列化 ->
单机噪声”顺序复核 clean `8f86192 -> f80b5bd`。三 seed 的 D7 均为 185 次调用，逐
seed pair 数、命令、mode 和 gate reason 一致；D7 与 main 计时路径没有源码差异。
200-pair 冻结输入的命令数组、完整命令、pair snapshot 和 mode transition 逐项相同，
同一源码的 12 次交错复测变异系数为 `12.26%`。本轮把汇总 `+7.337%` 记为未归因
墙钟波动，没有实施代码优化，也没有修改任何公式、阈值或安全门。

下一轮性能计划由 main 将集成计时拆成输入查找/permission 组装、D7 内核、世界数组
复制和 publication，并保存每次调用分布及主机状态。D7 只有在冻结输入重复试验出现
稳定差分，同时命令数组、mode、gate reason、pair state 和状态迁移保持一致时，才
评估小范围非算法优化。三 seed 描述性审计不关闭 200 对实时 P1。
