...
之前开始，到世界加速度数组复制结束，包含上游对象查找、输入数据对象构造、D7
批处理和数组复制。其他模块负载、诊断分位数、内存分配和处理器调度会影响下一次
D7 调用，即使 D7 源码和输入输出语义没有变化。

本轮 20-seed 整栈观测为 `+6.243%`，但固定 200-pair replay 的两版内核差异只有
`+0.626%`，置信区间包含零。两版命令、世界加速度、最终 pair 状态、模式和拒绝原因
严格一致。该证据要求把当前现象解释为系统级运行环境差异，不能通过放宽门控、减少
滤波或改变比例导引公式来追求墙钟数字。

成本分段用于确定今后的测量边界。命令对象物化、输入适配、比例导引计算、航迹滤波
和批处理循环是主要组成；LOS/TTC 在本次以中段为主的 replay 中占比较小。占比高只
说明绝对成本位置，不等于存在回归。局部优化必须同时满足冻结输入、严格业务等价和
可复现差分三个条件。

## 2026-07-21 隔离实验控制原理

成对实验要求 control 和 treatment 从相同初始世界与相同外生传感器、通信、故障
时序出发，但各自消费本 arm 的分配计划并独立推进状态。D7 用两个独立的
`IsolatedArmGuidanceExecutor3D` 保证这一点。每个执行器内部仍以资源编号和中心
全局航迹编号作为 pair key；arm 身份位于更外层，因此同名资源-航迹对不会在两臂间
共享航迹滤波、LOS 滤波、检测框面积历史或模式状态。

命令血缘由三组摘要组成。第一组是实验身份：实验、随机种子、arm、episode 和隔离
世界。第二组是计划身份：源计划编号、递增版本和完整计划载荷 SHA-256。第三组是
执行身份：资源-全局航迹 binding、生成时刻、控制模式、加速度与命令 SHA-256。只要
其中一项与当前 arm 不一致，命令就不能进入对应世界。

`command_generated` 只表示 D7 已形成候选命令；`held` 表示安全外壳已将该索引归零；
`control_applied_to_world` 只由 main 在克隆世界实际消费命令后确认。最后一种确认仍是
隔离质点仿真记录，不是生产运行确认。D4 不允许、resource-track binding 不一致，
以及 main 明确要求末端视觉门控但 D5 未通过时，结果均为 held。中段未要求 D5 时仍
可依据 D2 GlobalTrack 使用位置比例导引。

该外壳没有改动后文任何导引公式。位置比例导引、视觉比例导引、LOS 卡尔曼滤波、
检测框面积 TTC 和短时 coast 保持原实现。2026-07-21 的 9 个专项回归与 D7 全量
`213 passed` 只验证血缘、隔离、篡改检测和状态语义，其中规模样本为 200 个独立
assignment pair；多周期物理效果仍需 main/D6
完成。
