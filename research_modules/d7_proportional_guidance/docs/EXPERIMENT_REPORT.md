...
## 2026-07-25 200-pair 状态生命周期冻结输入验收

本轮未启动 AirSim。样本为一个确定性冻结输入序列，共 200 个资源-目标 pair、9 个
批次。前三批建立中段和视觉状态，第四批混合 D4 pending、D5 reacquire、带检测重获
和锁定后无图像帧，第五批恢复视觉，第六批把计划从 v1 升至 v2。第七批升级至 v3，
同时改绑 40 个资源、标记 10 个目标 lost，并从权威批次快照移除 20 个分配。第八批注入 10 个 v2
旧计划输入，第九批恢复当前 v3 输入。
...
| 指标 | 验收条件 | 结果 |
| --- | ---: | ---: |
| 最终有效 pair / 状态 | 状态数等于有效 pair | `170 / 170` |
| 峰值状态 | 批前、对账后和逐 pair 计算阶段均不超过输入规模 200 | `200` |
| 状态上界违规 | `0` | `0` |
| 暂态滤波状态保持 | D4 pending、D5 reacquire、丢帧均保持 | 通过 |
| 旧状态回收 | 改绑、lost、撤销、旧版本全部回收 | 通过 |
| 旧计划拒绝 / 接受 | 10 个拒绝，0 个接受 | `10 / 0` |
| `global_track_id` 改写 | `0` | `0` |
| 非有限命令阻断 | 无非有限输出 | `0` |
| 命令饱和 | 记录，不设本轮准入阈值 | `0` |
| 重复资源输入 | 状态对账前拒绝，既有状态不变 | 通过 |
| D7 全量测试 | 零失败 | `220 passed` |

累计状态事件为：创建 250、复用 1120、计划版本重置 330、回收 80。回收原因中，
资源改绑 40、目标丢失 10、批次撤回 20、旧计划版本 10。命令模式样本为三维中段
1380、末端视觉 280、短时视觉 coast 20、保持 40。所有命令使用输入 binding 中的
中心 `global_track_id`。

专项回归还分别注入 binding revoked、有效期到期、D4 action revoke 和 D4 reason
revoked。四类输入均回收原状态。短时 D4/D5/丢帧序列在 `0.25 s` 窗口内保持视觉
滤波历史；超过窗口的无图像帧清空视觉状态，但保留当前有效 pair 的航迹状态。
同批次重复 `resource_id` 的输入在对账前抛出错误，原有有效状态数量保持不变。

单次本地测量启用了 Python `tracemalloc`。逐 pair 时延 P50/P95 为
`0.201/0.721 ms`，批次时延 P50/P95 为 `75.639/111.547 ms`，跟踪到的峰值 Python
内存为 `6,429,201 bytes`。这些数字受内存跟踪和同机负载影响，只用于确认诊断字段
可计算，不作为实时准入指标。

本轮证明状态生命周期不会随历史分配无界保留，并证明暂态控制门不会错误删除同一
有效 pair 的滤波历史。状态对账在命令计算前执行，峰值统计覆盖批内新状态建立过程，
因此资源改绑期间也未出现旧、新状态叠加超过 200。它不提供 200v200 长时物理成功率、多 seed 置信区间、AirSim
平台响应或真实相机吞吐结论。PN、视觉 PNG、LOS、TTC 和外推公式未修改。

## 2026-07-23 200-pair 固定输入性能实验

本轮未启动 AirSim，未修改 Python 代码。输入证据包括 `0d2da25` 和 `5263e2b` 的
nominal 200v200、10 秒、seeds 1000-1019，以及一个确定性
200-pair/185-frame replay。

整栈 D7 累计墙钟均值为 `3.637837 -> 3.858795 s`，逐 seed 变化均值
`+6.243%`，95% bootstrap 区间 `[+2.954%, +9.825%]`。候选 episode 的逐调用
P50/P95 均值为 `19.572/22.520 ms`；参考版本未保存分位数。20/20 跨构建语义审计
通过，D7 命令、模式和拒绝原因一致。

冻结 replay 在两个工作树各运行 6 次。两版 replay、37,000 条命令、世界加速度和
最终 200 个 pair 状态 SHA-256 均一致。参考/候选内核均值为
`1.837673/1.849179 s`，变化 `+0.626%`，95% bootstrap 区间
`[-1.828%, +3.178%]`。分段 profile 的成本占比为：命令数据对象物化 `23.74%`、
输入适配 `14.61%`、PN/PNG `14.58%`、航迹滤波 `12.31%`、计划/安全门 `3.60%`、
LOS/TTC `0.45%`、批处理和内联几何残差 `30.71%`。

分段 trial 中两版各类别占比相同，没有候选独有热点。当前结论为“整栈差异存在，
D7 内核回归未确认”。D7 全量 `213 passed`，验收阈值为零失败。本轮不修改位置 PN、
视觉 PNG、LOS/TTC、切换安全门、阈值或调用频率。完整方法、逐 seed 数据和限制见
`subagent_reviews/D7_SCALABLE_3D_GUIDANCE_PERFORMANCE_AUDIT_20260723.md`。
...
结果：2026-07-14 当日全量 `188 passed`，验收阈值为零失败。位置 PN、VM/TTC PNG、LOS 滤波、外推公式、D3/D4/D5 门控和 `global_track_id` 均未修改。
