...
`module.d7_guidance` 逐 seed 变化均值为 `+6.243%`，95% bootstrap 区间为
`[+2.954%, +9.825%]`。两个提交的 86 个 D7 受控源文件和
`scalable_3d_guidance.py` blob 完全相同；20/20 跨构建语义审计证明规范化 D7
命令、模式、拒绝原因和计划血缘一致。

固定 200 pair、185 frame replay 在两个工作树各执行 6 次。37,000 条 pair 命令、
世界加速度和最终 pair 状态严格一致；内核均值为 `1.837673/1.849179 s`，候选变化
`+0.626%`，95% bootstrap 区间为 `[-1.828%, +3.178%]`。冻结差异未排除零，因此
没有确认 D7 内核回归，也没有实施代码优化。

分段 profile 显示主要成本为命令数据对象物化 `23.74%`、输入适配 `14.61%`、
PN/PNG 计算 `14.58%`、航迹滤波 `12.31%` 和批处理/内联几何残差 `30.71%`。
两版占比近似一致，候选偶发变慢时各类别同比例变化。当前把整栈差异保留为
CPU、缓存、调度或同进程负载连带效应，不修改 PN、视觉 PNG、LOS/TTC、安全门、
阈值或调用频率。详细证据见
