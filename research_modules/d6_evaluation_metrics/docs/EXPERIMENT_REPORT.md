...
既有 90 项离线制品中，严格身份交换为 `73/90 available`。可用项中 57 项为 0、16 项
为非零，可用部分合计 143。其余 17 项保持失败关闭：16 项为一条全局航迹对应多个真值
目标，1 项为源观测超出谱系窗口。所有 90 项均声明真值隔离通过、来源为离线身份评价、
验证模式为 SHA-256 绑定且未回填。该统计是既有 episode 的派生评估结果，不是新仿真。

哈希损坏、schema/episode 不匹配、严格指标不可用和在线不可用均已纳入单元回归。现有
135 个 episode 只需重新生成 D6 CSV、聚合 JSON、正式后验审计和矩阵准入结果。原始
episode、执行计划和 producer 来源提交保持不变；新报告单独记录 evaluator 来源。
