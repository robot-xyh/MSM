...
长距离配准评估将“有记录”和“能证明”分开处理。每个数值都包装为数值、可用性、不可用原因和
来源。旧版 episode 没有保持或绑定事件时，D6 不把缺文件解释为零次振荡或零次过期。这样可以
兼容历史结果，同时避免由缺证据得到错误通过结论。

无 accepted association 或时序事件时，main 可以生成无表头空 CSV。文件存在只证明生产者完成
写入，不证明对应指标为零。D6 将文件状态标为不可用并记录 `empty_file_no_header` 原因，相关门
保持失败关闭。

评估分为相机流和全局两层。相机流层分别统计实测短缺口、短缺口时长、有界预测保持帧、同编号
恢复、保持过期、长期重发现、绑定切换、实际交叉窗口和身份切换。全局层只在输入都可用时求和
或加权；关联准确率按可评分记录数加权，交叉纯度和连续性按可评分窗口数加权。任一 episode
缺少必要字段时，相应汇总保持不可用，不按部分样本外推。

结构门检查七项：连续可见身份切换、保持后的有效短缺口中断、绑定振荡、重复分配、在线真值
使用、全局航迹编号改写均应为零，关联准确率应不低于 0.95。实际交叉门另要求可评分窗口不少于
10 个，且占全部实际交叉窗口的比例不低于 0.30。几何预检只证明轨迹设计可能交叉，没有证明
两个目标被相机同时观测，因此不得替代实际窗口。

冻结 v2 单 seed 的短缺口为 3，D6 在没有保持事件的条件下将有效中断同样记为 3；绑定振荡因
缺时序事件而不可用。实际交叉窗口只有 `3/31`。即使关联准确率达到 `0.9979317476732161`，整体
仍失败关闭。该判定保留了“平均关联正确”和“困难交叉场景证据不足”之间的区别。
...
资源或多个相机观测，各流使用不同 `local_track_id` 是正常现象。末端本地身份切换应在
`global_track_id + resource_id + camera_id/stream_id` 的固定流键内比较；跨流差异应由跨视角
冲突或注册一致性指标评价，不能计入单流 ID switch。

2026-08-01 的真实 AirSim 2 对 2 受控专项中，每个全局航迹只出现于一个资源/相机流。
main bus 的两次末端切换分别来自两个流内部的本地号变化，因此本批值不是跨流误计。现有实现
只按全局航迹分组，尚不能保证多资源共拦和多相机场景的语义正确性。该限制为 P1；在分流验证
完成前，post-control execution 未提供该指标时继续保持 unavailable，不用诊断值补齐。
...
“已验证”状态开始。实际普通文件集合必须与 `SHA256SUMS` 完全一致，每个摘要重新计算。
manifest 中的 100 个 descriptor 必须分别等于独立 descriptor 文件，并绑定唯一的在线压缩
流和离线文件。审计过程中还记录设备、inode、文件大小、修改时间和权限模式；任一文件发生
变化即失败关闭。

在线流的完整性由三层绑定构成。header 绑定 episode、seed、来源身份和来源域；snapshot 与
camera-feedback 的对象键由对象内容规范化后计算 SHA-256；footer 绑定样本数、唯一对象数和
完整样本索引摘要。样本索引包含顺序号、样本键、观测键、快照键和相机反馈键，任何缺失引用、
重复身份、顺序断裂或摘要不一致都会阻断审计。

离线文件不因与在线文件物理分离就被默认可信。D6 解析每个离线 JSON，核对 episode 身份和
schema，再要求每个离线 label 的 sample key、observation key 与在线流逐项同序。该检查只
确认离线文件与在线样本的完整绑定，不评价 reward、counterfactual、动作角色覆盖或模型训练
价值。

2026-08-01 的 A3 v2 语料覆盖 seed `22100-22199`、100 个 episode 和 159502 个样本。
302 个清单工件和 `SHA256SUMS` 全部闭合；16 项来源检查与 13 项固定候选检查共 29/29 通过。
在线 truth、actor、object 标识均为 0，保留 seed `1000-1019` 的数值交集为 0。状态只确认
`simulation_research` 来源完整性。D5 训练门、模型准入和运行权限仍是独立问题。

生产计划摘要由 main 作为外部冻结锚点提供。计划文件未包含在受审根目录，因此 D6 明确记录
`generation_plan_content_recomputed=false`。这种写法保留证据边界，避免把已知摘要误写成
对计划文件内容的独立复算。
...
推导精确分片集合、分片身份和单元集合，再独立校验 manifest、checksum、压缩 payload、
文件 inventory 与 tar 成员。完整性分母是计划规定的归档子目录集合。普通 pack/verify
sidecar 文件不是分片，可保留在根目录；缺少分片、额外目录、符号链接或非普通项会使完整
范围结论关闭。路径检查先针对未解引用路径执行，不能以 `resolve()` 后可读替代 symlink 门。

空间受限时不需要同时恢复 20 个 shard。D6 每次只恢复一片，在临时目录内重算 episode
证据和后验指标，得到内存行后立即释放该片。最终 900 项结论由这些低层重算行形成。
producer 的 D6 报告只接受哈希绑定检查，不参与指标重算，因此不能用一份自称通过的汇总
替代原始 episode 证据。

归档 merge 也是待审计索引。D6 将 cell CSV 与冻结计划、低层 cell 摘要和 artifact tree
摘要逐项对账，将逻辑 episode index 与 CSV 顺序对账，再复算 archive binding 与五类报告
文件的路径、大小和 SHA-256。core 文件、D6 artifact 及其父目录不能是 symlink。D6 binding
中的 evaluator schema、Git 提交、工作树 dirty 状态和源码树摘要也必须为非空、格式合法
的容器。源码树摘要使用 `sha256:<64位小写十六进制>`，与 D6 evaluator 实际来源字段
一致；裸摘要和其他算法前缀不进入兼容路径。任一层矛盾都会使全局完整性门失败关闭。

## 分片评估与全局聚合

单个 episode 的离线评估只依赖该 episode 持久化制品。输出行保留阶段记录、失败原因、
availability、严格离线身份、真值隔离和 producer/evaluator 来源，因此可以作为归档释放前
形成的评估快照。该快照不包含新的在线输入，也不会把离线 truth 送回控制链。

跨 episode 聚合必须在全部行可用后统一执行。D6 先对行的 schema 和安全字段做失败关闭
检查，再在深拷贝上补齐全批次阶段列、终结 episode 证据状态，并生成 bootstrap、实验
矩阵、性能证据和报告。目录入口和预评估行入口共用该流程。分片恢复顺序不会改变统计口径，
调用方行也不会因最终写包而变化。

## 候选制品与当前源

候选审计同时绑定候选制品和生成该制品的源代码。历史候选绑定的源文件发生变化后，候选
制品仍可保留用于历史复核，但不能再代表当前实现。审计器必须先比较 clean 源提交和逐文件
摘要；任一文件不同即失败关闭，不能通过更新配置中的期望摘要把旧候选重新解释为当前候选。

2026-07-29 的 D4 v4/v5 候选绑定 `fd85745`。D4 后续在 `20895c7` 增加建议发布代次
安全门，`region_resource.py` 从 `1b534b4...` 变为 `1f47de6...`。D6 因此将两份候选
定位为历史证据并保持未准入。当前实现需要新的候选版本和独立审计链，旧报告不得覆盖。

## 身份交换的在线与离线边界

在线 D2 只掌握航迹和量测，不掌握目标真值。其 `id_switch_count=None`、
`id_switch_count_available=false` 是正确诊断，D6 通过独立字段原样保存。在线字段用于
检查 producer 是否越权使用真值，不参与严格身份交换统计。

严格身份交换在仿真结束后计算。D6 先用观测谱系把离线真值与航迹历史绑定，再读取
真值隔离 episode record 中的 `d2_identity.metrics.id_switch_count`。读取前必须完成清单、
文件哈希、episode 身份、schema、真值隔离和非回填合同复核。若同一全局航迹对应多个
真值目标，或源观测超出谱系窗口，指标保持不可用。可用值为 0 表示证据完整且没有发生
身份交换；不可用值不能解释为 0。

episode 的 `git_commit` 记录仿真 producer 来源。派生报告另记 D6 evaluator 的提交、
工作树状态、schema 和源码树摘要。两类来源不能相互替代。

## 正式 R0 前 450 项证据边界（2026-07-31）

main 在不重跑 episode 的条件下，使用 D6 v12 evaluator `b6289c5` 读取 clean producer
`80e55eb` 的 shard 0-9。450 项均通过有限状态检查，严格身份制品的哈希和合同也全部
通过。严格身份交换在 414 项中可判定，36 项失败关闭；可判定项合计 893 次身份交换，
其中 169 个 episode 为非零。不可判定原因只有两类：27 项一轨多真值，9 项源观测超出
谱系窗口。

同一批次的在线 producer 身份交换可用性为 `0/450`。该结果不与严格离线指标冲突：前者
证明在线链路没有使用真值，后者在仿真结束后使用隔离真值评分。修复前 90-cell 通用汇总
曾把在线 unavailable 当成严格指标，得到 `0/90 available`；其离线严格诊断实际为
`73/90 available`。该 90-cell 结果只作为接线修复前后的历史对照。

450 项是正式 R0 的半程派生汇总。它可以验证评估语义和来源分离，不能替代 900-cell
full posterior，也不能触发 post-run experiment matrix admission。两项正式步骤必须在
shard 0-19 全部完成后使用同一冻结范围执行。

## 建议证据的代次分类（2026-07-31 修复后复核）
...
错代，不能因其为 shadow 输出或没有改写正式决策而计为有效。修复前 `49e43ea` clean
smoke 的四个重规划 episode 均出现一条 superseded-valid 和一条
stale-at-publication，且没有当前计划建议。

修复后 `b063535` clean smoke 继续保留历史 v1 建议，但四个重规划 episode 的第二条建议
已绑定当前 v2。12 条建议均满足 \(k_i=k_i^{\mathrm{formal}}\)，发布时旧代为 0；每项至少
一条建议满足 \(k_i=k^*\)，最终计划覆盖为 `6/6`。其中 10 条带故障投影拒绝、hold 或
request-replan 的诊断仍属于当前代，不能误记为 stale。它们均为 shadow、非 assist，
正式决策未改变。没有消费记录时，D6 将采用保持 unavailable，而不是补写为 0。

## 时期租约可用性闭环（2026-07-31）

v5 开发态批次验证了“缺证据不补默认值”的完整闭环。D3 当前权威发布携带通用时期、
通用租约、区域最大时期和区域最小租约。D6 先验证四字段存在，再把 D3 区域摘要与 D4
最终 ownership 逐区域比较。100 个 episode 的时期和租约均为 available 且 match。

同身份评价刷新只改变诊断信息，不构成新的权威发布。D6 以权威发布计数、计划身份计数和
运行时计划确认数三方守恒核对该语义。本批次为 `151/151/151`，同身份重复为 0，
刷新抑制为 48。该证据关闭开发态 availability 缺口，不改变 dirty source 不能进入正式
分母的原则。

## 最终目标域一致性（2026-07-30）

外层计划键一致只说明 D3 和 D4 使用同一代计划标识。联盟目标还必须属于最后 D3
assignments。设最后 D3 的目标域为
\(\mathcal{T}_3=\{j\mid R_j\ne\varnothing\}\)，最后 D4 的必需提交目标域为
\(\mathcal{T}_4^c\)。D6 增加约束

\[
\mathcal{T}_4^c\subseteq\mathcal{T}_3.
\]

该约束阻止已从最后 D3 计划删除的目标凭旧提交状态重新进入当前联盟分母。对每个必需目标
还要求最终 D4 提交数等于 1。较早时刻曾 committed、但最终快照提交数为 0 的目标仍判为
未闭合。

## 当前计划与联盟代次审计（2026-07-30）

正式 R0 的 D3 和 D4 是异步发布者。设最后 D3 计划键为
\(k_3=(p_3,v_3)\)，最后 D4 区域 \(r\) 的权威键为
\(k_{4,r}=(p_{4,r},v_{4,r})\)。当前计划绑定条件为

\[
B_{\mathrm{plan}}=
\bigwedge_r
\left[
p_{4,r}=p_3 \land v_{4,r}=v_3
\right].
\]

只有 \(B_{\mathrm{plan}}=1\) 时，D4 的联盟状态才属于当前 D3 计划。旧代 D4 即使为
committed，也不能证明新计划已经完成仲裁。D3 和 D4 同时发布区域 epoch 或 lease 时，
D6 再比较对应值；缺少一侧字段时写为 unavailable，不假定二者相等。无论能否交叉比较，
D4 权威租约和联盟租约在最后决策时刻都必须有效。

设当前 D3 对目标 \(j\) 的资源集合为 \(R_j\)。目标进入必需联盟集合的条件为
\(|R_j|>1\)，或同代 D4 对该目标发布 `commit_required=true`。单成员 assignment 也可能
携带 `coalition_id`，该字段在此处只用于来源追踪，不能单独证明存在原子提交要求。

对当前计划中目标 \(j\) 的必要成员集合 \(R_j\)、已确认集合 \(A_j\) 和缺失集合
\(M_j\)，联盟通过条件为

\[
B_{\mathrm{coalition},j}=
\mathbf{1}[s_j\in\{\mathrm{committed},\mathrm{executing}\}]
\land(R_j=A_j)
\land(M_j=\varnothing)
\land a_j
\land e_j
...
其中 \(a_j\) 表示原子提交，\(e_j\) 表示执行授权，\(\tau_j\) 为租约到期时刻。
`collecting_acks` 和 `proposed` 都令该条件为 0。D6 同时比较 D3 分配成员与 D4 required
成员，防止旧成员集合或部分 ACK 被当作当前联盟。

逐消息通信处置是独立可用性维度。文件存在时，D6 核对每条消息的 transport ID、主题、
源宿、发送/到达时间、最终处置和重试代次。文件缺失不转换为零丢包或零 pending，只记录
unavailable。该证据用于区分丢包、在途和已交付，不能替代计划与联盟状态本身。

## 规则节点与转移残差的独立评价（2026-07-30）

D4 v7 保留确定性 R0 的区域节点动作，只允许学习模型对有向资源转移提出残差。D6 对每个
冻结快照 \(s\) 独立计算三类结果：R0 基准动作 \(a_0(s)\)、标签目标动作
\(a^*(s)\) 和冻结 actor 原始输出经确定性投影后的候选动作 \(\hat a(s)\)。评价过程直接
调用冻结低层模型和数据结构，不调用 D4 v7 高层评价器。

完整动作由区域动作元组和有向转移集合组成：

\[
\operatorname{Action}(s)=
\left(
  \{q_r, b_r, h_r, p_r,\ldots\}_{r\in R},
  \{(u,v,e,n)\}
\right),
\]

其中 \(q_r\) 为区域配额，\(b_r\) 为预备量，\(h_r\) 和 \(p_r\) 表示保持或重规划等
控制字段；有向转移由源区域 \(u\)、目标区域 \(v\)、边标识 \(e\) 和资源数量 \(n\)
确定。D6 序列化并逐字段比较完整 R0 action tuple，避免只检查转移边而遗漏节点动作被
意外改写。

规则正类仍按同快照目标签名是否不同于 R0 定义：

\[
y_{\mathrm{rule}}(s)=
\mathbf{1}\left[
\operatorname{sig}(a^*(s))\ne\operatorname{sig}(a_0(s))
\right].
\]

精确正动作要求投影后完整签名等于目标，并通过投影和干预不变量：

\[
y_{\mathrm{exact}}(s)=
\mathbf{1}\left[
y_{\mathrm{rule}}(s)=1,\
\operatorname{sig}(\hat a(s))=\operatorname{sig}(a^*(s)),\
\operatorname{safe}(\hat a(s))=1
\right].
\]

负类精确 R0 要求 \(y_{\mathrm{rule}}=0\) 且候选完整签名保持 R0。actor-derived
positive 的分母只统计模型实际形成可执行差异、通过不变量且无投影拒绝的帧。v7 聚合有
3 个此类帧，但均位于负类，因此精确命中为 `0/3`；validation 和 test 分母为 0，其比率
保持不可用。规则正类分母为 42，故规则正类召回是可用的 `0/42`。

有向转移错误按方向、边和数量拆分。预测 \((u,v)\) 而目标为 \((v,u)\) 计错误方向；
预测边不在目标集合且不存在反向目标计错误边；边相同而 \(n\) 不同计错误数量。规则负类
上的任一候选转移变化计虚假转移。v7 的三次变化全部是错误边和虚假转移。

投影前后分开审计。原始节点动作必须与 R0 完整元组相同；转移进入确定性投影后，区域配额
可能随资源守恒联动。v7 原始 R0 动作元组偏差为 0，投影后有 3 帧动作元组变化，来源正是
三次错误转移，不应误判为节点学习头输出。

来源独立性继续采用只含在线可观测图张量的键。冻结 v4 train+validation 为
425 帧/251 个唯一键，v7 外部数据为 128 帧/92 个唯一键，精确重合为 0。训练、正式
留出、既有评价、pilot 和本次独立评价 seed 分区互斥；模型拟合、调门、正式留出和既有
评价 payload 读取均为 0。

审计可信边界由固定哈希、执行期不可变性和独立对账组成。raw source、标签导出、标签
dataset、冻结 v4 和 v7 候选五棵输入树在推理前后复哈希；D4 评价树单独复哈希。D6 先
完成逐帧重算，再读取 D4 JSONL、CSV、summary 和 artifact manifest 对账。summary 不能
改变 D6 指标，任何逐帧、汇总、文件或内容摘要不一致均失败关闭。

本轮审计执行通过，只表示审计链条完整。规则正动作 `0/42`、负类虚假转移 3 次，因此
候选评价为失败关闭。D6 不产生注册、置信校准、正式留出、分配、降级、联盟、导引、控制
或物理权限。
...
机器可读制品还要求稳定行尾。逐 split CSV 固定使用 LF，不依赖 `csv` 模块方言的 CRLF
默认值。测试直接检查文件字节中没有 CR，并拒绝空格或制表符行尾，避免版本控制差异检查把
换行控制字符报告为行尾空白。
...
权重和实现是否来自冻结候选。分布层回答输入特征是否落在训练支持域。影子动作层记录模型
是否给出可辨识非零建议。实际采用层回答建议是否经安全链形成后继计划和物理结果。四层不能
互相替代。

模型来源层固定 clean commit、候选清单摘要和权重摘要。原始候选字节来自受版本控制的 D4
`model_registry/region_resource_a2_current_lineage_development_v1/`，clean clone 不依赖被
忽略的 `outputs/`。D6 重读清单、源实现摘要、训练配置、训练摘要、数据集摘要和模型包，
核对数据划分、模型可加载性、参数有限性和权限。通过时：

\[
V_{\mathrm{source}}=1
\]

只表示 `model_source_verified=true`。候选仍为开发/影子状态，全部权限为 false。

运行分布层从原始 shadow 记录重算。设受审记录数为 \(N\)，有限和非有限记录数为
\(N_f,N_{nf}\)，分布内和分布外记录数为 \(N_i,N_o\)。兼容门要求：

\[
N>0,\quad N_f+N_{nf}=N,\quad N_{nf}=0,\quad
N_i+N_o=N,\quad N_o=0
\]

每个 OOD 特征按 node/edge 特征名保留计数。模型动作数 \(N_a\)、动作缺失数 \(N_m\) 和规则
回退数 \(N_r\) 仍写入同一诊断事实，并检查 \(N_a+N_m=N\)，但不参与分布兼容布尔值。
分布内 no-op 或 hold 是合法状态；它可以通过分布门。

影子动作前置条件要求至少出现一个绑定到固定模型的可辨识非零动作。若
\(N_a=0\)，则记录动作缺失；若同时 \(N_r=N\)，再记录
`rule_fallback_only_not_treatment`。这些原因只阻断 rollout treatment，不把输入误写成
分布外。

实际采用层以预注册未见 seed 为单位。candidate 与 R0 必须共享外生配置，但使用不同
episode 和事件日志。candidate 还需具备 D3 严格后继、runtime/owner/coalition ACK 和确认后
物理窗口。在线真值使用必须为 0，受审状态必须有限，候选/R0 指标分母必须完整且相等。
规则回退、no-op 和普通规则重规划不能形成 treatment。缺证据时输出 unavailable，不补 0。

D6 确定性合同 fixture 使用 5 资源/5 目标、2 区域和 6 帧，结果为 6/6 OOD。该结果只用于
合同回归，不是 main 运行证据。main 真实预检分别为 5 资源/5 目标、2 区域、seed 2000 的
3/3 OOD，以及 200 资源/200 目标、8 区域、seed 2001 的 2/2 OOD。三组证据均表明当前
候选不满足运行分布兼容门，但其来源和样本数不能混写。

另一个分布内 no-op 回归证明：分布兼容可以为 true，而影子动作前置条件、采用和配对收益
仍为 unavailable。D6 只消费和审计，不授予准入、辅助、权属、分配、故障接管或控制权限。

## 模型来源信任链（2026-07-28）

D6 readiness v2 当前有两类可信来源适配器。冻结未见种子适配器从训练注册表、共享划分和
四个模块数据集清单重算种子隔离。G1 模型来源适配器从 D5 正式 v5 生产包、外部审计、
装配后审计、校验清单、保留集、成对影子评测和 episode 谱系重算模型来源事实。其余来源
schema 继续拒绝。

模型来源 reference 不提供通过结论。它只登记 13 个文件的位置和摘要。D6 依次执行路径边界
检查、逐文件 SHA-256 复核、固定候选 allow-list 核对、外部审计重算、装配后审计重算和实现
谱系复核：

\[
\text{model-source fact}
=
\operatorname{PostAudit}\!\left(
\operatorname{ExternalAudit}(\operatorname{Rehash}(\text{original artifacts}))
\right)
\]

持久化审计结果、v5 包内嵌外审结果和重新计算结果必须精确一致。模型权重指纹固定为
`sha256:7fb5db8b...1ca71`。替代模型即使拥有完整自签清单，也不能替换已登记的模型身份和
实现谱系。reference 中出现 facts、权限、绝对路径、上级目录、符号链接或摘要错配时直接
失败关闭。

真实正向验证显式使用
`/tmp/MSM-d5-g1-formal-evidence-8d5e02e-20260727`。clean source worktree 是同级
`/tmp/MSM-d5-g1-formal-8d5e02e`。适配器没有目录发现逻辑，不会扫描 `/tmp`，也不会修改
证据树。仓库根缺少 reference 指向的 13 项原制品，因此以仓库根调用时仍为 unavailable。
仓库中的审计 JSON 只是持久化结果，不能单独作为信任根。

该实现只关闭 G1 `model_source` 软件适配缺口。模型来源审计不证明模型已在某个 episode
实际采用。它也不提供运行确认、物理窗口、唯一同键规则基线、成对非退化、同运行真值使用
分母、有限状态分母或外部授权。G1 仍有八门 unavailable。C1/F1 需要四组件完整来源，
单个 `d5_graph` 组件不能使组合变体就绪。所有模型晋级、分配、接管、相机和控制权限保持
关闭。
...
模型被加载、输出建议或通过影子评测，都不能证明建议进入了运行系统。D6 将证据拆成政策评估、
确定性安全投影、实际发布、运行确认、物理状态窗口和同键规则基线六类阶段。每个阶段只使用对应
模块已经持久化的事实，缺一层时停止在已证实的最高阶段。

审计覆盖三个变体。A1 是 D3 分配代价修正，A2 是 D4 区域资源建议，A3 是 D5 主动视觉动作。
三者不能共享“采用”定义。A1 要证明学习修正确实改变计划并被运行时消费；A2 要证明区域建议
形成后继 D3 计划并被当前权属节点确认；A3 要证明模型动作经安全投影后形成相机命令、收到运行
确认，并由相机反馈证实位姿和视场已经生效。

输入合同分为两个版本。v1 只携带 A1/A2/A3 记录，继续用于旧调用方。v2 增加
`a3_pairing_dispositions`，要求把每个 A3 配对候选的 pairable 或 unpairable 处置结果全部
送入 D6。审计输出和 consumer 当前为 v4：v2 负责完整 disposition 分母，v3 增加候选阶段
细分及输出严格复载，v4 增加候选观测结果清单。旧 v1 输入仍可审计原有 paired 记录，但
disposition 分母明确不可用。
...
计数携带独立 availability。没有记录、缺运行确认、缺物理窗口或缺同键 R0 时，值为
`null/unavailable`。显式记录到“规则 fallback 已执行”时，实际学习采用可记为可用的 0；
该 0 不能扩展为物理窗口或收益 0。这样可以区分“已观察到没有采用”和“没有证据判断是否采用”。
旧 `auditable_benefit_count` 作为等值兼容字段保留。第四级只说明输入可供后续收益审计，
输出写作 `audit_input_available`，不声明正收益或非退化。

A2 在确定性投影后仍可能被安全采用链拒绝。例如 D3 后继计划版本没有严格递增时，D4 记录
`safe_adoption_rejected`，并清空未获认可的后继计划和执行证据。D6 只有在 preparation 与投影
有效、拒绝原因非空、后继计划至物理窗口的全部证据均不存在时，才将实际采用计为可用的 0。
该 0 不产生物理窗口、同键 R0 或收益输入。拒绝记录被篡改、缺原因或夹带后续证据时，四级
计数全部失败关闭。

### A3 配对分母

D5 paired evidence 只覆盖能够形成候选窗口和同键规则窗口的记录。它不能说明有多少候选因未
采用模型动作、缺候选物理窗口、缺规则窗口、规则窗口重复、身份不一致或物理结果不完整而无法
配对。D6 因此把 disposition inventory 作为独立分母。

每条 disposition 先由 D5 公共严格校验器重构。D6 随后要求
`adoption_trace_sha256` 唯一。pairable disposition 必须对应一条且仅一条顶层 A3 paired
evidence，内嵌证据与顶层记录必须逐字段相同。顶层 A3 记录也不得游离在 pairable disposition
之外。候选数 \(N\)、可配对数 \(N_p\) 和不可配对数 \(N_u\) 满足：

\[
N=N_p+N_u,\qquad r_p=\frac{N_p}{N}
\]

其中 \(r_p\) 为配对覆盖率。D6 对 D5 枚举的每个 reason code 输出计数，并再次检查 reason
计数总和等于候选数。字段、摘要、trace 唯一性、嵌套证据或计数任一不一致时，inventory
失败关闭。

D5 disposition v2 在顶层原因之外携带候选阶段证据。该证据将命令有效期、运行确认、相机
反馈、匿名观测双时间戳和物理窗口状态绑定到同一候选 trace。D6 以多标签方式统计运行确认
缺失、确认未生效、命令过期或时序不一致、反馈缺失、匿名观测缺失或不完整、物理窗口确认
缺失或不完整。一个候选可以同时命中多个细分原因，因此细分 assignment 数不要求等于候选数；
带细分和无细分的记录数必须守恒。

顶层 `candidate_physical_window_missing` 构成独立细分分母。D6 分别记录已有明确阶段证据和
仍未解决的数量。D5 disposition v1 没有阶段证据，只保留顶层原因并计入未解决。D6 不从后续
窗口、离线真值或空字段推断缺失阶段。

合法 unpairable 记录仍可形成完整原因清单。它不能补出该候选的实际采用、物理窗口、同键
规则参考或收益。只要 inventory 中存在 unpairable，A3 四级执行和收益计数保持 unavailable。
`inventory_completeness=true` 只说明调用方提供的 disposition 集合内部守恒且与顶层 paired
evidence 一致，不代表模型在全部场景上的证据完整。输出
`complete_model_evidence_claimed=false`。因此 152 条 pairable 子集不能替代 536 个候选的完整
分母。阶段细分完整也不改变该权限和可用性边界。

### 开发批次结果

main 使用 seeds 1000-1019 生成开发批次并交由当前 D6 v2 consumer 只读审计。A2 共评估 20/20
个候选，没有识别到区域干预，实际采用和 A2/R0 收益审计均为 0。20 条原因全部是
`identifiable_regional_intervention_missing`。该结果证明无操作候选没有被归因成学习采用，
不证明区域策略有效或无效。A2 批次 SHA-256 为
`ff3c10a089b6a94582451ae05d8a884af3a2bd7485acd4df0496442ea7e0ec55`。

A3 完整清单有 536 条 disposition。152 条可配对，384 条不可配对，覆盖率为 28.36%；20/20
个 seed 都有可配对子集。全部不可配对原因是 `candidate_physical_window_missing`。完整清单
下 `a3_auditable_pair_count=0`，实际采用、物理窗口、同键 R0 和收益计数均不可用。该结果说明
物理窗口证据链没有覆盖完整候选分母。它不支持主动视觉模型收益、非退化或准入结论。A3 批次
SHA-256 为 `455d181076553a485ff824618abc6d037a4477bb6342877d1d1e427fd28583a9`。
该冻结批次没有 D5 disposition v2 候选阶段证据，其自身仍不能进一步归因。

main 随后用同配置 seeds 1000-1019 进行一次内存态开发探针。536/536 个候选均形成阶段证据，
但完整可审计 seed 仍为 0。344 条候选同时出现匿名观测缺失和物理窗口确认缺失，另有 40 条
observation inventory 不完整但没有 stage reason。D6 不从 inventory 状态推导一个未声明的
细分枚举，而是将物理窗口缺失的 384 条分母保留为 344 条 evidenced 和 40 条 unresolved，
detail completeness 为 false。运行 ACK 缺失、运行确认缺失、命令过期、命令时序错配和相机
反馈缺失均为 0。开发摘要 SHA-256 为
`1ba6040e7c3e7e3b9e7d5506dfd20cf3539ce12c5aac13cca7f02799f0cd99ef`。该运行保持
`source_worktree_clean=false`、`formal_evidence=false`、
`persisted_full_pair_inventory=false`。这些细分只缩小当前断点范围，不替换冻结 v1 批次，
也不改变四级指标和全部权限的失败关闭状态。

第二次同配置开发复跑接入 D5 v2 零检测帧后，候选数由 536 调整为 492，其中 488 条可配对、
4 条不可配对，配对覆盖率为 99.18699%。候选窗口包含 159 个 v1 locked 帧和 329 个 v2
reacquire 帧。329 个 v2 帧均表示图像已处理但没有检测，不是目标可见证据，其覆盖率为 0，
locked 和 ambiguous 计数为 0。4 条缺失来自默认 1% 通信丢包；相应 4 个 seed 关闭丢包后均
可配对。该复跑来自未提交工作树，没有持久化完整逐候选清单，也没有未见 seed 证明。旧
536/152/384 冻结批次及其摘要不变。

### 零检测帧口径

D6 将“物理观测证据可审计”和“目标被看见”作为两个变量。D5 v2 零检测帧证明相机在命令生效
后处理了一帧图像，因而可以关闭原来的“没有观测事实”缺口。有中心分配目标时，该帧的关联
状态固定为重新捕获，分配目标可见标志固定为 false；无分配目标时，关联和覆盖结果均不可用。
它不创建局部轨迹，不绑定全局航迹，也不提供正向身份依据。

v4 的候选观测结果清单分别汇总普通轨迹帧、零检测帧、四类关联状态、分配引用数和可见引用数。
覆盖率为可见引用数与分配引用数之比，可以合法等于 0。清单可用只说明输入完整且数值可复算。
正收益、非退化和全部运行权限继续固定为 false。零检测帧若被序列化为 locked、ambiguous、
hold 或可见，输出严格复载失败。

### 严格边界

A3 的公共 validator 可以重算命令、运行确认、相机反馈、窗口摘要、同键身份和权限。D6 不依赖
adoption trace 上不存在的派生属性，而是用 trace 已有的场景、规模、seed、窗口、相机、资源、
目标引用、冻结外生摘要和计划/联盟/通信版本显式形成身份元组，再与候选/R0 窗口公开身份比较。
候选窗与 R0 窗可以来自两个独立 episode，但必须使用相同冻结外生配置摘要。

D6 从 sample key 提取 episode 标识。候选与 R0 episode 必须不同。同一 episode 的多个窗口
允许共享一个事件日志身份摘要，这与 main 当前按 `episode_id + stream + schema` 生成摘要的
方式一致；一个摘要不能绑定不同 episode，同一 episode 不能出现第二个日志身份。窗口不能跨
comparison key 复用，一个 R0 窗口不能支撑多个配对。synthetic 来源、在线真值计数和全局航迹
身份改写继续作为硬阻断。

A1 的核心 DTO 有公共 validator，可以确认候选、选择、发布和 lifecycle 摘要没有被改写。
当前 lifecycle 只保留运行确认和物理窗口的摘要计数，没有持久化运行来源载荷及 R0 配对身份。
D6 因此只报告“声明已通过模块校验”的最高阶段，不将其换算为实际采用或收益。

A2 按旧单臂和新配对两路审计。旧记录仍使用 D4 安全采用 DTO，逐层重建建议、后继计划、运行
分配确认、权属确认、联盟成员确认和物理窗口。完整链可报告实际采用和物理窗口；没有 R0 时
保持后两级 unavailable。

旧单臂记录的终止态单独处理。`candidate_rejected` 表示投影前拒绝；
`safe_adoption_rejected` 表示投影完成后，安全采用链在形成有效后继执行证据前拒绝。后一状态
要求 D3 计划、运行确认、权属回执、联盟提交对象和物理窗口均为空。无联盟需求时
`coalition_commit_available=true` 只表示该条件无需提交，不作为执行证据。两类合法拒绝都只
能证明实际采用为 0。

新 pair wrapper 使用 D4 已发布的 `RegionResourceA2BenefitAuditInput`，批量输入使用
`RegionResourceA2BenefitAuditBatch`。D6 先严格重建输入中的旧安全采用记录，并以其原始
`content_sha256` 建立唯一索引；随后根据 wrapper 的
`safe_adoption_evidence_sha256` 查找来源并调用 D4 public validator。来源缺失、重复或被多个
wrapper 复用时失败关闭。被引用的旧记录只用于验证来源，不再作为第二次采用计数。

D6 不采用 wrapper 的汇总计数，而是核对候选物理窗摘要，再比较场景、场景版本、规模、seed、
逻辑窗口、窗口时长和 `paired_exogenous_config_sha256`。候选与 R0 必须来自不同 execution
arm，日志摘要不能跨 episode，窗口不能跨键复用，一个 R0 不能重复配对。真实 D4 DTO、public
validator 和 batch 已通过正反向兼容测试。

A1 批量 inventory 仍没有公共 strict loader。D6 不按字段名称构造宽松替代实现，输出
`a1_batch_public_strict_loader_unavailable`。软件 fixture 只验证 consumer 合同；没有 main
持久化的真实 episode 记录时，不形成实际系统收益结论。

main 持久化的运行证据使用
`scalable3d-learning-adoption-evidence-records-v1`。D6 只读取调用方明确列出的
`learning_adoption_evidence.json`，校验固定字段、唯一 episode 标识和内容摘要后合并记录。
跨文件 A2 配对要求安全采用来源文件所在 episode 等于候选 execution arm，且候选/R0 episode
文件都在调用方列表中。A3 trace、候选窗和 R0 窗引用的 episode 也必须存在。D6 不搜索相邻
目录，也不把候选单臂和规则单臂自行组装成新事实；pair wrapper 由 D4/D5 公共装配器生成并只
持久化一次。

### 模块布局

同一公共合同在两种部署方式下具有不同的 Python 导入路径。安装包或显式配置
`PYTHONPATH` 时使用 D3、D4、D5 的顶层包名；在仓库根目录直接运行时，模块位于
`research_modules...` 命名空间。D6 先查找顶层包，只在异常明确指出请求模块或其父包不存在时
查找仓库路径。A2 的区域建议、运行确认和通信回执使用同一规则。

该回退只处理包布局。若公共模块存在，但导入时缺少内部依赖或执行失败，原异常继续向上抛出。
D6 不把模块故障降格成“证据不可用”，避免掩盖损坏的上游合同。

所有结果权限固定为 false。通过审计不允许模型晋级，不改变默认路径，也不授予分配、降级、
相机、控制或 `global_track_id` 修改权限。

当前软件验证口径为 D6 strict audit `64 passed, 1 warning in 11.79s` 和 main A3 paired
smoke `1 passed, 1 warning in 3.29s`。严格专项包含不依赖 D5 测试夹具的独立子进程输出复载，
并已将 A3 inventory validator 入口固定在公开输出校验器之前。v4 新增零检测帧 0 覆盖正例
和 locked/ambiguous/覆盖率篡改负例。当前 D6 全量回归为
`1106 passed, 1 warning in 100.94s`。此前 main 的
`paired_learning_adoption 5 passed`、scalable `345 passed, 1 warning`、cross-module
`8 passed` 和 D6 全量 `1093 passed, 1 warning in 98.33s` 都是当前 v3 改动前的冻结历史证据。
...
不能交给 v5 assembler。正式 v2 证据已于 2026-07-27 使用全新证据目录运行，旧目录未被复制、
重绑定或复用。

## D5 G1 正式证据链（2026-07-27）

正式证据链在 clean commit `8d5e02ec989259ce3d39e1e4ad6a90dd0d8d5b54` 上按三个阶段执行：

```text
冻结 development-v3、held-out、paired-shadow 和 paired lineage
  -> D6 external audit v2
  -> D5 生产 assembler 生成 bundle v5
  -> D5 strict/shadow loader
  -> D6 post-assembly v2
  -> D5 assist 权限门
```

external audit 输出为 `d6.d5-g1-external-audit.v2`，结果 `pass`、blocker 为空。文件
SHA-256 为 `cbd6c72b2d9e7b78bf3aa36f975e6627250d2bf18de5a0b0ebc2c8f6cf760cd6`，
规范内容 SHA-256 为
`334cf662e49c735931019ff358be1894d1358f1b4a5a868759eee41d3d282d15`。candidate lineage
包含 900 条记录和 900 个唯一 episode UID，文件 SHA-256 为
`83e105290f3e624f267d92ceaf050d32291bd5bbbabf98580846cd31498b1af1`。

D5 生产 assembler 生成的 v5 manifest 文件 SHA-256 为
`b431d066362005868374d038eb93a83b773c03715a53d8a9dfd0da21784f317d`。D5 strict loader
和 shadow loader 均通过。D6 装配后输出为 `d6.d5-g1-post-assembly-audit.v2`，结果
`pass`、blocker 为空，内容 SHA-256 为
`17dda42d06b4be1d21ff8f1f8baecc320fd49b532be06a9f9f6b304341763e1`，consumer 为
`d6.d5-g1-post-assembly-audit-consumer.v2`。

两层 D6 审计及 D5 authority contract 均把模型晋级、G1 辅助、默认路径变更、分配、故障
接管和控制权限设为 false。D5 assist 请求因此以
`bundle_g1_assist_authority_not_granted` 失败关闭。外审通过和
`g1_assist_eligible=true` 只表示证据资格，不表示运行授权。

真实相机泛化、中心全局航迹标识绑定正确性和物理闭环结果在两层审计中均显式 unavailable。
前者需要真实相机、外参漂移、遮挡和在线检测误差数据；中心绑定需要 binding 与隔离离线真值
的连接；物理结果需要导引、控制和拦截记录。三类证据不能由合成边分类、簇指标或装配完整性
代替。
...
辅助、默认路径变更、分配、故障接管和控制权写为 false。D5 装配器以该 JSON 为唯一 D6
输入，并自行验证
...
D6 独立计算的十文件当次实现摘要为 `408e71fe...f4fe`。20 个未见 seed、900 个 episode、
45 个场景规模单元、三项安全零计数、单特征 AUC 和五类扰动门均通过，正式外审 blocker 为空。
主 JSON 文件/内容 SHA-256 为 `10bf19f5...10b0` / `4e24ab33...9e54`。

64cb865 历史运行实现复核使用 clean commit `64cb865b...b05`。D6 重新计算十个运行文件后得到
`55066382...b8ea`，与 manifest、输入期望及 held-out/paired 联合证据一致。输入清单只绑定
当次批次的 registry、bundle、held-out、paired-shadow 和 lineage；文件与内容摘要、训练数据
谱系和 900 条唯一 lineage 均交叉一致。输入目录在审计前后没有变化。

该历史实例覆盖 20 个未见 seed、900 个 episode 和 45 个场景规模单元。held-out F1、错误
...
合成三维投影也不能代替真实相机、真实外参漂移、遮挡和检测误差测试。在 D6 之外完成独立运行
授权并通过实际运行作用域审计之前，规则路径继续保持默认。
...
修复的正式 R0 已在后继 clean source `1e5ed8d` 上启动。以下 177/900 数值是全量完成前的
阶段记录；当前全量结论见本文件末尾“完整范围、后验闭环与系统通过必须分栏”。
当时五个分片累计完成 177/900。
D6 对五个原失败 cell 重新读取 cell result、episode artifact tree、在线总线和 summary，
给出 5/5 clean-formal、实验矩阵 formal 和 generation verified 结论。进度分母与审计分母
分开管理：177 表示当时已执行，5 表示定向专项逐项审计。局部通过不能与旧批次拼接，也不能
外推到其余 172 个已执行 cell；该增量阶段的正式整体结论仍保持 895/900。

该口径由 9 个专项测试和 1243 个 D6 全量测试验证。测试同时覆盖目标输入清单、5 与 177
分母分离、clean-formal 失败关闭、generation 不可用失败关闭和报告范围声明。
...
## 规划建议的生效与收益必须分层（2026-07-29）

D4 给出的区域资源建议属于下一轮规划输入。它本身没有分配、联盟、接管或控制执行权限。D6
按以下层次独立记录：
...
合同链可用要求建议编号、源计划、消费记录和 D3 后继计划严格连接。真实干预要求资源到
`global_track_id` 的绑定集合或目标覆盖发生变化。计划版本、租约或 metadata 单独变化不构成
干预。

缺少独立同键 R0 时，可以比较 source 和 successor 的 assignments/unassigned，结果只能标为
描述性非退化。规则建议器形成的新绑定不能解释为学习模型收益。模型收益必须继续通过严格学习
采纳审计，核对独立 episode、comparison key、来源提交、实际采用和多 seed 结果。

故障代际变化使用另一条口径。旧建议被阻断、未消费且未形成后继时，D6 记录安全围栏通过。
该事件不进入模型性能失败计数。若故障围栏后仍出现旧建议消费或后继，则记录安全违规。

## 完整范围、后验闭环与系统通过必须分栏（2026-07-30）

执行范围完整只回答“预登记 cell 是否全部生成并可复核”。clean formal 回答来源、schema、
在线真值隔离和实验矩阵合同是否成立。后验闭环回答 D1 最终发布是否由 D2 实际消费，是否
存在错误 skip 或 pending。系统严格通过还要求 episode 不保留其他模块失败关闭原因。
四层分母不能互相替代。

source `1e5ed8d` 的正式 R0 说明了这种差异：

- 900 个 cell、20 个分片和全部 artifact tree 完整；
- clean formal、实验矩阵资格和 D1/D2 generation integrity 均为 `900/900`；
- skip 为 0，pending 全排空，在线真值使用为 0；
- 严格系统门为 `872/900`。

28 个严格失败项没有后验代次错误。它们全部位于高威胁 M 对 N 场景，episode 结束时 D4
联盟仍在收集成员确认。D6 保留 `d4_fail_closed:collecting_member_acks`，同时继续报告这些
cell 的 clean formal 和 generation 已通过。这样既不把安全失败关闭误写成感知关联断链，
也不把未完成联盟误写成系统成功。

可用性继续独立表达。D2 身份交换计数 900 项均不可用，因此总量为 `null`。D4/D5 部分安全
计数只在对应 runtime 发布的 700 项可用，另外 200 项不补零。任何跨场景总计只有在完整
分母可用时才生成数值。

本轮 900 项只属于 R0 单臂。完整父矩阵为 `900/5700`，没有 G1、A1、A2、A3 同范围结果。
范围完整和后验闭环不能推导学习算法收益、因果改进或生产准入。

## 来源独立评价的外部复算

D3 A1 来源独立 v2 评价采用两层证据。第一层是 D3 逐帧记录，描述规则矩阵、模型有界代价
修正、投影拒绝和最终有效绑定。第二层是 D6 外部审计。D6 不调用 D3 评价器，只读取冻结合同、
生成证据、匿名数据集和逐帧输出，并把 D3 汇总视为待核对声明。

D6 先验证输入身份。结果目录必须是固定普通文件集合，校验和覆盖全部载荷；模型 manifest、
state dict、bundle 树和评价源码树必须匹配预注册摘要。评价时提交可以早于当前分支 HEAD，
但该提交必须存在，且提交中的冻结文件摘要必须与合同一致。这样可以允许文档和后续代码提交
继续前进，同时保持评价对象不漂移。

数据审计按逐行方式执行。数据集目录必须解析为 generation root 下固定的
`learning_dataset/d3_assignment`，不能替换为结构相似的外部目录。每一帧检查有限值、匿名
资源和目标数量、目标×资源矩阵形状、候选边数量、seed 与 split 绑定。D6 从逐帧的
scenario、episode、seed 和 split 清单独立重算 split SHA-256，不采信 manifest 自报值。
字段名中出现 truth、Actor/Object 身份或全局目标身份时立即失败；相同扫描也用于评价
JSONL，只有值必须为 0 的 `online_truth_use_count` 审计字段例外。正式 seed 1000-1019 不进入
扫描范围。D6 仅在内存保留帧身份、容量、需求、动作掩码和矩阵摘要，不复制 169 MB 原始文件。

传输闭合先于指标结论。D6 按固定 21 列读取 292 行 CSV，将帧身份、教师机会、R0/candidate/
effective 三组选择边、换绑计数、教师与负类标志、OOD、拒绝原因和 fallback 标志与 JSONL
逐行派生值比较。仅修改 CSV 并同步重写结果目录校验和，仍会因 CSV/JSONL 内容不一致而失败。

指标从逐帧记录重建。教师机会帧构成正类分母，无机会帧构成负类分母。有效绑定相对规则绑定
发生安全变化时计入正类安全换绑；有效绑定与教师绑定完全一致时计入教师完全匹配；负类有效
绑定保持规则绑定时计入 exact-R0。投影拒绝帧还必须同时满足矩阵和绑定 exact-R0。任一重复
资源、硬禁边、M 对 N 原子性、版本、规则矩阵突变或模型越权输出都会使审计失败。

安全计数不采用 D3 自报值。选择边按“目标索引、资源索引”解释，D6 从匿名资源
`assignment_capacity` 计算容量超额，从目标需求槽检查每个目标的分配数只能为 0 或完整需求，
并用动作掩码检查硬禁边。R0、candidate、effective 三组独立结果分别与逐帧自报计数闭合；
总体机器门只消费 effective 独立结果。规则矩阵摘要也从数据集逐元素按连续小端双精度格式
重算，不能由评价 JSONL 自报摘要替代。

预注册总体门只按合同既定分母计算。2026-07-31 结果为安全换绑 `13/110`、教师完全匹配
`8/110`、负类规则保持 `182/182`，总体门通过。test 子组教师完全匹配为 `0/25`，该事实
单列为泛化限制。D6 不在看到结果后新增子组门限，也不把总体门通过解释为运行、控制、物理
或生产准入。

本轮补强后，CSV 为固定 21 列、292 行，逐行不一致数为 0；split hash 独立复算为
`f1380dd60fded50b2550e5ce63d6d41bb6066022f9e4b201925978acfa025ca5`。三组选择边各为
21637 条，索引越界、容量超额、硬禁边和 M 对 N 原子性违规均为 0。上述结果来自 D6
数据集重算，并与 D3 自报字段闭合。

## 外生输入、运行时与业务状态分层

D1 物化实现的配对评价不能直接比较整份在线日志哈希。在线日志同时记录传感器输入、模块
输出和通信调度结果，运行耗时变化会改变内部消息顺序或传输摘要。D6 将其分成三个集合：

```text
场景、离线真值、传感器观测 -> 外生输入等价
总线顺序、投递时刻、通信传输元数据 -> 运行时总线时序等价
D1 GlobalTrack 与 D2 关联输出 -> 业务等价
```

外生输入不等价时，两臂不可比较。外生输入相同而运行时总线不同时，D6 仍逐项核对 D1/D2。
状态、协方差、有效时刻、量测与到达时间、A95、航迹等级、全局身份、来源谱系或 D2 关联任一
变化，均记录业务退化。只有运行时元数据变化且 D1/D2 完全一致时，才允许给出业务等价结论；
运行时漂移仍单独保留，不参与外生输入哈希。
