...
树完整性强化后，同一真实 v4 bundle 的只读 dry audit 仍为 `pass`。实际树精确包含六个文件和
一个 `evidence/` 目录，结果内容 SHA-256 为 `37384441...d852`。首次正式输出没有覆盖；强化版
正式结果需在当前实现形成 clean commit 后写入新的独立目录。
