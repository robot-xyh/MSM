...
## 2026-08-10 D5长距离视觉配准专项

### 已完成

- [x] 新增只读 v2/v3 episode loader，兼容直接 episode 目录和包含 `coverage_safe` 的运行根目录；
  三个基础文件缺字段时保留不可用，两个时序文件为可选 v3 证据。
- [x] 按相机和全局汇总短缺口、保持、长期重发现、绑定事件、实际交叉窗口、关联准确率与安全
  计数；所有指标携带可用性、原因和来源。
- [x] 实现结构门：身份切换、有效短缺口中断、绑定振荡、重复分配、在线真值使用和全局编号
  改写均为 0，关联准确率不低于 0.95。
- [x] 实现实际交叉门：可评分窗口至少 10 个且比例不低于 0.30；几何预检不得替代实际检测证据。
- [x] 输出逐 episode CSV、聚合 JSON、中文 Markdown 和综合 PNG；增加失败关闭 CLI。
- [x] 单测覆盖冻结 v2、真实 main v3 行结构、缺字段不可用、无表头空 CSV、预检隔离和门控
  通过/失败，共 `7 passed, 1 warning`；D6 全量回归为
  `1425 passed, 16 skipped, 1 warning in 129.29s`。
- [x] 对冻结 seed `20260810` 只读复验，复现 `1930/1934` 正确关联、短缺口 `3`、长期重发现
  `48`、几何绑定切换 `7` 和交叉可评分 `3/31`，结论保持失败关闭。

### 待完成

- [ ] main 生成带 `temporal_binding_events.csv` 和 `dropout_events.csv` 的真实 v3 episode，D6
  再核验保持恢复、过期、绑定振荡和有效短缺口中断；当前只有软件夹具。
- [ ] main 运行至少 10 个原始尺度 AirSim seeds。D6 汇总交叉可用率和失败原因后，才能提交 P1
  关闭复核。
- [ ] 保持实际交叉证据与 `crossing_geometry_preflight` 分栏；任何 preflight 数值均不得进入正式门。
