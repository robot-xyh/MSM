...
from d6_evaluation_metrics.d1_opaque_source_identity_cache_multiseed import (
    CACHE_CAPACITY,
...
def test_cache_evaluator_binds_frozen_matrix_and_accepts_13_pairs(
    ...
    result = evaluate_d1_opaque_source_identity_cache_multiseed(manifest_path)
    ...
    assert all(
        pair["opaque_source_identity_cache_audit_passed"]
...
def test_cache_report_bundle_is_complete_deterministic_and_read_only(
    ...
    repeated = write_d1_opaque_source_identity_cache_multiseed_report(
        round_trip,
...
def test_candidate_identity_build_conservation_tamper_fails_closed(
    tmp_path: Path,
) -> None:
    manifest_path = _build_evidence(tmp_path)
    _mutate_final_diagnostics(
    ...
    with pytest.raises(
        D1OpaqueSourceIdentityCacheEvidenceError,
        match="identity build conservation",
    ):
        evaluate_d1_opaque_source_identity_cache_multiseed(
            manifest_path, raise_on_invalid=True
        )
...
def test_cache_capacity_mismatch_fails_closed(tmp_path: Path) -> None:
    ...
    profile["configuration"]["d1_opaque_source_identity_cache_capacity"] = 127
...
def test_cache_efficiency_gates_reject_low_hit_candidate(
    ...
    def lower_hit_ratio(diagnostics: dict[str, Any]) -> None:
        operations = diagnostics["operation_counts"]
        operations["cache_hit_count"] = 90_000
        operations["cache_miss_count"] = 10_000
        operations["identity_build_count"] = 10_000
...
def test_manifest_provenance_tamper_fails_closed(
    ...
    with pytest.raises(D1OpaqueSourceIdentityCacheEvidenceError, match=match):
        evaluate_d1_opaque_source_identity_cache_multiseed(
            manifest_path, raise_on_invalid=True
        )


def test_incomplete_arm_returns_unavailable_and_writes_fail_closed_report(
    tmp_path: Path,
) -> None:
    manifest_path = _build_evidence(tmp_path)
    manifest = _read_json(manifest_path)
    manifest["cases"][0]["arms"]["candidate"]["status"] = "failed"
    _write_json(manifest_path, manifest)
    ...
    paths = write_d1_opaque_source_identity_cache_multiseed_report(
        result,
        tmp_path / "unavailable_report",
    )
    assert "plot_png" not in paths
    assert "失败关闭" in paths["markdown"].read_text(encoding="utf-8")


def test_non_source_only_runtime_fails_closed(tmp_path: Path) -> None:
    manifest_path = _build_evidence(tmp_path)
    manifest_file = (
        manifest_path.parent
        / "short_seed_1101"
    ...
    manifest = _read_json(manifest_file)
    profile = manifest["runtime_profile"]
    profile["configuration"]["d1_publish_opaque_source_key"] = False
    manifest["runtime_profile_sha256"] = _canonical_sha256(profile)
    _write_json(manifest_file, manifest)

    with pytest.raises(
        D1OpaqueSourceIdentityCacheEvidenceError,
        match="not source-only",
    ):
        evaluate_d1_opaque_source_identity_cache_multiseed(
            manifest_path, raise_on_invalid=True
        )
...
def test_long_d2_ratio_of_means_gate_fails_without_outlier_removal(
    tmp_path: Path,
) -> None:
    manifest_path = _build_evidence(tmp_path)
    for case_id in ("long_seed_1101", "long_seed_1102", "long_seed_1103"):
        _set_stage_wall(
            manifest_path.parent
            / case_id
            / "candidate_episode"
    ...
    gate = result["admission_gates"][
        "maximum_long_d2_association_mean_increase_pct"
    ]
    assert gate["actual"] == pytest.approx(6.0)
    ...
    for case in matrix["cases"]:
        ...
        for arm in ("reference", "candidate"):
            ...
            arms[arm] = {
                ...
                "expected_commit": D1_OPAQUE_SOURCE_IDENTITY_CACHE_SOURCE_COMMIT,
...
def _write_episode(
    ...
    *,
    ...
    config = {
        ...
        "scenario_name": "d1_opaque_source_identity_cache_test",
        "scenario_version": "d1-opaque-source-identity-cache-test-v1",
        "seed": seed,
    ...
    initial_diagnostics = {
        "schema_version": "d1.opaque_source_identity_cache_diagnostics.v1",
        "implementation_id": implementation_id,
        "candidate_enabled": candidate,
        "cache_capacity": CACHE_CAPACITY,
        "cache_entry_count": 0,
        "cache_generation": None,
        "conservation": _cache_conservation(),
        "operation_counts": {},
    }
    if candidate:
        operation_counts = {
            "request_count": 100_000,
            "identity_build_count": 500,
            "reference_bypass_count": 0,
            "cache_hit_count": 99_500,
        ...
        operation_counts = {
            "request_count": 100_000,
            "identity_build_count": 100_000,
            "reference_bypass_count": 100_000,
            "cache_hit_count": 0,
            "cache_miss_count": 0,
            "cache_eviction_count": 0,
            "peak_entry_count": 0,
    ...
    diagnostics = {
        ...
        "cache_generation": ["D1_FUSION", "test-epoch-v1"],
        "conservation": _cache_conservation(),
    ...
    runtime_profile = {
        ...
        "configuration": {
            ...
            "d1_opaque_source_identity_cache_capacity": CACHE_CAPACITY,
        ...
        "d1_opaque_source_identity_implementation": implementation,
        "d1_opaque_source_identity_cache_diagnostics": initial_diagnostics,
    }
    manifest = {
        "git_commit": D1_OPAQUE_SOURCE_IDENTITY_CACHE_SOURCE_COMMIT,
    ...
    governance = {
        ...
        "d1_opaque_source_identity_implementation": implementation,
        "d1_opaque_source_identity_cache_diagnostics": copy.deepcopy(diagnostics),
        "d1_publish_opaque_source_key": True,
        "d1_d2_structural_ambiguity_hold_enabled": False,
        "d1_scan_events": [],
    }
...
def _cache_conservation() -> dict[str, bool]:
    return {
...
def _set_stage_wall(
    ...
    *,
    ...
    for row in rows:
        ...
        row["mean_wall_time_ms"] = str(wall_time_s * 100.0)
        row["p50_wall_time_ms"] = str(wall_time_s * 80.0)
        row["p95_wall_time_ms"] = str(wall_time_s * 110.0)
        row["max_wall_time_ms"] = str(wall_time_s * 120.0)
    with path.open("w", newline="", encoding="utf-8") as stream:
...
def _mutate_final_diagnostics_for_episode(
    ...
    locations = (
        ...
        governance["d1_opaque_source_identity_cache_diagnostics"],
    )
