...
from d6_evaluation_metrics.online_truth_guard_multiseed import (
...
def test_evaluator_binds_frozen_matrix_and_accepts_13_pairs(
    ...
    result = evaluate_online_truth_guard_multiseed(manifest_path)
    ...
    assert all(
        pair["truth_guard_audit_passed"] for pair in result["pairs"]
    )
    assert all(
        pair[arm]["online_message_count"]
        == pair[arm]["truth_guard_audit"]["validation_count"]
...
def test_report_bundle_is_complete_deterministic_and_read_only(
    ...
    repeated = write_online_truth_guard_multiseed_report(
...
def test_cli_writes_report_and_prints_separate_conclusions(
    ...
    assert (
        output / "online_truth_guard_multiseed_evaluation.json"
    ).is_file()


def test_validation_count_mismatch_fails_closed(tmp_path: Path) -> None:
    manifest_path = _build_evidence(tmp_path)
    summary_path = (
    ...
    summary["online_truth_guard_diagnostics"]["validation_count"] = 2
    _write_json(summary_path, summary)

    with pytest.raises(
        OnlineTruthGuardEvidenceError,
        match="validation_count does not equal message_count",
    ):
        evaluate_online_truth_guard_multiseed(manifest_path)
...
def test_implementation_identity_tamper_fails_closed(
    ...
    summary = _read_json(summary_path)
    summary["online_truth_guard_implementation"] = (
        REFERENCE_IMPLEMENTATION
    )
    _write_json(summary_path, summary)
...
def test_publication_and_core_gates_reject_small_candidate_gain(
    tmp_path: Path,
) -> None:
    manifest_path = _build_evidence(
        tmp_path,
        candidate_publication_main_s=0.99,
        candidate_publication_finalize_s=0.20,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            lambda manifest: manifest.__setitem__(
                "truth_guard_diagnostics_schema_version", "old"
            ),
            "diagnostics schema",
        ),
    ],
...
def test_manifest_provenance_tamper_fails_closed(
    ...
    with pytest.raises(OnlineTruthGuardEvidenceError, match=match):
        evaluate_online_truth_guard_multiseed(manifest_path)
...
def test_episode_dirty_state_fails_closed(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        OnlineTruthGuardEvidenceError, match="repository is dirty"
    ):
        evaluate_online_truth_guard_multiseed(manifest_path)
...
def test_reused_arm_fails_closed(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        OnlineTruthGuardEvidenceError, match="fresh complete arm"
    ):
        evaluate_online_truth_guard_multiseed(manifest_path)
...
def _build_evidence(
    tmp_path: Path,
    *,
    candidate_publication_main_s: float = 0.72,
    candidate_publication_finalize_s: float = 0.15,
    ...
    for case in matrix["cases"]:
        ...
        for arm in ("reference", "candidate"):
            ...
            _write_episode(
                ...
                publication_main_s=(
                    candidate_publication_main_s
                    if is_candidate
                    else 1.0
                ),
                publication_finalize_s=(
                    candidate_publication_finalize_s
                    if is_candidate
                    else 0.2
                ),
                core_wall_s=(
                ...
                external_elapsed_s=(
                    10.1 if is_candidate else 10.2
                ),
                rss_kib=101_000 if is_candidate else 100_000,
                real_time_factor=0.55 if is_candidate else 0.54,
                d1_fusion_wall_s=1.01 if is_candidate else 1.0,
                d2_association_wall_s=0.505 if is_candidate else 0.5,
            )
            ...
            resource_path.write_text(
                "\n".join(
                    [
                        ...
                        + ("101000" if is_candidate else "100000"),
                        "Exit status: 0",
                        "",
                    ]
                ),
            ...
            arms[arm] = {
                ...
                "expected_truth_guard_implementation": implementation,
                "validation_kind": "online_truth_guard",
                "expected_commit": ONLINE_TRUTH_GUARD_SOURCE_COMMIT,
    ...
    manifest = {
        "schema_version": ONLINE_TRUTH_GUARD_EVIDENCE_SCHEMA_VERSION,
        "experiment_id": ONLINE_TRUTH_GUARD_EXPERIMENT_ID,
        "matrix_path": str(matrix_path),
        "matrix_sha256": ONLINE_TRUTH_GUARD_MATRIX_SHA256,
        "matrix": matrix,
        "source_worktree": str(ROOT.resolve()),
        "source_commit": ONLINE_TRUTH_GUARD_SOURCE_COMMIT,
...
def _write_episode(
    ...
    *,
    ...
    arm: str,
    implementation: str,
    publication_main_s: float,
    publication_finalize_s: float,
    core_wall_s: float,
    external_elapsed_s: float,
    rss_kib: int,
    real_time_factor: float,
    d1_fusion_wall_s: float,
    d2_association_wall_s: float,
) -> None:
    del external_elapsed_s, rss_kib
    ...
    manifest = {
        ...
        "git_commit": ONLINE_TRUTH_GUARD_SOURCE_COMMIT,
    ...
    stage_values = {
        ...
        "module_publication_bus": publication_main_s,
        "module_publication_bus_finalize": publication_finalize_s,
    }
    summary = {
        "episode_id": episode_id,
        ...
        "online_truth_guard_diagnostics": {
            ...
            "implementation": implementation,
            "candidate_enabled": arm == "candidate",
            "validation_count": 1,
        },
