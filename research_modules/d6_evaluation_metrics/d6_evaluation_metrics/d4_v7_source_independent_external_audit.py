"""Independent, read-only D6 audit of the frozen D4 v7 actor.

The D4 summary is treated only as a claim to reconcile. Metrics are rebuilt
from the frozen labeled dataset, the exact v7 model bundle, and the common D4
...
D4_V7_EXTERNAL_AUDIT_INPUT_SCHEMA = (
    "d6.d4-v7-source-independent-external-audit-input.v1"
)
D4_V7_EXTERNAL_AUDIT_SCHEMA = (
    "d6.d4-v7-source-independent-external-audit.v1"
)
D4_V7_EXTERNAL_RECORD_SCHEMA = (
    "d4-region-resource-v7-source-independent-frame-evaluation-v1"
)
D4_V7_EXTERNAL_AUDIT_STATUS = (
...
_EXPECTED_NEGATIVE_COUNTS = {"train": 66, "validation": 11, "test": 9}
...
_EXPECTED_PRIOR_EVALUATION_SEEDS = frozenset(
    tuple(range(3000, 3040)) + tuple(range(4000, 4080))
)
_EXPECTED_PILOT_SEEDS = frozenset(range(5200, 5216))
...
_EXPECTED_EXTERNAL_UNIQUE_KEY_COUNT = 92
_EXPECTED_CANDIDATE_ID = (
...
_REQUIRED_HASHES = {
    ...
    "candidate_source_binding_content",
...
_D4_METRIC_FIELDS = (
    ...
    "actor_raw_transfer_change_frame_count",
    ...
    "raw_wrong_direction_count",
    "raw_wrong_quantity_count",
    "raw_wrong_edge_count",
    ...
    "r0_action_tuple_difference_count",
    "projected_r0_action_tuple_difference_frame_count",
...
class D4V7ExternalAuditError(ValueError):
    """Stable fail-closed error for invalid v7 audit evidence."""
...
class D4V7ExternalAuditInputs:
    ...
    schema_version: str = D4_V7_EXTERNAL_AUDIT_INPUT_SCHEMA
    ...
    def __post_init__(self) -> None:
        ...
        if self.schema_version != D4_V7_EXTERNAL_AUDIT_INPUT_SCHEMA:
    ...
    def from_mapping(
        ...
        *,
        ...
        _require_exact_keys(value, expected, "v7 external audit input")
        return cls(
            ...
            candidate_v7_root=Path(str(value["candidate_v7_root"])),
...
def load_d4_v7_external_audit_inputs(
    path: str | Path,
    *,
    repository_root: str | Path,
) -> D4V7ExternalAuditInputs:
    ...
    payload = _read_json(Path(path), "v7 external audit input")
    return D4V7ExternalAuditInputs.from_mapping(
        payload,
        repository_root=repository_root,
    )


def audit_d4_v7_source_independent_external(
    inputs: D4V7ExternalAuditInputs,
) -> dict[str, Any]:
    """Recompute the D4 v7 external evaluation without trusting its summary."""
    ...
    result = {
        "schema_version": D4_V7_EXTERNAL_AUDIT_SCHEMA,
        "audit_id": inputs.audit_id,
        "evaluated_at_utc": inputs.evaluated_at_utc,
        "status": D4_V7_EXTERNAL_AUDIT_STATUS,
        ...
        "scope": {
            ...
            "d4_high_level_evaluator_call_count": 0,
        },
        ...
        "admission_conclusion": {
            ...
            "reason_codes": [
                ...
                "validation_transfer_activation_absent",
                "test_transfer_activation_absent",
                "train_false_transfer_three",
                "actor_derived_exact_positive_zero_of_three",
        ...
        "limitations": [
            ...
            "validation_and_test_raw_transfer_change_counts_are_zero",
            "three_train_actor_derived_actions_are_false_transfers",
            "no_confidence_calibrator_exists_and_no_confidence_gate_was_applied",
            "formal_holdout_and_prior_evaluation_payloads_were_not_read",
        ],
    }
...
def write_d4_v7_external_audit_report(
    output_dir: str | Path,
    ...
    try:
        json_path = temporary / "d4_v7_external_audit_summary.json"
        csv_path = temporary / "d4_v7_external_audit_by_split.csv"
        records_path = temporary / "d4_v7_recomputed_records.jsonl"
        report_path = temporary / "D4_V7_SOURCE_INDEPENDENT_EXTERNAL_AUDIT_CN.md"
...
def _capture_input_summaries(
    ...
    return {
        ...
        "candidate_v7_tree_sha256": _tree_sha256(inputs.candidate_v7_root),
...
def _audit_hashes_bindings_and_load(
    ...
    *,
    ...
    candidate_manifest_path = (
        inputs.candidate_v7_root
        / "v7_rule_node_transfer_residual_candidate_manifest.json"
    )
    candidate_bundle_manifest_path = (
        inputs.candidate_v7_root / "bundle/manifest.json"
    )
    candidate_state_path = inputs.candidate_v7_root / "bundle/state_dict.pt"
    candidate_training_audit_path = (
        inputs.candidate_v7_root / "training_audit.json"
    )
    candidate_source_binding_path = (
        inputs.candidate_v7_root / "source_binding.json"
    )
    candidate_training_config_path = (
        inputs.candidate_v7_root / "training_config.json"
    )
    candidate_manifest = _read_json(
        candidate_manifest_path,
        "candidate v7 rule-node transfer residual manifest",
    )
    candidate_bundle_manifest = _read_json(
        candidate_bundle_manifest_path,
        "candidate v7 rule-node transfer residual bundle manifest",
    )
    candidate_training_audit = _read_json(
        candidate_training_audit_path,
        "candidate v7 rule-node transfer residual training audit",
    )
    candidate_source_binding = _read_json(
        candidate_source_binding_path,
        "candidate v7 rule-node transfer residual source binding",
    )
    candidate_model, loaded_bundle_manifest = api["_load_v7_model_bundle"](
        inputs.candidate_v7_root / "bundle",
    ...
    actual = {
        ...
        "candidate_tree": _tree_sha256(inputs.candidate_v7_root),
        ...
        "candidate_source_binding_content": _verify_content_sha256(
            candidate_source_binding,
            "candidate v7 source binding",
        ),
...
def _verify_candidate_bindings(
    *,
    ...
    candidate_bindings = {
        ...
        "source_binding_content_sha256": actual[
            "candidate_source_binding_content"
        ],
        "source_a_dataset_sha256": actual["frozen_v4_dataset"],
        "source_a_split_sha256": actual["frozen_v4_split"],
    }
    ...
    sources = _mapping(source_binding.get("sources"), "v7 training sources")
    source_a = _mapping(
        sources.get("frozen_v4_train_validation"),
        "v7 frozen v4 source",
    )
    source_b = _mapping(
        sources.get("m16n24_labeled_train_validation"),
        "v7 M16N24 source",
    )
    source_a_bindings = {
        ...
        "base_v4_model_state_sha256": str(
            ...
            else source_a["base_v4_model_state_sha256"]
        ),
    }
    for field, expected in source_a_bindings.items():
        if source_a.get(field) != expected:
            _fail("candidate_source_a_binding_mismatch", field)
    for field in ("dataset_sha256", "dataset_split_sha256"):
        manifest_field = (
            "source_b_dataset_sha256"
            if field == "dataset_sha256"
            else "source_b_split_sha256"
        )
        if source_b.get(field) != candidate_manifest.get(manifest_field):
            _fail("candidate_source_b_binding_mismatch", field)
    if (
        ...
        or source_binding.get("independent_evaluation_payload_read_count") != 0
        or source_a.get("test_payload_read_count") != 0
        or source_a.get("formal_holdout_payload_read_count") != 0
        or source_a.get("prior_evaluation_payload_read_count") != 0
        or source_a.get("independent_evaluation_payload_read_count") != 0
        or source_a.get("v5_candidate_consumed") is not False
        or source_b.get("test_payload_read_count") != 0
        or source_b.get("formal_holdout_payload_read_count") != 0
        or source_b.get("prior_evaluation_payload_read_count") != 0
        or source_b.get("independent_evaluation_payload_read_count") != 0
    ):
    ...
    *,
    ...
    for label, payload in (
        ...
        for field in (
            ...
            "prior_external_evaluation_payload_read_count",
            "independent_evaluation_payload_read_count",
        ):
            if field in payload and payload.get(field) != 0:
                _fail("forbidden_payload_read_detected", f"{label}:{field}")
    if (
        training_audit.get("test_payload_fit_count") != 0
        or training_audit.get("validation_fit_count") != 0
        or training_audit.get("validation_weight_fit_count") != 0
        or training_audit.get("confidence_calibration_available") is not False
        ...
        or d4_integrity.get("confidence_calibration_count") != 0
        or d4_integrity.get("candidate_mutation_count") != 0
        or d4_integrity.get("registration_count") != 0
        or d4_integrity.get("admission_count") != 0
    ):
    ...
    for split_name in _SPLIT_ORDER:
        ...
        for episode in loaded.episodes(split):
            for frame in episode.frames:
                ...
                decision = _infer_v7_transfer_residual(
                    frame.snapshot,
                    model=model,
                ...
                raw_signature, raw_payload = api["executable_signature"](
                    projector.build_advisory_contract(frame.snapshot, raw)
                )
                ...
                r0_action_tuple_preserved = bool(
                ...
                records.append(
                    {
                        "schema": D4_V7_EXTERNAL_RECORD_SCHEMA,
...
def _infer_v7_transfer_residual(
    snapshot: Any,
    *,
    model: Any,
    rule_policy: Any,
    ...
    """Rebuild one v7 raw recommendation from low-level frozen components."""

    baseline = rule_policy.recommend(snapshot)
    graph = api["snapshot_to_region_graph"](
        snapshot,
        device=api["_model_device"](model),
    )
    fractions = api["_baseline_transfer_fractions"](graph, baseline)
    with api["torch"].no_grad():
        output = model(graph, fractions)
    api["_validate_v7_output"](output, edge_count=graph.edge_count)
    activated_indices = api["_decode_active_edges"](
        output.activation_logits,
        frame_activation_logit=output.frame_activation_logit,
        threshold=float(candidate_manifest["activation_threshold"]),
    )
    baseline_counts = api["_transfer_count_by_key"](baseline)
    transfers = dict(api["_transfer_by_key"](baseline))
    ...
    for index in activated_indices:
        ...
        if count <= 0:
            ...
            continue
        transfers[key] = api["RegionTransferSuggestion"](
    ...
    recommendation = api["RegionResourceRecommendation"](
        ...
        policy_version=_EXPECTED_MODEL_VERSION,
        source=api["RecommendationSource"].LEARNED,
        confidence=0.0,
        actions=baseline.actions,
        transfers=tuple(transfers[key] for key in sorted(transfers)),
        projected=False,
        model_sha256=str(candidate_manifest["model_state_content_sha256"]),
...
def _summarise_records(
    ...
    return {
        ...
        "actor_raw_residual_activation_count": sum(
            int(row["actor_raw_residual_activation_count"])
            for row in available
        ),
        "actor_raw_transfer_change_count": sum(
            int(row["actor_raw_transfer_change_count"])
            for row in available
        ),
        "actor_raw_transfer_change_frame_count": sum(
            bool(row["actor_raw_transfer_change_count"])
            for row in available
        ),
        ...
        "actor_projected_transfer_change_count": sum(
            int(row["actor_projected_transfer_change_count"])
            for row in available
        ),
        "raw_correct_directed_edge_count": sum(
            int(row["raw_correct_directed_edge_count"])
            for row in available
        ),
        "raw_wrong_direction_count": sum(
            int(row["raw_wrong_direction_count"]) for row in available
        ),
        "raw_wrong_quantity_count": sum(
            int(row["raw_wrong_quantity_count"]) for row in available
        ),
        "raw_wrong_edge_count": sum(
            int(row["raw_wrong_edge_count"]) for row in available
        ),
        ...
        "r0_action_tuple_preservation_failure_count": sum(
            not bool(row["r0_action_tuple_preserved"])
            for row in available
        ),
        "r0_action_tuple_difference_count": sum(
            int(row["r0_action_tuple_difference_count"])
            for row in available
        ),
        "projected_r0_action_tuple_difference_frame_count": sum(
            bool(row["projected_r0_action_tuple_difference_count"])
            for row in available
        ),
...
def _validate_recomputed_inventory(
    ...
    expected_aggregate = {
        ...
        "rule_negative_count": 86,
        "actor_raw_residual_activation_count": 10,
        "actor_raw_transfer_change_count": 3,
        "actor_raw_transfer_count": 3,
        "actor_projected_transfer_count": 3,
        "actor_projected_transfer_change_count": 3,
        "raw_wrong_edge_count": 3,
        "projected_exact_positive_action_count": 0,
        "negative_exact_r0_count": 83,
        "wrong_direction_count": 0,
        "wrong_quantity_count": 0,
        "wrong_edge_count": 3,
        "false_transfer_count": 3,
        "projection_rejection_count": 0,
        "invariant_failure_count": 0,
        "r0_action_tuple_preservation_failure_count": 0,
        "r0_action_tuple_difference_count": 0,
        "projected_r0_action_tuple_difference_frame_count": 3,
        "actor_derived_positive_denominator_count": 3,
    }
    ...
    if (
        actor_metric["availability"] != "available"
        or actor_metric["value"] != 0.0
        or actor_metric["numerator"] != 0
        or actor_metric["denominator"] != 3
    ):
        _fail("actor_derived_rate_misreported", str(actor_metric))
...
def _reconcile_d4_evaluation(
    ...
    *,
    ...
    d6_jsonl_sha256 = _jsonl_records_sha256(recomputed_records)
    d4_jsonl_sha256 = anchors["actual_sha256"]["d4_records_jsonl_file"]
    if d6_jsonl_sha256 != d4_jsonl_sha256:
        _fail(
            "d4_d6_jsonl_byte_sha256_mismatch",
            f"d4={d4_jsonl_sha256}:d6={d6_jsonl_sha256}",
        )
    ...
    return {
        ...
        "d4_jsonl_byte_sha256": d4_jsonl_sha256,
        "d6_jsonl_byte_sha256": d6_jsonl_sha256,
        "d4_d6_jsonl_byte_identical": True,
...
def _audit_no_confidence_gate(
    *,
    ...
    if status.get("fixed_minimum_confidence_gate_applied") is not False:
        invalid.append("summary_fixed_confidence_gate_applied")
    if status.get("confidence_calibration_count") != 0:
        invalid.append("summary_confidence_calibration_count")
    if d4_integrity.get("confidence_calibration_count") != 0:
        invalid.append("integrity_confidence_calibration_count")
    ...
    return {
        ...
        "admission_evaluation_count": 0,
        "reserved_manifest_value": None,
        "fixed_confidence_gate_applied": False,
        "reason": (
            "v7 actor has no confidence calibrator and no fixed confidence "
            "gate was applied"
        ),
    }
...
def _audit_closed_permissions(
    ...
    if (
        ...
        or any(
            ...
            for value in permission_values.values()
        )
    ):
    ...
    return {
        ...
        "candidate_permission_values": dict(sorted(permission_values.items())),
        "d4_summary_permission_values": dict(
            sorted(summary_permissions.items())
        ),
            ...
            writer.writerow(
                {
                    ...
                    "actor_raw_residual_activation_count": row[
                        "actor_raw_residual_activation_count"
                    ],
                    "actor_raw_transfer_change_count": row[
                        "actor_raw_transfer_change_count"
                    ],
                    ...
                    "actor_projected_transfer_change_count": row[
                        "actor_projected_transfer_change_count"
                    ],
                    ...
                    "r0_action_tuple_preservation_failure_count": row[
                        "r0_action_tuple_preservation_failure_count"
                    ],
                    "projected_r0_action_tuple_difference_frame_count": row[
                        "projected_r0_action_tuple_difference_frame_count"
                    ],
...
def _render_chinese_report(result: Mapping[str, Any]) -> str:
    ...
    reconciliation = result["d4_artifact_reconciliation"]
    lines = [
        "# D4 v7 来源独立评价 D6 盲审",
        "",
        "## 结论",
        "",
        (
            "D6 从冻结标签数据、v7 模型参数、同快照确定性 R0 规则和投影约束重新"
            "计算 128 帧动作。D4 高层评价器调用数为 0，D4 汇总字段没有作为指标"
            "来源。D6 重算记录与 D4 逐帧记录完全一致。"
        ),
        (
            "规则正类共 42 帧，精确正动作命中为 0/42。validation 和 test 均未"
            "形成转移变化。train 出现 3 次可执行转移变化，全部位于负类且均为错误边，"
            "对应 3 次虚假转移。候选没有形成来源独立的正确转移能力。"
        ),
        (
            "投影拒绝、干预不变量失败和原始 R0 完整动作元组偏差均为 0。该安全结果"
            "不能抵消正动作泛化失败。评价结论为失败关闭；置信校准、正式留出、运行"
            "预检、D3、D7、联盟、控制和物理权限全部关闭，系统继续使用 R0。"
        ),
        "",
        "## 数据边界",
        "",
    ...
    for split in _SPLIT_ORDER:
        ...
        lines.append(
            ...
            f"{row['actor_raw_residual_activation_count']} | "
            f"{row['actor_raw_transfer_change_count']} | "
            f"{row['projected_exact_positive_action_count']} | "
            f"{row['negative_exact_r0_count']} | "
            f"{row['wrong_edge_count']} | "
            f"{row['false_transfer_count']} |"
        )
    lines.extend(
        [
            "",
            (
                ...
                f"{aggregate['negative_exact_r0_rate']:.6f}。错误方向和错误数量均为 0，"
                f"错误边和虚假转移均为 {aggregate['wrong_edge_count']}。"
            ),
            (
                f"模型产生 {aggregate['actor_raw_residual_activation_count']} 次原始边激活，"
                f"只有 {aggregate['actor_raw_transfer_change_count']} 次改变了 R0 转移。"
                "其余激活经资源数量解码后没有形成转移变化。"
            ),
            (
                "validation 和 test 的 actor-derived positive 分母均为 0，相关比率保持 "
                "unavailable；聚合分母为 3，精确正动作仍为 0/3。"
            ),
            (
                f"原始 R0 完整动作元组偏差为 "
                f"{aggregate['r0_action_tuple_preservation_failure_count']} 帧；"
                f"投影后动作元组变化为 "
                f"{aggregate['projected_r0_action_tuple_difference_frame_count']} 帧。"
                "后者来自 3 次错误转移进入确定性投影后的配额联动，不属于节点学习头。"
            ),
            "",
            "## 来源独立性",
            "",
            ...
            (
                f"| v7 外部数据 | {observable['external_frame_count']} | "
            ...
            f"- labeled dataset tree：`{anchors['labeled_dataset_tree']}`",
            f"- frozen v4 tree：`{anchors['frozen_v4_tree']}`",
            ...
            (
                f"- D4/D6 JSONL 字节 SHA-256："
                f"`{reconciliation['d4_jsonl_byte_sha256']}`"
            ),
            "",
            (
                "五棵冻结输入树在审计前后保持一致。D4 评价树也未变化，"
                f"`input_mutation_count={immutability['input_mutation_count']}`。"
            ),
            (
                "D4 的 CSV 与 JSONL 传输值一致；summary、input integrity、"
                "observable overlap 和 artifact manifest 的文件摘要、内容摘要及相互"
                "绑定均通过核对。"
            ),
            "",
            "## 下一门",
            "",
            (
                "v7 停止晋级。后续候选需先解决跨来源转移边识别：validation 和 test "
                "必须出现非零转移变化及非零精确正动作，同时保持零虚假转移、零投影"
                "拒绝、零不变量失败和零原始 R0 动作元组偏差。满足该门后，才可单独"
                "设计置信校准和正式留出评价。"
            ),
            "",
        ]
...
def _load_d4_api(repository_root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        from research_modules.d4_distributed_fallback.d4_distributed_fallback.region_resource_v7_rule_node_residual_candidate import (
            _baseline_transfer_fractions,
            _decode_active_edges,
            _load_v7_model_bundle,
            _model_device,
            _transfer_by_key,
            _transfer_count_by_key,
            _validate_v7_output,
        )
        import torch
    ...
    return {
        ...
        "RegionResourceRecommendation": RegionResourceRecommendation,
        "RegionTransferSuggestion": RegionTransferSuggestion,
        ...
        "_baseline_transfer_fractions": _baseline_transfer_fractions,
        "_decode_active_edges": _decode_active_edges,
        "_load_v7_model_bundle": _load_v7_model_bundle,
        "_model_device": _model_device,
        "_transfer_by_key": _transfer_by_key,
        "_transfer_count_by_key": _transfer_count_by_key,
        "_validate_v7_output": _validate_v7_output,
        "torch": torch,
    }
...
def _write_jsonl(
    ...
    path.write_bytes(_jsonl_records_bytes(rows))


def _jsonl_records_bytes(
    rows: Sequence[Mapping[str, Any]],
) -> bytes:
    return "".join(
        json.dumps(
...
def _jsonl_records_sha256(
    rows: Sequence[Mapping[str, Any]],
) -> str:
    return sha256(_jsonl_records_bytes(rows)).hexdigest()
...
def _fail(code: str, detail: str) -> None:
    raise D4V7ExternalAuditError(code, detail)
