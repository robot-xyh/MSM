"""Strict read-only D1 scan-input same-commit multi-seed evaluation.
...
D1_SCAN_INPUT_MULTISEED_AGGREGATE_SCHEMA_VERSION = (
    "d6.d1_scan_input_multiseed_aggregate.v1"
)
...
D1_SCAN_INPUT_MATRIX_SCHEMA_VERSION = (
    "scalable3d-d1-scan-input-multiseed-matrix-v1"
)
D1_SCAN_INPUT_EXPERIMENT_ID = "d1-scan-input-multiseed-20260724-v1"
D1_SCAN_INPUT_MATRIX_SHA256 = (
    "3e852e4036d17d4da7c80dbb4ddea75b6ed7e27ee9d0be3195c2d1b5e30a531d"
)
D1_SCAN_INPUT_EVALUATION_DATE = "2026-07-24"
D1_SCAN_INPUT_REFERENCE_IMPLEMENTATION = "reference_v1"
D1_SCAN_INPUT_CANDIDATE_IMPLEMENTATION = "candidate_v2"
D1_SCAN_INPUT_EXECUTION_CONFIG_SCHEMA_VERSION = (
    "d1.scan_input.execution_config.v1"
)
D1_SCAN_INPUT_PERFORMANCE_DIAGNOSTICS_SCHEMA_VERSION = (
    "d1.scan_input.performance_diagnostics.v2"
)
D1_SCAN_INPUT_BOOTSTRAP_RESAMPLES = 10_000
D1_SCAN_INPUT_BOOTSTRAP_RNG_SEED = 20_260_724
D1_SCAN_INPUT_SHORT_SEEDS = tuple(range(1101, 1111))
D1_SCAN_INPUT_LONG_SEEDS = tuple(range(1101, 1104))
D1_SCAN_INPUT_SHORT_DURATION_S = 2.2
D1_SCAN_INPUT_LONG_DURATION_S = 10.0
D1_SCAN_INPUT_TARGET_COUNT = 200
D1_SCAN_INPUT_RESOURCE_COUNT = 200
D1_SCAN_INPUT_RECON_COUNT = 2
D1_SCAN_INPUT_STAGE = "module.d1_scan_input"
...
_GROUPS = ("short", "long")
_IMPLEMENTATIONS = {
    _REFERENCE_ARM: D1_SCAN_INPUT_REFERENCE_IMPLEMENTATION,
    _CANDIDATE_ARM: D1_SCAN_INPUT_CANDIDATE_IMPLEMENTATION,
}
_RUN_FLAGS = (
    "--integrated-stack",
...
_EXPECTED_BOUNDARY = {
    ...
    "reference_implementation": D1_SCAN_INPUT_REFERENCE_IMPLEMENTATION,
    "candidate_implementation": D1_SCAN_INPUT_CANDIDATE_IMPLEMENTATION,
...
_IMPLEMENTATION_MARKER = "D6_PREREGISTERED_SCAN_INPUT_TREATMENT"
_TREATMENT_DERIVED_ID_MARKER = "D6_TREATMENT_DERIVED_EPISODE_ID"
_METRICS = (
    "d1_scan_input_wall_s",
    "d1_scan_input_p50_ms",
    "d1_scan_input_p95_ms",
    "d1_scan_input_max_ms",
    "core_wall_s",
    "external_elapsed_s",
    "maximum_rss_kib",
    "real_time_factor",
...
class D1ScanInputEvidenceError(ValueError):
    """Raised when persisted scan-input evidence violates the frozen contract."""
...
class D1ScanInputPairBinding:
    ...
    def __init__(
        ...
        *,
        ...
        arms: Mapping[str, D1ScanInputArmBinding],
    ) -> None:
...
class D1ScanInputEvidence:
    ...
    def __init__(
        ...
        *,
        ...
        pairs: Sequence[D1ScanInputPairBinding],
    ) -> None:
...
def load_d1_scan_input_evidence_manifest(
    source: str | Path,
) -> D1ScanInputEvidence:
    ...
    if manifest.get("status") != "episodes_complete_pending_d6":
        raise D1ScanInputEvidenceError(
    ...
    if matrix_sha256 != D1_SCAN_INPUT_MATRIX_SHA256:
        raise D1ScanInputEvidenceError(
            "matrix_sha256 does not match the frozen D1 scan-input matrix"
        )
    ...
    pairs: list[D1ScanInputPairBinding] = []
    ...
    for raw_case, expected in zip(raw_cases, _EXPECTED_CASES, strict=True):
        ...
        arm_bindings: dict[str, D1ScanInputArmBinding] = {}
        ...
        for arm in _ARMS:
            ...
            expected_command = _expected_command(
                ...
                seed=seed,
                resource_count=D1_SCAN_INPUT_RESOURCE_COUNT,
                target_count=D1_SCAN_INPUT_TARGET_COUNT,
                recon_count=D1_SCAN_INPUT_RECON_COUNT,
                episode_dir=episode_dir,
            )
            ...
            arm_bindings[arm] = D1ScanInputArmBinding(
                arm=arm,
                implementation=implementation,
                episode_dir=episode_dir,
                resource_path=resource_path,
...
def evaluate_d1_scan_input_multiseed(
    source: str | Path,
) -> dict[str, Any]:
    """Evaluate one complete 13-pair same-commit evidence manifest."""

    evidence = load_d1_scan_input_evidence_manifest(source)
    ...
    return {
        "schema_version": D1_SCAN_INPUT_MULTISEED_EVALUATION_SCHEMA_VERSION,
        "evaluation_date": D1_SCAN_INPUT_EVALUATION_DATE,
        ...
        "input_contract": {
            ...
            "matrix_schema_version": D1_SCAN_INPUT_MATRIX_SCHEMA_VERSION,
            "experiment_id": D1_SCAN_INPUT_EXPERIMENT_ID,
            ...
            "bootstrap_resamples": D1_SCAN_INPUT_BOOTSTRAP_RESAMPLES,
            "bootstrap_rng_seed": D1_SCAN_INPUT_BOOTSTRAP_RNG_SEED,
        ...
        "scope": {
            ...
            "resource_count": D1_SCAN_INPUT_RESOURCE_COUNT,
            "recon_count": D1_SCAN_INPUT_RECON_COUNT,
            "short_seeds": list(D1_SCAN_INPUT_SHORT_SEEDS),
            "long_seeds": list(D1_SCAN_INPUT_LONG_SEEDS),
            "short_duration_s": D1_SCAN_INPUT_SHORT_DURATION_S,
            "long_duration_s": D1_SCAN_INPUT_LONG_DURATION_S,
            "truth_is_online_control_input": False,
            "allowed_business_equivalence_differences": [
                "d1_scan_input_implementation_identity",
...
def write_d1_scan_input_multiseed_report(
    ...
    if result.get("schema_version") != (
        ...
        raise ValueError("unsupported D1 scan-input evaluation schema")
    ...
    evaluation_path = directory / "d1_scan_input_multiseed_evaluation.json"
    aggregate_path = directory / "d1_scan_input_multiseed_aggregate.json"
    csv_path = directory / "d1_scan_input_multiseed_pairs.csv"
    markdown_path = directory / "D1_SCAN_INPUT_MULTISEED_EVALUATION_CN.md"
    plot_path = directory / "d1_scan_input_multiseed_improvement_curve.png"
    ...
    markdown_path.write_text(
        render_d1_scan_input_multiseed_markdown(result),
        encoding="utf-8",
    )
...
def render_d1_scan_input_multiseed_markdown(
    result: Mapping[str, Any],
) -> str:
    ...
    lines = [
        ...
        "",
        (
            f"D1 扫描输入候选实现准入结论为 **{'通过' if admitted else '不通过'}**。"
            "评估只读取同一干净提交产生的 13 组参考/候选 episode，"
            "未参与仿真运行和控制决策。"
        ),
        (
            f"系统实时性缺口 **{'已关闭' if realtime else '未关闭'}**。"
            "该结论单独依据候选臂实时因子判断，不由优化准入结论推导。"
        ),
        "",
        "## 证据条件",
        "",
    ...
    for group in _GROUPS:
        summary = groups[group]["metrics"]["d1_scan_input_wall_s"]
...
def _validate_matrix(matrix: Mapping[str, Any]) -> None:
    ...
    for field, expected in (
        ...
        ("resource_count", D1_SCAN_INPUT_RESOURCE_COUNT),
        ("recon_count", D1_SCAN_INPUT_RECON_COUNT),
        ("bootstrap_seed", D1_SCAN_INPUT_BOOTSTRAP_RNG_SEED),
        ("bootstrap_resamples", D1_SCAN_INPUT_BOOTSTRAP_RESAMPLES),
    ):
...
def _evaluate_pair(
    pair: D1ScanInputPairBinding,
    evidence: D1ScanInputEvidence,
) -> dict[str, Any]:
...
def _evaluate_arm(
    ...
    *,
    ...
    stage = _load_scan_input_stage(paths["stage_timings.csv"])
    ...
    execution_config = _required_mapping(
        summary.get("d1_scan_input_execution_config"),
        f"{pair.case_id} {binding.arm} execution_config",
    )
    diagnostics = _required_mapping(
        summary.get("d1_scan_input_performance_diagnostics"),
        f"{pair.case_id} {binding.arm} performance_diagnostics",
    )
    metrics = {
        "d1_scan_input_wall_s": stage["wall_time_s"],
        "d1_scan_input_p50_ms": stage["p50_wall_time_ms"],
        "d1_scan_input_p95_ms": stage["p95_wall_time_ms"],
        "d1_scan_input_max_ms": stage["max_wall_time_ms"],
    ...
    return {
        ...
        "performance_diagnostics": copy.deepcopy(diagnostics),
        "governance_performance_diagnostics": copy.deepcopy(
...
def _validate_arm_provenance(
    ...
    *,
    ...
    if manifest.get("git_commit") != expected_commit:
        raise D1ScanInputEvidenceError(f"{context} source commit mismatch")
    if manifest.get("repository_dirty") is not False:
        raise D1ScanInputEvidenceError(f"{context} repository is dirty")
    config_sha256 = _canonical_sha256(config)
    if manifest.get("config_sha256") != config_sha256:
        raise D1ScanInputEvidenceError(f"{context} config_sha256 mismatch")
    runtime_profile = _required_mapping(
        manifest.get("runtime_profile"),
    ...
    for mapping, label, field, expected in (
        ...
        (config, "config", "target_count", D1_SCAN_INPUT_TARGET_COUNT),
        (summary, "summary", "target_count", D1_SCAN_INPUT_TARGET_COUNT),
        (config, "config", "resource_count", D1_SCAN_INPUT_RESOURCE_COUNT),
        (summary, "summary", "resource_count", D1_SCAN_INPUT_RESOURCE_COUNT),
        (config, "config", "recon_count", D1_SCAN_INPUT_RECON_COUNT),
        (summary, "summary", "recon_count", D1_SCAN_INPUT_RECON_COUNT),
    ):
    ...
    if summary.get("finite_state") is not True:
        raise D1ScanInputEvidenceError(f"{context} finite_state must be true")
...
def _validate_implementation_identity(
    ...
    *,
    ...
    runtime_execution = _required_mapping(
        runtime_profile.get("d1_scan_input_execution_config"),
        f"{context} runtime execution_config",
    )
    summary_execution = _required_mapping(
        summary.get("d1_scan_input_execution_config"),
        f"{context} summary execution_config",
    )
    summary_diagnostics = _required_mapping(
        summary.get("d1_scan_input_performance_diagnostics"),
        f"{context} summary performance_diagnostics",
    )
    final = _required_mapping(
        summary.get("module_final_diagnostics"),
        f"{context} module_final_diagnostics",
    )
    final_execution = _required_mapping(
        final.get("d1_scan_input_execution_config"),
        f"{context} final execution_config",
    )
    final_diagnostics = _required_mapping(
        final.get("d1_scan_input_performance_diagnostics"),
        f"{context} final performance_diagnostics",
    )
    governance_execution = _required_mapping(
        governance.get("d1_scan_input_execution_config"),
        f"{context} governance execution_config",
    )
    governance_diagnostics = _required_mapping(
        governance.get("d1_scan_input_performance_diagnostics"),
        f"{context} governance performance_diagnostics",
    )
    locations = {
        ...
        "manifest.runtime_profile.execution_config": runtime_execution.get(
            "implementation"
        ),
        "summary": summary.get("d1_scan_input_implementation"),
        "summary.execution_config": summary_execution.get("implementation"),
        "summary.performance_diagnostics": summary_diagnostics.get(
            "implementation"
        ),
        "summary.module_final_diagnostics": final.get(
            "d1_scan_input_implementation"
        ),
        "summary.module_final_execution_config": final_execution.get(
            "implementation"
        ),
        "summary.module_final_performance_diagnostics": final_diagnostics.get(
            "implementation"
        ),
        "governance": governance.get("d1_scan_input_implementation"),
        "governance.execution_config": governance_execution.get(
            "implementation"
        ),
        "governance.performance_diagnostics": governance_diagnostics.get(
            "implementation"
        ),
    }
    mismatches = [
    ...
    for execution in (
        ...
        if execution.get("schema_version") != (
            ...
            raise D1ScanInputEvidenceError(
                f"{context} execution_config schema mismatch"
            )
    diagnostics_set = (
        summary_diagnostics,
        final_diagnostics,
    ...
    expected_keys = set(summary_diagnostics)
    for diagnostics in diagnostics_set:
        if diagnostics.get("schema_version") != (
            D1_SCAN_INPUT_PERFORMANCE_DIAGNOSTICS_SCHEMA_VERSION
        ):
            raise D1ScanInputEvidenceError(
                f"{context} performance diagnostics schema mismatch"
            )
        if set(diagnostics) != expected_keys:
            raise D1ScanInputEvidenceError(
                f"{context} performance diagnostics key mismatch"
            )
        for key, value in diagnostics.items():
            if key in {"schema_version", "implementation"}:
                continue
            if (
                ...
                raise D1ScanInputEvidenceError(
                    f"{context} performance diagnostic {key} is invalid"
                )
...
def _implementation_identity_locations(
    ...
    runtime_execution = _required_mapping(
        runtime_profile["d1_scan_input_execution_config"],
        "runtime execution_config",
    )
    summary_execution = _required_mapping(
        summary["d1_scan_input_execution_config"],
        "summary execution_config",
    )
    summary_diagnostics = _required_mapping(
        summary["d1_scan_input_performance_diagnostics"],
        "summary performance_diagnostics",
    )
    return {
        "manifest_runtime_profile": str(
            runtime_profile["d1_scan_input_implementation"]
        ),
        "manifest_runtime_configuration": str(
            configuration["d1_scan_input_implementation"]
        ),
        "manifest_execution_config": str(runtime_execution["implementation"]),
        "summary_top_level": str(summary["d1_scan_input_implementation"]),
        "summary_execution_config": str(summary_execution["implementation"]),
        "summary_performance_diagnostics": str(
            summary_diagnostics["implementation"]
        ),
        "governance_top_level": str(
            governance["d1_scan_input_implementation"]
        ),
    }
...
def _normalized_runtime_profile(
    ...
    normalized["d1_scan_input_implementation"] = _IMPLEMENTATION_MARKER
    configuration = _mutable_mapping(
        normalized.get("configuration"),
        "normalized runtime configuration",
    )
    configuration["d1_scan_input_implementation"] = _IMPLEMENTATION_MARKER
    execution = _mutable_mapping(
        normalized.get("d1_scan_input_execution_config"),
        "normalized runtime execution_config",
    )
    execution["implementation"] = _IMPLEMENTATION_MARKER
    return normalized
...
def _normalized_summary(summary: Mapping[str, Any]) -> dict[str, Any]:
    ...
    _normalize_scan_input_fields(normalized, "normalized summary")
    final = _mutable_mapping(
        normalized.get("module_final_diagnostics"),
        "normalized module_final_diagnostics",
    )
    _normalize_scan_input_fields(final, "normalized module final")
...
def _normalized_governance(governance: Mapping[str, Any]) -> dict[str, Any]:
    ...
    _normalize_scan_input_fields(normalized, "normalized governance")
...
def _normalize_scan_input_fields(
    mapping: dict[str, Any],
    context: str,
) -> None:
    if "d1_scan_input_implementation" not in mapping:
        raise D1ScanInputEvidenceError(
            f"{context} lacks d1_scan_input_implementation"
        )
    mapping["d1_scan_input_implementation"] = _IMPLEMENTATION_MARKER
    execution = _mutable_mapping(
        mapping.get("d1_scan_input_execution_config"),
        f"{context} execution_config",
    )
    execution["implementation"] = _IMPLEMENTATION_MARKER
    diagnostics = _mutable_mapping(
        mapping.get("d1_scan_input_performance_diagnostics"),
        f"{context} performance_diagnostics",
    )
    diagnostics.clear()
    diagnostics["value"] = _PERFORMANCE_MARKER


def _load_scan_input_stage(path: Path) -> dict[str, Any]:
    required_fields = {
        "schema_version",
        "stage",
        "call_count",
    ...
    try:
        with path.open("r", newline="", encoding="utf-8") as stream:
            ...
            rows = [row for row in reader if row.get("stage") == D1_SCAN_INPUT_STAGE]
    ...
    if len(rows) != 1:
        raise D1ScanInputEvidenceError(
            "stage_timings.csv must contain exactly one module.d1_scan_input row"
        )
    row = rows[0]
    if row.get("schema_version") != _STAGE_TIMING_SCHEMA_VERSION:
        raise D1ScanInputEvidenceError("scan-input stage schema mismatch")
    call_count = _parse_positive_integer(row.get("call_count"), "call_count")
    if str(row.get("distribution_available", "")).strip().lower() != "true":
        raise D1ScanInputEvidenceError(
            "scan-input timing distribution must be available"
        )
    if str(row.get("distribution_unavailable_reason", "")).strip():
        raise D1ScanInputEvidenceError(
            "scan-input timing has an unexpected unavailable reason"
        )
    values = {
    ...
    if values["wall_time_s"] <= 0.0:
        raise D1ScanInputEvidenceError(
            "scan-input wall_time_s must be positive"
        )
    if not (
        ...
        raise D1ScanInputEvidenceError(
            "scan-input timing percentiles are not ordered"
        )
    ...
    if not math.isclose(
        ...
        raise D1ScanInputEvidenceError(
            "scan-input mean/call_count does not reconstruct wall_time_s"
        )
    if values["max_wall_time_ms"] > values["wall_time_s"] * 1000.0 + 1.0e-6:
        raise D1ScanInputEvidenceError(
            "scan-input max timing exceeds accumulated wall time"
        )
    return {
        "schema_version": row["schema_version"],
        "stage": D1_SCAN_INPUT_STAGE,
...
def _load_resource_metrics(path: Path) -> dict[str, float | int]:
    ...
    if values["process_exit_status"] != 0:
        raise D1ScanInputEvidenceError("GNU time process exit status is not zero")
    return values
...
def _summarize_group_metric(
    ...
    *,
    ...
    lower, upper = _bootstrap_mean_ci(
        raw,
        resamples=D1_SCAN_INPUT_BOOTSTRAP_RESAMPLES,
        rng_seed=D1_SCAN_INPUT_BOOTSTRAP_RNG_SEED,
    )
    return {
        "metric": metric,
        "direction": (
...
def _admission_gates(
    ...
    core = [groups[group]["metrics"]["core_wall_s"] for group in _GROUPS]
    ...
    return {
        ...
        "short_candidate_faster_at_least_8_of_10": _gate(
            short_scan["candidate_better_count"] >= 8,
            "short_candidate_faster_count_below_8",
        ),
        "short_scan_input_mean_improvement_at_least_5_pct": _gate(
            short_scan["improvement_pct"]["mean"] >= 5.0,
            "short_scan_input_mean_improvement_below_5_pct",
        ),
        "short_scan_input_bootstrap_raw_ci_upper_below_zero": _gate(
            short_scan["raw_relative_change"]["bootstrap_95_ci"]["upper"] < 0.0,
            "short_scan_input_bootstrap_raw_ci_upper_not_below_zero",
        ),
        "long_candidate_faster_at_least_2_of_3": _gate(
            long_scan["candidate_better_count"] >= 2,
            "long_candidate_faster_count_below_2",
        ),
        "long_scan_input_mean_improvement_at_least_5_pct": _gate(
            long_scan["improvement_pct"]["mean"] >= 5.0,
            "long_scan_input_mean_improvement_below_5_pct",
        ),
        "core_wall_mean_degradation_within_5_pct": _gate(
            all(
                summary["raw_relative_change"]["mean"] <= 0.05
                for summary in core
            ),
            "short_or_long_core_wall_mean_degradation_above_5_pct",
        ),
...
def _aggregate_output(result: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": D1_SCAN_INPUT_MULTISEED_AGGREGATE_SCHEMA_VERSION,
...
def _write_improvement_plot(
    ...
    for group in _GROUPS:
        ...
        axis.plot(
            ...
            [
                float(
                    pair["performance"]["d1_scan_input_wall_s"][
                        "improvement_pct"
                    ]
                )
    ...
    axis.axhline(
        ...
        linewidth=1.0,
        linestyle="--",
        label="Admission threshold 5%",
    )
    axis.set_xlabel("Seed")
    axis.set_ylabel("Scan-input improvement (%)")
    axis.set_title("D1 scan-input paired improvement")
...
def _distribution(values: Sequence[float]) -> dict[str, float]:
    if not values:
        raise D1ScanInputEvidenceError("distribution has no values")
    if not all(math.isfinite(value) for value in values):
        raise D1ScanInputEvidenceError("distribution contains nonfinite values")
...
def _bootstrap_mean_ci(
    ...
    *,
    ...
    if not values:
        raise D1ScanInputEvidenceError("bootstrap has no paired values")
...
def _strict_jsonl_digest(path: Path) -> str:
    ...
    try:
        ...
        raise D1ScanInputEvidenceError(f"unable to read {path}: {exc}") from exc
...
def _validate_truth_state_finite(path: Path) -> None:
    try:
        ...
        if isinstance(exc, D1ScanInputEvidenceError):
            raise
        raise D1ScanInputEvidenceError(
...
def _load_strict_json_mapping(
    ...
    if not isinstance(value, dict):
        raise D1ScanInputEvidenceError(f"expected JSON object: {path}")
...
def _parse_positive_integer(value: Any, context: str) -> int:
    try:
        ...
        raise D1ScanInputEvidenceError(f"{context} must be an integer") from exc
...
def _parse_finite_nonnegative_text(value: Any, context: str) -> float:
    try:
        ...
        raise D1ScanInputEvidenceError(f"{context} must be numeric") from exc
...
def _required_mapping(value: Any, context: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise D1ScanInputEvidenceError(f"{context} must be an object")
    return value


def _mutable_mapping(value: Any, context: str) -> dict[str, Any]:
    if not isinstance(value, dict):
        raise D1ScanInputEvidenceError(f"{context} must be a JSON object")
    return value
...
def _required_sequence(value: Any, context: str) -> Sequence[Any]:
    if (
        ...
        raise D1ScanInputEvidenceError(f"{context} must be an array")
    return value
...
def _required_text(value: Any, context: str) -> str:
    ...
    if not result:
        raise D1ScanInputEvidenceError(f"{context} must be non-empty")
    return result
...
def _explicit_path(
    ...
    *,
    ...
    if not path.is_absolute():
        raise D1ScanInputEvidenceError(f"{context} must be an absolute path")
    resolved = path.resolve()
    if require == "file" and not resolved.is_file():
        raise D1ScanInputEvidenceError(f"{context} is not a file: {resolved}")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = evaluate_d1_scan_input_multiseed(args.evidence_manifest)
    paths = write_d1_scan_input_multiseed_report(result, args.output_dir)
