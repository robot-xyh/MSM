...
200 三档规模各使用 seed `7/17`。42 个核心制品、配置哈希、有限状态、在线真值隔离、
当前计划标识/版本/时期/租约和 49 个联盟目标均为 `6/6` 通过。10 次权威发布、10 个
计划身份和 10 次确认守恒，16101 条通信处置通过验证。

12 条 D4 建议全部匹配发布时最新代次，旧代发布为 0。四个重规划 episode 均有最终
v2 建议，最终计划建议覆盖为 `6/6`。低层 clean formal 从旧批次的 `2/6` 恢复为
`6/6`。10 条故障诊断建议均为 shadow、非 assist，正式决策没有改写；原始制品没有
消费记录，采用指标保持 unavailable。

D4 建议代次的 6-cell 预准入通过。该批不是正式 900-cell，不含 execution plan 和
矩阵 metadata，不能形成 targeted/full posterior 正式通过率。完整 ID Switch 仍为
`3/6 available`。详细证据见
`reports/HIGH_THREAT_CLEAN_SMOKE_B063535_REVALIDATION_20260731_CN.md`；旧报告保留为
修复前对照。

## 2.48 高威胁 M 对 N 时期租约复验

2026-07-31，D6 只读复核 v5 的 100 个开发态 episode。最终 D3-D4 计划标识、版本、
时期、租约和当前联盟闭合均为 `100/100`。151 次权威发布对应 151 个不同计划身份和
151 次运行时计划确认；48 次同身份评价刷新被抑制为诊断，没有第二次发布或续租。

当前计划共审计 644 个多成员联盟目标。通信处置 `100/100 available/verified`，共
195838 条。D4 在 28 个 episode 使用无真值航迹回退，共 391 个快照；D7 门控未放宽。
D2 身份切换为 `88/100 available`，可用部分合计 52，12 项保持 unavailable。

200 对 200 的墙钟均值/P95 为 `14.209/15.566` 秒，实时倍率均值为 0.142。51 个真实
重规划 episode 的旧计划建议属于 v5 历史批次；该运行时断点已在后续 `b063535` clean
smoke 中关闭。100 个 manifest 均声明 `repository_dirty=true`，不能替代 clean formal
900 项。完整证据见
`reports/HIGH_THREAT_PRECHECK_V5_REVALIDATION_20260730_CN.md`。

## 2.47 高威胁 M 对 N 开发态 100 项修复复验

2026-07-30，D6 只读重算 5、20、50、100、200 五个规模、每规模 20 seeds 的 v4
开发态批次。有限状态、在线真值零使用、最终 D3-D4 计划标识/版本和当前联盟闭合均为
`100/100`。当前计划共包含 644 个多成员联盟目标，全部在最终 D4 快照中闭合。v3 的
三个 `GT3D-000011` 最终快照断点不再出现。

151 次权威 D3 发布对应 151 个不同计划身份，同一 `(plan_id, plan_version)` 重复发布
为 0。`payload_digest_mismatch` 与 `cross_binding_invalid` 均为 0。区域时期编号和
租约因 D3 未发布对照字段而保持 `0/100 available`，未写成通过。

通信处置 `100/100 available/verified`，共 195838 条。离线身份指标 `88/100`
available，可用部分合计 52 次；12 项分别因谱系窗口不足或一轨对应多个真值目标保持
unavailable。200 对 200 的 wall time 均值/P95 为 `12.928/14.296` 秒，实时因子均值为
`0.156`。

100 个 manifest 均指向提交 `2790b165`，同时均声明 dirty source。本批次不具备正式 R0
资格，不得替代或拼接正式 900 项。详细证据见
`reports/HIGH_THREAT_PRECHECK_V4_REVALIDATION_20260730_CN.md`。

## 2.46 正式 R0 当前计划绑定审计器

2026-07-30，D6 完成最后 D3 计划与最后 D4 决策的严格绑定检查。D4 必须与最后 D3 的
`plan_id/plan_version` 同代；可用时继续核对权威 epoch 和 lease。多成员联盟必须完成
当前计划的全部 ACK、原子提交和执行授权。旧代 committed、`collecting_acks` 和
`proposed` 均不能通过。

真实 runtime 合同允许普通一对一 assignment 携带非空 `coalition_id`。D6 已将必需联盟
目标限定为“同一目标有多个当前 D3 资源”或“同代 D4 明确
`commit_required=true`”。20 对 20 合同 smoke 从错误的期望 16、提交 2，修正为期望 2、
已审计 2，审计状态为 pass。该 smoke 只验证合同解释，不是正式矩阵结果。

代表性测试覆盖同代通过、D3 v2 对 D4 v1 拒绝、当前计划 ACK 未闭合拒绝、proposed
拒绝、epoch/lease 错代拒绝、通信处置缺失 availability 和混合单成员/多成员分配。
专项组合 `27 passed`，D6 全量 `1261 passed, 1 warning in 128.21s`。

本项是审计器代码验证，不是正式矩阵重跑。没有读取新的正式保留集，也没有改写既有正式
制品。main runtime 的同代租约冻结、ACK 重评、有限重发、重发耗尽失败关闭、尾部排空和
逐消息处置落盘已经代码就绪，D6 已核对消费合同。历史 `872/900` 保留为旧门禁结果；新
正式结论须在新的 clean source 上整体重跑 900 项后形成。

## 2.45 D4 v7 来源独立外部评价盲审

### 结论

2026-07-30，D6 对冻结的 D4 v7 规则节点与转移残差候选完成来源独立盲审。输入为
M16N24、8 区域、64 episode、128 帧，seed 为 `5216-5279`。D6 不调用 D4 v7
高层评价器，从冻结模型、同快照 R0、标签动作、确定性投影和干预不变量重新计算全部
逐帧结果。

规则正类为 42 帧，精确正动作命中为 `0/42`。train 出现 10 次原始边激活和 3 次
可执行转移变化，三次均是负类错误边和虚假转移；validation/test 转移变化均为 0。
聚合 actor-derived positive 分母为 3，精确命中为 `0/3`。候选未建立来源独立的正确
转移能力。

错误方向、错误数量、投影拒绝、干预不变量失败和原始 R0 完整动作元组偏差均为 0。
负类精确保持 R0 为 `83/86=0.965116`。投影后动作元组变化 3 帧，均由错误转移触发的
资源配额联动造成。
...
冻结 v4 train+validation 的 425 帧形成 251 个唯一可观测键，外部 128 帧形成
92 个，精确重合为 0。训练、正式留出、既有设计与评价、pilot 和本次独立评价 seed
两两无交集。模型拟合、检查点更新、阈值调整、置信校准、输入或候选修改、注册、准入、
正式留出及 prior evaluation payload 读取均为 0。

raw source、labeled export、labeled dataset、冻结 v4 和 v7 候选五棵输入树在审计
前后保持不变，D4 评价树也未变化。D6 重算 JSONL 与 D4 JSONL 逐字节 SHA-256 均为
`7785ded96360869edfb694c425321fa3323450cf1624607b53edf5d3eca6a5cd`。
D4 CSV、summary、input integrity、observable overlap 和 artifact manifest 对账均通过。

### 结论边界

审计链条执行成功，候选能力评价失败关闭。v7 保持 unregistered、admission closed 和
rule fallback required；置信校准、正式留出、运行预检、D3、D7、降级、接管、联盟、
控制和物理权限全部为 false。下一候选需在全新未见数据上取得 validation/test 非零且
充分的精确正动作，并将虚假转移保持为 0。

完整报告见 `docs/D4_V7_SOURCE_INDEPENDENT_EXTERNAL_AUDIT_CN.md`。专项测试
`11 passed, 1 warning in 4.65s`，D6 全量回归
`1234 passed, 1 warning in 126.73s`。
...
各完成 45 个 cell，shard 8、18 各完成 21 个 cell，执行进度为 177/900。D6 新专项绕过
原聚合，从 cell result、episode 文件和低层评估合同独立复核五个原失败 cell：
...
| delayed_noisy 5v5 | 1008 | 13 / 13 | 5 / 5 / 8 | 0 | true | true / true | verified | `[]` |
| delayed_noisy 5v5 | 1018 | 14 / 14 | 6 / 6 / 8 | 0 | true | true / true | verified | `[]` |
