...
D6 只读审计 clean commit `49e43eacf627315a75093a3b381c9f810b11905e` 生成的
6 个高威胁 episode。范围为 5、100、200 三档规模，每档 seed `7/17`、仿真 2 秒。
核心制品、配置哈希、有限状态、在线真值隔离、最终 D3-D4 计划标识/版本/时期/租约和
当前联盟闭合均为 `6/6`。10 次权威发布、10 个计划身份和 10 次计划确认守恒；49 个
当前联盟目标闭合，16101 条通信处置通过验证。

100 和 200 规模的 4 个重规划 episode 在新 D3/D4 计划发布后，又发布一条仍绑定旧计划
的 D4 区域建议。四项均没有匹配最终计划的建议，导致低层 clean formal 资格只有
`2/6`。这些建议为规则 `shadow` 输出且没有改写正式决策，但现有 targeted/full formal
门仍会失败关闭。900-cell 准入判断为“需先修复”，6-cell smoke 不作为正式结果。完整
报告见 `reports/HIGH_THREAT_CLEAN_SMOKE_49E43EA_20260731_CN.md`。
