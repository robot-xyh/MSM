...
D6 新增 `d3_a1_source_independent_v2_audit.py` 和命令行入口
`scripts/run_d3_a1_source_independent_v2_external_audit.py`。该入口只使用 Python 标准库，
不导入 D3 评价器、模型或汇总函数。D3 的 `aggregate.json` 只作为待核对声明，结论由合同、
生成记录、匿名数据集和 292 条逐帧评价记录独立重算。

审计覆盖以下硬门：

1. 结果目录必须恰好包含合同规定的 5 个普通文件，不允许符号链接、缺文件、额外文件或
   `SHA256SUMS` 覆盖不完整；
2. 冻结模型 manifest、state dict、bundle 树和冻结评价源码树必须与预注册摘要一致；
3. 生成源必须为 100 个 episode、seed 20000-20099、D3-only export、有限状态、在线真值
   使用为 0，且不得读取正式 seed 1000-1019；
4. 数据集路径必须严格绑定生成根目录的 `learning_dataset/d3_assignment`。数据集必须为
   292 帧，episode 分组为 60/20/20，帧分组为 178/57/57，并从逐帧 episode、seed、split
   清单独立重算 split SHA-256；
5. 数据集和评价 JSONL 都禁止出现 truth、Actor、Object 或全局身份字段；评价记录中只允许
   `online_truth_use_count` 这一审计计数字段；
6. 独立解析固定 21 列、292 行的 CSV，把身份、三组选择边、换绑数、正负类、拒绝、OOD 和
   fallback 标志与 JSONL 逐行闭合；
7. 从数据集规则矩阵按连续小端双精度规范重算每帧 SHA-256；从动作掩码、目标需求槽和匿名
   资源容量独立重算 R0、candidate、effective 三组边的索引、容量、硬禁边和 M 对 N 原子性，
   再与逐帧自报计数闭合；
8. 任何权限冒充、计数伪造、CSV 与校验和联合篡改、矩阵或 split 摘要冒充、真值字段、文件
   篡改或输入审计前后变化均失败关闭。

正式审计读取 100 个 episode、292 帧。总体正类安全换绑为 `13/110=11.82%`，教师完全匹配
为 `8/110=7.27%`，负类保持规则绑定为 `182/182=100%`；94 个拒绝帧的矩阵和绑定全部
回退为规则基线。非零代价修正为 98 帧，分布外为 27 帧。重复资源、硬禁边、M 对 N
原子性、版本、规则矩阵突变，以及模型 assignment/plan/runtime 输出均为 0。预注册总体
机器门通过。

补强审计确认 CSV 为 `21` 列、`292` 行，逐行不一致数为 0。split SHA-256 独立复算为
`f1380dd60fded50b2550e5ce63d6d41bb6066022f9e4b201925978acfa025ca5`。R0、candidate、
effective 各复算 `21637` 条选择边，三组的索引越界、资源容量超额、硬禁边和 M 对 N
原子性违规均为 0；机器门中的 effective 安全计数来自该独立复算，不再采用 D3 自报计数。

test 子组教师完全匹配为 `0/25`。合同门限预注册为 292 帧总体聚合门，D6 不在结果产生后
增设子组门限。该事实作为泛化限制保留，不能据此授予运行、辅助、分配、计划、控制、物理、
正式或生产权限。完整产物位于
[`reports/D3_A1_SOURCE_INDEPENDENT_V2_EXTERNAL_AUDIT_20260731/`](reports/D3_A1_SOURCE_INDEPENDENT_V2_EXTERNAL_AUDIT_20260731/)。

专项测试增至 `18 passed`，其中新增 9 个失败关闭负例；D6 全量回归
