...
本项 D1-owned 热点据此关闭，系统实时与超线性 P1 不关闭。下一轮先对当前候选重新冻结函数级
预算，优先研究非雷达扫描代价矩阵（代表剖析累计 35.469 s）和航迹物化（16.470 s）。任何后续
优化仍须保持逐扫描语义、固定滞后窗、双时间戳、协方差、来源谱系和终态输出严格一致。另行由
main 统计 scan input、进程常驻集、长于 10 s 的增长率和端到端实时倍率。
