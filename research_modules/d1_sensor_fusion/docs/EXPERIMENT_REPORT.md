...
**范围：D1 合成量测关联热点，默认关闭候选的模块级同进程交错微基准**

### 试验设计

reference 为 `disabled_v1`，candidate 为
`modality_conservative_quadratic_bound_v1`。每个变体先建立相同的 120 条雷达航迹，再
处理一个包含 120 条观测的更新扫描。每模态有 14,400 个候选 pair；每个变体预热 1 次，
随后交错运行 7 次。计时只覆盖一次 state-only 关联更新，不包含初始航迹建立和最终快照
物化。性能 schema 为 `d1.association_sparse_prefilter_performance.v2`；诊断固定桶为
`radar/lidar/acoustic/acoustic_3d/eo/other`。`other` 不构造计时输入，因为当前公共
`SensorObservation` 合同拒绝未知模态，但固定诊断仍保留该 fail-open 桶。

验收要求五类规范输出一致、逐模态 exact-gate-pass 相等、逐模态操作数稳定，
LiDAR/二维声学/三维声学/光电均减少精确求解，且四类合计 P50 改善至少 `5%`。奇异、
近奇异和非有限协方差必须保守回退，不得预筛。

### 结果

| 模态 | Reference P50 / s | Candidate P50 / s | 精确求解 | 精确门内通过 R/C | 预筛剔除 | Candidate 更快 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 雷达 | `0.085590` | `0.085779` | `4,261 -> 4,261` | `4,261/4,261` | `10,139` | `2/7` |
| LiDAR | `0.127432` | `0.111972` | `14,400 -> 3,126` | `3,110/3,110` | `11,274` | `7/7` |
| 二维声学 | `0.124354` | `0.116331` | `14,400 -> 6,306` | `6,306/6,306` | `8,094` | `6/7` |
| 三维声学 | `0.145342` | `0.134237` | `14,400 -> 6,306` | `6,306/6,306` | `8,094` | `7/7` |
| 光电 | `0.140760` | `0.125076` | `14,400 -> 3,295` | `3,243/3,243` | `11,105` | `7/7` |

LiDAR、二维声学、三维声学和光电合计 P50 为 `0.538083 -> 0.487310 s`，改善
`9.436%`。雷达 selector on/off 走同一旧下界，candidate P50 本轮慢 `0.221%`，不记为
新增收益。五类规范输出 SHA-256 一致，精确门内 pair 数一致，正常输入 fallback 为 0。
专项回归为 `30 passed in 1.14s`，全量回归为 `473 passed in 24.45s`。
...
## 真值隔离质量基准开发验证（2026-07-25）

### 验证范围

本轮验证质量基准的数据隔离、指标可用性和规模接口。现有融合算法、门限和航迹生命周期保持
不变。在线观测经过身份字段和离线真值令牌双重检查；评分 sidecar 只在融合完成后使用。

### 20-seed 快速夹具

配置为 5 目标、0.61 s、扫描周期 0.30 s、暖机 0.10 s、seeds 2000-2019。该夹具用于回归
批量入口，不作为 200 目标性能结论。
...
20/20 个运行完成，累计 OOSM 观测为 74，共评分 248 条唯一接受观测。同日三次开发运行的
20-seed 单扫描处理 P95 汇总为 11.84--12.65 ms。墙钟受本机负载影响，不进入单元测试门限。
较高 NEES/NIS 和重复航迹率属于测量结果，未通过调参消除。

### 200 目标短时冒烟

配置为 200 目标、0.61 s、扫描周期 0.30 s、seed 1000，漏检率 0.10，虚警率每目标
0.005。3 个发布帧全部完成，终态 201 条 D1 航迹，OOSM 观测 164 条，共评分 532 条唯一
接受观测，谱系映射覆盖率 1.0。暖机召回率为 0.8183。同日三次开发运行的单扫描处理时间
P95 为 166.41--310.96 ms，该范围只描述本机短时运行。

该运行持续时间短，不能反映虚警航迹长期累积、遮挡后重捕获或稳定速度估计。它只确认 200
目标场景可实际进入融合和离线评分。
...
| 在线真值使用 | 0 | 通过 |
| D2 规范身份写入 | 0 | 通过 |
| 快速批量完成 | 20/20 seeds | 通过 |
| 谱系映射覆盖 | 100% | 通过 |
| 不可用指标 | `value=None` 且有原因 | 通过 |
| 专项测试 | 全部通过 | `8 passed` |
| D1 全量回归 | 全部通过 | `496 passed in 33.19s` |
...
## GlobalTrack 完整物化性能候选（2026-08-01）

### 输入与候选

试验使用现有非正式冻结输入
`2p2s_seed_42000/online_observations.jsonl`，持续 2.2 秒，含 86 个扫描和 2,051 条匿名观测。
输入 SHA-256 为
`c6dcc69d58b0fc9a51e9cfcf2368b4faeb882d5a90991ffcdf1f7605bba55e53`。未运行正式 seeds
1000--1019，未运行正式 R0。

reference/candidate 分别为逐航迹 A95 摘要和同发布帧批量 A95 摘要。候选只合并二维位置
协方差的特征值计算，完整 `GlobalTrack` 仍逐条构造。开关默认关闭。

### 预注册方法

门限使用 7 对交替 fresh-process。每个 arm 从冻结输入重新构造 scan organizer 和融合器，
profile 使用额外进程，不进入计时样本。预注册条件为：candidate 更快比例至少 80%，模块墙钟
中位改善至少 10%，配对墙钟差 bootstrap 95% 上界小于 0，逐扫描语义 7/7，业务操作数守恒
7/7。P50、P95、max、峰值 RSS 和工作量计数均为必报项。

### 性能结果

| 指标 | Reference | Candidate |
| --- | ---: | ---: |
| 模块墙钟 P50 / s | 0.228742 | 0.190582 |
| 模块墙钟 P95 / s | 0.229523 | 0.194719 |
| 模块墙钟 max / s | 0.229629 | 0.195816 |
| 单次发布 P50 / ms | 3.168315 | 2.519885 |
| 单次发布 P95 / ms | 4.999508 | 4.184879 |
| 单次发布 max / ms | 24.928044 | 27.490811 |
| 完整融合 P50 / s | 2.977062 | 2.928923 |
| 完整融合 P95 / s | 2.990879 | 2.946391 |
| 完整融合 max / s | 2.993028 | 2.946844 |
| 峰值 RSS P50 / KiB | 165528 | 165312 |
| 峰值 RSS P95 / KiB | 165853 | 165987 |
| 峰值 RSS max / KiB | 165908 | 166000 |

候选 7/7 更快，中位模块墙钟改善 `16.6824%`。配对差 bootstrap 95% 区间为
`[-0.044637, -0.031457] s`，上界满足小于 0 的门限。

### 语义与工作量

7/7 对的逐扫描后验、协方差、NIS、门控观测编号、完整 publication payload、一致性证据、
业务操作数和最终离线 export 严格一致。每臂执行 59 次完整发布并物化 11,188 条航迹。
reference 执行 11,188 次标量 A95；candidate 处理同样 11,188 个矩阵，执行 56 次非空批量
特征值调用。空发布帧保留批量构建诊断但不调用求解器。在线真值使用和
`global_track_id` 写权限均为 0。

### 判定

D1 模块门通过。候选继续默认关闭，状态为
`module_gate_passed_candidate_remains_default_off_pending_main`。报告 JSON/Markdown 的
SHA-256 分别为
`b0f776c08192b6441a6dafb4d6ce20a1fcc5f3c59f07ed419e2581720e91cded` 和
`7c8fed8460c6735cf2e07080dea1ef03888d0534dd280819cec714e716d94786`。

本结果不证明 200v200 系统实时闭合，不包含 AirSim、目标硬件、正式 R0 或正式融合精度证据。
是否进入 main 由独立集成审查决定。
