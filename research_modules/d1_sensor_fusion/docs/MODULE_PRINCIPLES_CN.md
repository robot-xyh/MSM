...
研究候选 `fixed_lag_checkpoint_prefix_cumulative_summary_v1` 为完整 checkpoint 前缀保存
不可变累计摘要。摘要包括 checkpoint 修订号、观测身份与顺序、NIS tuple、门控 ID tuple、
一致性观测顺序和证据结构修订号。候选只复用该摘要，不复制或引用可变 checkpoint 列表。
默认 `per_checkpoint_prefix_rebuild_v1` 仍按原方式逐条重建。
...
`d1.fixed_lag_replay_prefix_summary.v1`，NIS、排序键和 consistency 序列长度必须一致。
第三类是 evidence 完整性：回放上下文、证据结构修订、缓存策略和来源航迹必须匹配。任一
条件失败，系统先物化未决计数，再执行原逐条路径。部分前缀、无 checkpoint、迟到量测插入、
...
剔除、精确求解、精确门内通过和 fallback。

2026-07-25，D6 对 clean source commit
`9302ccede2ca513c2235370e1a464fc88bc41150`、matrix SHA-256
`a7162d014d1c3c0f207355b24a5d7159bf3486d134ca21876f7469d1e915b71d` 完成正式评估。
场景为 200 个目标、200 个资源和 2 个侦察节点；short seeds 1131-1140、long seeds
1131-1133，共 13 pair/26 个 fresh episode。13/13 pair 的业务语义、有限状态、online
truth use=0 和逐模态 exact gate-pass 相等均通过。非雷达精确求解削减
`86.636767%`，但它只是局部操作数证据。

正式性能门中，short 更快数 `7/10 < 8/10`、D1 fusion 改善
`0.228437% < 1%`、bootstrap 原始变化 95% 上界 `0.443531% > 0%`、core 改善
`0.091096% < 0.25%`，long D1 fusion 改善 `0.713776% < 1%`。因此 D6 verdict 为
`reject`，reference `disabled_v1` 继续作为默认；candidate 只保留为显式研究入口。冻结
矩阵和门限不因局部求解削减而调整。

此前 120 航迹模块微基准作为历史保留：LiDAR、二维声学、三维声学和光电合计 P50 改善
`9.436%`，四类精确求解分别减少
`78.292%/56.208%/56.208%/77.118%`。该历史结果用于说明候选机制，不构成主线准入。
...
D1 状态为
\[
x=[p_n,p_e,p_d,v_n,v_e,v_d]^\mathsf{T}.
\]
声学方位、光电针孔投影和激光雷达位置观测只由 \(p_n,p_e,p_d\) 决定。无径向速度雷达的
距离、方位和俯仰也只依赖位置。因此这些模型满足
\[
\frac{\partial h}{\partial v_n}
=\frac{\partial h}{\partial v_e}
=\frac{\partial h}{\partial v_d}=0.
\]
含径向速度的四维雷达观测同时依赖位置和速度，不采用该三列省略。

reference 数值雅可比先计算一次 \(h(x)\) 取得输出维数，再对六列执行
\[
J_{:,i}=\frac{h(x+\delta_i e_i)-h(x-\delta_i e_i)}{2\delta_i},
\qquad
\delta_i=10^{-5}\max(1,|x_i|).
\]
候选直接使用量测模型已知的输出维数。雅可比先置零，只对结构活动集合 \(A\) 中的列执行
同一中心差分。声学、光电、激光雷达和无径向速度雷达取
\(A=\{0,1,2\}\)；含径向速度雷达取 \(A=\{0,\ldots,5\}\)。候选没有使用解析近似，也没有
...
协方差限幅先得到对称矩阵 \(P\)。原路径计算全部特征值，并在
\(\lambda_{\min}(P)\geq0\) 时原样返回；出现负特征值时，继续执行相关矩阵投影。六维严格
正定矩阵也可由 Cholesky 分解判定：

\[
P=LL^\mathsf{T}.
\]

浮点 Cholesky 在机器精度附近可能接受带极小负特征值的矩阵。因此分解成功后还要计算
归一化行列式，并与 `9.094947017729282e-13` 的安全门限比较。只有通过安全门的矩阵才直接
返回。分解失败或安全门拒绝时，候选仍执行原有特征值检查和投影。半正定、近奇异和不定矩阵
均保留 reference 行为。候选没有用容差把负特征值改写为零，也没有跳过必要投影。

该路径通过 `cholesky_covariance_psd_fast_path=True` 显式启用，默认关闭。适用范围固定为
有限 `6x6` 状态协方差，观测侧常见 1 至 4 维矩阵继续使用特征值检查。诊断记录实现 ID、
安全门限、尝试、成功和回退，并验证尝试数等于成功数与回退数之和。输出始终是新数组，
不与输入别名。

当前 v2 确定种子模块基准的中位墙钟为 `0.558490 -> 0.588263 s`，candidate 慢 `5.33%`，
`0/9` 配对更快；数学输出和原因严格一致。20,000 次检查中有 19,800 次直接成功、200 次
回退。该路径没有性能收益，只保留为研究对照，不进入 main 默认配置。安全门前旧计时不再
作为当前实现证据。D1 全量 `404 passed in 21.39s`。该结论不代表完整融合或系统实时能力。

### 匀速模型矩阵可以复用，状态传播不能合并

匀速模型在给定时间差 \(\Delta t\) 和过程噪声谱密度 \(q\) 后，状态转移矩阵 \(F\) 与过程
噪声矩阵 \(Q\) 已完全确定：

\[
x_k=F(\Delta t)x_{k-1},\qquad
P_k=F(\Delta t)P_{k-1}F^\mathsf{T}(\Delta t)+Q(\Delta t,q).
\]

200v200 函数剖析中，`predict_to()` 执行 32,345 次，矩阵构造执行 32,217 次。大量航迹在
同一扫描和重放时刻具有相同的精确 \(\Delta t\)，因此可以复用 \(F,Q\)。每条航迹的
\(x,P\) 仍独立计算，协方差限制、固定滞后重放和业务发布没有合并。

候选 `d1.fusion.cv_motion_model.bounded_exact_lru.v1` 只接受精确
`(dt, process_noise)` 键。矩阵写保护，缓存有固定容量和确定性淘汰；非有限键及非正时间差
使用 reference。过程噪声变化自然生成新键，避免跨配置污染。D1 默认仍为逐预测构造路径，
候选必须显式启用。

模块专项使用 200 个状态、100 个传播步和 7 次交替采样，reference/candidate 中位耗时
`0.220679/0.103950 s`，矩阵构造 `20,000/8`，最终状态 SHA-256 相同。专项与全量测试分别为
`6 passed` 和 `395 passed in 21.41s`。该结果证明重复纯计算可以消除，但不能单独外推
整个融合链路的收益。

正式准入使用 clean source commit
`44223566439a446fc49f2a3fd861d1d51bd676b9` 和冻结矩阵 SHA-256
`9898656598f0fa282620afe2384a3d656b7496f8957109c413bcb62069fd2e9a`。场景含
200 个目标、200 个资源和 2 个侦察节点；short 10 pair 各 2.2 s，long 3 pair 各
10 s，共 26 个全新 arm。13/13 业务语义、有限状态、在线真值隔离、实现身份和缓存审计
全部通过。

short 的 D1 fusion 均值由 `3.289739 s` 降至 `3.061518 s`，逐 pair 平均改善
`6.9271%`，10/10 更快；配对原始相对变化 bootstrap 95% 区间为
`[-7.7968%, -6.0841%]`。long 由 `23.304548 s` 降至 `21.776847 s`，改善
`6.6103%`，3/3 更快。核心墙钟 short/long 改善 `2.4060%/2.4537%`，D2 association
变化 `-0.1082%/-2.6729%`。RSS 均值增幅 `0.0145%/0.2959%`，任一 pair 最大
`0.8629%`。

896,820 次预测请求中，reference 构造 875,031 次模型；candidate 构造 3,535 次、命中
871,496 次，构造减少率和命中率均为 `99.5960%`。D6 据此给出
`d1_optimization_admitted=true`。D1 模块构造器默认仍为逐预测构造路径，main 集成默认
已晋级为 `bounded_exact_lru_v1`，并通过 scalable 3D 全量 `212 tests`。两层默认不同是
兼容边界，不是算法分歧。

候选最低实时因子只有 `0.1739499`，所以 `system_realtime_gap_closed=false`。正式证据来自
三维质点环境，不含 AirSim、目标硬件、RMSE、NEES 或 NIS；这些 P1 继续独立验收。

### 发布审计元数据 v2 已通过系统准入

同一扫描的 association、latency 和 sensor-health 审计树现在可按精确不可变合同共享。
正式实现 ID 为 `d1.publication_metadata.immutable_shared_audit.v2`，合同版本为
`d1.publication_audit_tree.v2`。结构验证通过后，D2 对每个新对象执行一次内容审计，后续只
按同一强引用对象复用。

clean source commit `be399e138762f5e660f553c8caa812d52ab38c61` 的 200 目标、200 资源、
2 侦察节点矩阵覆盖 short 10 seed 和 long 3 seed。D1 fusion 改善
13.5447%/26.8298%，核心墙钟改善 6.5677%/18.2438%，D2 association 耗时降低
16.1939%/35.6213%；13/13 合同和业务检查通过。main 已默认选择 v2，D1 构造器默认仍保留
reference。最低实时因子为 `0.1730801`，系统实时和逐批审计明细继续作为 P1。

### 扫描输入候选已通过正式同提交准入

正式评估使用 clean commit
`d14285e4fdeb2f2e2cd32fad2f6d42e30f9e73a7` 的同一套代码，只切换
`reference_v1/candidate_v2` 实现。short 组包含 seeds 1101-1110、每组 2.2 s；long
组包含 seeds 1101-1103、每组 10 s。

short scan-input 累计墙钟均值由 `1.2124522798461839 s` 降为
`1.145650333847152 s`，逐 pair 平均改善 `5.360121886647966%`，9/10 更快；
原始相对变化 bootstrap 95% CI 为
`[-8.208165356448217%, -3.0841406102053194%]`。long 由
`6.687633245543111 s` 降为 `6.3406803108907 s`，改善
`5.142481684491682%`，3/3 更快；95% CI 为
`[-8.837128529506151%, -1.6693612946922343%]`。

13/13 pair 的业务语义、有限状态、在线真值隔离和实现身份均通过，RSS 门通过。核心墙钟
short/long 仅改善约 `0.7187%/0.5792%`。D6 因此准入 `candidate_v2`，扫描输入正式矩阵
P1 关闭；这项结论只说明同一数学与业务合同下的实现替换可以进入默认路径。

系统实时性没有随之关闭。候选最低实时因子为 `0.14342687633969603`，
`system_realtime_gap_closed=false`。矩阵来自三维质点环境，没有 AirSim、实机、目标硬件、
RMSE、NEES、NIS 或长于 10 s 的容量证据。
...
目标硬件不在计时内。后续正式 13-pair 矩阵已按上节完成；系统实时 P1 不因实现准入关闭。

### 性能优化必须通过同输入、多 seed 和长时准入

正式 v3 试验把标量 reference
`a5a472cf81496d94a98db3deb88a3d5c6951f0ce` 与向量化 candidate
`064cbb979d3bab68fee995e476df25709eb666db` 绑定到同一运行配置。两臂共同包含完整
正半定修复和 D2 `e4147b8` 误警审计修复，唯一 treatment 是逐对协方差限制的标量或
向量实现。short 使用 seeds 1101-1110、2.2 s；long 使用 seeds 1101-1103、10 s。
13 组配对对应 26 个三维质点集成 episode，全部正常退出，13/13 跨构建语义检查通过。

short 的 D1 融合累计墙钟均值由 `4.029165 s` 降至 `3.652252 s`，改善
`9.35462%`，10/10 更快；配对原始变化 95% CI 为
`[-10.914359,-8.113134]%`。long 由 `32.954357 s` 降至 `30.768826 s`，改善
`6.631993%`，3/3 更快；95% CI 为 `[-7.279095,-5.406805]%`。单次融合 P95 在
short/long 分别改善 `6.652902/6.655511%`。

D6 给出 `d1_optimization_admitted=true`。这关闭完整正半定输出 P0 和向量化准入 P1。
candidate 最低实时因子为 `0.143397`，所以 `system_realtime_gap_closed=false`。正式
manifest SHA-256 为
`40669d10fff8367aa31e24624bab802d8bc3de6b01aaa1e5c92d054753ed93ec`。该试验不含
RMSE、NEES、NIS、AirSim 或目标硬件，不能用于证明定位精度或系统实时能力。

### 航迹协方差必须满足完整正半定

对每个交叉项限制相关系数不能替代完整矩阵检查。seed 1103 的六维航迹在限制前最小特征值为
正，旧限制器只降低一个位置交叉项后却得到 `-9.247657800e-04`。这是多个状态分量相关关系
共同决定的结果。故障前 58,776 次标量与向量实现逐元素一致，排除了向量化顺序差异。

当前先执行原对角上下界和逐对相关限制，再检查完整特征值。非正定矩阵在标准化相关空间完成
特征值 floor 和单位对角恢复，然后向单位阵收缩到相关上界内。该凸组合保持正半定，恢复后的
对角仍是治理后的原值。有限轮复核仍失败时使用同一对角矩阵，D1 不把异常留给 D2 放宽容差。

航迹 metadata 记录投影原因和操作数。记录内容包括相关裁剪 pair 数、投影迭代数、特征值
floor 数、相关收缩数和对角回退数。在线观测仍先做有限、对称和正半定 fail-closed 校验；
治理没有删除观测、缩短 6 s fixed-lag、改写双时间戳或来源谱系，也不读取 truth。

失败回归和 1 至 6 维随机/极端性质测试通过。D1 全量为 `352 passed in 20.52s`。修复后的
200v200、seed 1103、10 s 运行完成，10,554 条在线观测、有限状态、在线 truth 0。该结果关闭
D1 输出 P0；上节又完成正式多 seed/长时回归。系统实时、AirSim、目标硬件和融合精度 P1
仍未关闭。

### 协方差限制仍逐次执行，成对裁剪改为向量运算

状态预测、量测更新和固定滞后重放都会改变协方差。最新 seed 1100 冻结回放中，
状态协方差限制共执行 12,833 次，其中 10,832 次位于状态时间推进之后，1,789 次位于更新后
重放，202 次位于航迹新生。它们不是同一已限制后验的重复检查，不能通过缓存标记直接跳过。
同一当前状态在航迹物化前的重复限制已由 `current_state_covariance_limited` 阻止。

本轮只替换协方差内部的成对实现。原路径对六维协方差上三角的 15 对元素逐个调用
`np.clip`；新路径先由当前对角线构造同一上三角限幅向量，一次裁剪后镜像到下三角。1 至
6 维只复用不可写的三角索引，不缓存协方差。任意非对角元素仍满足：

\[
\ell_{ij}=0.999\sqrt{\max(P_{ii},0)\max(P_{jj},0)},\qquad
P_{ij}=P_{ji}=\operatorname{clip}(P_{ij},-\ell_{ij},\ell_{ij}).
\]

标量实现通过 `vectorized_covariance_limit=False` 保留为 reference。默认优化路径为
`True`。对角 floor/ceiling、限制原因、对称化、非法状态重置和在线观测 covariance
有限/对称/半正定拒绝均在原位置执行。该实现没有减少观测、缩短 6 s fixed-lag、改变门限、
近似滤波数学或使用 truth。

同一 89 扫描、2,035 匿名观测冻结输入先预热，再交错运行 5 轮。纯融合 P50
`3.011440 -> 2.614061 s`，P95 `3.023308 -> 2.660813 s`，5/5 轮更快。cProfile 中
协方差限制累计耗时 `1.047145 -> 0.426826 s`，状态预测总入口
`1.098530 -> 0.584526 s`。逐扫描后验、物化航迹、双时间戳、来源谱系、分级、操作计数、
累计诊断、终态航迹和一致性证据均严格等价。

另用 10 s 冻结夹具只执行一对长语义对照。771 扫描、11,889 观测覆盖 4,009 次固定滞后
重基和 11,888 条乱序量测，两臂语义一致。专项测试为 `18 passed`，D1 全量为
`342 passed in 19.73s`。上述结果是正式矩阵之前的冻结质点基线。上节已完成 clean 全栈
多 seed 准入，但仍不是 AirSim、目标硬件、实时或精度结论。
...
## 24. 共享发布信息必须不可变（2026-07-24）

一个完整发布扫描中的 association audit、latency audit 和 sensor-health 是同一时刻的全局
审计快照，与具体航迹无关。为每条航迹重复复制这些树不会增加信息，却会随航迹数和传感器数
共同增长。共享只能建立在不可变性上；普通可变字典直接复用会使一个下游消费者的写入污染同
扫描其他航迹。

D1 候选先对三棵审计树做一次递归快照，再转换为 `frozenset` 键值对承载的映射和 tuple
承载的序列。各航迹的
顶层 metadata 仍独立，状态、协方差、航迹来源、关联诊断和身份似然仍逐条复制。共享类型不
继承 `dict/list`，因此基类写入方法没有可修改的底层存储。任何常规写入调用都会明确失败；
`GlobalTrack.to_dict()` 在 JSON 边界恢复普通字典和列表。

共享对象必须通过公开的版本化递归验证，不能凭 `__eq__`、marker 或任意 `Mapping` 判断。
验证器只接受精确合同类型、字符串键和有限 JSON 标量，拒绝可变 backing store、循环结构和
不支持叶值。结构认证不替代内容安全检查。D2 仍要对一个经认证对象执行一次 truth-free 审计，
随后只能对同一强引用对象身份复用审计结论。

该原则不允许减少完整物化扫描，也不允许删减审计字段。候选准入取决于同一 clean source
commit 上的逐发布完整字典等价、变异隔离、多 seed 性能门和下游兼容性。2026-07-24 的 v1
正式矩阵因 D2 重复审计导致核心墙钟只改善 1.65%/1.21%，未达到 5% 门，因此保持拒绝。

v2 随后在 clean source commit `be399e138762f5e660f553c8caa812d52ab38c61` 上完成 short
10 seed 和 long 3 seed 的 13 对、26 臂正式矩阵。场景为 200 目标、200 资源和 2 个侦察节点，
全部 arm 重新执行且无失败。D1 fusion 改善 13.5447%/26.8298%，核心墙钟改善
6.5677%/18.2438%，D2 association 耗时降低 16.1939%/35.6213%。业务语义、有限状态、在线
真值隔离、实现身份、D2 审计和 RSS 门 13/13 通过。702 次合同验证、702 次内容审计和
139,920 次同对象身份复用均未出现合同拒绝，D6 判定 `d1_optimization_admitted=true`。

D1 构造器的 `immutable_shared_publication_metadata` 默认仍为 `False`，用于维持独立调用兼容。
main promotion commit `f5b350b` 已把可扩展三维仿真的默认 selector 晋级为
`immutable_shared_v2`，并保留 reference 供显式对照。该区分避免把模块 API 默认与系统运行
默认混为一谈。最低实时因子仍只有 `0.1730801`，系统实时缺口未关闭；本组也不构成 AirSim、
硬件、RMSE、NEES、NIS 或物理拦截证据。

## 真值隔离质量评估

融合质量评估必须把在线计算和离线身份对齐分开。在线观测只携带双时间戳、量测、协方差、
传感器信息和匿名源谱系。目标真值、Actor 名称、Object 名称及离线身份标签不能进入
`SensorObservation`、融合器状态或 `GlobalTrack` 元数据。

离线评分使用单独 sidecar。sidecar 保存真值轨迹和匿名源谱系映射，通过被融合器实际接受的
观测谱系确定 D1 航迹对应的离线目标。一个航迹含多个真值谱系，或同时含真值与虚警谱系时，
以支持数最多的真值谱系作为评分归属，同时记录混合谱系率；谱系纯度分母包含全部真值和虚警
谱系。同一真值被多条航迹占用时计为重复航迹。纯虚警谱系形成的航迹计入虚假航迹及其可见
寿命。该对齐只服务评估，不能反向影响关联、滤波或航迹编号。谱系覆盖率按融合器保留的唯一
观测编号计算，同一历史观测不会在多个发布帧重复计数。

暖机后第 \(k\) 个发布帧的召回率定义为

\[
R_k = \frac{|\{j:\text{至少一条航迹的主谱系为真值 }j\}|}{N}
\]

重复航迹率定义为

\[
D_k = \frac{\sum_j \max(0,n_{j,k}-1)}{N}
\]

其中 \(n_{j,k}\) 为该帧归属于真值 \(j\) 的航迹数。位置均方根误差使用每个真值谱系支持最多
的一条航迹计算。NEES 使用六维状态误差和航迹 `6x6` 协方差；NIS 直接读取在线创新证据。
没有有效对齐、协方差不可逆或没有在线创新时，相应指标必须标记不可用并给出原因，不能补 0。

2026-07-25 的实现已验证 5/20/50/100/200 场景生成、20-seed 快速批量入口和 200 目标短时
执行。它没有调整融合算法，也没有增加航迹删除逻辑。正式 200 目标长时多 seed 门限仍待
冻结。

## EO 关联风险影子分类

D1 对光电关联的低归一化创新平方不能直接解释为几何合理。投影接近奇异面时，创新协方差会
急剧增大，图像外的已选边仍可能得到较小的归一化代价。原始
`d1.association-risk-evidence.v1` 因此在量测更新前记录已选边和有界候选边的几何诊断，但
不参与关联或滤波。

版本化分类使用独立 `AssociationRiskClassificationEvidence`。固定开发 profile
`d1-eo-pathological-projection-composite-development-v2` 只有在候选数至少 2、已选投影在
图像外、保留替代候选中至少一个在图像内、检测框面积不大于 4 像素平方且置信度不大于 0.10
时给出 `positive`。其余 raw evidence 给出 `negative`，并保存每项命中和未命中判据。分类
一一对应有界 raw evidence，不建立长期在线列表。

该分类固定为 default-off、shadow、`evidence_only`。它不读取 actor、target 或 truth 标识，
不抑制 assignment，不修改后验、协方差、航迹命中数、`GlobalTrack` 或
`global_track_id`，也不向 D2 发出执行请求。相机后方等无法形成合法诊断 DTO 的候选只跳过
旁路诊断，不得中断规范融合。

2026-07-31 的 37-episode 同源开发证据中，严格身份结果与冻结 R0 `37/37` 一致，在线真值
使用为 0；17 个已知相机致错事件 `17/17` 触发，20 个严格身份可用对照 `1/20` 触发。该数据
缺少独立留出，只用于形成 development profile，不能给出正式误报率，也不能支持 D2
enforcement。

后续留出执行使用 seeds 2000--2019 的 nominal 100v100 与 200v200，共 40 个 2.0 s episode，
没有使用正式 shards 10--19。D2 只读因果重放完成 `40/40`。排除 4 个非相机阻断样本后，36 个
case 产生 1,015 条分类；在线结果与离线 v2 复算 `1015/1015` 一致。13 个相机因果故障事件命中
11 个，召回率为 `0.8461538462`；25 个严格身份可用对照均未告警。样本量门通过，性能门因召回
低于 0.90 而失败。

影子分类的治理原则由该结果进一步冻结。留出执行集只能评价既有 v2，不能反向用于调参；逐条
复算一致不能替代分类性能门。100v100 seed 2006 和 200v200 seed 2001 的业务等价复核确认，去除
风险旁路字段后 D1/D2 总线、严格身份评估语义与 truth NPZ 的 SHA 均不变。v2 仍为默认关闭的
旁路证据，不形成正式准入，不允许 D2 执行门控。下一候选必须来自新的开发数据，并使用新的
独立留出集。

## GlobalTrack 完整物化的批量质量摘要

完整发布必须继续为每条航迹构造独立的 `GlobalTrack`。状态、六维协方差、双时间戳、来源谱系、
身份似然、质量等级和轨迹专属元数据不能因性能优化而被省略或共享为可变对象。允许消除的工作
仅限同一发布帧内输入已经确定、公式完全相同的重复数值摘要。

二维位置 95% 置信半径为

\[
A_{95}=\sqrt{\chi^2_{2,0.95}\max(\lambda_{\max}(P_{xy}),0)}.
\]

reference 对每条航迹单独执行一次对称特征值分解。候选先按原顺序完成协方差限制，再把同一
发布帧的所有 \(P_{xy}\) 组成 `(N,2,2)` 数组，调用一次批量 `eigvalsh`。所得 `A95` 仍逐条送入
原质量分档与完整物化函数。该批处理不改变协方差本身，也不改变分级门限和输出值。

2026-08-01 使用 seed 42000、2.2 秒的非正式 200v200 冻结输入开展 7 对交替 fresh-process
验证。输入含 86 个扫描和 2,051 条匿名观测，未使用正式 seeds 1000--1019 或正式 R0。候选
7/7 更快，模块墙钟中位数由 `0.228742 s` 降至 `0.190582 s`，改善 `16.6824%`；配对差
bootstrap 95% 区间为 `[-0.044637, -0.031457] s`。每臂仍执行 59 次完整发布并物化
11,188 条航迹，标量 A95 调用由 11,188 次改为 56 次非空批量调用。

逐扫描后验、协方差、归一化创新平方、门控观测编号、完整发布载荷、一致性证据、业务操作数和
最终离线导出在 7/7 对中严格一致。在线真值使用和 `global_track_id` 写权限均为 0。候选身份为
`d1.publication.global_track_materialization.batched_a95_summary.v1`，开关默认关闭。该结果只
通过 D1 模块门，等待 main 独立集成审查，不关闭系统实时、AirSim、目标硬件或正式融合精度
P1。
