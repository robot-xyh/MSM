...
D1 候选先对三棵审计树做一次递归快照，再转换为拒绝变异的 JSON 兼容容器。各航迹的顶层
metadata 仍独立，状态、协方差、航迹来源、关联诊断和身份似然仍逐条复制。共享对象支持读取、
比较、JSON 编码和深拷贝；深拷贝可安全返回原不可变对象。任何写入共享审计树的调用都会明确
失败，不能静默影响其他航迹。

该原则不允许减少完整物化扫描，也不允许删减审计字段。候选是否可作为默认实现，取决于同一
冻结输入上的逐发布完整字典等价、变异隔离测试、多 seed 性能区间和下游兼容性。2026-07-24
只有单 seed 候选证据，系统实时缺口保持开放。
